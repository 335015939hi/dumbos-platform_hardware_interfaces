...
// constexpr uint32_t VENDOR_SPACE_OFFSET_IN_MISC = 2 * 1024;
constexpr uint32_t WIPE_PACKAGE_OFFSET_IN_MISC = 16 * 1024;
constexpr uint32_t WIPE_PACKAGE_SIZE_MAX = 48 * 1024;

BootloaderMessage::BootloaderMessage(std::string misc_block_device)
    : misc_block_device_(std::move(misc_block_device)) {}

BootloaderMessage::BootloaderMessage() : BootloaderMessage("") {}
...
bool BootloaderMessage::ReadMiscPartition(void* p, uint32_t size, uint32_t offset,
                                          std::string* err) {
...
bool BootloaderMessage::WriteMiscPartition(const void* p, uint32_t size, uint32_t offset,
                                           std::string* err) {
    auto misc_blk_device = GetMiscBlockDevice(err);
    if (misc_blk_device.empty()) {
        return false;
...
Return<void> BootloaderMessage::UpdateBootloaderMessageWithRecoveryOptions(
    ...
    Message message;
    if (!clear_fields) {
        ReadBootloaderMessage([&message](const MessageStatus& status, const Message& read_message) {
            if (!status.result) {
...
BootloaderMessage* HIDL_FETCH_IBootloaderMessage(const char* name) {
    fprintf(stderr, "fetching bootloader message %s\n", name);
