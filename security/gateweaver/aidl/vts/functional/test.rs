...
/// In this test, vts acts as a sink and also an authgraph orchestrator.
/// It tests the gateweaver's api and its authgraph key exchange and management functions.
// TODO at this moment it is not known how can vts can get gateweaver's source id.
//  More checks pertaining to source id policy will be added once this is known.
#[test]
fn test_gw_success() {
    let protector_key = vec![0; AES_256_KEY_LEN];
    let new_protector_key = vec![1u8; AES_256_KEY_LEN];
    let slot_id = 0;
    let challenge = 1;
    ...
    let cred_arc = match verify_resp {
    ...
    // Test authgraph key management as a local sink
    let mut sink_ctx = vts_km::test_agkm_participant(false).expect("failed to create a local sink");
    vts_km::km_source::test_agkm_mainline(&mut sink_ctx,
                                          cred_arc,
                                          src_out_arc,
                                          sink_in_arc,
                                          require_km!());
    ...
    let (id_hash, cred_arc) = validate_re_enroll_resp(slot_id, &uid, &enroll_resp);
    assert!(id_hash == source_id_hash, "source id hashes:{:?} \n {:?}",id_hash, source_id_hash);
    let verify_resp = gw_ta
        .verify(&cred_arc, &new_protector_key, challenge, None)
        .expect("verify response must be valid");
    let _cred_arc = match verify_resp {
        VerifyResponse::Success(ref resp) => {
            validate_success_resp(&uid, challenge, 0, resp)
        }
        VerifyResponse::Error(ref resp) => {
    ...
    let verify_resp = gw_ta.verify(&create_resp, &protector_key, challenge, None)
                           .expect("verify response must be valid");
    match verify_resp {
...
/// In this test, vts acts as sink and orchestrator and tests the throttling
/// functionality of gateweaver by verify wrong protector key.
#[test]
fn test_gw_throttled() {
    let protector_key = vec![0; AES_256_KEY_LEN];
    let wrong_protector_key = vec![1; AES_256_KEY_LEN];
    let slot_id = 1;
    let challenge = 1;
    let gw_ta = require_gw!();
    let create_resp =
        gw_ta.create(slot_id, &protector_key).expect("create response should be valid");
    let (uid, _source_id_hash) = validate_create_resp(slot_id, &create_resp.arc);
    // verify with wrong protector key
    let verify_resp = gw_ta.verify(&create_resp, &wrong_protector_key, challenge, None)
                            .expect("verify response must be valid");
    match verify_resp {
        VerifyResponse::Success(ref resp) => panic!(
            "Expected Throttled error - instead received status code: {:?}",
            resp.statusCode
        ),
        VerifyResponse::Error(ref resp) =>{
          assert!( resp.errorCode == Error::THROTTLED,
          "Un-expected error: {:?}", resp.errorCode);
          // wait for the time = waitTime
          sleep(Duration::new((resp.waitTime + 2) as u64, 0));
        }
    };
    // verify with correct protector key
    let verify_resp = gw_ta.verify(&create_resp, &protector_key, challenge, None)
        .expect("verify response must be valid");
    let _cred_arc = match verify_resp {
        VerifyResponse::Success(ref resp) => {
...
fn validate_re_enroll_resp(
    ...
    match enroll_resp {
        ReEnrollResponse::Success(ref resp) => {
            ...
            (head_values.source_id_hash.unwrap(),resp.newGWBlob.clone())
        }
