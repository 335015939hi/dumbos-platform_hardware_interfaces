...
interface IGateWeaver {
     ...
     *    iv) source_id - `Identity` of the GW
     *
     ...
     * 3) Decrypt the `gateWeaverBlob` with the encryption_key, return error: INVALID_BLOB if the
     *    decryption fails, otherwise, proceed.
     * 4) If the `Identity` in the `source_id` doesn't match the current identity of the GW, return
     *    error: RE_ENROLL_REQUIRED
     * 5) Retrieve the UID and the ratelimiting_schedule id from the protected headers of the
     *    `gateWeaverBlob`
     * 6) If the failure_count > length of the rate limiting schedule, return error:
     *    MAXIMUM_ATTEMPTS_REACHED, else proceed.
     * 7) If the failure_count > 0, check if now < wait_time. If so, return error: THROTTLED
     *    along with time till next allowed attempt = wait_time - now. Otherwise, proceed.
     * 8) Decrypt the output of step #3 with the `protectorKey` to recover K_cred
     * 9) If step #6 succeeds;
     ...
     *       ii) set wait_time = now + ratelimiting_schedule[failure_count - 1]
