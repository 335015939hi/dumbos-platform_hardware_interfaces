...
interface IGateWeaver {
     ...
     *   iii) source_id - `Identity` of the GW
     *
     ...
     *           protected headers:
     *               a) UID = the UID obtained from the protected headers of the GW blob
     *               b) KeyType = CredentialKey
     *               c) challenge = the challenge from the input
     *               d) Timestamp = the timestamp obtained from the timestamp arc
     *      iii) check whether the `Identity` in slot_id matches the current identity of the GW. If
     *           not, return the arc created in step ii, along with the error code
     *           RE_ENROLL_REQUIRED. Otherwise, return the arc with the error code OK.
     * 8) If step #6 fails;
     *        i) set failure_count = failure_count + 1
     *       ii) set wait_time = now + wait time defined by the ratelimiting schedule for the
     *           current failure count
     *      iii) return error: THROTTLED along with time till next allowed attempt = wait_time
     *
     ...
     *
     * @return arc from step 7.ii on success or the corresponding error response on failures as
     * described above
     */
    ...
    /**
     * Reset all the slots in the GW storage, except the slot identified by the given slot id.
     * This is useful to support factory reset protection.
     */
    void invalidateAllExcept(in int slotID);
     ...
     * timestamp. If it is successful, move on to creating the new GW blob. Otherwise, return the
     * error returned from `verify`.
     * Follow the steps of `create` to create a new GW blob encrypted by the new
     * protector key and the encryption key associated with the given slot id. However, instead of a
     * randomly generated uniique identifier, use the UID included in the protected headers of the
     * GW blob provided as input.
     * Return the outputs of internal calls to `create` and `verify`.
     *
     * It is the responsibility of the caller to perform the subsequent operations to complete the
     * task of re-enroll, based on the use case. In all three use cases, the caller should make sure
     * that the downstream secrets protected from the old K_cred are migrated to be protected by
     * the new K_cred. In use case 1, the old GW blob should be deleted. In use case 3, the old
     * GW blob as well as the downstream secrets protected by the old K_cred should be deleted. The
     * caller should maintain a reference count for all the downstream parent secrets and make sure
     * that any parent secret is deleted only when its reference count is zero (i.e. all child
     * secrets are migrated to the new parent secrets).
     *
