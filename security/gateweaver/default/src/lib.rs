...
use std::array;
use gateweaver::{CredentialStore, ta::GateWeaverTa, ag_err, wire::{self, Error, ErrorCode},};
use authgraph_core::key::{EcSignKey, Identity};
...
impl LocalTa {
    ...
    pub fn new() -> Self {
        ...
        std::thread::spawn(move || {
            ...
            let (identity_sign_key, identity, allowed_sinks) =
                crate::get_ids().expect("Failed to read test ids");
            // create gatewear ta
            let gw_ta = Rc::new(RefCell::new(
                GateWeaverTa::new(
                  &mut boring::crypto_trait_impls(),
                  identity_sign_key,
                  identity,
                  allowed_sinks,
                  storage_impl,
...
pub struct InMemoryStore {
  store: [InMemorySlot; GW_MAX_SLOTS],
}
impl InMemoryStore{
  pub fn new() -> Self{
    Self{store: array::from_fn(|_|InMemorySlot::default()),}
  }
}
impl CredentialStore for InMemoryStore {
    ...
    fn create(
        &mut self,
        slot_id: usize,
        k_cred_arc: &[u8] // Arc.cddl
    ) -> Result<(), wire::Error>{
      if !(slot_id < GW_MAX_SLOTS) {
        return Err(ag_err!(StorageError, "slot id is invalid"));
      }
      self.store[slot_id].k_cred = k_cred_arc.to_vec();
      self.store[slot_id].attempts = 0;
      self.store[slot_id].wait_time = 0;
      Ok(())
    }

    // read the slot data.
    fn get(&self, slot_id: usize) -> Result<(Vec<u8>, u32, u64), wire::Error>{
      if !(slot_id < GW_MAX_SLOTS) {
        return Err(ag_err!(StorageError, "slot id is invalid"));
      }
      Ok((self.store[slot_id].k_cred.to_vec(), self.store[slot_id].attempts,self.store[slot_id].wait_time))
    }
    // Set the attempts and wait time to new value in the slot.
    fn set(&mut self, slot_id: usize, attempts: u32, wait_time: u64) -> Result<(), wire::Error>{
      if !(slot_id < GW_MAX_SLOTS) {
        return Err(ag_err!(StorageError, "slot id is invalid"));
      }
      self.store[slot_id].attempts = attempts;
      self.store[slot_id].wait_time = wait_time;
      Ok(())
    }
}

/// This method generates common test ids used by authgraph library which are used
/// for nonsecure gateweaver implementation in actual HW backed secure implementation
/// these ids will be shared in a implementation specific way.
pub fn get_ids() -> Result<(EcSignKey, Identity, Vec<Identity>), wire::Error>{
  todo!()
}
