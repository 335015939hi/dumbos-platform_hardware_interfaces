...
use log::error;
//use gateweaver_core::ta::GateWeaverTa;
use std::cell::RefCell;
use std::rc::Rc;
use std::sync::mpsc;
use std::sync::{Arc, Mutex};

/// Implementation of the Gateweaver TA that runs locally in-process (and which is therefore
/// insecure).
pub struct LocalTa {
...
impl LocalTa {
    ...
    pub fn new() -> Self {
        ...
        let gw_ta = boring::test_device::AgDevice::default_managed_device()
        ...
        std::thread::spawn(move || {
            let gw_ta = Rc::new(RefCell::new(gw_ta));
...
//                        //gw_ta.borrow_mut().process(&req_data[1..])
//                        todo!()
//                    }
