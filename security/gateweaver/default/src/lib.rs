...
use gateweaver_ta::{InMemoryStore, GateWeaverTa};
use authgraph_test_ids as test_ids;

/// Max attempts for the nonsecure local TA
pub const GW_MAX_ATTEMPTS: u32 = 10;
/// Rate limiting schedule for the nonsecure local TA
const GW_RATE_LIMIT_SCHEDULE: [u64; GW_MAX_ATTEMPTS as usize] = [
0, 0, 0, 10, 30, 30, 60, 60, 900, 3600,
];
...
impl LocalTa {
    ...
    pub fn new() -> Self {
        ...
        std::thread::spawn(move || {
            let storage_impl = Box::new(InMemoryStore::new());
            let (identity_sign_key, identity) = test_ids::test_id_for_source()
            ...
            let gw_ta = Rc::new(RefCell::new(
                GateWeaverTa::new(
                  ...
                  storage_impl,
                  GW_MAX_ATTEMPTS,
                  GW_RATE_LIMIT_SCHEDULE.to_vec(),
              )
              .expect("Failed to create local Gateweaver TA"),
            ));
