...
interface IAuthGraphCommon {
    /**
     * Initializes an ephemeral elliptic curve (EC) key pair, with X25519, in order to establish
     * a secret channel between two domains via elliptic curve diffie hellman (ECDH) key agreement.
     * This outputs the signed public key of the key pair in InitChannelResult. The signature is
     * created using the persistent signing key of the domain, via EdDSA.
     ...
     * invocations of the two methods related to channel establishment, it may include an arc from
     ...
     *
     * @param signingKeySetupInfo: Each domain is supposed to have a persistent signing key,
     *        with Ed25519, which is handed over to them via BCC (Boot Certificate Chain).
    ...
    /**
     * Verifies the signature on the ephemeral EC public of the other party, which is supplied as
     * the first argument. If the verification is successful, computes two encryption keys via ECDH,
     * which are used as the channel keys agreed by the two domains. One key is to used to decrypt
     * incoming messages and the other key is to encrypt outgoing message.
     *
     * Domains should output an arc from PBK to each channel key, attaching the public signing key
     * of the other party and the relevant label ('incoming'/'outgoing') in D. These are called
     * channel arcs. These outputs are included in the returned type CreateChannelResult.
     *
     * In order to verify that the channel keys are usable for further communication, domains should
     * also output the nonce sent by the other party encrypted with the outgoing channel key, which
     * will then be verified by the other party by decrypting it using their incoming channel key.
     *
     ...
     *
     * @param nonceToBeEncrypted: The value output by the other domain in the field
     *         'nonceToBeEncrypted'
     *        in InitChannelResult.
     *
    ...
    CreateChannelResult createChannel(in byte[] signedPublicKeyOfOtherParty,
            in byte[] nonceToBeEncrypted, in @nullable InitChannelResult initChannelResult);

    /**
     * Verifies that the channel is usable by decrypting 'encryptedNonce' using the incoming channel
     * key and checking if the decrypted value is equal to the 'nonceToBeEncrypted' value in
     * InitChannelResult.
     *
     * ErrorCode::OK must be returned if the verification is successful. Otherwise, return
     * ErrorCode::CHANNEL_VERIFICATION_FAILED
     *
     * @param encryptedNonce: encryption of 'nonceToBeEncrypted' value in initChannelResult, which
     *        which is output by self in the 'initChannel' method.
     *        The encryption is output by the other party in the 'createChannel' method.
     *
     * @param createChannelResult: CreateChannelResult output by self in 'createChannel' method.
     *        This is passed in for the domains to have access to their channel keys.
     *
     * @param initChannelResult: InitChannelResult output by self in the 'initChannel' method.
     *        This is passed in for the domains to have access to the nonce they created.
     *
     */
    void verifyChannel(in byte[] encryptedNonce, in CreateChannelResult createChannelResult,
            in InitChannelResult initChannelResult);
}
