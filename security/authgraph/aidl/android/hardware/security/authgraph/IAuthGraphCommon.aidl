 ...
 * be used without the source domain's secret being available. And when the source domain's
 * secret is available, it can be communicated to the sink domain via a secure channel
 * pre-established between the two domains.
 * E.g. An auth-bound key created in KeyMint TA is encrypted using a key derived from the
 * fingerprint secret that resides in Fingerprint TA, s.t. a user's keys are
 ...
 * of a payload P with a key K and additional authentication data (AAD) D (i.e. Arc = Enc(K, P, D)).
 *
