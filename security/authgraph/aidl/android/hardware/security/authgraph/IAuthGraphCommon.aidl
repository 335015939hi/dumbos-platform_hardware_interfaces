...
interface IAuthGraphCommon {
    ...
    /**
     * Verifies the signature on the ehemeral EC public of the other party, which is supplied as the
     * first argument. If the verification is successful, computes two encryption keys via ECDH,
     * which are used as the channel keys agreed by the two domains. One key is to encrypt incoming
     * messages to this domain by the other domain and the other key is to encrypt outgoing messages
     * to the other domain by this domain.
     *
