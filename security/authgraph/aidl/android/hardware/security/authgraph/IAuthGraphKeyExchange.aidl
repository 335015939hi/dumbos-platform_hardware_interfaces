...
interface IAuthGraphKeyExchange {
     ...
     *    of the arc).
     * However, P1 should maintain some of in-memory record to be able to verify that the input
     ...
     *    instance of the key agreement protocol.
     * ii.A way to minimize the in-memory storage of P2 allocated for key agreement.
     ...
     *        `ownKey` is not in the in-memory reocords for unfinished instances of a key agreement
     *        protocol, return error: INVALID_KE_KEY. Similarly, if the public key or the arc
     *        containing the private key in `ownKey` is invalid, return INVALID_PUB_KEY_IN_KEY and
     *        INVALID_PRIV_KEY_ARC_IN_KEY respectively.
     ...
     * i. A mapping to correlate the key agreement protocol taken place with a particular peer and
