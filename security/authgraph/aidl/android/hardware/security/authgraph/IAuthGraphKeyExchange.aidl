...
/**
 * Authgraph interface definition for authenticated key exchange between two parties: P1 and P2.
...
interface IAuthGraphKeyExchange {
     ...
     *     6. mark the shared key arcs as auth completed = false in their protected headers
     *     7. derive a MAC key
     ...
     *
     * @param nonce - nonce created by the peer
     *
     * @return KEResult including the `Key` containing the public key of the created key pair and
     * an arc from the per-boot key to the private key, the nonce, the persistent identity, two
     * shared key arcs from step #5, session id and the signature over the session id.
     */
    KEResult init(in PubKey peerDHKey, in Identity peerId, in byte[] nonce);
     ...
     *     7. mark the shared key arcs as auth completed = true in their protected headers
    ...
    KEResult finish(in PubKey peerDHKey, in Identity peerId, in KESignature peerSignature,
            in byte[] nonce, in Arc ownDHKey);

    /**
     * This method is invoked on P2.
     * Perform the following steps:
     *     1. verify the signature over the session id attached to the session key arcs
     *     2. mark the session key arcs as auth completed = true in their protected headers
     *
     * @param peerSignature - the signature created by the peer over the session id computed by the
     *                        peer
     *
     ...
     *
     * @param KEResult including the shared key arcs marked as auth completed = true
     */
    KEResult authComplete(in KESignature peerSignature, in Arc[] sharedKeys);
}
