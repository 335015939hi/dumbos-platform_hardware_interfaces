...
/*
 * The IAuthGraphKeyManagement interface defines a protocol to manage the authentication
 * graph (i.e. authgraph) made up of connected arcs. An arc is simply AES-GCM encryption of a
 ...
 *
 * There are 3 operations defined by this interface which are used to manage the
 * authgraph by creating, modifying and unlocking the arcs in a authgraph.
 * These three operations are as follows:
 *     1. create: Creates a new persistent arc from a existing ephemeral arc. Arc is defined in
 * Arc.aidl.
 *     2. mint: Creates a new persistent arc from two existing ephemeral key arcs.
 *     3. snap: Creates a new ephemeral arc from combination of ephemeral and persistent arcs.
 *
 * Here, ephemeral arc originate from per-boot key, which is a symmetric encryption
 * key kept in memory with per-boot life time of authgraph participant implementing
 * the IAuthGraphKeyManagement interface.
 *
 * Thus, create and mint are used during the construction/modification of the
 * persistent authentication graph. Whereas mint and snap are used during the unlocking of the
 * authentication graph. Thus, by using these 3 operations, a complete management of the authgraph
 * is possible.
 *
 * One of the main usecase for this protocol is that one of the authgraph participant with a role
 * sink can obtain a symmetric encryption key that is cryptographically protected with a key
 * held by another paricipant with a role source, where the sink and the source exchange
 ...
 *
 * Pre-requisites:
 *     1. Each IAuthGraphKEyMAnagement instance should should have following data
 *        which is used to create new arcs and validate parameters in the interface methods:
 *          - allowed_sources: SourceIds that are allowed by this instance. Used if
 *                             this instance is a source.
 *          - allowed_sinks: SinkIds that are allowed bu this instance. Used if
 *                             this instance is a sink.
 *          - allowed_roles: Roles suppoted by this instance which are source,
 *                            sink or both.
 *          - UID policy: Identifies whether this instance allows arcs with single or multple UIDs.
 *            Note: sourceIds and sinkIDs are of type IdentityEncoded.
 *            See Arc.cddl for details.
 *
 *     2. Each pair of source and sink should have:+
 *              i. a shared key set up by executing the authenticated key exchange protocol defined
 ...
 *
 *     3. A user who wants to create a secret at the sink, that is protected by a key held by the
 *        source, should have set up a key that is unlocked at the source
 *        (called "cred key"), via a source-specific functionality. TThe cred key is associated
 *        with the authenticated party via a unique identifier (called UID). The authenticated
 *        party can be user, server, etc.
 *
...
interface IAuthGraphKeyManagement {
    /*
     * This method creates a new secret key and returns a persistent arc
     ...
     * is a positive statement about the usage such as "the arc can be
     * used by Keymint to encrypt a user specific secret for a user with
     * a specific user Id". The limitation is a negative statement about the
     * arc's usage such as "the arc cannot be used for more then 10 times
     * and after 15 minutes period has passed after a latest successfull user
     * authentication".
     *
     * This operation can be invoked at TA which is either source or sink.
     * The input wrappingArc to this operation must be encrypted by the
     * per-boot key.
     *
     * Before creating the new Arc, the IAuthGraphKeyManagement instance performs
     * a series of checks to verify that the creation should be allowed. If
     * any check fails, the instance must return an ENFORCEMENTS_FAILED service
     * specific error is returned.
     *
     * Perform the following steps with respect to input parameter role:
     * 1. CHECK whether role is supported by IAuthKeyGraphManagement instance.
     *
     * Perform the following steps with respect to input parameter wrappingArc:
     *  1. If the IAuthKeyGraphManagement instance is a source TA then CHECK if the
     *     sourceId in the wrappinArc is supported by the TA or if the
     *     IAuthKeyGraphManagement instance is a sink TA then CHECK if the
     *     sinkId in the wrappinArc is supported by the TA.
     ...
     *                 childArcCaps's policy cannot be single.
     ...
     *              - If the IAuthKeyGraphManagement instance is source TA and input parameter role
     * is source then add sourceId of the IAuthKeyGraphManagement instance TA to the permission.
     *              - If the IAuthKeyGraphManagement instance is sink TA and input parameter role is
     * sink then add Sink Id of the IAuthKeyGraphManagement instance TA to the permission.
     *              - Generate "MintingAllowed" permission by combining "MintingAllowed"
     *                of the wrappingArc (if defined) and adding new permissions
     *                and limitations to it from childArcCaps. If the input
     *                parameter role is source then add TA's sourceId to
     *               "MintingAllowed" if not already present else if the input
     *                parameter role is sink then add TA's sink Id to "MintingAllowed"
     *                if not already present. Add UIDPolicy=single if neither
     *                wrappingArc or childArcCaps specifies it.
     ...
     *    2. CHECK the sourceId of parentArc matches the id of the
     *       IAuthGraphKeyManagement instance if it is a Source TA. Else CHECK sinkId
     *       matches the id of the IAuthGraphKeyManagement instance if it is a Sink TA.
     ...
     *    4. Decrypt the parentArc using perboot key of the TA and CHECK if that happens
     *       successfully.
    ...
    /*
     * This method creates a new persistent arc by using the sourceArc's
     ...
     *              allowed by the TA.
     *    4. Decrypt the sourceArc and destArc using the perboot key of the TA and
