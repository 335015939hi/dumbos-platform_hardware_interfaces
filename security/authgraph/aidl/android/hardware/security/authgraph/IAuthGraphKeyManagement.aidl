...
interface IAuthGraphKeyManagement {
     ...
     * producing an ephemeral arc. Both these arcs are returned to the caller.
     *
     * Perform the following steps:
     *     1. CHECK whether the input: `credKey` is an arc that is encrypted with the source's
     *        per-boot key and has KeyType = CredKey in the protected headers.
     ...
     *                   b. TODO: DICE verification
     *                   c. Create an arc that encrypts the source key with the per-boot key and add
     ...
     *                   d. Create a SourceKey to be returned to the caller with no `persistent` arc
