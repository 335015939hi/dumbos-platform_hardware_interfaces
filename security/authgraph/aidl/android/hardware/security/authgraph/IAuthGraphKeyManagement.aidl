 ...
 *
 * Such cryptographically protected user secrets created at the sink can be decrypted if and only if
 ...
 *
 * There are 3 operations defined by IAuthGraphKeyManagement api:
 *     1. create: Creates a new persistent arc from a existing ephemeral arc.
 ...
 *
 * Here, ephemeral arc originate from per-boot key, which is only valid until the next reboot of the
 * device. Thus, reate and mint are used during the construction/modification of the
 * persistent authentication graph. Whereas, mint and snap are used during the unlocking of the
 * authentication graph. Thus, using this 3 operations complete management of the authgraph
 * is possible.
 *
...
interface IAuthGraphKeyManagement {
     ...
     *     supported by the TA or if this is a sink TA then CHECK if the sinkId in
     *     the wrappinArc is supported by the TA.
     *  2. CHECK if the payloadType is secretKey.
     *  3. CHECK if "MintineAllowed" permission is defined then source Ids, sinkIds
     ...
     *                parameter role is source then add TA's source Id to
     ...
     * @param childArcCaps - these are set of source ids, sink ids and UIDPolicy.
    ...
    /*
     * This operation method creates a new persistent arc by using the sourceArc's
