...
interface IAuthGraphKeyManagement {
     ...
     *    3. CHECK whether the "source_id"s and "sink_id"s in the protected headers of the two input
     *       arcs are equal
     *    4. Deserialize the public key found in the protected header: `PlainPubKeyEncoded` of
     *       the input: `primaryExchangePubKey` arc
     *    5. Create an EC key pair on NIST curve: P-256 for ECDH
     *    6. Create a nonce
     *    7. Compute the Diffie-Hellman shared secret: Z, using the public key from step #4
     *       and the key pair created in step #5 above
     *    8. Extract a cryptographic secret S from Z, using the nonce created in step #6 as the
     *       salt
     *    9. Derive a symmetric encryption key of 256 bits from S with b"KEY_WRAPPING_KEY" as the
     *       context
     *   10. Derive a MAC key of 256 bits from S with b"KEY_WRAPPING_KEY_CONFIRMATION_KEY" as the
     *       context
     *   11. Compute "session_id" as the 256 bits HMAC over the nonce created in step #6 with
     *       the key derived in step #10
     *   12. Encode the public key of the key pair created in step #5 and add the custom
     *       header: KeyType = SecondaryExchangePubKey
     *   13. Create an arc with PayloadType = Empty and the encrypting key being the long term
     *       encryption key of the sink. Add the following protected headers:
     *         i. PlainPubKeyEncoded = the serialized COSE key from step #12 above
     *        ii. Nonce = nonce created in step #6
     *       iii. session_id = the one computed in step #11
     *   14. Create an arc that encrypts the symmetric encryption key derived in step #9 with
     ...
     *   15. Create a `KeyWrappingKeyPackage` to be returned, containing the arc from step #14 as
     *       the `keyWrappingKey` and the arc from step #13 as the `keyWrappingKeyHandle`
     *
     * Note: If any of the enforcements marked as `CHECK` above fails, return the error:
     * ENFORCEMENTS_FAILED
     *
     ...
     *        has KeyType = SharedKey, "source_id" and "sink_id" in the protected headers.
    ...
    KeyWrappingKeyPackage recoverKeyWrappingKeyWithKeyExchange(
            in KeyWrappingKeyDeriveInfo keyWrappingKeyDeriveInfo, in Arc sharedKey, in Arc authKey);
