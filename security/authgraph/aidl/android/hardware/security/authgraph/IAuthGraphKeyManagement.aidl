...
import android.hardware.security.authgraph.SourceKey;
...
interface IAuthGraphKeyManagement {
     ...
     * the cred key is unlocked.
     * This method creates a unique source key for a particular user. If a source key has already
     * been created, this method unlocks the source key. The source key is encrypted by the
     * cred key, producing a persistent arc. The source key is encrypted by the per-boot key,
     * producing an ephemeral arc. Both these arcs are returned to the caller. These arcs contain
     * the same UID that is attached to the cred key by the source.
     *
     * Perform the following steps:
     *     1. CHECK whether the input: `credKey` is an arc that is encrypted with the source's
     *        per-boot key and has KeyType = CredKey and source_id= `Identity` of the source in the
     *        protected headers.
     *     2. Depending on whether a `sourceKey` is present,
     *            i. If a `sourceKey` is present;
     *                   a. CHECK whether `persistent` field in the SourceKey is an arc that is
     *                      encrypted with the cred key (i.e. the payload key of input 1) and
     *                      has KeyType = SourceKey_w_CredKey in the protected headers.
     *                   b. Create an arc that encrypts the source key with the per-boot key and add
     *                      the following in the protected headers:
     *                      i. source_id = `Identity` of the source
     *                      ii. KeyType = SourceKey
     *                      iii.challenge = the value of the "challenge" protected header, if it
     *                           was present in the input arc: `credKey`
     *                   c. Create a `SourceKey` to be returned to the caller with no `persistent`
     *                      arc and the arc created in step c above being the `ephemeral` arc.
     *           ii. If an `sourceKey` is not present:
     *                   a. Create a 256-bit cryptographic key for AES-GCM encryption (source key)
     *                   b. Create an arc that encrypts the source key with the per-boot key and add
     *                      the following in the protected headers:
     *                      i.   source_id = `Identity` of the source
     *                      ii. KeyType = SourceKey
     *                   c. Create an arc that encrypts the source key with the cred key and add
     *                      the following in the protected headers:
     *                      i.   source_id = `Identity` of the source
     *                      ii. KeyType = SourceKey_w_CredKey
     *                   d. Create a SourceKey to be returned to the caller, with the arc created in
     *                      step b above being the `ephemeral` arc and the arc created in step c
     *                      above being the `persistent` arc.
     ...
     * ENFORCEMENTS_FAILED
     *
     * @param credKey - an arc encrypting the user's cred key with the source's per-boot key
     *
     * @param sourceKey -
     *
     * @return SourceKey -
     */
    SourceKey createOrUnlockSourceKeys(in Arc credKey, in SourceKey sourceKey);

    /**
     * This method is invoked on a source, right after the user authenticates with the source and
     * the cred key and source key are unlocked.
     ...
     *                   b. CHECK whether the DICE chain in the "sink_id" protected header of the
     *                      `sharedKey` arc in the AuthKeyPackage matches the identity verification
     *                      policy in the "sink_id" protected header of the `persistent` arc of the
     *                      AuthKey in the AuthKeyPackage.
     *                   c. Create an arc that encrypts the auth key with the shared key and add
     *                      the following in the protected headers:
     *                      i.   source_id = `Identity` of the source
     *                      ii.  sink_id = `Identity` of the sink from the protected headers of the
     *                           `persistent` arc of the AuthKey in the AuthKeyPackage
     *                      iii. KeyType = AuthKey_w_SharedKey
     *                      iv.  challenge = the value of the "challenge" protected header, if it
     *                           was present in the input arc: `sourceKey`
     ...
     *                      ii.  sink_id = `Identity` of the sink from the protected headers of the
     *                           shared key arc
     *                      iii. KeyType = AuthKey_w_SourceKey
     *                   d. Create an AuthKeyPackage to be returned to the caller, with `sharedKey`
     ...
     *     3. CHECK whether the "source_id"s and the "sink_id"s in the protected headers of the two
     *        input arcs are equal.
     ...
     *       sink's long term encryption key and has the protected headers: `PlainPubKeyEncoded`
     *       (with a serialized COSE key as the value), "source_id" and "sink_id"
     *    2. CHECK whether the headers of the COSE key has KeyType = PrimaryExchangePubKey
     *    3. Deserialize the public key from the COSE key
     *    4. CHECK whether the input: `sharedKey` is an arc that is encrypted with the sink's
     *        per-boot key and has KeyType = SharedKey, "source_id" and "sink_id" in the protected
     *        headers.
     *    5. CHECK whether the "source_id"s and "sink_id"s in the protected headers of the two input
     *       arcs are equal
     *    6. Create an EC key pair on NIST curve: P-256 for ECDH
     *    7. Create a nonce
     *    8. Compute the Diffie-Hellman shared secret: Z, using the public key from step #3
     *       and the private key of the key pair created in step #6 above
     *    9. Extract a cryptographic secret S from Z (via HKDF-extract) using the nonce created in
     *       step #7 as the salt
     *   10. Derive a symmetric encryption key of 256 bits from S with b"KEY_WRAPPING_KEY" as the
     *       context (via HKDF-expand)
     *   11. Derive a MAC key of 256 bits from S with b"KEY_WRAPPING_KEY_CONFIRMATION_KEY" as the
     *       context (via HKDF-expand)
     *   12. Compute "session_id" as the 256 bits HMAC over the nonce created in step #7 with
     *       the key derived in step #11
     *   13. Encode the public key of the key pair created in step #6 as a COSE key and add the
     *       custom header: KeyType = SecondaryExchangePubKey
     *   14. Create an arc with PayloadType = Empty and the encrypting key being the long term
     *       encryption key of the sink. Add the following protected headers:
     *         i. PlainPubKeyEncoded = the serialized COSE key from step #13 above
     *        ii. Nonce = nonce created in step #7
     *       iii. session_id = the one computed in step #12
     *   15. Create an arc that encrypts the symmetric encryption key derived in step #10 with
     ...
     *   16. Create a `KeyWrappingKeyPackage` to be returned, containing the arc from step #15 as
     *       the `keyWrappingKey` and the arc from step #14 as the `keyWrappingKeyHandle`
     *
     * Note: If any of the enforcements marked as `CHECK` above fails, return the error:
     * ENFORCEMENTS_FAILED
     *
     ...
     */
    KeyWrappingKeyPackage createKeyWrappingKeyWithKeyExchange(
            in Arc primaryExchangePubKey, in Arc sharedKey);
     ...
     *     2. CHECK whether the input: `authKey` is an arc that is encrypted with the key shared
     *        between the sink and the source (i.e. the payload key of input 1) and has
     *        KeyType = AuthKey_w_SharedKey, "source_id" and "sink_id" in the protected headers.
     *     3. CHECK whether the "source_id"s and "sink_id"s in the protected headers of the two
     *        input arcs are equal.
     *     4. Create a 256-bit cryptographic key for AES-GCM encryption (key wrapping key).
     *     5. Create an arc that encrypts the key wrapping key with the sink's per-boot key and add
     *        the following in the protected headers:
     *            i. source_id = "source_id" from the input arcs
     *           ii. KeyType = KeyWrappingKeyGen
     *     6. Create an arc that encrypts the key wrapping key with the auth key and add the
     *        following in the protected headers:
     *            i. source_id = "source_id" from the input arcs
     *           ii. KeyType = KeyWrappingKey_w_AuthKey
     *          iii. sink_id = `Identity` of the sink
     ...
     *                  `sharedKey`)
     *
     * @return KeyWrappingKeyPackage - contains an ephemeral arc encrypting the key wrapping key to
     *                                 be used for encrypting the user's secret key material created
     *                                 at the sink and a persistent arc encrypting the key wrapping
     *                                 key to be used when unlocking the user's secret key material.
     */
     ...
     *        between the sink and the source (i.e. the payload key of input 1) and has
     *        KeyType = AuthKey_w_SharedKey, "source_id" and "sink_id" in the protected headers.
     *     3. CHECK whether `primaryExchangePrivKey` in the input: `keyWrappingKeyDeriveInfo` is an
     *        arc that is encrypted with the auth key (i.e. the payload key of input 2) and has
     *        KeyType = PrimaryExchangePrivKey_w_AuthKey, "source_id" and "sink_id" in the protected
     *        headers.
     *     4. CHECK whether the "source_id"s and the "sink_id"s in the protected headers of the
     *        above three arcs are equal.
     *     5. CHECK whether `secondaryExchangePubKey` in the input: `keyWrappingKeyDeriveInfo` is an
     *        arc that is encrypted with the sink's long term encryption key and has the protected
     *        headers: `PlainPubKeyEncoded` (with a serialized COSE key as the value), "Nonce" and
     *        "session_id"
     *     6. CHECK whether the headers of the COSE key has KeyType = SecondaryExchangePubKey
     *     7. Deserialize the public key from the COSE key
     *     8. Compute the Diffie-Hellman shared secret: Z, using the public key from step #7
     *        and the payload key from the arc: `primaryExchangePrivKey` in the input:
     *        `keyWrappingKeyDeriveInfo`, as the private key
     *     9. Extract a cryptographic secret S from Z (via HKDF-extract), using the nonce from the
     *        protected headers of the arc: `secondaryExchangePubKey` in the input:
     *        `keyWrappingKeyDeriveInfo`, as the salt
     *    10. Derive a symmetric encryption key of 256 bits from S with b"KEY_WRAPPING_KEY" as the
     *        context (via HKDF-expand)
     *    11. Derive a MAC key of 256 bits from S with b"KEY_WRAPPING_KEY_CONFIRMATION_KEY" as the
     *        context (via HKDF-expand)
     *    12. Compute "session_id" as the 256 bits HMAC over the nonce used in step #7 with
     *        the key derived in step #11
     *    13. CHECK whether the session_id computed in step #12 matches the session_id obtained from
     *        the protected headers of the arc: `secondaryExchangePubKey` in the input:
     *        `keyWrappingKeyDeriveInfo`, if not, return the error: KEY_CONFIRMATION_FAILED
     *    14. Create an arc that encrypts the symmetric encryption key derived in step #10 with
     *        the per-boot key of the sink. Add the following protected headers:
     *          i. source_id = "source_id" from the input arcs
     *         ii. challenge, if it was included in the protected headers of the `authKey` arc
     *        iii. KeyType = KeyWrappingKeyUse
     *    15. Create an arc that encrypts the key wrapping key with the auth key and add the
     *        following in the protected headers:
     *         i. source_id = "source_id" from the input arcs
     *        ii. KeyType = KeyWrappingKey_w_AuthKey
     *    16. Create a `KeyWrappingKeyPackage` to be returned, containing the arc from step #14 as
     *        the `keyWrappingKey` and the arc from step #15 as the `keyWrappingKeyHandle`
     *
     * Note: If any of the enforcements marked as `CHECK` above fails, return the error:
     * ENFORCEMENTS_FAILED
     *
     ...
     *                  `sharedKey`)
     *
     * @param keyWrappingKeyDeriveInfo - contains two persistent arcs, one encrypting the private
     *                                   key of the key pair created in `setupAuthKeyPair`, and the
     *                                   other authenticating the public key of the key pair created
     *                                   in `createKeyWrappingKeyWithKeyExchange`, in order to
     *                                   enable this method to re-derive the same key wrapping key
     *                                   that was derived in `createKeyWrappingKeyWithKeyExchange`.
     *
     * @return KeyWrappingKeyPackage - contains an ephemeral arc encrypting the key wrapping key to
     *                                 be used for decrypting the user's secret key material held
     *                                 at the sink and a persistent arc encrypting the key wrapping
     *                                 key to be used for unlocking the user's secret key material
     *                                 subsequently (i.e. without having to call this method again).
     */
    KeyWrappingKeyPackage recoverKeyWrappingKeyWithKeyExchange(
            in Arc sharedKey, in Arc authKey, in KeyWrappingKeyDeriveInfo keyWrappingKeyDeriveInfo);
     ...
     *        between the sink and the source (i.e. the payload key of input 1) and has
     *        KeyType = AuthKey_w_SharedKey, "source_id" and "sink_id" in the protected headers.
     *     3. CHECK whether the input: `keyWrappingKeyHandle` is an arc that is encrypted with the
     *        auth key (i.e. the payload key of input 2) and has KeyType = KeyWrappingKey_w_AuthKey,
     *        "source_id" and "sink_id" in the protected headers.
     *     4. CHECK whether the "source_id"s and the "sink_id"s in the protected headers of the
     *        above three arcs are equal.
     *     5. Create an arc that encrypts the key wrapping key (i.e. the payload key of input 3)
     *        with the sink's per-boot key and add the following in the protected headers:
     *            i. source_id = "source_id" from the input arcs
     *           ii. challenge, if it was included in the protected headers of the `authKey` arc
     *          iii. KeyType = KeyWrappingKeyUse
     *     6. Create a `KeyWrappingKeyPackage` to be returned, containing the arc from step #5 as
     *        the `keyWrappingKey` and none for the `keyWrappingKeyHandle`
     *
     *
     * Note: If any of the enforcements marked as `CHECK` above fails, return the error:
     * ENFORCEMENTS_FAILED
     ...
     *                  `sharedKey`)
     *
     * @param keyWrappingKeyHandle - an arc encrypting the key wrapping key with the auth key (i.e.
     *                               payload key in `authKey`)
     *
     * @return KeyWrappingKeyPackage - contains an ephemeral arc encrypting the key wrapping key to
     *                                 be used for decrypting the user's secret key material held
     *                                 at the sink
    ...
    KeyWrappingKeyPackage recoverKeyWrappingKey(
            in Arc sharedKey, in Arc authKey, in Arc keyWrappingKeyHandle);
}
