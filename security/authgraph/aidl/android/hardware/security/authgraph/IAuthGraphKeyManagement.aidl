...
/**
 * AuthGraph interface definition for cryptographically protecting one party's (P2 - sink) secrets
 * with a key held by the other party (P1 - source). Such cryptographically protected secrets at
 * the sink can be decrypted if any only if the key held by the source is unlocked. The mechanism of
 ...
 *     2. The user who wants to create secrets at the sink, that are protected by the source, should
 *        have setup a unique key at the source (called "source key"). The source key is encrypted
 *        a with secret known to the user and then with the source's long term encryption key.
 *
