...
interface IAuthGraphKeyManagement {
     ...
     *
     * @return KeyWrappingKeyPackage - contains the arc from step #13 as the `keyWrappingKey` and
     *                                 the arc from step #12 as the `keyWrappingKeyHandle`
     */
     ...
     * `createKeyWrappingKeyWithKeyExchange`, when the user's secret material that is encrypted with
     ...
     * `createKeyWrappingKey`, when the user's secret material that is encrypted with
     * that key needs to be unlocked, once the user's secret at the source is unlocked.
     */
