...
parcelable SessionInitiationInfo {
     ...
     *    the shared keys.
     * Analysis: if a man-in-the-middle wanted the two parties to use an older (vulnerable) version
     * of the protocol, they can invoke `init` with a version that is lower than the version
     ...
     * signature on the session id in `finish`, at which point the the protocol aborts. Therefore,
