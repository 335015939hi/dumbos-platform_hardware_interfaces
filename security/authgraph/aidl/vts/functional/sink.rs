...
pub fn test(
    source: &mut ke::AuthGraphParticipant,
    sink: binder::Strong<dyn IAuthGraphKeyExchange>,
) {
    test_mainline(source, sink.clone());
    test_corrupt_sig(source, sink.clone());
    test_corrupt_keys(source, sink);
}
...
pub fn test_mainline(
    ...
    let source_init_info = source.create().expect("failed to create() with local impl");
    ...
    let sink_verification_key = source
    ...
    let decrypted_shared_keys = source
