...
pub fn test(sink: binder::Strong<dyn IAuthGraphKeyExchange>) {
    test_mainline(sink.clone());
    test_corrupt_sig(sink.clone());
    test_corrupt_keys(sink);
}

/// Perform mainline AuthGraph key exchange with the provided sink.
pub fn test_mainline(sink: binder::Strong<dyn IAuthGraphKeyExchange>) {
    let mut impls = test_impls();

    // Step 1: create an ephemeral ECDH key at the (local) source.
    let source_init_info = ke::create(&mut impls).expect("failed to create() with local impl");
    ...
    let sink_verify_key = verify_key_from_identity(&impls, &sink_init_info.identity.identity);
    ...
    // The AuthGraph core library will verify the session ID signature, but do it here too.
    let source_verify_key = verify_key_from_identity(&impls, &source_init_info.identity);
    ke::verify_signature_on_session_id(
        &source_verify_key,
...
pub fn test_corrupt_sig(sink: binder::Strong<dyn IAuthGraphKeyExchange>) {
    let mut impls = test_impls();
...
pub fn test_corrupt_keys(sink: binder::Strong<dyn IAuthGraphKeyExchange>) {
    let mut impls = test_impls();
