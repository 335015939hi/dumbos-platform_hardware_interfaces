...
fn test_nonsecure_source_mainline() {
    ...
    let mut sink_ke = vts_km::test_agke_participant().expect("failed to create a local sink");
...
fn test_nonsecure_sink_mainline() {
    // First perform key exchange
    let mut source_ke = vts_km::test_agke_participant().expect("failed to create a local sink");
