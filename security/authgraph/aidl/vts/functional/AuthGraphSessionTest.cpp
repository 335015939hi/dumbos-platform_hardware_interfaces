  ...
  public:
    ...
    struct ReturnedError {
        ...
        friend bool operator==(const ReturnedError& lhs, const ReturnedError& rhs) {
            if (lhs.err_type != rhs.err_type) {
                return false;
            }
            switch (lhs.err_type) {
                case ErrorType::AIDL_ERROR:
                    if (lhs.err_val.aidl_error != rhs.err_val.aidl_error) {
                        return false;
                    }
                    break;
                case ErrorType::BINDER_ERROR:
                    if (lhs.err_val.binder_error != rhs.err_val.binder_error) {
                        return false;
                    }
                    break;
            }
    ...
    ReturnedError GetReturnError(const ::ndk::ScopedAStatus& result) {
        if (result.isOk()) {
            ReturnedError ok = {.err_type = ErrorType::AIDL_ERROR, .err_val.aidl_error = Error::OK};
            return ok;
        }
