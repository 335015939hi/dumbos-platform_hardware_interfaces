...
pub fn test(source: binder::Strong<dyn IAuthGraphKeyExchange>) {
    test_mainline(source.clone());
    test_corrupt_sig(source.clone());
    test_corrupt_key(source);
}

/// Perform mainline AuthGraph key exchange with the provided source.
pub fn test_mainline(source: binder::Strong<dyn IAuthGraphKeyExchange>) {
    ...
    let sink_verify_key = verify_key_from_identity(&impls, &sink_init_info.identity);
    ...
    // The AuthGraph core library will verify the session ID signature, but do it here too.
    let source_verify_key = verify_key_from_identity(&impls, &source_init_info.identity.identity);
    ke::verify_signature_on_session_id(
        &source_verify_key,
    ...
    let _sink_arcs = ke::authentication_complete(
        &mut impls,
        &source_info.signature.signature,
...
/// ID signature.
pub fn test_corrupt_sig(source: binder::Strong<dyn IAuthGraphKeyExchange>) {
    let mut impls = test_impls();
...
/// a corrupted key.
pub fn test_corrupt_key(source: binder::Strong<dyn IAuthGraphKeyExchange>) {
    let mut impls = test_impls();
