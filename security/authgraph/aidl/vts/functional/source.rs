...
pub fn test(
    sink: &mut ke::AuthGraphParticipant,
    source: binder::Strong<dyn IAuthGraphKeyExchange>,
) {
    test_mainline(sink, source.clone());
    test_corrupt_sig(sink, source.clone());
    test_corrupt_key(sink, source);
}
...
pub fn test_mainline(
    ...
    let sink_verification_key = key::Identity::from_slice(&sink_init_info.identity)
        ...
        .cert_chain
        .root_key;
    sink.verify_signature_on_session_id(
    ...
    // The AuthGraph core library will verify the session ID signature, but do it here too.
    let source_verification_key = sink
    ...
    let decrypted_shared_keys = sink
