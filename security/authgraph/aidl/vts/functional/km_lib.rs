...
pub struct AgkmTestContext{
    ...
    pub rotation_src_id: key::Identity,
    /// sink identity that will cause key rotation in the participant
    pub rotation_sink_id: key::Identity,
...
pub fn test_agkm_participant(source: bool) -> Result<AgkmTestContext, AgError> {
    ...
    let (_, self_id) = test_ids::test_id_for_self()
    ...
    let (_, rotation_src_id) = test_ids::test_id_for_source_key_rotation()
        .expect("source identity for key rotation should be created");
    let (_, rotation_sink_id) = test_ids::test_id_for_sink_key_rotation()
        .expect("sink identity for key rotation should be created");
    ...
    if source {
      ...
      dev.set_identity((src_pvt_key, src_id.clone()), iana::Algorithm::EdDSA);
      ...
      dev.set_identity((sink_pvt_key, sink_id.clone()), iana::Algorithm::EdDSA);
    }
    ...
    Ok(AgkmTestContext{
      agkm, self_id, src_id, sink_id, rotation_src_id, rotation_sink_id,cred_arc,authbound_arc_caps,
    })
}
