...
fn extract_plain_pub_key(pub_key: &Option<PubKey>) -> &PlainPubKey {
    match pub_key {
        ...
        Some(PubKey::SignedKey(_)) => panic!("expect unsigned public key from init()"),
        None => panic!("expect pubKey to be populated by sink on init()"),
    }
}

fn verify_key_from_identity(impls: &traits::TraitImpl, identity: &[u8]) -> key::EcVerifyKey {
