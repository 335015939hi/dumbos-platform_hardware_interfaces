        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                let rsp_data = match req_data[0] {
                    ...
                    SK_MESSAGE_PREFIX => sk_ta.borrow_mut().process(&req_data[1..]),
...
fn main() {
    ...
    // TODO: get rid of nested Arc/Mutex
    let service = SecretkeeperService::new_as_binder(
        Arc::new(Mutex::new(sk_channel)),
        Arc::new(Mutex::new(ag_channel)),
    );
