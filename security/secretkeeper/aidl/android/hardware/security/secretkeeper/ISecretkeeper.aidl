...
interface ISecretkeeper {
    /**
     * secretManagement method is used for interacting with the Secret Management API.
     *
     * Secret Management API: The clients can use this API to store (& get) 64 bytes of data.
     * The API is a CBOR based protocol, which follows request/response model.
     * See SecretManagement.cddl for the API spec.
     *
     * Further, the requests (from client) & responses (from service) must be wrapped into
     * ProtectedRequestPacket & ProtectedResponsePacket using symmetric keys agreed between
     * the client & service. For this, service (& client) must implement a key exchange protocol,
     * which is critical for establishing the secure channel.
     *
     ...
     *
     * @param ProtectedRequestPacket serialized into a byte array. See SecretManagement.cddl
     * @return ProtectedResponsePacket serialized into a byte array. See SecretManagement.cddl
     */
    byte[] secretManagement(in byte[] request);
}
