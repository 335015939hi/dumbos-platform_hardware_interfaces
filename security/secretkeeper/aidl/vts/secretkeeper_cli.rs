...
use secretkeeper_test::{
    diags::{diagnostic_bcc, Diagnostic, Indent},
    dice_sample::make_explicit_owned_dice,
    AUTHORITY_HASH, CONFIG_DESC, MODE, SECURITY_VERSION, SUBCOMPONENT_AUTHORITY_HASH,
...
fn main() -> Result<()> {
    ...
    if cli.dice {
        ...
        for line in dice.artifacts().diagnostic_indented(Indent(1), Indent(1)) {
            println!("{}", line);
        }
    }
    ...
    match cli.command {
        ...
        Command::Store(args) => {
            ...
            if cli.dice {
                ...
                for line in sealing_policy.diagnostic_indented(Indent(1), Indent(1)) {
                    println!("{}", line);
                }
            }
        ...
        Command::DumpBcc(args) => {
            let bcc = std::fs::read(args.filename).context("failed to read file")?;
            println!("BCC:");
            for line in
                diagnostic_bcc(&bcc, Indent(1)).map_err(|e| anyhow!("failed to parse BCC: {e}"))?
            {
                println!("{}", line);
            }
        }
