...
use dice_policy::{Constraint, ConstraintType, DicePolicy, NodeConstraints};
    ...
    fn diagnostic(&self) -> String {
        ...
        for (label, value) in &self.params {
            let (l, v) = match (label, value) {
                ...
                {
                    ("crv".to_string(), diag_curve(curve.clone().try_into().unwrap()))
                }
    ...
    fn diagnostic(&self) -> String {
        ...
        for (label, value) in &self.rest {
            let (l, v) = match (label, value) {
                ...
                (ClaimName::PrivateUse(crate::RKP_VM), Value::Null) => {
                    ("rkp_vm".to_string(), "true".to_string())
                }
...
fn diag_curve(curve: i64) -> String {
    match curve {
...
impl Diagnostic for Constraint {
    fn diagnostic(&self) -> String {
        let typ = match ConstraintType::n(self.0) {
            Some(ConstraintType::ExactMatch) => "ExactMatch".to_string(),
            Some(ConstraintType::GreaterOrEqual) => "GreaterOrEqual".to_string(),
            _ => format!("{}", self.0),
        };
        let path: Vec<String> = self.1.iter().map(|claim| diag_claim(*claim)).collect();
        format!("Constraint {{ type={}, path={:?}, value={} }}", typ, path, self.2.diagnostic())
    }
}
