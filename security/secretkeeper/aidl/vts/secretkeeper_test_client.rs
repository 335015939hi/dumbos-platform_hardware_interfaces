...
fn secretkeeper_check_identity(instance: String) {
    ...
    // failure arises from validation which happens in the local participant.
    let mut any_other_key = CoseKey::default();
    any_other_key.canonicalize(CborOrdering::Lexicographic);
    let session_res = SkClient::with_expected_sk_identity(&instance, any_other_key).unwrap_err();
    assert!(matches!(
        session_res,
