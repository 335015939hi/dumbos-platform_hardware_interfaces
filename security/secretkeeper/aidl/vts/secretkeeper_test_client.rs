...
fn secret_management_store_get_secret_found() {
    ...
    let get_request = GetSecretRequest{id: Id(ID_EXAMPLE), updated_sealing_policy: None};
...
fn secret_management_store_get_secret_not_found() {
    ...
    let get_request = GetSecretRequest{id: Id(ID_NOT_STORED), updated_sealing_policy: None};
    let get_request = get_request.serialize_to_packet().to_vec().unwrap();
