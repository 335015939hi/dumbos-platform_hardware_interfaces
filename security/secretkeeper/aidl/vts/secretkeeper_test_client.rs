...
use secretkeeper_test::{
    ...
    dice_sample::{make_explicit_owned_dice_for_uds, make_explicit_owned_dice, CDI_SIZE}
};
use std::fs;
use std::path::Path;
...
#[rdroidtest(get_instances())]
fn secretkeeper_get_secret_across_sameid_sessions(instance: String) {
...
#[rdroidtest(get_instances())]
fn secretkeeper_not_get_secret_across_diffid_sessions(instance: String) {
    // Store a secret with one session.  Note that we need to hang on to the
    // test client because it auto-deletes entries on drop.
    let mut sk_client1 = SkClient::new(&instance).unwrap();
    sk_client1.store(&ID_EXAMPLE, &SECRET_EXAMPLE).unwrap();
...
#[rdroidtest(get_instances())]
fn secretkeeper_many_sessions(instance: String) {
    const SESSION_COUNT: usize = 32;
    let mut sk_clients = Vec::new();
    for idx in 0..SESSION_COUNT {
        sk_clients
            .push(SkClient::new(&instance).expect(&format!("failed to establish session {idx}")));
    }
}
