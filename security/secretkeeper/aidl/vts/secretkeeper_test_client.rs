...
// This test checks that Secretkeeper uses the correct expected [`RequestSeqNum`] as aad while
...
fn secret_management_replay_protection_seq_num() {
    ...
    // Lets now layout with aad should be (in req & expected in resp)
    let mut seq_a = SeqNum::new();
    let (seq_0, seq_1, seq_2) = (
        seq_a.get_then_increment().unwrap(),
        seq_a.get_then_increment().unwrap(),
        seq_a.get_then_increment().unwrap(),
    );
...
// This test checks that Secretkeeper uses fresh [`RequestSeqNum`] for new sessions.
#[test]
