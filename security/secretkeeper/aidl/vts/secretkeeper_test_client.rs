...
macro_rules! setup_client {
    {} => {
        match SkClient::with_default_identity() {
            Some(sk) => sk,
            None => {
    ...
    {$dice_chain:ident} => {
        match SkClient::with_identity($dice_chain) {
            Some(sk) => sk,
            None => {
                warn!("Secretkeeper HAL is unavailable, skipping test");
                return;
...
impl From<secretkeeper_comm::wire::ApiError> for Error {
    fn from(e: secretkeeper_comm::wire::ApiError) -> Self {
        Self::InfraError(secretkeeper_client::Error::from(e))
    }
}
...
fn is_entry_not_found(e: Error) {
    if !matches!(e, Error::SecretkeeperError(SecretkeeperError::EntryNotFound)) {
...
fn secret_management_store_get_secret_not_found() {
    ...
    is_entry_not_found(sk_client.get(&ID_NOT_STORED).unwrap_err());
}
...
fn secretkeeper_store_delete_ids() {
    ...
    let res = sk_client.get(&ID_EXAMPLE);
    is_entry_not_found(res.unwrap_err());
    assert_eq!(sk_client.get(&ID_EXAMPLE_2).unwrap(), SECRET_EXAMPLE);

    sk_client.delete(&[&ID_EXAMPLE_2]);

    is_entry_not_found(sk_client.get(&ID_EXAMPLE).unwrap_err());
    is_entry_not_found(sk_client.get(&ID_EXAMPLE_2).unwrap_err());
}
...
fn secret_management_replay_protection_seq_num() {
    ...
    let sk_client = setup_client!(dice_chain);
...
#[test]
fn secret_management_policy_gate() {
    let dice_chain = make_explicit_owned_dice(/*Security version in a node */ 100);
    let mut sk_client = setup_client!(dice_chain);
    sk_client.store(&ID_EXAMPLE, &SECRET_EXAMPLE).unwrap();
    ...
    let mut sk_client_upgraded = setup_client!(dice_chain_upgraded);
    ...
    let mut sk_client_downgraded = setup_client!(dice_chain_downgraded);
    assert!(matches!(
