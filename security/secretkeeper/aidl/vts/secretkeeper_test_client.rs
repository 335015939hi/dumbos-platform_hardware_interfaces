...
impl SkClient {
    /// Create an `SkClient` using the default `ExplicitAndOwnedDiceArtifacts` for identity.
    ...
    /// Create an `SkClient` using the given `ExplicitAndOwnedDiceArtifacts` for identity.
    fn with_identity(dice_artifacts: ExplicitAndOwnedDiceArtifacts) -> Option<Self> {
    ...
    fn store(&mut self, id: &Id, secret: &Secret) {
        let sealing_policy =
            sealing_policy(self.dice_artifacts.dice_chain_in_explicit_key_format().unwrap());
        let store_request =
...
/// Construct a sealing policy on the dice chain. This method uses a the following set of
...
fn secret_management_replay_protection_seq_num() {
    ...
    let sealing_policy = sealing_policy(dice_chain.dice_chain_in_explicit_key_format().unwrap());
...
// Helper method that constructs 3 SecretManagement requests.
