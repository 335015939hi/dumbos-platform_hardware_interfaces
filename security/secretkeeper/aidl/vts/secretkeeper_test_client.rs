...
use dice_policy_builder::{ConstraintSpec, ConstraintType, MissingAction, policy_on_dice_chain};
...
fn sealing_policy(dice: &[u8]) -> Vec<u8> {
    ...
    policy_on_dice_chain(dice, &constraint_spec).unwrap().to_vec().unwrap()
}
