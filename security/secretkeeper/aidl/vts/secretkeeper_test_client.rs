...
fn secret_management_store_get_secret_found() {
    ...
    let store_request = StoreSecretRequest::new(
        Id(Box::new(ID_EXAMPLE)),
        Secret(Box::new(SECRET_EXAMPLE)),
    ...
    let get_request = GetSecretRequest::new(Id(Box::new(ID_EXAMPLE)), None);
...
fn secret_management_store_get_secret_not_found() {
    ...
    let get_request = GetSecretRequest::new(Id(Box::new(ID_NOT_STORED)), None);
    let get_request = get_request.serialize_to_packet().to_vec().unwrap();
