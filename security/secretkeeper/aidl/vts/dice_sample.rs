...
//! This module provides a set of sample DICE chain for testing purpose only. Note that this module
//! duplicates a large chunk of code in libdiced_sample_inputs. We avoid modifying the latter for
//! testing purpose because it is installed on device.
...
pub fn make_explicit_owned_dice(security_version: u64) -> ExplicitAndOwnedDiceArtifacts {
    let dice = make_sample_bcc_and_cdis(security_version);
    ExplicitAndOwnedDiceArtifacts::from(dice).unwrap()
}
