...
std::optional<std::string> AuthenticatedRequest::parse() {
    auto [parsedRequest, _, csrErrMsg] = cppbor::parse(mEncodedRequest);
    if (!parsedRequest) {
        return csrErrMsg;
    }
    ...
    mUdsCerts = std::move(*udsCerts);
    mDiceCertChain = std::move(*diceCertChain);
    mSignedData = std::move(*signedData);
...
ErrMsgOr<bytevec> AuthenticatedRequest::getUdsPubFromDiceChain_() {
    if (mDiceCertChain.size() == 0 || !mDiceCertChain.get(0)->asMap()) {
        return "AuthenticatedRequest DiceCertChain must be a non-empty Map";
    }

    auto udsPub = mDiceCertChain.get(0)->asMap()->encode();
    return udsPub;
}
...
std::optional<std::string> AuthenticatedRequest::validate() {
    ...
    auto encodedDiceCertChain = mDiceCertChain.encode();
    auto result = validateDiceCertChain(encodedDiceCertChain, *diceChainKind, mAllowAnyMode,
                                        mInstanceName);

    if (!result) {
        return result.message() + "\n" + prettyPrint(&mDiceCertChain);
    }
    auto [isProper, diceCertChainEntries] = *result;

    mIsProper = isProper;

    if (!mAllowDegenerate && !mIsProper) {
    ...
    if (mRequireUdsCerts && mUdsCerts.size() == 0) {
    ...
    auto error = validateUdsCerts(mUdsCerts, *udsPub);
    if (error) {
        return *error;
    }
    ...
    auto signedData = cppcose::verifyAndParseCoseSign1(&mSignedData, leafSigningKey, /*aad=*/{});
    if (!signedData) {
        return signedData.message();
    }

    auto csrPayload = parseAndValidateSignedData(*signedData, mChallenge);
    if (!csrPayload) {
        return csrPayload.message();
    }

    mCsrPayload = *csrPayload;
