...
inline const std::vector<uint8_t> kP256EekId{0x35, 0x73, 0xb7, 0x3f, 0xa0, 0x8a, 0x80, 0x89,
                                             0xb1, 0x26, 0x67, 0xe9, 0xcb, 0x7c, 0x75, 0xa1,
                                             0xaf, 0x02, 0x61, 0xfc, 0x6e, 0x65, 0x03, 0x91,
                                             0x3b, 0xd3, 0x4b, 0x7d, 0x14, 0x94, 0x3e, 0x46};
...
inline const std::vector<uint8_t> kLegacyCsrDiceChainRootPublicKey{
        0xa6, 0x01, 0x02, 0x03, 0x26, 0x04, 0x81, 0x02, 0x20, 0x01, 0x21, 0x58, 0x20, 0x45,
        0xdf, 0x69, 0x1d, 0x96, 0xd4, 0x27, 0xf6, 0xf8, 0x22, 0x57, 0x94, 0x18, 0x33, 0x7c,
        0x7e, 0xcf, 0x3e, 0x2c, 0x3c, 0x44, 0xbc, 0x6e, 0x95, 0x5f, 0x57, 0x64, 0xc3, 0x4b,
        0x02, 0x94, 0x5b, 0x22, 0x58, 0x20, 0x38, 0x8a, 0x66, 0xf3, 0x7d, 0x71, 0x9b, 0xa1,
        0xc1, 0x2a, 0xef, 0x84, 0xbb, 0x4d, 0x08, 0xb2, 0x66, 0x47, 0x75, 0x2e, 0x16, 0x3a,
        0x1d, 0x16, 0x67, 0x4c, 0x2e, 0x75, 0x2a, 0x81, 0x83, 0x20};
...
TEST(RemoteProvUtils, validateCsrV1Functionality) {
    ...
    auto diceChainPublicKeys = verifyProductionProtectedData(
            DeviceInfo{kLegacyCsrDeviceInfo}, keysToSign, kLegacyCsrKeysToSignMac,
            ProtectedData{kLegacyCsrProtectedData}, eekChain, kP256EekId, rpcHardwareInfo,
            DEFAULT_INSTANCE_NAME, kLegacyCsrChallenge);
    ASSERT_TRUE(diceChainPublicKeys) << diceChainPublicKeys.message();
    ASSERT_EQ(diceChainPublicKeys->size(), 1);
    ASSERT_EQ((*diceChainPublicKeys)[0].pubKey, kLegacyCsrDiceChainRootPublicKey)
            << "actual DICE chain root public key does not match expected value.";
}

TEST(RemoteProvUtils, validateCsrV1Error) {
    auto eekChain = EekChain{.chain = getProdEekChain(RpcHardwareInfo::CURVE_P256),
                             .last_pubkey = kP256EekPublic,
                             .last_privkey = kP256EekPrivate};
    auto rpcHardwareInfo = RpcHardwareInfo{.versionNumber = 2};

    // There should be two keys to sign.
    auto keysToSign = cppbor::Array();
    keysToSign.add(cppbor::EncodedItem(kLegacyCsrKeysToSign1));
    ...
    auto result = verifyProductionProtectedData(
            ...
            DEFAULT_INSTANCE_NAME, kLegacyCsrChallenge);
    ASSERT_FALSE(result);
    ASSERT_EQ(result.message(), "MAC tag mismatch");
}
