...
// The challenge that is in the CSR with UDS Certs and the CSR without UDS certs
...
TEST(RemoteProvUtilsTest, requireUdsCertsWhenPresent) {
    auto [keysToSignPtr, _, errMsg] = cppbor::parse(
            kKeysToSignForCsrWithUdsCerts.data(),
            kKeysToSignForCsrWithUdsCerts.data() + kKeysToSignForCsrWithUdsCerts.size());
    ...
    auto csr = verifyFactoryCsr(*keysToSign, kCsrWithUdsCerts, mockRpc.get(), kChallenge,
...
TEST(RemoteProvUtilsTest, requireUdsCertsWhenNotPresent) {
    ...
    auto csr = verifyFactoryCsr(/*keysToSign=*/Array(), kCsrWithoutUdsCerts, mockRpc.get(),
                                kChallenge, /*allowDegenerate=*/false, /*requireUdsCerts=*/true);
    ASSERT_FALSE(csr);
}
...
TEST(RemoteProvUtilsTest, dontRequireUdsCertsWhenNotPresent) {
    ...
    const auto* keysToSign = keysToSignPtr->asArray();
    auto csr = verifyFactoryCsr(*keysToSign, kCsrWithoutUdsCerts, mockRpc.get(), kChallenge,
                                /*allowDegenerate=*/false, /*requireUdsCerts=*/false);
    ASSERT_TRUE(csr) << csr.message();
}
