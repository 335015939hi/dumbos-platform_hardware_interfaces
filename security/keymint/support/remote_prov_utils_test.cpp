...
inline const std::vector<uint8_t> kCsrWithoutUdsCerts{
        0x85, 0x01, 0xa0, 0x82, 0xa4, 0x01, 0x01, 0x03, 0x27, 0x20, 0x06, 0x21, 0x58, 0x20, 0xca,
...
TEST(RemoteProvUtilsTest, checkModeOnLeafCertifcateInDiceChain) {
    auto hasNotNormalMode = hasNotNormalModeInDiceChain(kCsrWithDebugMode, DEFAULT_INSTANCE_NAME);
    ASSERT_TRUE(hasNotNormalMode) << hasNotNormalMode.message();
    ASSERT_TRUE(*hasNotNormalMode);

    auto hasNotNormalModeInDiceChain2 =
            hasNotNormalModeInDiceChain(kCsrWithUdsCerts, DEFAULT_INSTANCE_NAME);
    ASSERT_TRUE(hasNotNormalModeInDiceChain2) << hasNotNormalModeInDiceChain2.message();
    ASSERT_FALSE(*hasNotNormalModeInDiceChain2);
}
