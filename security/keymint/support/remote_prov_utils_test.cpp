...
inline const std::vector<uint8_t> kDegenerateDiceCertChain{
...
TEST(RemoteProvUtilsTest, validateDegenerateDiceCertChain) {
    auto result = validateDiceCertChain(kDegenerateDiceCertChain, hwtrust::DiceChain::Kind::kVsr16,
                                        /*allowAnyMode=*/false, DEFAULT_INSTANCE_NAME);
    ASSERT_TRUE(result);

    auto [isProper, bccContents] = *result;
    ASSERT_FALSE(isProper);
}
...
TEST(RemoteProvUtilsTest, compareUdsPubFromAuthenticatedRequests) {
    ...
    auto udsPub1 = getUdsPubFromDiceCertChain(kCsrWithSharedUdsRoot1, kChallenge, "default");
    ASSERT_TRUE(udsPub1) << "Error: " << udsPub1.message();

    auto udsPub2 = getUdsPubFromDiceCertChain(kCsrWithSharedUdsRoot2, kChallenge, "strongbox");
    ASSERT_TRUE(udsPub2) << "Error: " << udsPub2.message();

    auto udsPub3 = getUdsPubFromDiceCertChain(kCsrWithUdsCerts, kChallenge, "widevine");
