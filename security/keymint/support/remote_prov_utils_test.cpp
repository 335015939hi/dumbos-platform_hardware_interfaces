...
TEST(RemoteProvUtilsTest, checkModeOnLeafCertifcateInDiceChain) {
    auto isNormal = isNormalModeDiceChain(kCsrWithDebugMode, DEFAULT_INSTANCE_NAME);
    ASSERT_TRUE(isNormal) << isNormal.message();
    ASSERT_FALSE(*isNormal);

    auto isNormal2 = isNormalModeDiceChain(kCsrWithUdsCerts, DEFAULT_INSTANCE_NAME);
    ASSERT_TRUE(isNormal2) << isNormal2.message();
    ASSERT_TRUE(*isNormal2);
}
