...
class KeyMintAttestationFuzzer {
  ...
  private:
    void generateAttestKey(const AuthorizationSet& keyDesc,
                           const std::optional<AttestationKey>& attestKey, vector<uint8_t>* keyBlob,
    ...
    std::shared_ptr<IKeyMintDevice> mKeyMint = nullptr;
    FuzzedDataProvider mFdp;
};

void KeyMintAttestationFuzzer::generateAttestKey(const AuthorizationSet& keyDesc,
                                                 const std::optional<AttestationKey>& attestKey,
                                                 vector<uint8_t>* keyBlob,
                                                 vector<KeyCharacteristics>* keyCharacteristics,
                                                 vector<Certificate>* certChain) {
    AuthorizationSet keyDescPlusSign = keyDesc;
    keyDescPlusSign.push_back(TAG_PURPOSE, KeyPurpose::SIGN);
    generateKey(keyDescPlusSign, attestKey, keyBlob, keyCharacteristics, certChain);
}
...
KeyCreationResult KeyMintAttestationFuzzer::generateKey(
    ...
    AStatus result = mKeyMint->generateKey(keyDesc.vector_data(), attestKey, &creationResult);
...
void KeyMintAttestationFuzzer::process() {
    ...
    mKeyMint = std::move(IKeyMintDevice::fromBinder(binder));
    if (!mKeyMint) {
        return;
    }
    ...
    generateAttestKey(createAuthorizationSet(&mFdp), {}, &attestKey.keyBlob,
                      &attestKeyCharacteristics, &attestKeyCertChain);
    ...
    if (attestedKeyCertChain.size() > 0) {
        size_t idx = mFdp.ConsumeIntegralInRange<size_t>(0, attestedKeyCertChain.size() - 1);
        verifyAttestationRecord(attestedKeyCertChain[idx].encodedCertificate);
    }
}
