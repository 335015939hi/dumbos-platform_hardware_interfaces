...
class KeyMintRemoteProvSupport {
  public:
    KeyMintRemoteProvSupport(const uint8_t* data, size_t size) : mFdp(data, size) {}
    void process();
...
void KeyMintRemoteProvSupport::process() {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    KeyMintRemoteProvSupport kmRemoteProvSupport(data, size);
    kmRemoteProvSupport.process();
    return 0;
}
