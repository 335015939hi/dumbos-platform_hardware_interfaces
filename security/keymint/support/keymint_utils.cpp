...
uint32_t getPatchlevel(const char* patchlevel_str, bool include_days) {
    regex_t regex;
...
uint32_t getOsPatchlevel() {
    ...
    return getPatchlevel(patchlevel.c_str(), /* include_days = */ false);
}

uint32_t getVendorPatchlevel() {
    std::string patchlevel = wait_and_get_property(kVendorPatchlevelProp);
    return getPatchlevel(patchlevel.c_str(), /* include_days = */ true);
}
