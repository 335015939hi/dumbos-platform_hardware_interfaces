...
ErrMsgOr<std::vector<BccEntryData>> validateBcc(const cppbor::Array* bcc) {
    ...
    auto res = cert_request_validator::VerifyChain({encodedBcc.data(), encodedBcc.size()});
    if (!res.error.empty()) return (std::string)res.error;
    auto len = cert_request_validator::GetLength(*res.chain);
    std::vector<BccEntryData> result;
    for (auto i = 0; i < len; ++i) {
        auto key = cert_request_validator::GetPublicKey(*res.chain, i);
        result.push_back({bytevec(key.begin(), key.end())});
    }
    return result;
}
