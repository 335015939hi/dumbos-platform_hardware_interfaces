...
ErrMsgOr<std::tuple<std::vector<uint8_t>, std::vector<BccEntryData>>> validateDiceCertChain(
        const cppbor::Array* diceCertChain, const std::string& instanceName, bool allowAnyMode,
        bool allowDegenerate) {
...
ErrMsgOr<std::vector<uint8_t>> getUdsPubFromAuthenticatedRequest(
    ...
    auto result = validateDiceCertChain(authenticatedRequest->diceCertChain, instanceName,
                                        false /*allowAnyMode*/, false /*allowDegenerate*/);
    if (!result) {
        return result.message();
    }
...
ErrMsgOr<bytevec> parseAndValidateAuthenticatedRequest(const std::vector<uint8_t>& request,
    ...
    auto error = validateUdsCerts(*authenticatedRequest->udsCerts, udsPub);
    if (!error.empty()) {
        return error;
    }
    ...
    auto signedPayload =
            verifyAndParseCoseSign1(authenticatedRequest->signedData, kmDiceKey, {} /* aad */);
    if (!signedPayload) {
