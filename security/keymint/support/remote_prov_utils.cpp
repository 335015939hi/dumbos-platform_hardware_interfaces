...
ErrMsgOr<bool> isCsrWithProperDiceChain(const std::vector<uint8_t>& encodedCsr,
                                        const std::string& instanceName) {
    auto diceChain = getDiceChain(encodedCsr, false, true, instanceName);
    if (!diceChain) {
...
ErrMsgOr<bool> compareRootPublicKeysInDiceChains(const std::vector<uint8_t>& encodedCsr1,
    ...
    auto diceChain1 = getDiceChain(encodedCsr1, false, true, instanceName1);
    if (!diceChain1) {
    ...
    auto diceChain2 = getDiceChain(encodedCsr2, false, true, instanceName2);
    if (!diceChain2) {
...
ErrMsgOr<bool> verifyComponentNameInKeyMintDiceChain(const std::vector<uint8_t>& encodedCsr) {
    auto diceChain = getDiceChain(encodedCsr, false, true, DEFAULT_INSTANCE_NAME);
    if (!diceChain) {
        return diceChain.message();
    }
