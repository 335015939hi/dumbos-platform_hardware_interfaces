...
ErrMsgOr<std::unique_ptr<cppbor::Array>> parseAndValidateCsrPayload(
    ...
    if (signedKeys->encode() != keysToSign.encode()) {
        auto keys = signedKeys->encode();
        std::stringstream keySs;
        for (const auto& byte : keys) {
            keySs << "0x" << std::hex << std::setw(2) << std::setfill('0') << (int)byte << ", ";
        }

        auto otherKeys = keysToSign.encode();
        std::stringstream otherkeySs;
        for (const auto& byte : otherKeys) {
            otherkeySs << "0x" << std::hex << std::setw(2) << std::setfill('0') << (int)byte
                       << ", ";
        }
