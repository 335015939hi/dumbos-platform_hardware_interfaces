...
constexpr uint32_t kNumStrongBoxDeviceInfoEntries = 13;
...
ErrMsgOr<std::unique_ptr<cppbor::Map>> parseAndValidateDeviceInfo(
    ...
    const std::array<std::string, kNumTeeDeviceInfoEntries> kDeviceInfoKeys = {"brand",
                                                                               "manufacturer",
                                                                               "product",
                                                                               "model",
                                                                               "device",
    ...
    switch (info.versionNumber) {
        case 3:
            ...
            // StrongBox DeviceInfo has an optional entry.
            if (!isTeeDeviceInfo(*parsed) && (parsed->size() != kNumStrongBoxDeviceInfoEntries ||
                                              parsed->size() != kNumTeeDeviceInfoEntries)) {
                error += fmt::format(
                        "Err: Incorrect number of device info entries. Expected {} but got"
                        "{}\n",
                        kNumStrongBoxDeviceInfoEntries, parsed->size());
            }
            for (auto& [key, _] : *parsed) {
                if (std::find(kDeviceInfoKeys.begin(), kDeviceInfoKeys.end(),
                              key->asTstr()->value()) == kDeviceInfoKeys.end()) {
