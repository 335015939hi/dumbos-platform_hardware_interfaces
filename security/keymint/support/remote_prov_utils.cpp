...
ErrMsgOr<std::unique_ptr<cppbor::Array>> verifyCsr(
    ...
    if (challenge != *challengeInCsr) {
        return "challenges do not match";
    }
    ...
    if (encodedKeysToSign != *keysToSignInCsr) {
        return "KeysToSign do not match";
    }
    ...
    auto result = parseAndValidateDeviceInfo(*deviceInfo, provisionable, isFactory);
    if (!result) {
