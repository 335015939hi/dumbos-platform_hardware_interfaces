...
ErrMsgOr<bool> compareRootPublicKeysInDiceChains(const std::vector<uint8_t>& encodedCsr1,
                                                 const std::string& instanceName1,
                                                 const std::vector<uint8_t>& encodedCsr2,
                                                 const std::string& instanceName2) {
    auto diceChainKind = getDiceChainKind();
    if (!diceChainKind) {
        return diceChainKind.message();
    }
    ...
    if (!proper1) {
        return instanceName1 + " has a degenerate DICE chain:\n" + hexlify(encodedCsr1);
    }
    ...
    if (!proper2) {
        return instanceName2 + " has a degenerate DICE chain:\n" + hexlify(encodedCsr2);
    }
