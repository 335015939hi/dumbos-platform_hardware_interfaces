...
hwtrust::DiceChain::RkpInstance toRkpInstance(const std::string& instanceName) {
    if (DEFAULT_INSTANCE_NAME == instanceName) {
        return hwtrust::DiceChain::RkpInstance::kDefault;
    } else if (RKPVM_INSTANCE_NAME == instanceName) {
        return hwtrust::DiceChain::RkpInstance::kAvf;
    } else {
        // TODO(b/314128697): Assert that the instance name is for strongbox.
        return hwtrust::DiceChain::RkpInstance::kStrongbox;
    }
}
...
ErrMsgOr<std::vector<BccEntryData>> validateBcc(const cppbor::Array* bcc,
    ...
    auto chain =
            hwtrust::DiceChain::Verify(encodedBcc, kind, allowAnyMode, toRkpInstance(instanceName));
...
ErrMsgOr<std::unique_ptr<cppbor::Array>> verifyCsr(const cppbor::Array& keysToSign,
    ...
    bool isRkpVmChain = RKPVM_INSTANCE_NAME == instanceName;
...
ErrMsgOr<bool> isCsrWithProperDiceChain(const std::vector<uint8_t>& csr,
    ...
    auto chain = hwtrust::DiceChain::Verify(encodedDiceChain, *diceChainKind,
                                            /*allowAnyMode=*/false, toRkpInstance(instanceName));
    if (!chain.ok()) return chain.error().message();
