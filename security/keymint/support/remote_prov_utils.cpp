...
bytevec getProdEekChain(uint32_t curve) {
    bytevec prodEek;
    prodEek.reserve(1 +
                    std::max(sizeof(kCoseEncodedRootCert25519) + sizeof(kCoseEncodedGeekCert25519),
    ...
    if (curve == RpcHardwareInfo::CURVE_25519) {
        prodEek.insert(prodEek.end(), std::begin(kCoseEncodedRootCert25519),
                       std::end(kCoseEncodedRootCert25519));
        prodEek.insert(prodEek.end(), std::begin(kCoseEncodedGeekCert25519),
                       std::end(kCoseEncodedGeekCert25519));
    } else {
