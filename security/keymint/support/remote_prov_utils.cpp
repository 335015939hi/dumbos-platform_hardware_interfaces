...
bytevec getProdEekChain(int32_t supportedEekCurve) {
    ...
    if (supportedEekCurve == RpcHardwareInfo::CURVE_P256) {
        chain.add(cppbor::EncodedItem(
            bytevec(std::begin(kCoseEncodedEcdsaRootCert), std::end(kCoseEncodedEcdsaRootCert))));
        chain.add(cppbor::EncodedItem(
            bytevec(std::begin(kCoseEncodedEcdsaGeekCert), std::end(kCoseEncodedEcdsaGeekCert))));
    } else {
        chain.add(cppbor::EncodedItem(
