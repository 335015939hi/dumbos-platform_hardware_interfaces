...
typedef struct km_auth_list {
    ...
    ASN1_INTEGER_SET* user_secure_id;
...
ASN1_SEQUENCE(KM_AUTH_LIST) = {
        ...
        ASN1_EXP_SET_OF_OPT(KM_AUTH_LIST, user_secure_id, ASN1_INTEGER,
                            TAG_USER_SECURE_ID.maskedTag()),
...
template <Tag tag>
void copyAuthTag(const stack_st_ASN1_INTEGER* stack, TypedTag<TagType::ULONG_REP, tag> ttag,
                 AuthorizationSet* auth_list) {
    typedef typename TypedTag2ValueType<decltype(ttag)>::type ValueT;
    for (size_t i = 0; i < sk_ASN1_INTEGER_num(stack); ++i) {
        auth_list->push_back(
...
static ErrorCode extract_auth_list(const KM_AUTH_LIST* record, AuthorizationSet* auth_list) {
    ...
    copyAuthTag(record->user_secure_id, TAG_USER_SECURE_ID, auth_list);
