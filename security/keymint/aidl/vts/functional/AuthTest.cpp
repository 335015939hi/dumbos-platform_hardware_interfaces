...
// Test use of a key with multiple USER_SECURE_ID values.
TEST_P(AuthTest, TimeoutAuthenticationMultiSid) {
    ...
    // Create an AES key that requires authentication within the last 3 seconds from either secure
    // ID. Also allow any authenticator type.
    const uint32_t timeout_secs = 3;
    auto builder = AuthorizationSetBuilder()
                           .AesEncryptionKey(256)
                           .BlockMode(BlockMode::ECB)
    ...
    const string alt_ciphertext =
            EncryptMessage(keyblob, message, params, &out_params, alt_hat.value());
    const string alt_plaintext = DecryptMessage(keyblob, alt_ciphertext, params, alt_hat.value());
    EXPECT_EQ(message, alt_plaintext);
}
