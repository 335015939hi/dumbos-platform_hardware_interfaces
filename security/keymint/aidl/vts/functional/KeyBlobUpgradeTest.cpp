...
// 4) Run the "*After*" subset of these tests with the `--keyblob_dir <dir>` command-line argument
...
//
//    All tests should pass, and the `UpgradeKeyBlobs` test should only indicate that no keyblob
//    upgrade was needed if the patchlevels of the new environment are the same as before.
...
// with:
//    adb pull /data/local/tmp/keymint-blobs/
TEST_P(KeyBlobUpgradeTest, UpgradeKeyBlobsBeforeOrAfter) {
...
TEST_P(KeyBlobUpgradeTest, DeleteRRKeyBlobsAfter) {
    ...
    for (std::string name : keyblob_names(SecLevel())) {
        for (bool with_hidden : {false, true}) {
            if (!requires_rr(name)) {
                std::cout << "Skipping keyblob file  '" << name
            ...
            std::cout << "Deleting files for deleted key '" << name << ";";
