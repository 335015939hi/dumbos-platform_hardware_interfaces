...
std::vector<std::string> keyblob_names = {
        "aes-key",     "des-key",    "hmac-key",       "rsa-key",         "p256-key",
        "ed25519-key", "x25519-key", "rsa-attest-key", "p256-attest-key", "ed25519-attest-key"};
...
TEST_P(KeyBlobUpgradeTest, CreateKeyBlobs) {
    ...
    vector<std::pair<const std::string, AuthorizationSetBuilder>> keys_info = {
    ...
    for (const auto& info : keys_info) {
        for (bool with_hidden : {false, true}) {
            ...
            ASSERT_EQ(ErrorCode::OK, GenerateKey(builder, &keyblob, &key_characteristics))
                    << " failed for " << name;
