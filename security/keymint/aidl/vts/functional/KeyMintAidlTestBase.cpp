...
void verify_serial(X509* cert, const uint64_t expected_serial) {
    uint64_t serial = ASN1_INTEGER_get(X509_get_serialNumber(cert));
...
vector<uint8_t> set_serial_blob(uint64_t serial_int) {
    BIGNUM* serial = BN_new();
    BN_set_u64(serial, serial_int);

    int len = BN_num_bytes(serial);
    vector<uint8_t> serial_blob(len);
    if (BN_bn2bin(serial, serial_blob.data()) != len) return {};

    return move(serial_blob);
}

void verify_subject_and_serial(const Certificate certificate,   //
                               const uint64_t expected_serial,  //
                               string subject, bool self_signed) {
