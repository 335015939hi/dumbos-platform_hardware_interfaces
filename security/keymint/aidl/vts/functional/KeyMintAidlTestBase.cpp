...
ErrorCode KeyMintAidlTestBase::GenerateKey(const AuthorizationSet& key_desc,
    ...
    ByteArray blob;
    Status result =
            keymint_->generateKey(key_desc.vector_data(), &blob, key_characteristics, &certChain_);

    if (result.isOk()) {
        EXPECT_PRED2(KeyCharacteristicsBasicallyValid, SecLevel(), *key_characteristics);
        EXPECT_GT(blob.data.size(), 0);
        *key_blob = std::move(blob.data);
    }
...
ErrorCode KeyMintAidlTestBase::ImportWrappedKey(string wrapped_key, string wrapping_key,
    ...
    Status result = keymint_->importWrappedKey(
            ...
            unwrapping_params.vector_data(), 0 /* passwordSid */, 0 /* biometricSid */, &blob,
            &key_characteristics_);

    if (result.isOk()) {
        EXPECT_PRED2(KeyCharacteristicsBasicallyValid, SecLevel(), key_characteristics_);
        EXPECT_GT(blob.data.size(), 0);
        key_blob_ = std::move(blob.data);
    }
