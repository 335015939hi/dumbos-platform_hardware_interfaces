...
/*
 * NewKeyGenerationTest.EcdsaAttestationUniqueIdNoDateFail
 *
 * Verifies that attempting to create an attested ECDSA key with a UNIQUE_ID but
 * without a CREATION_DATETIME provided fails.
 */
TEST_P(NewKeyGenerationTest, EcdsaAttestationUniqueIdNoDateFail) {
    auto app_id = "foo";
    auto challenge = "hello";
    auto subject = "cert subj 2";
    vector<uint8_t> subject_der(make_name_from_str(subject));
    ...
    const AuthorizationSetBuilder builder =
            AuthorizationSetBuilder()
                    ...
                    .SetDefaultValidity();
    auto result = GenerateKey(builder);
    ASSERT_TRUE(result == ErrorCode::INVALID_KEY_BLOB || result == ErrorCode::INVALID_ARGUMENT);
}
