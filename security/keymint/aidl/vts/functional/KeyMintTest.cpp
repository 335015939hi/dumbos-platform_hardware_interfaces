...
/*
 * NewKeyGenerationTest.RsaAttestationSelfSign
 *
...
￼* works as expected.
 */
TEST_P(NewKeyGenerationTest, RsaAttestationSelfSign) {
    for (auto key_size : ValidKeySizes(Algorithm::RSA)) {
        vector<uint8_t> key_blob;
        vector<KeyCharacteristics> key_characteristics;
        ASSERT_EQ(ErrorCode::OK, GenerateKey(AuthorizationSetBuilder()
 ...
 *
 * Verifies that for Hmac key generation, no attestation will be generated even
 * if challenge and app id are provided.
 */
