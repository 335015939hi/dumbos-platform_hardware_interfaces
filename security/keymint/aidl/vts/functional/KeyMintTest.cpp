...
bool verify_chain(const vector<Certificate>& chain) {
    for (size_t i = 0; i < chain.size(); ++i) {
        X509_Ptr key_cert(parse_cert_blob(chain[i].certificate));
        X509_Ptr signing_cert;
        if (i < chain.size() - 1) {
            signing_cert.reset(parse_cert_blob(chain[i + 1].certificate));
        } else {
            signing_cert.reset(parse_cert_blob(chain[i].certificate));
        }
        ...
        if (dump_Attestations) std::cout << bin2hex(chain[i].certificate) << std::endl;
    }
...
TEST_P(NewKeyGenerationTest, RsaWithAttestation) {
    for (auto key_size : ValidKeySizes(Algorithm::RSA)) {
        ...
        ASSERT_TRUE(cert_chain_);
        EXPECT_TRUE(verify_chain(cert_chain_->chain));
        ASSERT_GT(cert_chain_->chain.size(), 0);
        EXPECT_TRUE(verify_attestation_record(challenge, app_id,                     //
                                              key_characteristics.softwareEnforced,  //
                                              key_characteristics.hardwareEnforced,  //
                                              SecLevel(), cert_chain_->chain[0].certificate));
