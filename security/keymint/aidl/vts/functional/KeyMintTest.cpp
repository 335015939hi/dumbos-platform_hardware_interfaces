...
class NewKeyGenerationTest : public KeyMintAidlTestBase {
  protected:
    ...
    AuthorizationSet CheckCommonParams(const vector<KeyCharacteristics>& keyCharacteristics) {
        ...
        // Must include vendor and boot patchlevels.
        /* TODO(drysdale): either fix the keymint provider or the spec definition
        (IkeyMintDevice.aidl L261) auto vendor_pl = auths.GetTagValue(TAG_VENDOR_PATCHLEVEL);
...
/*
 * NewKeyGenerationTest.AesWithAttestation
 *
 * Verifies that keymint can generate all required AES key sizes, and that the resulting keys
 * have correct characteristics.
 *
 ...
 */
TEST_P(NewKeyGenerationTest, AesWithAttestation) {
    for (auto key_size : ValidKeySizes(Algorithm::AES)) {
        for (auto block_mode : ValidBlockModes(Algorithm::AES)) {
            for (auto padding_mode : ValidPaddingModes(Algorithm::AES, block_mode)) {
                SCOPED_TRACE(testing::Message()
 ...
 *
 * Verifies that specifying an invalid padding on AES key generation returns
 * OK, but that subsequent operations fail.
 */
    ...
    for (auto key_size : ValidKeySizes(Algorithm::AES)) {
        for (auto block_mode : ValidBlockModes(Algorithm::AES)) {
            for (auto padding_mode : InvalidPaddingModes(Algorithm::AES, block_mode)) {
                ...
                // Key creation is OK.
                // TODO(drysdale): should the interface specify that GenerateKey should fail in this
                // case, so that it's not possible to generate an unusable key?
                ASSERT_EQ(ErrorCode::OK, GenerateKey(builder, &key_blob, &key_characteristics));

                // But attempting to begin an operation fails.
 ...
 *
 * Verifies that importing and using an 3DES key works.
 */
