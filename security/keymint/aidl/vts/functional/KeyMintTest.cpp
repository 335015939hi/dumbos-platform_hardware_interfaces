...
TEST_P(NewKeyGenerationTest, EcdsaAttestationUniqueId) {
    uint64_t cert_date = cert_date;  // Wed Apr 28 14:54:08 2021 in ms since epoch
    auto challenge = "hello";
    auto app_id = "foo";
    auto subject = "cert subj 2";
    vector<uint8_t> subject_der(make_name_from_str(subject));
    ...
    const AuthorizationSetBuilder base_builder =
            AuthorizationSetBuilder()
                    ...
                    .SetDefaultValidity();
    AuthorizationSetBuilder builder = base_builder;
    builder.AttestationApplicationId(app_id).Authorization(TAG_CREATION_DATETIME, cert_date);
    ...
    {
        // 1_619_621_648_000 / 2_592_000_000 is 624.85. Anything in the range
        // [624 * 2_592_000_000 = 1_617_408_000_000, 625 * 2_592_000_000 = 1_620_000_000_000)
        // should produce the same UNIQUE_ID.
        AuthorizationSetBuilder builder = base_builder;
        builder.AttestationApplicationId(app_id).Authorization(TAG_CREATION_DATETIME,
                                                               cert_date + 1000000);
        ASSERT_EQ(ErrorCode::OK, GenerateKey(builder));
        ASSERT_GT(key_blob_.size(), 0U);
        ASSERT_GT(cert_chain_.size(), 0);
    ...
    // A different attestation application ID should yield a different unique ID.
    {
        ...
        builder.AttestationApplicationId(app_id2).Authorization(TAG_CREATION_DATETIME, cert_date);
        ...
        EXPECT_TRUE(verify_attestation_record(challenge, app_id2, sw_enforced, hw_enforced,
                                              SecLevel(), cert_chain_[0].encodedCertificate,
                                              &unique_id3));
