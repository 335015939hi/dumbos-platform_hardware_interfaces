...
TEST_P(ModuleHashTest, SetSameValue) {
    ...
    auto mod_hash = module_hash();
    ASSERT_TRUE(mod_hash.has_value());

    // Setting the same value that's already there should work.
    vector<KeyParameter> info = {Authorization(TAG_MODULE_HASH, mod_hash.value())};
...
TEST_P(ModuleHashTest, SetDifferentValue) {
    ...
    auto mod_hash = module_hash();
    ASSERT_TRUE(mod_hash.has_value());
    vector<uint8_t> wrong_hash = mod_hash.value();
