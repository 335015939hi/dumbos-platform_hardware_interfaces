...
TEST_P(NewKeyGenerationTest, LimitedUsageRsa) {
    for (auto key_size : ValidKeySizes(Algorithm::RSA)) {
        ...
        // The usage count limit tag may appear in the hardware enforced list if IKeyMintDevice
        // implement this feature.
        AuthorizationSet software_auths(key_characteristics.softwareEnforced);
        AuthorizationSet hardware_auths(key_characteristics.hardwareEnforced);
        EXPECT_TRUE(software_auths.Contains(TAG_USAGE_COUNT_LIMIT, 1U) ||
                    hardware_auths.Contains(TAG_USAGE_COUNT_LIMIT, 1U))
...
TEST_P(NewKeyGenerationTest, LimitedUsageHmac) {
    for (auto digest : ValidDigests(false /* withNone */, true /* withMD5 */)) {
        ...
        // The usage count limit tag may appear in the hardware enforced list if IKeyMintDevice
        // implement this feature.
        EXPECT_TRUE(hardwareEnforced.Contains(TAG_USAGE_COUNT_LIMIT, 1U) ||
                    softwareEnforced.Contains(TAG_USAGE_COUNT_LIMIT, 1U))
                << "key usage count limit " << 1U << " missing";
...
TEST_P(UsageCountLimitTest, TestLimitAes) {
    ...
    AuthorizationSet software_auths(key_characteristics_.softwareEnforced);
    AuthorizationSet hardware_auths(key_characteristics_.hardwareEnforced);
    EXPECT_TRUE(software_auths.Contains(TAG_USAGE_COUNT_LIMIT, 1U) ||
                hardware_auths.Contains(TAG_USAGE_COUNT_LIMIT, 1U))
            << "key usage count limit " << 1U << " missing";
