...
/*
 * VerificationOperationsTest.HmacVerificationFailsForCurruptSignature
 *
 * Verifies HMAC signature verification should fails if message or signature is currupted.
 */
TEST_P(VerificationOperationsTest, HmacVerificationFailsForCurruptSignature) {
    string key_material = "HelloThisIsAKey";
    ...
    string curruptMessage = "This is b message.";  // Currupted message
    string output;
    EXPECT_EQ(ErrorCode::VERIFICATION_FAILED, Finish(curruptMessage, signature, &output));

    ASSERT_EQ(ErrorCode::OK,
              Begin(KeyPurpose::VERIFY, verification_key,
                    AuthorizationSetBuilder().Digest(Digest::SHA_2_256), &begin_out_params));

    signature[0] += 1;  // Currupt a signature
