...
ErrMsgOr<MacedPublicKey> corrupt_maced_key(const MacedPublicKey& macedPubKey) {
    ...
    auto unprotParams = coseMac0->asArray()->get(kCoseMac0UnprotectedParams)->asBstr();
    auto payload = coseMac0->asArray()->get(kCoseMac0Payload)->asBstr();
...
TEST_P(CertificateRequestTest, NonEmptyRequestCorruptMac_testMode) {
    ...
    bytevec keysToSignMac;
    ProtectedData protectedData;
    auto status = provisionable_->generateCertificateRequest(testMode, {keyWithCorruptMac},
                                                             eekChain_.chain, challenge_,
                                                             &keysToSignMac, &protectedData);
...
TEST_P(CertificateRequestTest, NonEmptyRequest_testKeyInProdCert) {
    ...
    bytevec keysToSignMac;
    ProtectedData protectedData;
    auto status = provisionable_->generateCertificateRequest(false /* testMode */, keysToSign_,
                                                             eekChain_.chain, challenge_,
                                                             &keysToSignMac, &protectedData);
    ASSERT_FALSE(status.isOk());
    ASSERT_EQ(status.getServiceSpecificError(),
