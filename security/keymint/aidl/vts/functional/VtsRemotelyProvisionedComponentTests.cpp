...
/**
 * Verify implementation supports at least MAX_NUM_KEYS_IN_CSR_FLOOR keys in a CSR.
 */
TEST_P(GetHardwareInfoTests, maxNumKeysInCsr) {
    RpcHardwareInfo hwInfo;
    ASSERT_TRUE(provisionable_->getHardwareInfo(&hwInfo).isOk());
    ASSERT_GE(hwInfo.maxNumKeysInCsr, RpcHardwareInfo::MAX_NUM_KEYS_IN_CSR_FLOOR);
}
...
TEST_P(CertificateRequestTest, NonEmptyRequest_testMode) {
    bool testMode = true;
    generateKeys(testMode, rpcHardwareInfo.maxNumKeysInCsr /* numKeys */);
...
TEST_P(CertificateRequestV2Test, NonEmptyRequestMultipleKeys) {
    generateKeys(false /* testMode */, rpcHardwareInfo.maxNumKeysInCsr /* numKeys */);
