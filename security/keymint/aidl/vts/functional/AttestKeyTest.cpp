 ...
 */
string get_imei() {
    string output = exec_command(TELEPHONY_CMD_GET_IMEI);

    if (output.empty()) {
        LOG(ERROR) << "Command failed. Cmd: " << TELEPHONY_CMD_GET_IMEI;
        return "";
    }
    ...
    if (out.size() != 1) {
        LOG(ERROR) << "Error in parsing the command output. Cmd: " << TELEPHONY_CMD_GET_IMEI;
        return "";
    }
...
TEST_P(AttestKeyTest, EcdsaAttestationID) {
    ...
    ASSERT_GT(imei.size(), 0);
    auto attestation_id_tags = AuthorizationSetBuilder().Authorization(TAG_ATTESTATION_ID_IMEI,
                                                                       imei.data(), imei.size());
