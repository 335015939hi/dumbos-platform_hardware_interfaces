...
TEST_P(AttestKeyTest, EcdsaAttestationID) {
    ...
    if (AidlVersion() >= 3 && SecLevel() != SecurityLevel::STRONGBOX) {
        ...
        if (!second_imei.empty() && second_imei.compare("null") != 0) {
            // Make sure first IMEI also exists.
            EXPECT_FALSE(imei.empty());
    ...
    for (const KeyParameter& tag : attestation_id_tags) {
        ...
        if (tag.tag == TAG_ATTESTATION_ID_SECOND_IMEI) {
            if (!imei.empty()) {
                ...
                imei_tag = Authorization(TAG_ATTESTATION_ID_IMEI, imei_blob);
        ...
        // If first IMEI exists, then include it along with second IMEI in expected auth list.
        if (tag.tag == TAG_ATTESTATION_ID_SECOND_IMEI) {
            ASSERT_EQ(imei_tag.tag, TAG_ATTESTATION_ID_IMEI);
...
TEST_P(AttestKeyTest, SecondIMEIAttestationIDWithoutFirstIMEIFails) {
    if (is_gsi_image()) {
        // GSI sets up a standard set of device identifiers that may not match
        // the device identifiers held by the device.
        GTEST_SKIP() << "Test not applicable under GSI";
    ...
    if (AidlVersion() < 3 || SecLevel() == SecurityLevel::STRONGBOX) {
        GTEST_SKIP() << "Test not applicable for KeyMint-Version <= 2 or strongbox sec level";
    }
    ...
    auto result = GenerateKey(builder, attest_key, &attested_key_blob,
                              &attested_key_characteristics, &attested_key_cert_chain);
    ASSERT_TRUE(result == ErrorCode::CANNOT_ATTEST_IDS) << "result = " << result;
    device_id_attestation_vsr_check(result);
