 ...
 *
 * This test creates self signed RSA attestation keys of various sizes, and varify it can be
...
TEST_P(AttestKeyTest, AllRsaSizes) {
    for (auto size : ValidKeySizes(Algorithm::RSA)) {
        ...
        EXPECT_TRUE(verify_attestation_record("foo2", "bar2", sw_enforced2, hw_enforced2,
                                              SecLevel(),
                                              attested_key_cert_chain[0].encodedCertificate));
 ...
 *
 * This test creates RSA attestation key signed by factory keys, and varify it can be
 * used to sign other RSA and EC keys.
 */
TEST_P(AttestKeyTest, RsaAttestedAttestKeys) {
     ...
     */
    attest_key.issuerSubjectName = make_name_from_str("cert subj 2");
    vector<uint8_t> attested_key_blob;
    vector<KeyCharacteristics> attested_key_characteristics;
    vector<Certificate> attested_key_cert_chain;
    EXPECT_EQ(ErrorCode::OK,
    ...
    verify_subject_and_serial(attested_key_cert_chain[0], serial_int, subject, false);
}
...
TEST_P(AttestKeyTest, RsaAttestKeyChaining) {
    auto subject = "attest key chaining";
    vector<uint8_t> subject_der(make_name_from_str(subject));

    uint64_t serial_int = 78;
    vector<uint8_t> serial_blob(build_serial_blob(serial_int));

    vector<vector<uint8_t>> key_blob_list(6);
    vector<vector<Certificate>> cert_chain_list(6);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i > 0) {
            attest_key.issuerSubjectName = make_name_from_str("attest key chaining");
        ...
        EXPECT_EQ(ErrorCode::OK,
                  GenerateKey(AuthorizationSetBuilder()
                                      ...
                                      .Authorization(TAG_CERTIFICATE_SUBJECT, subject_der)
                                      .SetDefaultValidity(),
                              attest_key, &key_blob_list[i], &attested_key_characteristics,
                              &cert_chain_list[i]));
...
TEST_P(AttestKeyTest, EcAttestKeyChaining) {
    auto subject = "Ec attest key chaining";
    vector<uint8_t> subject_der(make_name_from_str(subject));
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i > 0) {
            attest_key.issuerSubjectName = make_name_from_str("Ec attest key chaining");
            attest_key.keyBlob = key_blob_list[i - 1];
        } else {
            attest_key = {};
        }
...
TEST_P(AttestKeyTest, AlternateAttestKeyChaining) {
    auto subject = "alt attest key chaining";
    vector<uint8_t> subject_der(make_name_from_str(subject));
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i > 0) {
            attest_key.issuerSubjectName = make_name_from_str("alt attest key chaining");
            attest_key.keyBlob = key_blob_list[i - 1];
        } else {
            attest_key = {};
        }
