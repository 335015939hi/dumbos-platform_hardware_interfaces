...
static bool isValidSBKeySize(string transform, int keySize) {
    ...
    case Algorithm::AES:
        return (keySize == 128 || keySize == 256) ? true : false;

    case Algorithm::HMAC:
        return (keySize % 8 == 0 && keySize >= 64 && keySize <= 512) ? true : false;

    case Algorithm::TRIPLE_DES:
        return (keySize == 168) ? true : false;

    case Algorithm::RSA:
        return (keySize == 2048) ? true : false;

    case Algorithm::EC:
        return (keySize == 256) ? true : false;
    }
    return false;
}
