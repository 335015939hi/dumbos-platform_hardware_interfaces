...
class KeyMintBenchmarkTest {
  public:
     ...
     * Require return value to skip the benchmark test case from further execution incase
...
static void keygen(benchmark::State& state, string transform, int keySize) {
    ...
    if (keymintTest->securityLevel_ == SecurityLevel::STRONGBOX &&
        ...
        state.SkipWithError(("Skipped for STRONGBOX: Keysize: " + std::to_string(keySize) + " is not supported in StrongBox for algorithm: " +
            keymintTest->getAlgorithmString(transform)).c_str());
        return;
    }
...
static void sign(benchmark::State& state, string transform, int keySize, int msgSize) {
    ...
    if (keymintTest->securityLevel_ == SecurityLevel::STRONGBOX) {
        ...
        if (keymintTest->getDigest(transform) != Digest::SHA_2_256) {
            state.SkipWithError(("Skipped for STRONGBOX: Digest: " + keymintTest->getDigestString(transform) +
                " is not supported in StrongBox").c_str());
            return;
        }
    }
    addDefaultLabel(state);
