...
enum Tag {
     ...
     *
     *    R is 1 if Tag::RESET_SINCE_ID_ROTATION was provided to attested key generation or 0 if
     *      the tag was not provided.
     *
     ...
     * generation methods, which must place the value in the KeyDescription SEQUENCE of the
     ...
     * only with attested key generation operations.
     *
     ...
     * Android, to attested key generation operations.  This field must be set only when requesting
     ...
     * in Android, to attested key generation operations.  This field must be set only when
     ...
     * generation operations.  This field must be set only when requesting attestation of the
     ...
     * Build.MANUFACTURER in Android, to attested key generation operations.  This field must be set
     * only when requesting attestation of the device's identifiers.
     *
     * If the device does not support ID attestation (or destroyAttestationIds() was previously
     * called and the device can no longer attest its IDs), any key attestation request that
     * includes this tag must fail with ErrorCode::CANNOT_ATTEST_IDS.
    ...
    /**
     * DEVICE_UNIQUE_ATTESTATION is an argument to IKeyMintDevice::attested key generation
