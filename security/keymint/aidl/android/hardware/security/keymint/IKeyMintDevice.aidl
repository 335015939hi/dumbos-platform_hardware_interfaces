...
interface IKeyMintDevice {
    ...
    /**
     * Send a signed root of trust record, generated by the bootloader, to KeyMint.
     *
     * To verify this record, KeyMint must know how to verify the bootloader's DICE
     * certificate chain. The details of the verification are vendor-defined, but
     * for TEE KeyMint instances, the KeyMint certificate chain and the bootloader's
     * DICE certificate chain will generally share a common root (UDS Pub) and some
     * number of other common entries before the chains differ.  KeyMint must validate
     * that the expected common parts of the chain match, and then use the sequence
     * of expected component names from the Configuration Descriptors in the subsequent
     * to validate that the bootloader DICE chain contained in the root of trust
     * record is the correct chain for the bootloader.
     *
     * For StrongBox KeyMint, the StrongBox KeyMint implementation must be able to
     * verify the bootloader's DICE certificate chain, which means that StrongBox
     * KeyMint must be configured in the factory with knowledge of the bootloader's
     * UDS public key and the expected structure of the bootloader's DICE chain.
     *
     * In addition, TEE KeyMint must verify that the boot nonce contained in the root
     * of trust record is not zero and matches the boot nonce it received along with
     * its own CDI Attest private key.  StrongBox KeyMint must verify the bootloader's
     * boot nonce is not zero and matches the value received...
     *     TODO(swillden): Define with Hasini's help.
     *
     * If the bootloader's DICE chain validates correctly and the boot nonce matches,
     * KeyMint must use the provided root of trust values in subsequent attestations.
     * If the bootloader
     *
     * If KeyMint is asked to produce an attestation prior to having received a valid
     * root of trust record via this method, it must return
     *
     * @param in signed_root_of_trust is the root of trust record, formatted per RoT
     *     in the following CBOR CDDL schema:
     *
     * RoT = [
     *     DiceCertChain,
     *     RoTSigningKeyCertificate,
     *     SignedRoTData,
     * ]
     *
     * DiceCertChain is defined in IRemotelyProvisionedComponent.aidl.
     *
     * RoTSigningKeyCertificate = ([        ; COSE_Signature1
     *     protected: bstr .cbor {
     *         1 : AlgorithmEdDSA / AlgorithmESP256 / AlgorithESP384
     *     },
     *     unprotected {},
     *     payload: bstr .cbor CertificateCWT,
     *     signature: bstr PureEd25519(CDI_Attest, CertificateInput) /
     *                     ECDSA(CDI_Attest, CertificateInput)
     * ]),
     *
     * RoTSigningKeyCertificateInput = [ ; COSE_Signature1 sig structure
     *     context: "Signature1",
     ...
     * ]
     *
     * RoTSigningKeyCertificateCWT = {     ; CWT (RFC8392)
     *     1 : tstr,                       ; Issuer (CDI_ID)
     *     2 : "KeyMint RoT Signing Key",
     *     -4670552 : bstr .cbor RoTSigningKey
     *     -4670553 : bstr h'0100'         ; key_usage : digitalSignature
     * }
     *
     * RoTSigningKey = [ ; COSE_Key
     *     1: 1,                           ; Key type : Octet Key Pair
     *     3: -8,                          ; Algorithm : AlgorithmEdDSA
     *     -1 : 6,                         ; Curve : Ed25519
     *     -2 : bstr                       ; Ed25519 public key
     * ]
     *
     * SignedRoTData = #6.18([                    ; COSE_Signature1
     *         protected: bstr .cbor { 1 : -8 },  ; Algorithm EdDSA
     *         unprotected: {},
     *         payload: bstr .cbor RoTData,
     *         signature: bstr PureEd25519(.cbor RoTDataInput, RoTSigningKey),
     * ])
     *
     * RoTDataInput = [ ; COSE_Signature1 sig structure
     *     context: "Signature1",
     *     body_protected: bstr .cbor { 1 : -8 },
     *     external_aad: bstr containing UTF-8 encoding of
     *                        "Bootloader-signed KeyMint RoT",
     *     payload: bstr .cbor RoTData
     * ]
     *
     * RoTData = {
     *     1: BootNonce
     *     2: bstr .size 32,           ; SHA-256 of VB Key0
     *     3: bool,                    ; True iff bootloader is locked
     *     4: VBState,
     *     5: bstr .size 32,           ; vbmeta digest, using SHA-256
     *     6: uint .size 4,            ; os_version
     *     7: uint .size 4,            ; os patchlevel
     *     8: uint .size 4,            ; vendor patchlevel
     *     9: uint .size 4,            ; boot patchlevel
     * }
     *
     * BootNonce = uint .size 8;
     *
     * VBState = (
     *     verified : 0,
     *     self_signed : 1,
     ...
     * )
     *
     * @param out dice_cert_chain is KeyMint's DiceCertChain, used for testing.
     *
     * @param out boot_nonce is KeyMint's boot nonce, used for testing, as a
     *     CBOR-encoded integer (see BootNonce in the above schema).
     */
    void sendSignedRootOfTrust(
            in byte[] signed_root_of_trust, out byte[] dice_cert_chain, out byte[] boot_nonce);
}
