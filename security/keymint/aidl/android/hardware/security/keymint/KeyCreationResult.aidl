...
parcelable KeyCreationResult {
     ...
     *     caller must know the certificate chain for the provided key.
     *
     * 3.  Asymmetric key non-attestation with signing key.  If Tag::ATTESTATION_CHALLENGE is not
     *     provided and the generated/imported key has KeyPurpose::SIGN, then the returned certificate
     *     chain must contain only a single self-signed certificate with no attestation extension.
     *
     * 4.  Asymmetric key non-attestation with non-signing key.  If TAG::ATTESTATION_CHALLENGE is not
     *     provided and the generated/imported key does not have KeyPurpose::SIGN, then the returned
     *     certificate chain must contain only a single certificate with an empty signature and no
     *     attestation extension.
     *
     * 5.  Symmetric key.  If the generated/imported key is symmetric, the certificate chain must
     *     return empty, any ATTESTATION_CHALLENGE inputs, if provided, are ignored.
     */
