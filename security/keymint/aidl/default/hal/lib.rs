...
pub fn attestation_id_info() -> kmr_wire::AttestationIdInfo {
    let prop = |name| {
        let prop_val =
            get_property(format!("ro.product.{}_for_attestation", name).as_str())
        ...
        if !prop_val.is_empty() {
            ...
            let prop_val = get_property(format!("ro.product.vendor.{}", name).as_str())
                .unwrap_or_else(|_| "".to_string())
                .as_bytes()
                .to_vec();
            if !prop_val.is_empty() {
                ...
                get_property(format!("ro.product.{}", name).as_str())
