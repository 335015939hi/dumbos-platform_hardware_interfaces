...
//! Common functionality for main KeyMint executables.
...
/// Set up a local keymint instance.
pub fn local_keymint_setup(instance: &str) -> Result<(), HalServiceError> {
    ...
    let service_name = format!("{}/{}", SECRET_SERVICE_NAME, instance);
    ...
    let service_name = format!("{}/{}", KM_SERVICE_NAME, instance);
    ...
    let service_name = format!("{}/{}", RPC_SERVICE_NAME, instance);
    ...
    let service_name = format!("{}/{}", CLOCK_SERVICE_NAME, instance);
    ...
    info!("Successfully registered KeyMint HAL services for /{instance}.");
    Ok(())
}
