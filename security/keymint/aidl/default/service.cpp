...
int main() {
    ...
    status = AServiceManager_addService(sharedsecretser->asBinder().get(), instanceName.c_str());
    CHECK(status == STATUS_OK);
    ...
    LOG(INFO) << "sharedsecret instance: " << instanceName;
    status = AServiceManager_addService(secureclockser->asBinder().get(), instanceName.c_str());
    CHECK(status == STATUS_OK);
