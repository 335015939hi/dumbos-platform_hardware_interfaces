...
RemotelyProvisionedComponent::RemotelyProvisionedComponent(
        std::shared_ptr<::keymaster::AndroidKeymaster> km) {
...
ScopedAStatus RemotelyProvisionedComponent::generateEcdsaP256KeyPair(bool testMode,
    ...
    // TODO(jbires): The followiing should move from ->GenerateKey to ->GenerateRKPKey and
    // everything
    ...
    request.key_description.Reinitialize(params, array_length(params));
    ...
    if (response.certificate_chain.entry_count != 1) {
        return ScopedAStatus::ok();  // Error: Need the single non-signed certificate with the
                                     // public key in it
    }
    keymaster_blob_t* kmCert = response.certificate_chain.begin();
    const uint8_t* temp = kmCert->data;
    X509* cert = d2i_X509(NULL, &temp, kmCert->data_length);
    EVP_PKEY* pubKey = X509_get_pubkey(cert);
    EC_KEY* ecKey = EVP_PKEY_get0_EC_KEY(pubKey);
    if (ecKey == nullptr) {
        // Error: this isn't an ec key, somehow
    }
    ...
    EC_POINT_get_affine_coordinates_GFp(EC_KEY_get0_group(ecKey), jacobian_coords, &x, &y, ctx);
    bytevec x_bytestring(32);
    bytevec y_bytestring(32);
    BN_bn2binpad(&x, x_bytestring.data(), 32 /* length */);
    BN_bn2binpad(&y, y_bytestring.data(), 32 /* length */);
    cppbor::Map cosePublicKeyMap = cppbor::Map()
                                           .add(CoseKey::KEY_TYPE, EC2)
                                           .add(CoseKey::ALGORITHM, ES256)
                                           .add(CoseKey::CURVE, cppcose::P256)
                                           .add(CoseKey::PUBKEY_X, x_bytestring)
                                           .add(CoseKey::PUBKEY_Y, y_bytestring);
    BN_CTX_free(ctx);
