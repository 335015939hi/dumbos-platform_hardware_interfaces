...
- KeyMint supports *attestation* of public keys: when an asymmetric keypair is created, the secure
  environment produces a chain of signed certificates, starting from a Google-signed root
  certificate, and terminating in a leaf certificate that holds the public key and which may also
...
*hardware auth token*) is signed with a per-boot HMAC key.
...
use a local, vendor-specific, method to agree on the shared HMAC key.
...
optional ... more secure password protection, where more secure h/w is available ... feeds into
Gatekeeper, doesn't replace it though
...
e.g. fingerprint, face auth if it passes the criteria to be a strong authenticator

.... needs access to HMAC key

## Background: Keymaster
