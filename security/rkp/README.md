...
unique device secret (UDS) as a root of trust to create an authenticated, secure
channel. We refer to the public half of this asymmetric key pair as UDS\_pub.

In order for the provisioning service to learn that UDS\_pub should be trusted
we ask device OEMs to use one of two mechanisms:
...
1.  The device OEM signs the UDS\_pub and stores the certificates on the device.
    This has the advantage that they don't need to upload a UDS\_pub for every
    device immediately, but has the disadvantage that they have to manage their
...
establish a secure channel. Instead, UDS\_pub is just the first public key in a
chain of public keys that end the KeyMint public key. All keys in the chain are
derived from the UDS and joined in a certificate chain following the
specification of the [Android Profile for DICE](#android-profile-for-dice).

### Phases

RKP will be deployed with phased management the root of trust
binding between the device and the backend. To briefly describe them:

* Degenerate DICE: A software root of trust manages UDS\_pub and UDS\_pub is
  used to establish the secure channel; a single self-signed certificate
  signifies this phase.
* DICE: A hardware ROM root of trust manages UDS\_pub with key material blown
  into fuses which are only accessible to the ROM code; the boot process follows
  the [Android Profile for DICE](#android-profile-for-dice).
* SoC vendor certified DICE: This is identical to Phase 2, except the SoC
  vendor also does the public key extraction or certification in their
...
*   **rkpd** is a system service that is responsible for communicating with the
    server and all of the system components that require key certificates from
    the server. It also implements the policy that defines how many key pairs
    each client should keep in their pool.
...
If no fields are relevant, an empty map should be encoded. Additional
implementation-specific fields may be added using key values not in the range
\[-70000, -70999\] (these are reserved for future additions here).
