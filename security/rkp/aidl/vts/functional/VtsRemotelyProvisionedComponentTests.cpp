...
TEST_P(CertificateRequestV2Test, DeviceInfo) {
    ...
    // This key should only have either TEE or Strongbox enforced characteristics.
    ASSERT_EQ(key_characteristics.size(), 1);
    ...
    auto auths = AuthorizationSet(key_characteristics[0].authorizations);

    auto attestedSecurityLevel = key_characteristics[0].securityLevel;
    ...
    auto attestedBootPatchLevel = auths.GetTagValue(TAG_OS_PATCHLEVEL);
    ...
    ASSERT_TRUE((securityLevel->value() == "tee" &&
                 attestedSecurityLevel == SecurityLevel::TRUSTED_ENVIRONMENT) ||
                (securityLevel->value() == "strongbox" &&
                 attestedSecurityLevel == SecurityLevel::STRONGBOX));
