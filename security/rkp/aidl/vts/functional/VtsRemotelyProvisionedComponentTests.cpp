 ...
 * is "orange", then the mode on at least one certificate in the DICE chain is non-normal
 */
TEST_P(CertificateRequestV2Test, unlockedBootloaderStatesImpliesNonnormalDiceChain) {
    ...
    ASSERT_EQ(deviceState != "locked" || verifiedBootState != "green", *nonNormalMode)
            << "ro.boot.vbmeta.device_state is '" << deviceState
            << "' and ro.boot.verifiedbootstate is '" << verifiedBootState << "', and it is "
            << *nonNormalMode
            << " that the DICE chain has a certificate with "
               "a non-normal mode set.";
}
