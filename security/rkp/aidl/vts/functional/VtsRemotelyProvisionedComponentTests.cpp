...
TEST(NonParameterizedTests, requireUdsPubInDiceCertChainForRkpVmAndPrimaryKeyMintInstances) {
    ...
    auto rkpVmUdsPub = getUdsPubFromDiceCertChain(rkpVmCsr, RKPVM_INSTANCE_NAME);
    ...
    auto primaryKeyMintPub = getUdsPubFromDiceCertChain(primaryKeyMintCsr, DEFAULT_INSTANCE_NAME);
...
TEST_P(CertificateRequestTest, EmptyRequest_testMode) {
    ...
    for (size_t eekLength : {2, 3, 7}) {
        ...
        auto result = verifyProductionProtectedData(
                ...
                rpcHardwareInfo.supportedEekCurve, rpcHardwareInfo, GetParam(), challenge_);
        ASSERT_TRUE(result) << result.message();
    }
}
...
TEST_P(CertificateRequestTest, NewKeyPerCallInTestMode) {
    ...
    auto firstBcc = verifyProductionProtectedData(
            ...
            eekId_, rpcHardwareInfo.supportedEekCurve, rpcHardwareInfo, GetParam(), challenge_);
