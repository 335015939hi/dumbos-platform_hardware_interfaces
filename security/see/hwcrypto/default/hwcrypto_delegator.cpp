...
class HwCryptoKey : public ndk_hwcrypto::BnHwCryptoKey {
  ...
  public:
    HwCryptoKey(std::string tipcDev) : mTipcDevice(std::move(tipcDev)) {}

    static std::shared_ptr<HwCryptoKey> Create(std::string tipc_dev) {
        std::shared_ptr<HwCryptoKey> hwCrypto =
                ndk::SharedRefBase::make<HwCryptoKey>(std::move(tipc_dev));
        ...
        auto ret = hwCrypto->connectToTrusty();
        if (!ret.ok()) {
    ...
    ndk::ScopedAStatus importClearKey(const ndk_hwcrypto::types::ExplicitKeyMaterial& keyMaterial,
        ...
        cpp_hwcrypto::types::ExplicitKeyMaterial ExplicitKeyMaterialCpp;
        Status status = Status::fromExceptionCode(Status::EX_ILLEGAL_ARGUMENT);
        ...
        status = mHwCryptoServer->importClearKey(explicitKeyCpp, cppKeyPolicy.value(),
                                                 &binder_return);
        if (status.isOk()) {
