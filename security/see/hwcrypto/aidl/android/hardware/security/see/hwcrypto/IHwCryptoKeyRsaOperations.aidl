...
interface IHwCryptoKeyRsaOperations {
     ...
     */
    IAccumulatingOperation begin_sign(in IOpaqueKey key, in RsaSignParameters parameters);
     ...
     *      Ok(IAccumulatingOperation) on success, specific error code on error.
     */
    IAccumulatingOperation begin_decrypt(in IOpaqueKey key, in RsaDecryptParameters parameters);
    ...
    byte[] get_public_key(in IOpaqueKey key);
}
