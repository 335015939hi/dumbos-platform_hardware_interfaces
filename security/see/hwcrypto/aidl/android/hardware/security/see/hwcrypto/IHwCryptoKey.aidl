...
import android.hardware.security.see.hwcrypto.base_types.NullableKeyOrigin;
...
interface IHwCryptoKey {
     ...
     * @import_domain:
     *     domain where we want to import the new key (HW Block, Secure Execution Environment,
     *     SW library).
     *
     * Return:
     *      Ok(HwCryptoKeyMaterial) on success, Err(HAlErrorCode) on error.
     */
    HwCryptoKeyResult import_key_into_domain(in HwCryptoKeyMaterial key_to_be_imported,
            in KeyPolicy new_key_policy, in NullableKeyOrigin import_domain);
     ...
     * @key_implementation:
     *     domain that should create the key (HW Block, Secure Execution Environment, SW library).
     *
     * Return:
     *      Ok(HwCryptoKeyMaterial) on success, Err(HAlErrorCode) on error.
     */
    HwCryptoKeyResult generate_key(in KeyPolicy policy, in KeyType key_type,
            in @nullable NullableKeyOrigin key_implementation);
