...
interface IHwCryptoKey {
     ...
     *
     * @return:
     *     A DiceCurrentBoundKeyResult containint the versioned key tied the the current client
     *     version on success, service specific error based on <code>HalErrorCode</code> otherwise.
     */
     ...
     *
     * @return:
     *      A HwCryptoKeyMaterial containing the derived key on success, service specific error
     *      based on <code>HalErrorCode</code> otherwise.
     */
     ...
     *
     * @return:
     *      An IOpaqueKey that can be directly be used on the local HWCrypto service on
     *      success, service specific error based on <code>HalErrorCode</code> otherwise.
     */
    IOpaqueKey keyTokenImport(in OpaqueKeyToken requestedKey, in byte[] sealingDicePolicy);

    /*
     * getKeyslotData() - Gets the keyslot key material referenced by slotId. This interface is used
     *                    to access device specific keys with known types and uses. Because the
     *                    returned key is opaque, it can only be used through the different HwCrypto
     *                    interfaces. Because the keys live in a global namespace the identity of
     *                    the caller needs to be checked to verify that it has permission to
     ...
     *
     * @return:
     *      An IOpaqueKey corresponding to the requested key slot on success, UNAUTHORIZED if the
     *      caller cannot access the requested key, another specific error code otherwise.
     */
