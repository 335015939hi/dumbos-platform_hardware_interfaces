...
fn test_hwcrypto_key_derive_current_device_dice_policy_key() {
    ...
    // derive an opaque key
    let opaque_policy = KeyPolicy {
    ...
    let serialized_policy =
        hwcryptohal_common::key_policy_to_cbor(&opaque_policy).expect("couldn't serialize policy");
    derived_key_parameters.keyPolicy = DerivedKeyPolicy::OpaqueKey(serialized_policy);
    let opaque_derived_key =
        hw_crypto_key.deriveKey(&derived_key_parameters).expect("didn't get a derived key back");
    let DerivedKey::Opaque(opaque_derived_key) = opaque_derived_key else {
        panic!("should have received an opaque key");
    };
    assert!(opaque_derived_key.is_some(), "Didn't received a key");
}
