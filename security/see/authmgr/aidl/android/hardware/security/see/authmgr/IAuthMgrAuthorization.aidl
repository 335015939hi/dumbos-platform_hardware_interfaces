...
interface IAuthMgrAuthorization {
     ...
     * input: `instanceIdentifier`. An instance identifier that is not included in the DICE
     * certificate chain should be known to the AuthMgr BE in an out-of-band mechanism as a valid
     * instance identifier. The instance identifier is used by the AuthMgr BE in this step to detect
     * and reject any duplicate authentication attempts. The error code
     * `INSTANCE_ALREADY_AUTHENTICATED` should be returned in this case.
     *
     ...
     * the storage of the pVM context is constructed using the instance identifier.
     *
     ...
     * The requests for for phase 2 of the authorization protocol are allowed only on authenticated
     * connections.
     *
     ...
     * the client's context is constructed as: instance sequence number + client ID.
     *
     * In the subsequent invocations of this method, the AuthMgr BE matches the given DICE leaf
     * to the stored DICE policy in order to enforce rollback protection. If that succeeds and if
     * the given DICE policy is different from the stored DICE policy, the AuthMgr BE replaces the
     * stored DICE policy with the given DICE policy.
     *
