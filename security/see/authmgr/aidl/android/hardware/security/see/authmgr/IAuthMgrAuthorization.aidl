 ...
 *
 * The following assumptions must be true for underlying IPC mechanism and the transport layer:
 ...
 *        boot cycle of a pVM instance. This is important to prevent man-in-the-middle (MITM)
...
interface IAuthMgrAuthorization {
     ...
     * handle to the new connection created at the beginning of phase 2. Now an authorized
     * connection is setup between the client and the trusted service, which they can use to
     * communicate independent of the AuthMgr FE and the AuthMgr BE.
     *
