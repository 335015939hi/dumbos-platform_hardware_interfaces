 ...
 *        with the AuthMgr BE in phase 1, in order to authorize a client and the aforementioned new
 *        connection, so that an authorized connection is established between the client and the
 *        trusted service. The same token that was sent over the new connection is sent as an input
 *        to `authorizeAndConnectClientToTrustedService`, in order to authorize the new connection.
 *
 * Note that this interface defines a one-way authentication protocol from the AuthMgr FE to AuthMgr
 * BE. This API does not cover mutual authentication.
 *
 ...
 * the AuthMgr BE's secure storage to store the context of the remote pVM instances and the clients,
 * 2) a unique identifier for each client, in order to prevent the "use-after-destroy" threat.
...
interface IAuthMgrAuthorization {
     ...
     * to detect and reject any duplicate authentication attempts. The error code
     * `INSTANCE_ALREADY_AUTHENTICATED` should be returned in this case. The instance identifier is
     * used in step 2 to build the file path in the secure storage to store the instance's context.
     *
     ...
     * another AuthMgr BE. Both transport IDs are included for completenss, although it is
     ...
     * in the input: `dicePolicy`. The error code `DICE_POLICY_MATCHING_FAILED` should be returned
     * if this fails. If this is the first invocation of this method during the lifetime of the
     * AuthMgr FE, the AuthMgr BE stores the DICE policy in the secure storage as part of the pVM
     * instance's context, upon successful matching of DICE chain to the policy. The file path for
     * the storage of the pVM context is constructed using the instance identifier. Note that
     * the creation of a pVM instance's context in the secure storage is allowed only during the
     * factory, for the first version of this API. In the future, we expect to allow the creation of
     * a pVM instance's context in the secure storage even after the device leaves the factory,
     * based on hard-coded DICE policies and/or via a separate `IAuthMgrInstanceContextMaintenance`
     * API. The error code `INSTANCE_CONTEXT_CREATION_DENIED` should be returned if this is not
     * allowed.
     *
     ...
     *
     * Upon successful execution of this method, the AuthM BE should store some state associated
     ...
     * connections.
     *
     * @param signedResponseForChallenge - signature from AuthMgr FE (CBOR encoded according to
     ...
     * underlying connection of this method call is already authenticated. Otherwise the error code
     * `CONNECTION_NOT_AUTHENTICATED` is returned.
     *
     ...
     * The error code `DICE_POLICY_MATCHING_FAILED` should be returned if this fails.
     ...
     * the client's context is constructed as: instance sequence number + client ID. There is no use
     * case for deleting a client context or a pVM context created in the secure storage, for the
     * first version of this API, outside of the the factory reset. In the future, we expect to
     ...
     * authorization along wit the tokens sent over them and the transport ID of the pVM instance
