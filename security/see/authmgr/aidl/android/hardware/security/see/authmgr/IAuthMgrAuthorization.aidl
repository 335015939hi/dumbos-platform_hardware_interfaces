 ...
 *        to `authorizeAndConnectClientToTrustedService`, in order to authorize the new conenction.
 *
 ...
 * the AuthMgr BE's secure storage to store the context the remote pVM instances and the clients,
...
interface IAuthMgrAuthorization {
    ...
    void authorizeAndConnectClientToTrustedService(in byte[] clientID, String serviceName,
            in byte[32] token, in ClientDiceArtifactsForPhase2 clientDiceArtifacts);
}
