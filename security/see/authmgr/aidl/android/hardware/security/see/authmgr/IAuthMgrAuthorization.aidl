 ...
 *        validated DICE certificate chain. Rollback protection is achieved by matching the DICE
 *        certificate chain against the stored DICE policy.
 ...
 * wiped upon factory reset. This sequence number is used to build: 1) unique file paths for the
 * clients in the AuthMgr BE's secure storage, 2) a unique identifier to introduce each client to
 * the trusted services with, in order to prevent the use-after-destroy threat. This means that
 * even if a client is created with the same identifier(s) of a deleted client, the new client
 * should not be able to access the deleted client's secrets/resources.
 *
...
interface IAuthMgrAuthorization {
     ...
     * a file path constructed using the instance identifier.
     *
     ...
     * requests for phase 2 are allowed only on authenticated connections. Along with the
     * authentication status, the AuthMgr BE may store other re-usable artifacts such as instanceID,
     * transport ID, DICE chain and DICE policy of AuthMgr FE in the state associated with the
     * connection to serve multiple phase 2 requests over the authenticated connection.
     *
     */
