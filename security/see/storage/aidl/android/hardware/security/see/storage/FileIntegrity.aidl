...
enum FileIntegrity {
    /** REE cannot prevent or tamper with file operations. */
    TAMPER_PROOF,
     ...
     * those changes are rollbacks that occured because a write was accepted
     * during an A/B update, then a boot failure caused a rollback to a checkpoint.
     */
    TAMPER_DETECT_IGNORE_ROLLBACK_AB_BOOT_FAILURE,
    /**
     * REE may alter written data. Changes other than rollbacks will be detected
     * and reported. (A reset is a rollback to the initial state.)
     */
