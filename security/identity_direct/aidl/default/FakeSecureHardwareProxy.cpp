...
FakeSecureHardwareProxy::FakeSecureHardwareProxy() {
    mProvisionData.resize(kMaxCredentialSlots, nullptr);
}

int32_t FakeSecureHardwareProxy::createCredential(int32_t credentialSlot, bool testCredential,
    ...
    mProvisionData[credentialSlot] =
            std::make_shared<CredentialData>(testCredential, challenge, kMaxCredentialDataSize);
...
int32_t FakeSecureHardwareProxy::deleteCredential(int credentialSlot) {
    if ((credentialSlot < 0) || credentialSlot > (kMaxCredentialSlots - 1)) {
        return IMDocStore::STATUS_FAILED;
    }
    if (mProvisionData[credentialSlot] != nullptr) {
...
int32_t FakeSecureHardwareProxy::getMaximumCredentialDataSize(int64_t* maxCredDataSize) {
    *maxCredDataSize = kMaxCredentialDataSize;
    return IMDocStore::STATUS_OK;
}

int32_t FakeSecureHardwareProxy::getNumberOfCredentialSlots(int32_t* slots) {
    *slots = kMaxCredentialSlots;
    return IMDocStore::STATUS_OK;
}

int32_t FakeSecureHardwareProxy::lookupMDocCredential(int credentialSlot) {
    if ((credentialSlot < 0) || credentialSlot > (kMaxCredentialSlots - 1)) {
        return IMDocStore::STATUS_FAILED;
    }
    if (mProvisionData[credentialSlot] == nullptr) {
...
int32_t FakeSecureHardwareProxy::presentationPackageGenerate(
    ...
    presentationPackage->first = std::move(ret->first);
    presentationPackage->second = std::move(ret->second);
    return IMDocStore::STATUS_OK;
}

int32_t FakeSecureHardwareProxy::getEncryptedCredData(uint32_t credentialSlot,
...
int32_t FakeSecureHardwareProxy::currentPresentationPackageSet(
...
int32_t FakeSecureHardwareProxy::getUsageCount(int credentialSlot, int32_t* out) {
    if ((credentialSlot < 0) || credentialSlot > (kMaxCredentialSlots - 1)) {
        return IMDocStore::STATUS_FAILED;
    }
...
int32_t FakeSecureHardwareProxy::getCurrentSigningCertificate(uint32_t credentialSlot,
                                                              std::vector<uint8_t>* out) {
    if ((credentialSlot < 0) || credentialSlot > (kMaxCredentialSlots - 1)) {
        return IMDocStore::STATUS_FAILED;
    }
...
int32_t FakeSecureHardwareProxy::clearPresentationPackage(uint32_t credentialSlot) {
    if ((credentialSlot < 0) || credentialSlot > (kMaxCredentialSlots - 1)) {
        return IMDocStore::STATUS_FAILED;
    }
