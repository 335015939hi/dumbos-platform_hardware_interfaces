...
struct EArcMock : public BnEArc {
    ...
    ::ndk::ScopedAStatus getConnectionState(int32_t in_portId,
                                            IEArcConnectionStatus* _aidl_return) override;
    ...
    ::ndk::ScopedAStatus reportCapabilities(const std::vector<uint8_t> capabilities,
                                            int32_t portId);
    ::ndk::ScopedAStatus changeConnectionState(const IEArcConnectionStatus status, int32_t portId);
  ...
  private:
    ...
    std::vector<IEArcConnectionStatus> mPortConnectionStatus;
