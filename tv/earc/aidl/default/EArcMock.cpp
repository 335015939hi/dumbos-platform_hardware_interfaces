...
ScopedAStatus EArcMock::getConnectionState(int32_t portId, IEArcConnectionStatus* _aidl_return) {
...
ScopedAStatus EArcMock::changeConnectionState(const IEArcConnectionStatus status, int32_t portId) {
    if (mCallback != nullptr) {
        mCallback->onConnectionStateChange(status, portId);
        return ScopedAStatus::ok();
    } else {
        return ScopedAStatus::fromExceptionCode(EX_NULL_POINTER);
    }
...
EArcMock::EArcMock() {
    ...
    mPortConnectionStatus[0] = IEArcConnectionStatus::STATUS_IDLE;
