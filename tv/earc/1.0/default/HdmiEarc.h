...
struct HdmiEarc : public IHdmiEarc {
    ...
    static void eventCallback(const hdmi_earc_event_t* event, void*) {
        ...
        if ((!mCallbackVector.empty()) && event != nullptr) {
            if (event->type == EARC_STATUS_CHG) {
                ...
                int i = 0;
                while (i < (int)mCallbackVector.size()) {
                    sp<IHdmiEarcCallback> cb_client = mCallbackVector[i];
                    if (cb_client->ping().isOk()) {
                        cb_client->onEarcEvent(earcEvent);
                        i++;
                    } else {
                        // skip callback event and erase the client from mCallbackVector
                        mCallbackVector.erase(mCallbackVector.begin() + i);
                    }
                }
            }
        }
  ...
  private:
    static std::vector<sp<IHdmiEarcCallback>> mCallbackVector;
