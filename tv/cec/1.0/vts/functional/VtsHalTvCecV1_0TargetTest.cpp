...
TEST_P(HdmiCecTest, PhysicalAddress) {
    ...
    EXPECT_NE(addr, INVALID_PHYSICAL_ADDRESS);
}
...
TEST_P(HdmiCecTest, GetPortInfo) {
    ...
    for (size_t i = 0; i < ports.size(); ++i) {
        ...
        if (!hasDeviceType(CecDeviceType::TV)) {
            EXPECT_NE(ports[i].physicalAddress, TV_PHYSICAL_ADDRESS);
        }
    }
...
TEST_P(HdmiCecTest, EnableAudioReturnChannel) {
    ...
    for (size_t i = 0; i < ports.size(); ++i) {
        if (ports[i].arcSupported) {
            hdmiCec->enableAudioReturnChannel(ports[i].portId, true);
        }
    }
}
...
TEST_P(HdmiCecTest, IsConnected) {
    ...
    bool status = false;
    for (size_t i = 0; i < ports.size(); ++i) {
        status |= hdmiCec->isConnected(ports[i].portId);
    }
    if (!hasDeviceType(CecDeviceType::TV)) {
        EXPECT_TRUE(status);
    } else {
