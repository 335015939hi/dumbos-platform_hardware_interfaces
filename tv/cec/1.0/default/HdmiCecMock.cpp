...
Return<void> HdmiCecMock::setCallback(const sp<IHdmiCecCallback>& callback) {
    ...
    if (callback != nullptr) {
        ...
        // Mock hotplug event here
        // HotplugEvent hotplugEvent{.connected = true, .portId = static_cast<uint32_t>(0)};
        // mCallback->onHotplugEvent(hotplugEvent);
        mFd1 = open(CEC_MSG_IN_FILE, O_RDWR);
        mFd2 = open(CEC_MSG_OUT_FILE, O_RDWR);
...
int HdmiCecMock::readMessageFromFile(unsigned char* buf, int msgCount) {
    if (msgCount <= 0 || !buf) {
        return 0;
    }
...
int HdmiCecMock::sendMessageToFile(const CecMessage& message) {
    ...
    mFd2 = open(CEC_MSG_OUT_FILE, O_WRONLY);
    if (mFd2 < 0) {
    ...
    ret = write(mFd2, msgBuf, length + 1);
    close(mFd2);
    if (ret < 0) {
        ALOGE("[halimp] write :%s failed, ret:%d\n", CEC_MSG_OUT_FILE, ret);
        return -1;
    }
    return ret;
}
...
void HdmiCecMock::threadLoop() {
    ...
    while (mFd1 < 0) {
        usleep(1000 * 1000);
        mFd1 = open(CEC_MSG_IN_FILE, O_RDONLY);
    }
    ALOGD("[halimp] file open ok, fd = %d.", mFd1);
    ...
    while (mCecThreadRun) {
        ...
        r = readMessageFromFile(msgBuf, CEC_MESSAGE_BODY_MAX_LENGTH);
        if (r <= 1) {
