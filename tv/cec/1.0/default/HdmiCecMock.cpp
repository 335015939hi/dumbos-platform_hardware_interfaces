...
void HdmiCecMock::cec_set_option(int flag, int value) {
    ...
    switch (flag) {
        ...
        case HDMI_OPTION_ENABLE_CEC:
            option_enable_cec = value;
            break;
        case HDMI_OPTION_SYSTEM_CEC_CONTROL:
            option_system_cec_control = value;
            break;
        case HDMI_OPTION_SET_LANG:
            option_language = value;
    }
}
...
Return<Result> HdmiCecMock::addLogicalAddress(CecLogicalAddress addr) {
    ...
    int size = logical_addresses.size();
    logical_addresses.resize(size + 1);
    logical_addresses[size + 1] = addr;
