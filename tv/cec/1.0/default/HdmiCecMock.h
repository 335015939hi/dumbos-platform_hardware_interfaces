...
#define CEC_MSG_IN_FILE "/dev/cec_in_pipe"
#define CEC_MSG_OUT_FILE "/dev/cec_out_pipe"
...
struct HdmiCecMock : public IHdmiCec, public hidl_death_recipient {
  ...
  private:
    ...
    void threadLoop();
    int readMessageFromFile(unsigned char* buf, int msgCount);
    int sendMessageToFile(const CecMessage& message);
