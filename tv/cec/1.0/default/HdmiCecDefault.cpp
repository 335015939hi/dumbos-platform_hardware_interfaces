...
Return<Result> HdmiCecDefault::addLogicalAddress(CecLogicalAddress addr) {
    ...
    if (ret) {
        LOG(ERROR) << "addLogicalAddress ret = " << ret;
        return Result::FAILURE_BUSY;
    }
...
Return<void> HdmiCecDefault::clearLogicalAddress() {
    ...
    if (ret) {
        LOG(ERROR) << "clearLogicalAddress ret = " << ret;
    }
    return Void();
}
...
Return<void> HdmiCecDefault::getPhysicalAddress(getPhysicalAddress_cb callback) {
    ...
    switch (ret) {
        ...
        case -EBADF:
            ...
            break;
        default:
            callback(Result::FAILURE_UNKNOWN, addr);
            break;
    }
    return Void();
}
...
Return<SendMessageResult> HdmiCecDefault::sendMessage(const CecMessage& message) {
    ...
    if (ret) {
        LOG(ERROR) << "Send message failed, ret = " << ret << " errno = " << strerror(errno);
...
Return<Result> HdmiCecDefault::init() {
    ...
    if (mCecFd < 0) {
        LOG(ERROR) << "Failed to open " << path << ", ret = " << errno;
        return Result::FAILURE_NOT_SUPPORTED;
    }
    mExitFd = eventfd(0, EFD_NONBLOCK);
    if (mExitFd < 0) {
        LOG(ERROR) << "Failed to open eventfd, ret = " << errno;
        release();
        return Result::FAILURE_NOT_SUPPORTED;
    }
    ...
    if (ret) {
        LOG(ERROR) << "Unable to query cec adapter capabilities, ret = " << ret;
        release();
        return Result::FAILURE_NOT_SUPPORTED;
    }
    ...
    if (ret) {
        LOG(ERROR) << "Unable to set initiator mode, ret = " << ret;
        release();
        return Result::FAILURE_NOT_SUPPORTED;
    }

    /* thread loop for receiving cec messages and hotplug events*/
    if (pthread_create(&mThread, NULL, event_thread, NULL)) {
...
void* HdmiCecDefault::event_thread(void*) {
    ...
    while (1) {
        ...
        if (ufds[1].revents == POLLERR) { /* CEC Event */
            ...
            if (ret) {
                LOG(ERROR) << "CEC_DQEVENT error = " << ret;
                continue;
            }
        ...
        if (ufds[0].revents == POLLIN) { /* CEC Driver */
            ...
            if (ret) {
                LOG(ERROR) << "CEC_RECEIVE error = " << ret;
                continue;
            }
            ...
            if (mCallback != nullptr) {
                ...
                LOG(ERROR) << "no event callback for msg";
            }
        }
    }
    return NULL;
