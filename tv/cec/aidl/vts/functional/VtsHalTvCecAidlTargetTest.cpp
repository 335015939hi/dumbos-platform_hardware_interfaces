...
class HdmiCecTest : public ::testing::TestWithParam<std::string> {
  public:
    void SetUp() override {
        ...
        ASSERT_EQ(AIBinder_linkToDeath(hdmiCecCallback->asBinder().get(),
                                       hdmiCecDeathRecipient.get(), 0),
                  STATUS_OK);
    }
...
TEST_P(HdmiCecTest, EnableAudioReturnChannel) {
    ...
    for (size_t i = 0; i < ports.size(); ++i) {
        if (ports[i].arcSupported) {
            ASSERT_TRUE(hdmiCec->enableAudioReturnChannel(ports[i].portId, true).isOk());
        }
    }
}
