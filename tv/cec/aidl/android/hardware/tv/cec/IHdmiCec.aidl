...
import android.hardware.tv.cec.HdmiPortInfo;
import android.hardware.tv.cec.IHdmiCecCallback;
import android.hardware.tv.cec.OptionKey;
...
interface IHdmiCec {
    ...
    const int RESULT_FAILURE_INVALID_STATE = 3;
    const int RESULT_FAILURE_NOT_SUPPORTED = 4;
    const int RESULT_FAILURE_BUSY = 5;
     ...
     *         RESULT_FAILURE_INVALID_ARGS if the given logical address is invalid,
     *         RESULT_FAILURE_BUSY if device or resource is busy
     */
    int addLogicalAddress(in CecLogicalAddress addr);
    ...
    const int SEND_MESSAGE_SUCCESS = 0;
    const int SEND_MESSAGE_NACK = 1;
    const int SEND_MESSAGE_BUSY = 2;
    const int SEND_MESSAGE_FAIL = 3;
     ...
     * @return Result status of the operation. SEND_MESSAGE_SUCCESS if
     *         successful,
     *         SEND_MESSAGE_NACK if the sent message is not acknowledged,
     *         SEND_MESSAGE_BUSY if the CEC bus is busy.
     */
    int sendMessage(in CecMessage message);
     ...
     */
    void setOption(in OptionKey key, in boolean value);
}
