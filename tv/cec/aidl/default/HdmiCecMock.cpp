...
ScopedAStatus HdmiCecMock::setCallback(const std::shared_ptr<IHdmiCecCallback>& callback) {
    ...
    if (callback != nullptr) {
        AIBinder_linkToDeath(mCallback->asBinder().get(), mDeathRecipient.get(), 0 /* cookie */);
...
int HdmiCecMock::sendMessageToFifo(const CecMessage& message) {
    unsigned char msgBuf[CEC_MESSAGE_BODY_MAX_LENGTH + 1];
    int ret = -1;
...
void HdmiCecMock::printCecMsgBuf(const char* msg_buf, int len) {
    int i, size = 0, bufSize = CEC_MESSAGE_BODY_MAX_LENGTH * 2;
    // Use 2 characters for each char in the message.
    char buf[CEC_MESSAGE_BODY_MAX_LENGTH * 2] = {0};
...
void HdmiCecMock::handleCecMessage(unsigned char* msgBuf, int msgSize) {
    ...
    ALOGD("[halimp] msg init %hhd", message.initiator);
    message.destination = static_cast<CecLogicalAddress>((msgBuf[0] >> 0) & 0xf);
    ALOGD("[halimp] msg dest %hhd", message.destination);
...
HdmiCecMock::HdmiCecMock() {
    ALOGE("[halimp] Opening a virtual CEC HAL for testing and virtual machine.");
    mCallback = nullptr;
}
