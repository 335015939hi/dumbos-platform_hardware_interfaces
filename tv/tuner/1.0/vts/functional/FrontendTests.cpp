...
<<<<<<< TARGET BRANCH (3803a1 [conflict] Merge "KeyMint VTS: cope with ATTEST_KEY +/- SIGN)
...
=======
    if (mFeIds.size() > 0) {
        for (size_t i = 0; i < mFeIds.size(); i++) {
            ...
            feId = mFeIds[i];
            return;
>>>>>>> SOURCE BRANCH (88b7c1 [conflict] Merge "vts: fix some tunerhal vts fail without fr)
        }
    } else {
        feId = INVALID_ID;
    }
