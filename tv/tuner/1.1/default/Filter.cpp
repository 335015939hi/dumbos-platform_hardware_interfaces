...
#if 1
        if (mFilterId == 0) {
		input.open("/product/streams/stream.ts", ios::in |std::ios::binary );
...
#endif
    ALOGD("Inside Filter constructor tsfilter main type %d", mType.mainType );
    switch (mType.mainType) {
        case DemuxFilterMainType::TS:
            ALOGD("Inside Filter constructor tsfilter type TS");
            if (mType.subType.tsFilterType() == DemuxTsFilterType::AUDIO ||
                mType.subType.tsFilterType() == DemuxTsFilterType::VIDEO) {
                ALOGD("Inside Filter constructor tsfilter type A/V");
                mIsMediaFilter = true;
            }
            if (mType.subType.tsFilterType() == DemuxTsFilterType::PCR) {
                ALOGD("Inside Filter constructor tsfilter type PCR");
                mIsPcrFilter = true;
            }
            if (mType.subType.tsFilterType() == DemuxTsFilterType::RECORD) {
                ALOGD("Inside Filter constructor tsfilter type RECORD");
...
Filter::~Filter() {
    ...
    mFilterTsExtractThrdRunning = false; // Anbu - for testing
...
Return<Result> Filter::configure(const DemuxFilterSettings& settings) {
    ...
    switch (mType.mainType) {
        case DemuxFilterMainType::TS:
            ...
            mTableid = settings.ts().filterSettings.section().condition.sectionBits().filter;
            mTable = mTableid[0];
            ALOGD("%s> configure mtableId : %d", __FUNCTION__, mTableid[0]);
            ALOGD("%s> configure mtableId case1 : %d", __FUNCTION__, mTable);
            ALOGD("%s> configure mTpid : %d", __FUNCTION__, mTpid);
            break;
        case DemuxFilterMainType::MMTP:
            break;
        case DemuxFilterMainType::IP:
...
Return<Result> Filter::start() {
    ...
    mFilterTsExtractThrdRunning = true; // Anbu - testing
    ...
    startTsextractorLoop(); // Anbu -Testing
...
Result Filter::startTsextractorLoop() {
    //mFilterTsExtractThrdRunning = true;
    //pthread_create(&mFilterThread, NULL, __threadLoopTsExtractor, this);
    //pthread_setname_np(mFilterThread, "tsfile_extractor_loop");
    ALOGD("[Filter] startTsextractorLoopThread FilterID: %llu.", mFilterId);
    pthread_create(&mTSExtractorThread, NULL, __threadLoopTsExtractor, this);
    pthread_setname_np(mTSExtractorThread, "tsfile_extractor_loop");
...
void Filter::tsextractorThreadLoop() {
    ...
    if (!mFilterTsExtractThrdRunning)
     return;

    std::lock_guard<std::mutex> lock(mFilterextThreadLock); // Anbu -for testing
    while(mFilterTsExtractThrdRunning)
    {
        ALOGW("[Filter] Inside while infinite loop");
        //while(1)
        //while (readpacket(rawpacket, input))
        //{
            if (!readpacket(rawpacket, input)) {
              return;
            }
            //readpacket(rawpacket, input);
	        ALOGD("[Filter] readpacket byte case4: ");
	        MpegPacket* packet = new MpegPacket(rawpacket);
	        //ALOGD("[Filter] packet addr: %p", packet);
	        //ALOGD("[Filter] packet addr deference: %p", *packet);
	        if (nullptr == packet) {
	            ALOGD("[Filter] packet null: ");
	            return;
	        }
	        //table = new Table(packet);
	        //ALOGD("[Filter] table addr: %p", table);
	        //Table* table = new Table(packet);
	        //if (nullptr == table) {
	        //    ALOGD("[Filter] Table null: ");
	        //}
	        //ALOGD("[Filter] Read table_id : %d", table->table_id);
	        packets.push_back(packet);
	        mergePackets(packets);
	        packet->getPacket();
            vector<uint8_t> payload = packet->getPayload();

            //ALOGD("[Filter] Read table_id as from input : %d", mTableid[0]);
            //ALOGD("[Filter] Read table_id from stream : %d", /*table->payload[0]*/payload[0]);
            //if(packet->getPID() == SDT_PID)
            //if(mTableid[0] == table->table_id)
            //if(mTable == table->table_id)
            //if(mTable == table->table_id)
            if(mTable == payload[0])
            {
                ALOGD("[Filter] readpacket bytte case5: ");
                mDemux->updateFilterOutput(mFilterId,packet->getPayload());
                mDemux->startFilterHandler(mFilterId);
                //updateFilterOutput(packet->getPayload());
                //startFilterHandler();
            }
	        ALOGW("[Filter] Inside read raw packet..");
        //}
        ALOGW("[Filter] Inside while infinite loop end");
        //break;
    }
...
void Filter::filterThreadLoop() {
    ...
    while (mFilterThreadRunning) {
        ...
        for (int i = 0; i < SECTION_WRITE_COUNT; i++) {
			...
			ALOGD("[Filter] filterThreadLoop case10.1");
            ...
            if (i == SECTION_WRITE_COUNT - 1) {
				ALOGD("[Filter] filterThreadLoop case15 %llu", mFilterId);
