...
import android.hardware.tv.hdmi.connection.Result;
...
interface IEArc {
     ...
     * framework.
     * @return {@code SUCCESS} if the setting could be changed to the value passed.
     *         {@code FAILURE_NOT_SUPPORTED} if the setting is not supported.
     *         {@code FAILURE_INVALID_ARGS} if the arguments were invalid.
     *         {@code FAILURE_UNKNOWN} if there was an unknown failure.
     */
    Result setEArcEnabled(in boolean enabled);
