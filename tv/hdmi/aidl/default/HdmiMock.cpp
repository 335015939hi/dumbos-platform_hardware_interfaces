...
ScopedAStatus HdmiMock::setCallback(const std::shared_ptr<IHdmiCallback>& callback) {
    ...
    if (callback != nullptr) {
        ...
        mInputFile = open(HDMI_MSG_IN_FIFO, O_RDWR);
...
void HdmiMock::handleHotplugMessage(unsigned char* msgBuf) {
    ...
    if (portId > static_cast<int32_t>(mPortInfos.size())) {
        ALOGD("[halimp] ignore hot plug message, id %x does not exist", portId);
        return;
    }
...
void HdmiMock::threadLoop() {
    ...
    while (mInputFile < 0) {
        usleep(1000 * 1000);
        mInputFile = open(HDMI_MSG_IN_FIFO, O_RDONLY);
    }
...
HdmiMock::HdmiMock() {
    ALOGE("[halimp] Opening a virtual HDMI HAL for testing and virtual machine.");
    mCallback = nullptr;
