...
TEST_P(HdmiConnectionTest, HdpSignal) {
    ...
    ASSERT_TRUE(hdmiConnection->setHpdSignal(signal, *retVal).isOk());
    ASSERT_TRUE(retVal == IHdmiConnection::RESULT_SUCCESS);
    ...
    ASSERT_TRUE(hdmiConnection->getHpdSignal(&readSignal).isOk());
    EXPECT_EQ(readSignal, signal);
    ASSERT_TRUE(hdmiConnection->setHpdSignal(originalSignal, *retVal).isOk());
    ASSERT_TRUE(retVal == IHdmiConnection::RESULT_SUCCESS);
}
