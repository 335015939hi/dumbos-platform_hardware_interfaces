...
TEST_P(DumpstateHidl1_1Test, TestEnableDeviceLogging) {
    ...
    // Make sure we actually turned it on. Some HALs are lazy, so re-get if needed.
    if (!dumpstate->ping().isOk()) {
        ALOGW("IDumpstateDevice service appears to have exited lazily, attempting to get again");
        GetService();
    }
...
TEST_P(DumpstateHidl1_1Test, TestDisableDeviceLogging) {
    ...
    // Make sure we actually turned it off. Some HALs are lazy, so re-get if needed.
    if (!dumpstate->ping().isOk()) {
        ALOGW("IDumpstateDevice service appears to have exited lazily, attempting to get again");
        GetService();
    }
