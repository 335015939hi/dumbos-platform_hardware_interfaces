...
using aidl::android::hardware::dumpstate::DumpstateMode;
using aidl::android::hardware::dumpstate::DumpstateStatus;
...
class DumpstateAidlPerModeTest
    : public DumpstateAidlTestBase<std::tuple<std::string, DumpstateMode>> {
  protected:
    ...
    void AssertStatusForMode(const ::ndk::ScopedAStatus& status,
                             const DumpstateStatus& dumpstateStatus,
                             const DumpstateStatus expectedDumpstateStatus,
        ...
        if (GetMode() == DumpstateMode::DEFAULT) {
            ASSERT_EQ(expectedDumpstateStatus, dumpstateStatus)
                    << "Required mode (DumpstateMode::" << toString(GetMode())
                    << "): status should be DumpstateStatus::" << toString(expectedDumpstateStatus)
                    << ", but got DumpstateStatus::" << toString(dumpstateStatus);
        } else {
            // The rest of the modes are optional to support, but they MUST return either the
            // expected value or UNSUPPORTED_MODE.
            ASSERT_TRUE(dumpstateStatus == expectedDumpstateStatus ||
                        dumpstateStatus == DumpstateStatus::UNSUPPORTED_MODE)
                    << "Optional mode (DumpstateMode::" << toString(GetMode())
                    << "): status should be DumpstateStatus::" << toString(expectedDumpstateStatus)
                    << " or DumpstateStatus::UNSUPPORTED_MODE, but got DumpstateStatus::"
                    << toString(dumpstateStatus);
        }
        if (dumpstateStatus == expectedDumpstateStatus && additional_assertions != nullptr) {
...
TEST_P(DumpstateAidlPerModeTest, TestNullHandle) {
    ...
    DumpstateStatus dumpstateStatus;
    ...
    AssertStatusForMode(status, dumpstateStatus, DumpstateStatus::ILLEGAL_ARGUMENT);
}
...
TEST_P(DumpstateAidlPerModeTest, TestOk) {
    ...
    AssertStatusForMode(status, dumpstateStatus, DumpstateStatus::OK, [&fds]() {
...
TEST_P(DumpstateAidlPerModeTest, TestHandleWithTwoFds) {
    ...
    AssertStatusForMode(status, dumpstateStatus, DumpstateStatus::OK, [&fds1, &fds2]() {
...
TEST_P(DumpstateAidlGeneralTest, TestInvalidModeArgument_Negative) {
    ...
    DumpstateStatus dumpstateStatus;
    auto status = dumpstate->dumpstateBoard(dumpstateFds, static_cast<DumpstateMode>(-100),
                                            kDefaultTimeoutMillis, &dumpstateStatus);

    ASSERT_TRUE(status.isOk()) << "Status should be ok and return a more specific DumpstateStatus: "
                               << status.getDescription();
    ASSERT_EQ(dumpstateStatus, DumpstateStatus::ILLEGAL_ARGUMENT)
...
TEST_P(DumpstateAidlGeneralTest, TestInvalidModeArgument_Undefined) {
    ...
    DumpstateStatus dumpstateStatus;
    auto status = dumpstate->dumpstateBoard(dumpstateFds, static_cast<DumpstateMode>(9001),
                                            kDefaultTimeoutMillis, &dumpstateStatus);
...
TEST_P(DumpstateAidlPerModeTest, TestDeviceLoggingDisabled) {
    ...
    AssertStatusForMode(status, dumpstateStatus, DumpstateStatus::OK);
...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::Combine(
                testing::ValuesIn(android::getAidlHalInstanceNames(IDumpstateDevice::descriptor)),
                testing::ValuesIn(ndk::internal::enum_values<DumpstateMode>)),
