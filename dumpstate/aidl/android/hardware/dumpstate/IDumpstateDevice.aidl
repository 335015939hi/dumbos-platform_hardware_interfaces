...
import android.hardware.dumpstate.DumpstateMode;
import android.hardware.dumpstate.DumpstateStatus;
import android.os.ParcelFileDescriptor;

// Interface inherits from android.hardware.dumpstate@1.0::IDumpstateDevice but AIDL does not
// support interface inheritance.
@VintfStability
interface IDumpstateDevice {
    // Ignoring method dumpstateBoard from 1.0::IDumpstateDevice since a newer alternative is
    // available.

    // Changing method name from dumpstateBoard_1_1 to dumpstateBoard
    // Adding return type to method instead of out param DumpstateStatus status since there is only
    // one return value.
    /**
     ...
     * @return A DumpstateStatus value indicating the final result.
     */
    DumpstateStatus dumpstateBoard(
            in ParcelFileDescriptor[] fd, in DumpstateMode mode, in long timeoutMillis);

    // Adding return type to method instead of out param boolean enabled since there is only one
    // return value.
    /**
