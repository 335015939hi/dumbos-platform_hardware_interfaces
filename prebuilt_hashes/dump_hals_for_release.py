...
def filter_types(iterable):
    return iterable if Globals.include_types else \
        filter(lambda s: not s.endswith("::types"), iterable)

def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--pretty', help='Print pretty JSON', action='store_true')
    parser.add_argument('--include-types', help='Include types.hal as well', action='store_true')
    parser.add_argument('--package-root', metavar='PACKAGE:PATH',
        help='package root of current directory, e.g. android.hardware:hardware/interfaces', nargs='*')
    parser.add_argument('hashes', metavar='FILE',
        help='Locations of current.txt for each release.', nargs='*')
    ...
    for filename in Globals.hashes or tuple():
        ...
        assert api_level_to_int(api_level) is not None, \
            '--hash API_LEVEL must be an integer, but is {}'.format(api_level)

        assert api_level not in interfaces_for_level, \
            'Duplicate --hash API_LEVEL: {}'.format(api_level)

        if api_level not in interfaces_for_level:
            interfaces_for_level[api_level] = set()
        interfaces_for_level[api_level].update(filter_types(get_interfaces(filename)))
    ...
    for package_root in Globals.package_root or tuple():
        ...
        interfaces_for_level[Constants.CURRENT].update(filter_types(get_interfaces_from_package_root(*tup)))
