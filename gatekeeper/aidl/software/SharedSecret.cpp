...
::ndk::ScopedAStatus SoftSharedSecret::computeSharedSecret(
    ...
    if (nonce_.empty() || hmac_key_.size() > 0) {
        LOG(ERROR) << "wrong phase in shared secret calculation process";
    ...
    if (verification_input.Reset(sizeof(kMacVerificationString) - 1) == nullptr) {
        LOG(ERROR) << "verification input allocation failed";
        return ::ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
    ...
    if (!hmac_impl.Sign((const uint8_t*)kMacVerificationString, sizeof(kMacVerificationString) - 1,
                        sharing_check->data(), sharing_check->size())) {
        return ::ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
    LOG(INFO) << "Returning shared secret calculation checksum";
    return ::ndk::ScopedAStatus::ok();
}
