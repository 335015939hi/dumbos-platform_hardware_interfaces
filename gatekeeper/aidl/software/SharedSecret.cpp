...
::ndk::ScopedAStatus SoftSharedSecret::computeSharedSecret(
    ...
    if (label.Reset(strlen(KEY_AGREEMENT_LABEL)) == nullptr) {
        LOG(ERROR) << "label memory allocation failed";
        return ::ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
    std::memcpy(label.writable_data(), KEY_AGREEMENT_LABEL, strlen(KEY_AGREEMENT_LABEL));
    ...
    std::stringstream ckdf_input;
    ckdf_input << "key_agreement_key (" << key_agreement_key.key_material_size << ") {";
    for (auto i = 0; i < key_agreement_key.key_material_size; i++) {
        ckdf_input << (int)key_agreement_key.key_material[i] << ", ";
    }
    ckdf_input << "} label (" << label.data_length << ") { ";
    for (auto i = 0; i < label.data_length; i++) {
        ckdf_input << (int)label.data[i] << ", ";
    }
    ckdf_input << "}";
    for (const auto& blob : context_blobs) {
        ckdf_input << " context_blob (" << blob.data_length << ") { ";
        for (auto i = 0; i < blob.data_length; i++) {
            ckdf_input << (int)blob.data[i] << ", ";
        }
        ckdf_input << "} ";
    }
    LOG(INFO) << "Running CKDF with input " << ckdf_input.str();
    ...
    std::stringstream hmac_calculation;
    hmac_calculation << "hmac_key (" << hmac_key_.key_material_size << ") {";
    for (auto i = 0; i < hmac_key_.key_material_size; i++) {
        hmac_calculation << (int)hmac_key_.key_material[i] << ", ";
    }
    hmac_calculation << "} data (" << strlen(KEY_CHECK_LABEL) << ") {";
    for (auto i = 0; i < strlen(KEY_CHECK_LABEL); i++) {
        hmac_calculation << (int)KEY_CHECK_LABEL[i] << ", ";
    }
    hmac_calculation << "}";

    LOG(INFO) << "Running HMAC with " << hmac_calculation.str();
    ...
    std::stringstream hmac_out;
    hmac_out << "sharing_check (" << sharing_check->size() << ") {";
    for (auto i : *sharing_check) {
        hmac_out << (int)i << ", ";
    }
    hmac_out << "}";
    LOG(INFO) << "Returning shared secret calculation checksum: " << hmac_out.str();
    return ::ndk::ScopedAStatus::ok();
}
