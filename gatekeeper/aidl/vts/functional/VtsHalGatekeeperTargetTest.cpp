...
static void verifyAuthToken(GatekeeperVerifyResponse& rsp) {
    ...
    uint32_t auth_type = ntohl(static_cast<uint32_t>(rsp.hardwareAuthToken.authenticatorType));
    uint64_t auth_tstamp =
        ntohq(static_cast<uint64_t>(rsp.hardwareAuthToken.timestamp.milliSeconds));
