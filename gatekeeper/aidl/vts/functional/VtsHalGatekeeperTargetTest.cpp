...
#include <aidl/android/hardware/gatekeeper/GatekeeperResponse.h>
#include <aidl/android/hardware/gatekeeper/GatekeeperStatusCode.h>
...
static const hw_auth_token_t* toAuthToken(GatekeeperResponse& rsp) {
    const hw_auth_token_t* auth_token = reinterpret_cast<hw_auth_token_t*>(rsp.data.data());
...
class GatekeeperAidlTest : public ::testing::TestWithParam<std::string> {
  protected:
    ...
    void doEnroll(GatekeeperRequest& req, GatekeeperResponse& rsp) {
        while (true) {
            auto ret = gatekeeper_->enroll(uid_, req.curPwdHandle, req.curPwd, req.newPwd, &rsp);
    ...
    void doVerify(GatekeeperRequest& req, GatekeeperResponse& rsp) {
        while (true) {
            auto ret = gatekeeper_->verify(uid_, req.challenge, req.curPwdHandle, req.newPwd, &rsp);
            ASSERT_TRUE(ret.isOk());
            if (rsp.code != GatekeeperStatusCode::ERROR_RETRY_TIMEOUT) break;
            ALOGI("%s: got retry code; retrying in 1 sec", __func__);
            sleep(1);
    ...
    void doDeleteUser(GatekeeperResponse& rsp) {
        while (true) {
            auto ret = gatekeeper_->deleteUser(uid_, &rsp);
            ASSERT_TRUE(ret.isOk());
            if (rsp.code != GatekeeperStatusCode::ERROR_RETRY_TIMEOUT) break;
            ALOGI("%s: got retry code; retrying in 1 sec", __func__);
            sleep(1);
    ...
    void doDeleteAllUsers(GatekeeperResponse& rsp) {
        while (true) {
            auto ret = gatekeeper_->deleteAllUsers(&rsp);
            ASSERT_TRUE(ret.isOk());
            if (rsp.code != GatekeeperStatusCode::ERROR_RETRY_TIMEOUT) break;
            ALOGI("%s: got retry code; retrying in 1 sec", __func__);
            sleep(1);
    ...
    void checkVerify(GatekeeperResponse& rsp, uint64_t challenge, bool expectSuccess) {
        if (expectSuccess) {
    ...
    void enrollNewPassword(std::vector<uint8_t>& password, GatekeeperResponse& rsp,
                           bool expectSuccess) {
    ...
    void verifyPassword(std::vector<uint8_t>& password, std::vector<uint8_t>& passwordHandle,
                        uint64_t challenge, GatekeeperResponse& verifyRsp, bool expectSuccess) {
    ...
    if (delAllRsp.code == GatekeeperStatusCode::STATUS_OK) {
        ...
        for (size_t i = 0; i < sizeof(users) / sizeof(users[0]); ++i) {
            ...
            verifyPassword(users[i].password, users[i].enrollRsp.data, 0, users[i].verifyRsp,
                           false);
            EXPECT_EQ(GatekeeperStatusCode::ERROR_GENERAL_FAILURE, users[i].verifyRsp.code);
        }
