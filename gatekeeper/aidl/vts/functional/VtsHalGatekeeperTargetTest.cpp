...
class GatekeeperAidlTest : public ::testing::TestWithParam<std::string> {
  protected:
    ...
    Status doEnroll(GatekeeperRequest& req, GatekeeperResponse& rsp) {
        Status ret;
        while (true) {
    ...
    Status doVerify(GatekeeperRequest& req, GatekeeperResponse& rsp) {
        Status ret;
        while (true) {
    ...
    void checkEnroll(GatekeeperResponse& rsp, Status& ret, bool expectSuccess) {
        if (expectSuccess) {
    ...
    void checkVerify(GatekeeperResponse& rsp, Status& ret, uint64_t challenge, bool expectSuccess) {
        if (expectSuccess) {
            EXPECT_TRUE(ret.isOk());
