...
TEST_P(OemLockAidlTest, AllowedByDeviceCanBeToggled) {
    ...
    ASSERT_EQ(allowed.size(), 1);
    const bool originallyAllowed = allowed.front();
    ...
    ASSERT_EQ(allowed.front(), !originallyAllowed);
...
TEST_P(OemLockAidlTest, CarrierUnlock) {
    ...
    std::vector<bool> allowed;
    std::vector<OemLockSecureStatus> secure_status;
    ...
    ASSERT_TRUE(set_ret.isOk());
    ASSERT_EQ(secure_status.size(), 1);
    ASSERT_NE(secure_status.front(), OemLockSecureStatus::FAILED);
    const auto set_status = secure_status.front();
    ...
    ASSERT_TRUE(restore_ret.isOk());
    ASSERT_EQ(secure_status.front(), OemLockSecureStatus::OK);
