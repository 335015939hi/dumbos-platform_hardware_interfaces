...
static_assert(LIGHT_FLASH_NONE == (int) Flash::NONE,
    "Flash::NONE must match legacy value.");
static_assert(LIGHT_FLASH_TIMED == (int) Flash::TIMED,
    "Flash::TIMED must match legacy value.");
static_assert(LIGHT_FLASH_HARDWARE == (int) Flash::HARDWARE,
    "Flash::HARDWARE must match legacy value.");

static_assert(BRIGHTNESS_MODE_USER == (int) Brightness::USER,
    "Brightness::USER must match legacy value.");
static_assert(BRIGHTNESS_MODE_SENSOR == (int) Brightness::SENSOR,
    "Brightness::SENSOR must match legacy value.");
static_assert(BRIGHTNESS_MODE_LOW_PERSISTENCE == (int) Brightness::LOW_PERSISTENCE,
...
Return<Status> Light::setLight(Type type, const LightState& state)  {
    ...
    light_state_t legacyState {
        .color = state.color,
        .flashMode = (int) state.flashMode,
        .flashOnMS = state.flashOnMs,
        .flashOffMS = state.flashOffMs,
        .brightnessMode = (int) state.brightnessMode,
    };
...
light_device_t* getLightDevice(const char* name) {
    ...
    if (ret == 0) {
        ...
        if (ret != 0) {
            ALOGE("light_open %s failed: %d", LIGHTS_HARDWARE_MODULE_ID, ret);
        }
    } else {
        ALOGE("hw_get_module %s failed: %d", LIGHTS_HARDWARE_MODULE_ID, ret);
    }
...
ILight* HIDL_FETCH_ILight(const char* /* name */) {
    ...
    if (lights.size() == 0) {
        ALOGE("Could not open any lights.");
        return nullptr;
    }
