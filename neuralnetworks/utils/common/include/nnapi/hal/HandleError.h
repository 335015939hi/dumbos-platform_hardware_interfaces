...
nn::ExecutionResult<Type> makeExecutionFailure(nn::Result<Type> result, nn::ErrorStatus status) {
    if (!result.has_value()) {
        return nn::error(status, std::vector<nn::OutputShape>{}) << std::move(result).error();
    }
    return std::move(result).value();
}
...
nn::ExecutionResult<Type> makeExecutionFailure(nn::GeneralResult<Type> result) {
    if (!result.has_value()) {
        const auto status = result.error().code;
        return nn::error(status, std::vector<nn::OutputShape>{})
