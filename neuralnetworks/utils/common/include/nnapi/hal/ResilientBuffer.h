...
class ResilientBuffer final : public nn::IBuffer {
  ...
  public:
    using Factory = std::function<std::shared_ptr<const nn::IBuffer>(bool)>;

    static std::shared_ptr<const nn::IBuffer> create(Factory makeBuffer);

    explicit ResilientBuffer(PrivateConstructorTag tag, Factory makeBuffer,
                             std::shared_ptr<const nn::IBuffer> buffer);

    std::shared_ptr<const nn::IBuffer> getBuffer() const;
    std::shared_ptr<const nn::IBuffer> recover(const nn::IBuffer* failingBuffer,
                                               bool blocking) const;
  ...
  private:
    ...
    mutable std::shared_ptr<const nn::IBuffer> mBuffer GUARDED_BY(mMutex);
};
