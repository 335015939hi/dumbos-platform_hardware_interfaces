...
std::shared_ptr<const nn::IBuffer> ResilientBuffer::create(Factory makeBuffer) {
    CHECK(makeBuffer != nullptr);
    auto buffer = makeBuffer(/*blocking=*/true);
...
std::shared_ptr<const nn::IBuffer> ResilientBuffer::getBuffer() const {
    std::lock_guard guard(mMutex);
    return mBuffer;
}
std::shared_ptr<const nn::IBuffer> ResilientBuffer::recover(const nn::IBuffer* /*failingBuffer*/,
                                                            bool /*blocking*/) const {
    std::lock_guard guard(mMutex);
    return mBuffer;
}
