...
std::shared_ptr<const nn::IPreparedModel> ResilientPreparedModel::create(
        Factory makePreparedModel) {
    CHECK(makePreparedModel != nullptr);
    auto preparedModel = makePreparedModel(/*blocking=*/true);
...
ResilientPreparedModel::ResilientPreparedModel(
        PrivateConstructorTag /*tag*/, Factory makePreparedModel,
        std::shared_ptr<const nn::IPreparedModel> preparedModel)
    : kMakePreparedModel(std::move(makePreparedModel)), mPreparedModel(std::move(preparedModel)) {
    CHECK(kMakePreparedModel != nullptr);
    CHECK(mPreparedModel != nullptr);
}

std::shared_ptr<const nn::IPreparedModel> ResilientPreparedModel::getPreparedModel() const {
    std::lock_guard guard(mMutex);
    return mPreparedModel;
}

std::shared_ptr<const nn::IPreparedModel> ResilientPreparedModel::recover(
...
nn::GeneralResult<std::unique_ptr<const nn::IPreparedModel>>
ResilientPreparedModel::configureExecutionBurst(bool preferPowerOverLatency) const {
    return getPreparedModel()->configureExecutionBurst(preferPowerOverLatency);
}
