...
std::shared_ptr<PreparedModel> adaptPreparedModel(nn::SharedPreparedModel preparedModel,
    ...
    return ndk::SharedRefBase::make<PreparedModel>(std::move(preparedModel), userId);
}

void notify(IPreparedModelCallback* callback, PrepareModelResult result, uid_t userId) {
    if (!result.has_value()) {
        ...
        auto aidlPreparedModel = adaptPreparedModel(std::move(preparedModel), userId);
...
nn::GeneralResult<void> prepareModelFromCache(
    ...
    auto task = [device, nnDeadline, nnModelCache = std::move(nnModelCache),
                 nnDataCache = std::move(nnDataCache), nnToken, userId, callback] {
