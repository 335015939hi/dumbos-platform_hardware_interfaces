    ...
    ndk::ScopedAStatus getExecutionInfo(Timing* timingLaunched, Timing* timingFenced,
        ...
        if (result.ok()) {
            ...
            *errorStatus = ErrorStatus::NONE;
            ...
            const auto aidlStatus =
                    utils::convert(result.error().code).value_or(ErrorStatus::GENERAL_FAILURE);
...
nn::Duration makeDuration(int32_t durationNs) {
    return nn::Duration(std::chrono::nanoseconds(durationNs));
}

nn::GeneralResult<nn::OptionalDuration> makeOptionalDuration(int32_t durationNs) {
    if (durationNs < -1) {
...
nn::GeneralResult<nn::OptionalTimePoint> makeOptionalTimePoint(int32_t durationNs) {
    if (durationNs < -1) {
...
nn::GeneralResult<FencedExecutionResult> executeFenced(
    ...
    auto fileDescriptor = ndk::ScopedFileDescriptor(uniqueFd.release());
...
PreparedModel::PreparedModel(nn::SharedPreparedModel preparedModel, uid_t userId)
    : kPreparedModel(std::move(preparedModel)), kUserId(userId) {
