...
GeneralResult<SharedHandle> unvalidatedConvert(const NativeHandle& aidlNativeHandle) {
    if (aidlNativeHandle.fds.size() != 1 || aidlNativeHandle.ints.size() != 0) {
        return NN_ERROR()
               << "Unable to convert NativeHandle to Handle because it requires 1 fd and 0 ints";
    }
    base::unique_fd fd = NN_TRY(dupFd(aidlNativeHandle.fds.front().get()));
