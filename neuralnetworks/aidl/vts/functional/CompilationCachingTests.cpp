...
class CompilationCachingTestBase : public testing::Test {
  protected:
    ...
    void prapareModelFromCacheOrRetryWithModel(
            const std::vector<ndk::ScopedFileDescriptor>& modelCache,
            const std::vector<ndk::ScopedFileDescriptor>& dataCache,
            std::shared_ptr<IPreparedModel>* preparedModel, ErrorStatus* status, const Model* model,
            const TestModel& testModel) {
        // Don't attempt fallback with prepareModel
        prepareModelFromCache(modelCache, dataCache, preparedModel, status,
                              /*fallbackModel=*/nullptr);
        if (*status != ErrorStatus::INVALID_ARGUMENT) {
            ASSERT_EQ(*status, ErrorStatus::GENERAL_FAILURE);
        }
        ASSERT_EQ(preparedModel->get(), nullptr);

        if (*status == ErrorStatus::GENERAL_FAILURE) {
            // Fallback with prepareModel should succeed regardless of cache files
            prepareModelFromCache(modelCache, dataCache, preparedModel, status,
                                  /*fallbackModel=*/model);
            // Unless the model is not supported
            if (*status != ErrorStatus::GENERAL_FAILURE) {
                // But if it is, we should see a valid model.
                ASSERT_EQ(*status, ErrorStatus::NONE);
                ASSERT_NE(preparedModel->get(), nullptr);
...
TEST_P(CompilationCachingTest, SaveToCacheInvalidNumCache) {
    ...
    // Test with number of model cache files greater than mNumModelCache.
    {
        ...
        prapareModelFromCacheOrRetryWithModel(modelCache, dataCache, &preparedModel, &status,
                                              &model, testModel);
    }
