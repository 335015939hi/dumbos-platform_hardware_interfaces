...
TEST_F(NeuralnetworksHidlTest, GetCapabilitiesTest) {
  ...
  Return<void> ret = device->getCapabilities_1_2(
      [](ErrorStatus status, const Capabilities &capabilities) {
        ...
        EXPECT_TRUE(
            isPositive(capabilities.relaxedFloat32toFloat16PerformanceScalar));
        EXPECT_TRUE(
            isPositive(capabilities.relaxedFloat32toFloat16PerformanceTensor));
        const auto &opPerf = capabilities.operandPerformance;
        EXPECT_TRUE(std::all_of(opPerf.begin(), opPerf.end(),
                                [isPositive](const OperandPerformance &a) {
                                  return isPositive(a.info);
                                }));
