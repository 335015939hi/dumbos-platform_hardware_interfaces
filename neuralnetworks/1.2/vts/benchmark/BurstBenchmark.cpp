...
class ExecutionBurstBlocking : public ModelExecution {
   public:
    void SetUp(::benchmark::State& state) override {
        ...
        // variables
        burst = createExecutionBurstController(preparedModel, true);
        if (burst == nullptr) {
   ...
   protected:
    std::unique_ptr<ExecutionBurstController> burst;
};
...
class ExecutionBurstSpinning : public ModelExecution {
   public:
    void SetUp(::benchmark::State& state) override {
        ...
        // variables
        burst = createExecutionBurstController(preparedModel, false);
        if (burst == nullptr) {
            state.SkipWithError("createExecutionBurstController error.");
        }
    }
...
BENCHMARK_DEFINE_F(ModelExecution, BM_Async_Time)(benchmark::State& state) {
    for (auto _ : state) {
...
BENCHMARK_DEFINE_F(ModelExecution, BM_Sync_Time)(benchmark::State& state) {
    for (auto _ : state) {
        ErrorStatus status = ErrorStatus::GENERAL_FAILURE;
        Return<void> ret;

        {
            ...
            // start execution
            ret = preparedModel->executeSynchronously(
                    request, MeasureTiming::NO,
                    [&status](ErrorStatus errorStatus, const auto&, const auto&) {
                        status = errorStatus;
                    });
        }
...
BENCHMARK_REGISTER_F(ModelExecution, BM_Async_Time)->Range(0, kTestValues.size() - 1);
BENCHMARK_REGISTER_F(ModelExecution, BM_Sync_Time)->Range(0, kTestValues.size() - 1);
