...
static Return<ErrorStatus> ExecutePreparedModel(const sp<V1_0::IPreparedModel>& preparedModel,
                                                const Request& request,
                                                const sp<ExecutionCallback>& callback) {
    return preparedModel->execute(request, callback);
}
static Return<ErrorStatus> ExecutePreparedModel(const sp<V1_2::IPreparedModel>& preparedModel,
                                                const Request& request,
                                                const sp<ExecutionCallback>& callback) {
    return preparedModel->execute_1_2(request, callback);
}
static Return<ErrorStatus> ExecutePreparedModel(const sp<V1_0::IPreparedModel>&, const Request&) {
...
static std::unique_ptr<::android::nn::IExecutionBurstController> CreateBurst(
    const sp<V1_0::IPreparedModel>&, const Request&) {
    ADD_FAILURE() << "asking for burst execution at V1_0";
    return nullptr;
}
static std::unique_ptr<::android::nn::IExecutionBurstController> CreateBurst(
    const sp<V1_2::IPreparedModel>& preparedModel, const Request& request) {
    return ::android::nn::createExecutionBurstController(preparedModel, request, /*blocking=*/true);
}
...
void EvaluatePreparedModel(sp<T_IPreparedModel>& preparedModel, std::function<bool(int)> is_ignored,
                           ...
                           float fpRtol = kDefaultRtol, Executor executor = Executor::ASYNC) {
        ...
        switch (executor) {
            case Executor::ASYNC: {
                ...
                Return<ErrorStatus> executionLaunchStatus =
                    ExecutePreparedModel(preparedModel, request, executionCallback);
            ...
            case Executor::SYNC: {
                ...
                // execute
                Return<ErrorStatus> executionStatus = ExecutePreparedModel(preparedModel, request);
            ...
            case Executor::BURST: {
                ...
                // create burst
                const std::unique_ptr<::android::nn::IExecutionBurstController> controller =
                    CreateBurst(preparedModel, request);
                ASSERT_NE(nullptr, controller.get());

                // execute burst
                const bool launched = controller->launch(ErrorStatus::NONE);
                ASSERT_TRUE(launched);

                // get result status
                const ErrorStatus status = controller->getResult();
...
void EvaluatePreparedModel(sp<T_IPreparedModel>& preparedModel, std::function<bool(int)> is_ignored,
                           const std::vector<MixedTypedExample>& examples,
                           bool hasRelaxedFloat32Model, Executor executor) {
    EvaluatePreparedModel(preparedModel, is_ignored, examples, hasRelaxedFloat32Model, kDefaultAtol,
                          kDefaultRtol, executor);
}
