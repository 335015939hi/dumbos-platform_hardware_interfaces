...
TEST_P(PowerAidl, setThreadHints) {
    const pid_t tid = gettid();
    ASSERT_TRUE(power->setThreadHints(tid, ThreadHint::NONE).isOk());
    ASSERT_TRUE(power->setThreadHints(tid, ThreadHint::HIGH_UTILIZATION).isOk());
    ASSERT_TRUE(power->setThreadHints(tid, ThreadHint::LOW_LATENCY).isOk());
    const int32_t bothHints = static_cast<int32_t>(ThreadHint::HIGH_UTILIZATION) |
                              static_cast<int32_t>(ThreadHint::LOW_LATENCY);
    ASSERT_TRUE(power->setThreadHints(tid, static_cast<ThreadHint>(bothHints)).isOk());
    ASSERT_TRUE(power->setThreadHints(tid, ThreadHint::NONE).isOk());
}
