...
TEST_P(PowerAidl, fixedPerformanceSupportsAllOrNone) {
    ...
    // If one FIXED_PERFORMANCE mode is supported, all three should be.
    bool supported;
    ...
    ASSERT_TRUE(power->isModeSupported(Mode::FIXED_PERFORMANCE_MODERATE, &supported).isOk());
    ASSERT_EQ(supported, supportsMaxSustainable);
}

TEST_P(PowerAidl, getFixedPerformanceScaleFactor) {
    bool supportsMaxSustainable;
    const auto result = power->isModeSupported(Mode::FIXED_PERFORMANCE_MAXIMUM_SUSTAINABLE,
                                               &supportsMaxSustainable);
    ASSERT_TRUE(result.isOk());
    ...
    int scaleFactor;
    ASSERT_TRUE(power->getFixedPerformanceScaleFactor(&scaleFactor).isOk());
    if (supportsMaxSustainable) {
        ASSERT_GT(scaleFactor, 0);
    } else {
        ASSERT_EQ(scaleFactor, 0);
    }
}
...
TEST_P(PowerAidl, setDisplayUpdateImminent) {
    const int64_t nowNs = systemTime(SYSTEM_TIME_MONOTONIC);
    ASSERT_TRUE(power->setDisplayUpdateImminent(nowNs).isOk());
    ASSERT_TRUE(power->setDisplayUpdateImminent(nowNs + 10'000'000).isOk());
}

TEST_P(PowerAidl, setTopAppPackageName) {
    String16 packageName;
    ASSERT_TRUE(power->setTopAppPackageName(packageName).isOk());
    packageName.setTo(u"com.android.vtsTestPackageName");
    ASSERT_TRUE(power->setTopAppPackageName(packageName).isOk());
}

TEST_P(PowerAidl, notifyLoadChanged) {
    ASSERT_TRUE(power->notifyLoadChanged(0.5f, 0.5f).isOk());
    ASSERT_TRUE(power->notifyLoadChanged(0.5f, 2.0f).isOk());
    ASSERT_TRUE(power->notifyLoadChanged(2.0f, 0.5f).isOk());
    ASSERT_TRUE(power->notifyLoadChanged(2.0f, 2.0f).isOk());
}

TEST_P(PowerAidl, setThreadHint) {
    const pid_t tid = gettid();
    for (const auto& threadHint : kThreadHints) {
        ASSERT_TRUE(power->setThreadHint(tid, threadHint, true).isOk());
        ASSERT_TRUE(power->setThreadHint(tid, threadHint, false).isOk());
    }
}
