...
enum Hint {
     ...
     * the current top app. Only apps which opt into one of the dynamic
     * performance framework APIs are guaranteed to provide this hint.
     */
    TOP_APP,

    /**
     * Whether setRenderingRate() is supported, which allows an app to notify
     * the HAL of its rendering rate in order to better align load measurement
     * periods with the rendered frames.
     */
    RENDERING_RATE,
    ...
    /**
     * Whether setThreadHint() is supported, which allows an app to notify the
