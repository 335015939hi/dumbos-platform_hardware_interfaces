...
class MockEnergyConsumer : public PowerStats::IEnergyConsumer {
  public:
    MockEnergyConsumer(EnergyConsumerType type, std::string name) : mType(type), mName(name) {
        mResult.timestampMs = 0;
        mResult.energyUWs = 0;
        mResult.attribution = {};
    }

    ~MockEnergyConsumer() = default;
    ...
    std::optional<EnergyConsumerResult> getEnergyConsumed() override {
        mMockEnergyConsumerResult.update(&mResult);
        return mResult;
    }

  private:
    class MockEnergyConsumerResult {
      public:
        MockEnergyConsumerResult() : mDistribution(1, 100) {}
    ...
    std::string mName;
    MockEnergyConsumerResult mMockEnergyConsumerResult;
