...
class MockEnergyMeter : public PowerStats::IEnergyMeter {
  public:
    MockEnergyMeter(std::vector<std::pair<std::string, std::string>> channelNames) {
        int32_t channelId = 0;
    ...
    ndk::ScopedAStatus readEnergyMeter(const std::vector<int32_t>& in_channelIds,
        ...
        for (auto& measurement : mEnergyMeasurements) {
            mMockEnergyMeasurement.update(&measurement);
        }
  ...
  private:
    class MockEnergyMeasurement {
      public:
        MockEnergyMeasurement() : mDistribution(1, 100) {}
    ...
    MockEnergyMeasurement mMockEnergyMeasurement;
