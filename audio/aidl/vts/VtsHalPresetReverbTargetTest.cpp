...
class PresetReverbHelper : public EffectHelper {
  public:
    ...
    Parameter createPresetReverbParam(PresetReverb::Presets param) {
        return Parameter::make<Parameter::specific>(
                Parameter::Specific::make<Parameter::Specific::presetReverb>(
                        PresetReverb::make<PresetReverb::preset>(param)));
    }

    void setAndVerifyPreset(PresetReverb::Presets param) {
...
class PresetReverbParamTest : public ::testing::TestWithParam<PresetReverbParamTestParam>,
  ...
  public:
    ...
    PresetReverb::Presets mParamPreset = kDefaultPreset;
};
...
class PresetReverbProcessTest : public ::testing::TestWithParam<PresetReverbProcessTestParam>,
  ...
  public:
    ...
    void generateSineWaveInput() {
        int frequency = 1000;
        for (size_t i = 0; i < kSamplingFrequency; i++) {
            mInput[i] = sin(2 * M_PI * frequency * i / kSamplingFrequency);
        }
    }
    ...
    void setPresetAndProcess(PresetReverb::Presets preset, std::vector<float>& output) {
        ASSERT_NO_FATAL_FAILURE(setAndVerifyPreset(preset));
        ASSERT_NO_FATAL_FAILURE(
                processAndWriteToOutput(mInput, output, mEffect, &mOpenEffectReturn));
    }

    void validateIncreasingEnergy(std::vector<PresetReverb::Presets>& presets) {
