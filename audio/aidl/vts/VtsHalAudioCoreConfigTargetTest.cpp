...
class AudioCoreConfig : public testing::TestWithParam<std::string> {
  public:
    ...
    void ValidateAudioFormatDescription(const AudioFormatDescription& format) {
        // Fake placeholder validation. Remove when
        // legacy2aidl_audio_format_t_AudioFormatDescription is available
        EXPECT_FALSE(format.encoding.empty());
    }

    /**
     * Verify each of the formatFamilies has a non-empty primaryFormat
     * Verify each primary format is unique
     * Verify no subformat appears in two families
     * Verify no subformat is listed as a primary format
     */
    void ValidateSurroundSoundConfig() {
        std::set<AudioFormatDescription> primaryFormatSet;
        std::set<AudioFormatDescription> subformatSet;
        for (const SurroundSoundConfig::SurroundFormatFamily& family :
            ...
            EXPECT_TRUE(primaryFormatSet.insert(family.primaryFormat).second);
            for (const AudioFormatDescription& subformat : family.subFormats) {
                ...
                EXPECT_TRUE(subformatSet.insert(subformat).second);
                EXPECT_FALSE(primaryFormatSet.count(subformat));
            }
        }
    }
