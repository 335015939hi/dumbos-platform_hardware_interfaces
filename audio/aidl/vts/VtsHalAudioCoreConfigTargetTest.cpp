...
class AudioCoreConfig : public testing::TestWithParam<std::string> {
  public:
    ...
    static bool IsAudioFlagReservedForSystemUse(const AudioFlag& flag) {
        return (flag == AudioFlag::SYS_RESERVED_SCO);
    }
    ...
    const int& GetSupportedAudioFlagsMask() {
        static const int supportedAudioFlagsMask = []() {
            ...
            for (const auto& audioFlag : ndk::enum_range<AudioFlag>()) {
                if (!IsAudioFlagReservedForSystemUse(audioFlag)) {
     ...
     * Validate that streamType is a valid stream type if the associated
     * volumeGroup minIndex/maxIndex are INDEX_DEFERRED_TO_AUDIO_SERVICE
     */
