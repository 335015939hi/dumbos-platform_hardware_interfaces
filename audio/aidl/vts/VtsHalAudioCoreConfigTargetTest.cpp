...
class AudioCoreConfig : public testing::TestWithParam<std::string> {
  public:
    ...
    void ValidateAudioHalConfigurationRule(
            const std::string& rule, const std::vector<AudioHalCapCriterion>& criteria,
        ...
        std::erase_if(domainRule, [](auto c) { return c == '{' || c == '}'; });
        if (domainRule.empty()) {
            return;
        }
        ...
        for (const auto& criteriaRule : criteriaRules) {
            ...
            auto criteriaIt = find_if(criteria.begin(), criteria.end(), [&](const auto& criterion) {
                return criterion.name == criterionLiteral;
            });
            EXPECT_NE(criteriaIt, criteria.end())
                    << " Invalid rule criterion " << criterionLiteral.c_str();
            AudioHalCapCriterion matchingCriterion = *criteriaIt;

            auto criterionTypesIt = find_if(
                    criterionTypes.begin(), criterionTypes.end(), [&](const auto& criterionType) {
    ...
    void ValidateAudioHalCapDomain(const AudioHalCapDomain& domain,
        ...
        for (const std::string settingParameter : domain.capSettings[0].parameterSettings) {
