...
class AudioCoreConfig : public testing::TestWithParam<std::string> {
  public:
    ...
    void ValidateAudioHalCapCriterion(const AudioHalCapCriterionV2& criterionV2) {
        switch(criterionV2.getTag()) {
            case AudioHalCapCriterionV2::availableInputDevices: {
                ...
                EXPECT_TRUE(criterion.values.has_value());
            ...
            case AudioHalCapCriterionV2::telephonyMode: {
                ...
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForCommunication: {
                auto criterion =
                        criterionV2.get<AudioHalCapCriterionV2::forceUseForCommunication>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForMedia: {
                auto criterion = criterionV2.get<AudioHalCapCriterionV2::forceUseForMedia>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForRecord: {
                auto criterion = criterionV2.get<AudioHalCapCriterionV2::forceUseForRecord>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForDock: {
                auto criterion = criterionV2.get<AudioHalCapCriterionV2::forceUseForDock>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForSystem: {
                auto criterion = criterionV2.get<AudioHalCapCriterionV2::forceUseForSystem>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForHdmiSystemAudio: {
                auto criterion =
                        criterionV2.get<AudioHalCapCriterionV2::forceUseForHdmiSystemAudio>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForEncodedSurround: {
                auto criterion =
                        criterionV2.get<AudioHalCapCriterionV2::forceUseForEncodedSurround>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            case AudioHalCapCriterionV2::forceUseForVibrateRinging: {
                auto criterion =
                        criterionV2.get<AudioHalCapCriterionV2::forceUseForVibrateRinging>();
                EXPECT_TRUE(criterion.values.has_value());
                EXPECT_EQ(criterion.logic, AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE);
                break;
            }
            default:
                EXPECT_TRUE(false) << "Invalid criterion tag " << toString(criterionV2.getTag());
        }
    }
     ...
     */
    void ValidateAudioHalCapRuleMatchingRule(const AudioHalCapRule::MatchingRule matchingRule,
        ...
        } else if (logicalDisjunction == AudioHalCapCriterionV2::LogicalDisjunction::EXLUSIVE) {
            EXPECT_TRUE(matchingRule ==  AudioHalCapRule::MatchingRule::IS
                        || matchingRule ==  AudioHalCapRule::MatchingRule::IS_NOT);
        } else {
            EXPECT_TRUE(false) << "Invalid criterion Logical rule";
        }
    }
    ...
    void validateAudioHalCapRule(CriterionV2 criterionV2, Value value,
        ...
        EXPECT_TRUE(criterionV2.values.has_value());
        if (criterionV2.values.has_value()) {
            auto values = criterionV2.values.value();
            auto valueIt = find_if(values.begin(), values.end(), [&](const auto &typedValue) {
     ...
     */
    template<typename CriterionV2, typename Value>
    void validateAudioHalCapRuleOptionalValues(CriterionV2 criterionV2, Value value,
                const AudioHalCapRule::MatchingRule matchingRule) {
        ValidateAudioHalCapRuleMatchingRule(matchingRule, criterionV2.logic);
        EXPECT_TRUE(criterionV2.values.has_value());
        if (criterionV2.values.has_value()) {
            auto values = criterionV2.values.value();
            auto valueIt = find_if(values.begin(), values.end(), [&](const auto &typedValue) {
                return typedValue.has_value() && typedValue.value() == value;
            });
            EXPECT_NE(valueIt, values.end());
        }
    }
     ...
     */
    void ValidateAudioHalConfigurationRule(const AudioHalCapRule& rule,
        ...
        if (!rule.nestedRules.has_value() &&
                (!rule.criterionRules.has_value() || rule.criterionRules.value().empty())) {
        ...
        if (rule.nestedRules.has_value()) {
            for (const auto& nestedRule : rule.nestedRules.value()) {
                EXPECT_TRUE(nestedRule.has_value());
                ValidateAudioHalConfigurationRule(nestedRule.value(), criteria);
            }
        }
        if (rule.criterionRules.has_value()) {
            for (const auto& criterionRuleOptional : rule.criterionRules.value()) {
                EXPECT_TRUE(criterionRuleOptional.has_value());
                if (!criterionRuleOptional.has_value()) {
                    continue;
                }
                const auto& criterionRule = criterionRuleOptional.value();
                ...
                switch(selectionCriterion.getTag()) {
                    case AudioHalCapCriterionV2::availableInputDevices: {
                        ...
                        validateAudioHalCapRuleOptionalValues(matchingCriterion.get<
                                AudioHalCapCriterionV2::availableInputDevices>(),
                                criterionValue.get<TypeTag::availableDevicesType>(),
                                matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::availableOutputDevices: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::availableDevicesType);
                        validateAudioHalCapRuleOptionalValues(matchingCriterion.get<
                                AudioHalCapCriterionV2::availableOutputDevices>(),
                                criterionValue.get<TypeTag::availableDevicesType>(),
                                matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::availableInputDevicesAddresses: {
                        EXPECT_EQ(criterionValue.getTag(),
                                  TypeTag::availableDevicesAddressesType);
                        validateAudioHalCapRuleOptionalValues(matchingCriterion.get<
                                AudioHalCapCriterionV2::availableInputDevicesAddresses>(),
                    ...
                    case AudioHalCapCriterionV2::availableOutputDevicesAddresses: {
                        EXPECT_EQ(criterionValue.getTag(),
                                  TypeTag::availableDevicesAddressesType);
                        validateAudioHalCapRuleOptionalValues(matchingCriterion.get<
                                AudioHalCapCriterionV2::availableOutputDevicesAddresses>(),
                                criterionValue.get<TypeTag::availableDevicesAddressesType>(),
                                matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::telephonyMode: {
                        ...
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::telephonyMode>(),
                    ...
                    case AudioHalCapCriterionV2::forceUseForCommunication: {
                        ...
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForCommunication>(),
                    ...
                    case AudioHalCapCriterionV2::forceUseForMedia: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForMedia>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForRecord: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForRecord>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForDock: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForDock>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForSystem: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForSystem>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForHdmiSystemAudio: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForHdmiSystemAudio>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForEncodedSurround: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForEncodedSurround>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    case AudioHalCapCriterionV2::forceUseForVibrateRinging: {
                        EXPECT_EQ(criterionValue.getTag(), TypeTag::forcedConfigType);
                        validateAudioHalCapRule(matchingCriterion.get<
                                AudioHalCapCriterionV2::forceUseForVibrateRinging>(),
                                criterionValue.get<TypeTag::forcedConfigType>(), matchesWhen);
                        break;
                    }
                    default:
    ...
    /**
     * Get the number of occurence of a given parameter within a given vector of parameter.
        ...
        for(const auto& domainParameter: domainParameters) {
            ...
            switch(domainParameter.getTag()) {
                case AudioHalCapParameter::selectedStrategyDevice: {
                    ...
                    auto typedParam =
                            parameter.get<AudioHalCapParameter::selectedStrategyDevice>();
                ...
                case AudioHalCapParameter::strategyDeviceAddress: {
                    ...
                    auto typedParam =
                            parameter.get<AudioHalCapParameter::strategyDeviceAddress>();
                    if (typedDomainParam.id == typedParam.id) {
                        count += 1;
                    }
                    break;
    ...
    /**
     * Verify unique criterion is provided for a given Tag.
