...
class AudioCoreConfig : public testing::TestWithParam<std::string> {
  public:
    ...
    void ValidateAudioHalConfigurationRule(
        ...
        for (const auto& operand : rules.operands) {
            if (operand.getTag() == AudioHalCapRule::Operand::compoundRule) {
                auto compoundRule = operand.get<AudioHalCapRule::Operand::compoundRule>();
                EXPECT_TRUE(compoundRule == AudioHalCapRule::CompoundRule::ANY
                        || compoundRule == AudioHalCapRule::CompoundRule::ALL);
            }
            if (operand.getTag() == AudioHalCapRule::Operand::criterionRule) {
                auto criterionRule = operand.get<AudioHalCapRule::Operand::criterionRule>();
    ...
    void ValidateAudioHalCapDomain(const AudioHalCapDomain& domain,
        ...
        for (const AudioHalCapConfiguration& configuration : domain.configurations) {
            EXPECT_TRUE(configurationNames.insert(configuration.name).second);
            ValidateAudioHalConfigurationRule(configuration.rules, criteria, criterionTypes);
        }
