...
enum ParamDensityTest { DESCRIPTOR, TAG_VALUE, PARAM_VALUE, IS_INPUT_MUTE };
...
class EnvironmentalReverbDensityTest
  ...
  public:
    EnvironmentalReverbDensityTest() : EnvironmentalReverbHelper(std::get<DESCRIPTOR>(GetParam())) {
        mTag = std::get<TAG_VALUE>(GetParam());
        mParamValues = std::get<PARAM_VALUE>(GetParam());
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<EnvironmentalReverbDensityTest::ParamType>& info) {
            ...
            auto tag = std::get<TAG_VALUE>(info.param);
            auto value = std::get<PARAM_VALUE>(info.param);
