...
static std::array<Maker, static_cast<int>(EnvironmentalReverb::levelMb) + 1> maker = {
        nullptr,
        []() -> std::set<int> {
...
static std::vector<TagValuePair> buildSetAndGetTestParams() {
    ...
    for (EnvironmentalReverb::Tag tag : ndk::enum_range<EnvironmentalReverb::Tag>()) {
        ...
        if (intTag <= static_cast<int>(EnvironmentalReverb::levelMb) && maker[intTag] != nullptr) {
            values = maker[intTag]();
        }
...
class EnvironmentalReverbHelper : public EffectHelper {
  public:
    ...
    static constexpr std::array<Maker, static_cast<int>(EnvironmentalReverb::bypass) + 1> makers = {
            nullptr,
            ...
            [](int value) -> EnvironmentalReverb {
                return EnvironmentalReverb::make<EnvironmentalReverb::decayHfRatioPm>(value);
            },
            nullptr,
            nullptr,
            [](int value) -> EnvironmentalReverb {
    ...
    void createEnvParam(EnvironmentalReverb::Tag tag, int paramValue) {
        int intTag = static_cast<int>(tag);
        if (intTag <= static_cast<int>(EnvironmentalReverb::bypass) && makers[intTag] != NULL) {
            mEnvParam = makers[intTag](paramValue);
        } else {
