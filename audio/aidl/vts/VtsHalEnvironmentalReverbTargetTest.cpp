...
using ParamTagValuePair = std::pair<EnvironmentalReverb::Tag, std::vector<int>>;
using TagMinValuePair = std::pair<EnvironmentalReverb::Tag, int>;
...
std::vector<TagMinValuePair> minimumValueParams = {{EnvironmentalReverb::roomLevelMb, -6000},
                                                   {EnvironmentalReverb::decayTimeMs, 0},
                                                   {EnvironmentalReverb::levelMb, -6000}};
...
static std::vector<std::pair<int, EnvironmentalReverb::Tag>> buildParameters() {
    std::vector<std::pair<int, EnvironmentalReverb::Tag>> valueTag;
    for (EnvironmentalReverb::Tag tag : tags) {
        ...
        for (const auto& value : values) {
            valueTag.push_back(std::make_pair(value, tag));
        }
    }
 ...
 * - Specific parameter checking, general IEffect interfaces testing
 *   performed in VtsAudioEffectTargetTest.
 ...
 * - Validating the effect by comapring the outputs of the supported parameters.
 */
...
class EnvironmentalReverbHelper : public EffectHelper {
  public:
    ...
    float computeReverbOutputEnergy(const std::vector<float>& input, std::vector<float>& output) {
        if (!isAuxiliary()) {
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Combine(testing::ValuesIn(EffectFactoryHelper::getAllEffectDescriptors(
                           ...
                           testing::ValuesIn(buildParameters())),
        [](const testing::TestParamInfo<EnvironmentalReverbParamTest::ParamType>& info) {
            auto descriptor = std::get<0>(info.param).second;
            auto tag = std::get<1>(std::get<1>(info.param));
            auto val = std::get<0>(std::get<1>(info.param));
            std::string name =
...
class EnvironmentalDataTest
  ...
  public:
    ...
    void assertEnergyIncreasingWithParameter() {
        ...
        float baseEnergy = 0;
        for (int val : mParamPair.second) {
            createEnvParam(mParamPair.first, val);
            std::vector<float> output(kBufferSize);
            // Skipping the further steps for unnsupported decay time values
            if (!isParamValid(mEnvParam)) {
                continue;
            }
            ASSERT_NO_FATAL_FAILURE(setAndVerifyParam(EX_NONE, mEnvParam, mParamPair.first));
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<EnvironmentalDataTest::ParamType>& info) {
            ...
            std::string name = getPrefix(descriptor) + "_Tag_" + toString(val.first);
            return name;
        });
...
TEST_P(MinimumParamValueTest, MinimumValueTest) {
    ...
    generateSineWaveInput(input);
    std::vector<float> output(kBufferSize);
    // Output at minimum levelMb is zero only when roomLevelMb is set to minimum
    if (mParamPair.first == EnvironmentalReverb::levelMb) {
    ...
    createEnvParam(mParamPair.first, mParamPair.second);
    ASSERT_NO_FATAL_FAILURE(setAndVerifyParam(EX_NONE, mEnvParam, mParamPair.first));
    ASSERT_NO_FATAL_FAILURE(processAndWriteToOutput(input, output, mEffect, &ret));
    float energy = computeReverbOutputEnergy(input, output);
...
class EnvironmentalReverbBypassDataTest
    : public ::testing::TestWithParam<
              std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor>, bool>>,
      public EnvironmentalReverbHelper {
  public:
    EnvironmentalReverbBypassDataTest() : EnvironmentalReverbHelper(std::get<0>(GetParam())) {
...
TEST_P(EnvironmentalReverbBypassDataTest, BypassDataTest) {
...
INSTANTIATE_TEST_SUITE_P(
        EnvironmentalReverbTest, EnvironmentalReverbBypassDataTest,
        ::testing::Combine(testing::ValuesIn(EffectFactoryHelper::getAllEffectDescriptors(
                                   IFactory::descriptor, getEffectTypeUuidEnvReverb())),
                           testing::Bool()),
        [](const testing::TestParamInfo<EnvironmentalReverbBypassDataTest::ParamType>& info) {
            auto descriptor = std::get<0>(info.param).second;
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(EnvironmentalReverbBypassDataTest);
