...
static std::vector<TagValuePair> buildSetAndGetTestParams() {
    ...
    std::set<int> testValueSetDecayParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::decayTimeMs>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetDecayParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::decayTimeMs, value));
    }

    std::set<int> testValueSetRoomLevelMbParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::roomLevelMb>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetRoomLevelMbParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::roomLevelMb, value));
    }

    std::set<int> testValueSetRoomHfLevelMbParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::roomHfLevelMb>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetRoomHfLevelMbParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::roomHfLevelMb, value));
    }

    std::set<int> testValueSetDecayHfRatioPmParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::decayHfRatioPm>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetDecayHfRatioPmParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::decayHfRatioPm, value));
    }

    std::set<int> testValueSetlevelMbParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::levelMb>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetlevelMbParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::levelMb, value));
    }

    std::set<int> testValueSetDiffusionPmParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::diffusionPm>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetDiffusionPmParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::diffusionPm, value));
    }

    std::set<int> testValueSetDensityPmParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::densityPm>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetDensityPmParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::densityPm, value));
    }

    std::set<int> testValueSetBypassParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::bypass>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetBypassParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::bypass, value));
    }

    std::set<int> testValueSetdelayMsParam =
            EffectHelper::getTestValueSet<EnvironmentalReverb, int, Range::environmentalReverb,
                                          EnvironmentalReverb::delayMs>(
                    kDescPair, EffectHelper::expandTestValueBasic<int>);
    for (const auto& value : testValueSetdelayMsParam) {
        valueTag.push_back(std::make_pair(EnvironmentalReverb::delayMs, value));
    }
...
class EnvironmentalReverbDataTest
  ...
  public:
    ...
    void assertEnergyIncreasingWithParameter() {
        ...
        float baseEnergy = 0;
        if (mBypass) {
            createEnvParam(EnvironmentalReverb::bypass, mBypass);
            ASSERT_NO_FATAL_FAILURE(
                    setAndVerifyParam(EX_NONE, mEnvParam, EnvironmentalReverb::bypass));
        }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Combine(testing::ValuesIn(EffectFactoryHelper::getAllEffectDescriptors(
                                   IFactory::descriptor, getEffectTypeUuidEnvReverb())),
                           testing::ValuesIn(kParamsIncreasingVector), testing::Bool()),
        [](const testing::TestParamInfo<EnvironmentalReverbDataTest::ParamType>& info) {
            auto descriptor = std::get<0>(info.param).second;
            auto tag = std::get<1>(info.param).first;
            auto byPass = std::get<2>(info.param);
            std::string name = getPrefix(descriptor) + "_Tag_" + toString(tag) + "_Bypass_" +
                               std::to_string(byPass);
            return name;
        });
