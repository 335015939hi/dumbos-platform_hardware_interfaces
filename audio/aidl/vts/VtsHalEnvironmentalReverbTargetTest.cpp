...
enum ParamName { kDescriptorIndex, kTagValuePairIndex };
enum TagValue { kTagIndex, kValueIndex };
...
class EnvironmentalReverbParamTest
  ...
  public:
    EnvironmentalReverbParamTest()
        : EnvironmentalReverbHelper(std::get<kDescriptorIndex>(GetParam())) {
        std::tie(mTag, mParamValue) = std::get<kTagValuePairIndex>(GetParam());
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<EnvironmentalReverbParamTest::ParamType>& info) {
            auto descriptor = std::get<kDescriptorIndex>(info.param).second;
            auto tag = std::get<kTagValuePairIndex>(info.param).first;
            auto val = std::get<kTagValuePairIndex>(info.param).second;
            std::string name =
...
class EnvironmentalReverbDataTest
  ...
  public:
    EnvironmentalReverbDataTest()
        : EnvironmentalReverbHelper(std::get<kDescriptorIndex>(GetParam())) {
        std::tie(mTag, mParamValues) = std::get<kTagValuePairIndex>(GetParam());
...
class EnvironmentalReverbMinimumParamTest
  ...
  public:
    EnvironmentalReverbMinimumParamTest()
        : EnvironmentalReverbHelper(std::get<kDescriptorIndex>(GetParam())) {
        std::tie(mTag, mValue) = std::get<kTagValuePairIndex>(GetParam());
    }
    void SetUp() override {
        SetUpReverb();
