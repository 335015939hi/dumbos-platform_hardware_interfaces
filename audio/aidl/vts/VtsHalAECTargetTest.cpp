...
using AECParamTestParam = std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor>,
                                     int /* echoDelayMs */, bool /* mobileMode */>;
...
class AECParamTest : public ::testing::TestWithParam<AECParamTestParam>, public EffectHelper {
  public:
    ...
    Parameter::Specific getDefaultParamSpecific() {
        AcousticEchoCanceler aec = AcousticEchoCanceler::make<AcousticEchoCanceler::echoDelayMs>(0);
    ...
    void addEchoDelayParam(int delay) {
        ...
        aec.set<AcousticEchoCanceler::echoDelayMs>(delay);
        mTags.push_back({AcousticEchoCanceler::echoDelayMs, aec});
    }
    ...
    bool isTagInRange(const AcousticEchoCanceler::Tag& tag, const AcousticEchoCanceler& aec,
        ...
        switch (tag) {
            case AcousticEchoCanceler::echoDelayMs: {
                return isEchoDelayInRange(aecCap, aec.get<AcousticEchoCanceler::echoDelayMs>());
            }
    ...
    bool isEchoDelayInRange(const AcousticEchoCanceler::Capability& cap, int delay) const {
        return (delay >= 0 && delay <= cap.maxEchoDelayMs);
    }
    ...
    static std::vector<int> getEchoDelayTestValues() {
        ...
        int maxDelay =
                max->second.capability.get<Capability::acousticEchoCanceler>().maxEchoDelayMs;
