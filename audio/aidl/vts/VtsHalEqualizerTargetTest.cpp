...
class EqualizerTest : public ::testing::TestWithParam<EqualizerParamTestParam>,
  ...
  public:
    ...
    void SetUp() override {
        ...
        create(mFactory, mEffect, mIdentity);
        ...
        open(mEffect, common, specific, &ret, EX_NONE);
        ASSERT_NE(nullptr, mEffect);
        setTagRange();
    }
    void TearDown() override {
        close(mEffect);
        destroy(mFactory, mEffect);
    }
...
TEST_P(EqualizerTest, SetAndGetPresetOutOfLowerBound) {
    EXPECT_NO_FATAL_FAILURE(addPresetParam(mPresetIndex.second - 1));
...
TEST_P(EqualizerTest, SetAndGetPresetAtLowerBound) {
    EXPECT_NO_FATAL_FAILURE(addPresetParam(mPresetIndex.first));
    EXPECT_NO_FATAL_FAILURE(SetAndGetEqualizerParameters());
}

TEST_P(EqualizerTest, SetAndGetPresetAtHigherBound) {
    EXPECT_NO_FATAL_FAILURE(addPresetParam(mPresetIndex.second));
    EXPECT_NO_FATAL_FAILURE(SetAndGetEqualizerParameters());
}

TEST_P(EqualizerTest, SetAndGetPresetInBound) {
    EXPECT_NO_FATAL_FAILURE(addPresetParam((mPresetIndex.first + mPresetIndex.second) >> 1));
    EXPECT_NO_FATAL_FAILURE(SetAndGetEqualizerParameters());
}
