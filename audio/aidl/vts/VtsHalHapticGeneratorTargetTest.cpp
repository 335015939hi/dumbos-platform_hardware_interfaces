...
class HapticGeneratorDataTest : public ::testing::TestWithParam<HapticGeneratorDataTestParam>,
  ...
  public:
    HapticGeneratorDataTest() : mOutputLayout(std::get<DATA_LAYOUT>(GetParam())) {
        ...
        mOutputChannelCount = getChannelCount(
                AudioChannelLayout::make<AudioChannelLayout::layoutMask>(mOutputLayout),
                AudioChannelLayout::LAYOUT_HAPTIC_AB);
        mOutputBufferSize = kFrameCount * mOutputChannelCount;
        generateInput();
    }
    ...
    int mInputChannelCount =
            getChannelCount(AudioChannelLayout::make<AudioChannelLayout::layoutMask>(
                    AudioChannelLayout::LAYOUT_MONO));
    size_t kInputBufferSize = kFrameCount * mInputChannelCount;
...
TEST_P(HapticGeneratorDataTest, IncreaseingScaleTest) {
    std::vector<float> baseOutput(mOutputBufferSize);
    ...
    ASSERT_NO_FATAL_FAILURE(processAndWriteToOutput(mInput, baseOutput, mEffect, &ret));

    float baseEnergy = audio_utils_compute_energy_mono(baseOutput.data(), AUDIO_FORMAT_PCM_FLOAT,
                                                       baseOutput.size());
