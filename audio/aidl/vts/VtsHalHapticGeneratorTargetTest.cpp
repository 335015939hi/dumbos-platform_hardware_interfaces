...
std::vector<HapticGenerator::VibratorScale> kScaleValues = {HapticGenerator::VibratorScale::HIGH
...
class HapticGeneratorHelper : public EffectHelper {
  public:
    ...
    if(outputLayout== AudioChannelLayout::LAYOUT_HAPTIC_AB){
        kInputFrameCount=3*kInputSize;
        kOutputFrameCount=kInputFrameCount/2;
    }
    else{
        kInputFrameCount=2*kInputSize;
        kOutputFrameCount=kInputFrameCount;
    }
        Parameter::Common common = createParamCommon(
                0 /* session */, 1 /* ioHandle */, kSamplingFrequency /* iSampleRate */,
                kSamplingFrequency /* oSampleRate */, kInputFrameCount /* iFrameCount */,
                kOutputFrameCount /* oFrameCount */, inputchannelLayout, outputchannelLayout);
    ...
    HapticGenerator::VibratorInformation createVibratorInfo(float resonantFrequency, float qFactor,
     ...
     long kInputFrameCount;
    long kInputSize = 10000;
     long kHapticFrameCount ;
     long kOutputFrameCount ;
...
enum DataTestParam { EFFECT_INSTANCE, OUTPUT_LAYOUT };
...
class HapticGeneratorDataTest : public ::testing::TestWithParam<HapticGeneratorDataTestParam>,
                                public HapticGeneratorHelper {
  public:
    HapticGeneratorDataTest() : mOutputLayout(std::get<OUTPUT_LAYOUT>(GetParam())) {
        ...
        mHapticChannelCount = getChannelCount(
                ...
                AudioChannelLayout::LAYOUT_HAPTIC_AB);
    ALOGE("---constructor---kInputFrameCount=%ld-----kOutputFrameCount=%ld--------\n",kInputFrameCount,kOutputFrameCount);
    ...
    void validateIncreasingEnergy(HapticGenerator::Tag tag) {
        ...
        for (auto param : mHapticParam) {
            ...
            ALOGE("-------output=%zu---mInput=%zu-------\n",output.size(),mInput.size());
            ASSERT_NO_FATAL_FAILURE(setAndVerifyParameter(param, tag));
            SCOPED_TRACE("Param: " + param.toString());
            ALOGE("-------------before process------ output.size()-kInputFrameCount=%lu-----\n", output.size()-kInputFrameCount);
           processAndWriteToOutput(mInput, output, mEffect, &ret);
        FILE* fpout;
        fpout = fopen("data/local/tmp/output.raw", "w");
        fwrite(output.data(), sizeof(float), output.size(), fpout);
        fclose(fpout);
            float hapticOutputEnergy = audio_utils_compute_energy_mono(
                    output.data() + kInputFrameCount, AUDIO_FORMAT_PCM_FLOAT,
                    output.size()-kInputFrameCount);
    ...
    void findMaxAmplitude() {
        for (float amp = 0.1; amp <= 1; amp += 0.1) {
            ...
            float outAmplitude = findAbsMax(output.begin() + kInputSize, output.end());
    ...
    int kInputFrequency = 1000;
    int mInputChannelCount =
            getChannelCount(AudioChannelLayout::make<AudioChannelLayout::layoutMask>(
                    AudioChannelLayout::LAYOUT_MONO));
    size_t mInputBufferSize = kInputFrameCount * mInputChannelCount;
...
TEST_P(HapticGeneratorDataTest, IncreasingVibratorScaleTest) {
     mHapticOutputBufferSize = kOutputFrameCount * mHapticChannelCount;
        mOutputBufferSize = mHapticOutputBufferSize;
        mInput.resize(kInputFrameCount);
        ALOGE("--mInput.size()=%zu---mOutputBufferSize=%zu------mInputBufferSize=%zu----------------\n",mOutputBufferSize,mInputBufferSize,mInput.size());

    generateInput(mInput, kInputFrequency, kSamplingFrequency);
...
//      mHapticOutputBufferSize = kOutputFrameCount * mHapticChannelCount;
//         mOutputBufferSize = mHapticOutputBufferSize;
//         mInput.resize(mInputBufferSize);
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<HapticGeneratorDataTest::ParamType>& info) {
            ...
            std::string layout = std::to_string(std::get<OUTPUT_LAYOUT>(info.param));
