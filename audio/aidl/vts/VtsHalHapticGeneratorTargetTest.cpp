...
const std::vector<int> kHapticScaleIdValues = {MIN_ID - 1, MIN_ID, MAX_ID};
...
class HapticGeneratorParamTest : public ::testing::TestWithParam<HapticGeneratorParamTestParam>,
  ...
  public:
    ...
    bool isTagInRange(const HapticGenerator::Tag& tag, const HapticGenerator& hg) const {
        switch (tag) {
            case HapticGenerator::hapticScale: {
                HapticGenerator::HapticScale hapticScale = hg.get<HapticGenerator::hapticScale>();
                return isHapticScaleInRange(hapticScale);
            }
    ...
    bool isHapticScaleInRange(HapticGenerator::HapticScale hapticScale) const {
        if (hapticScale.id >= 0) return true;
        return false;
    }
...
INSTANTIATE_TEST_SUITE_P(
        HapticGeneratorTest, HapticGeneratorParamTest,
        ...
        [](const testing::TestParamInfo<HapticGeneratorParamTest::ParamType>& info) {
            ...
            std::string name = address.str() + "_UUID_timeLow_" +
                               ...
                               "_hapticScaleId" + hapticScaleID + "_hapticScaleVibScale" +
                               hapticScaleVibScale + "_resonantFrequency" + resonantFrequency +
