...
class HapticGeneratorHelper : public EffectHelper {
  public:
    ...
    Parameter createScaleParam(const std::vector<HapticGenerator::HapticScale>& hapticScales) {
        auto first = Parameter::make<Parameter::specific>(
...
class HapticGeneratorDataTest : public ::testing::TestWithParam<HapticGeneratorDataTestParam>,
  ...
  public:
    HapticGeneratorDataTest() : mOutputLayout(std::get<OUTPUT_LAYOUT>(GetParam())) {
        ...
        mOutputBufferSize = mHapticOutputBufferSize + mInputBufferSize;
    ...
    void generateInput() {
        for (size_t i = 0; i < mInputBufferSize; i++) {
            mInput[i] = sin(2 * M_PI * kInputFrequency * i / kSamplingFrequency);
        }
    }

    void generatePulseInput() {
        size_t cycleSize = kSamplingFrequency / kInputFrequency;
        size_t startSize = 0;
        while (startSize < mInputBufferSize) {
            for (size_t i = startSize; i < cycleSize + startSize; i++) {
                mInput[i] = sin(2 * M_PI * kInputFrequency * (i - startSize) / kSamplingFrequency);
            }
    ...
    void validateIncreasingEnergy(HapticGenerator::Tag tag) {
        ...
        for (auto param : mHapticParam) {
            ...
            float hapticOutputEnergy = audio_utils_compute_energy_mono(
                    output.data() + mInputBufferSize, AUDIO_FORMAT_PCM_FLOAT,
                    mHapticOutputBufferSize);
            ASSERT_GT(hapticOutputEnergy, baseEnergy);
...
TEST_P(HapticGeneratorDataTest, IncreasingVibratorScaleTest) {
    ...
    validateIncreasingEnergy(HapticGenerator::hapticScales);
}

TEST_P(HapticGeneratorDataTest, IncreasingMaxAmplitudeTest) {
    std::vector<float> increasingAmplitudeValues = {0.25, 0.5, 0.75, kDefaultMaxAmp};
    generateInput();
    setBaseScaleParam();
    ...
    validateIncreasingEnergy(HapticGenerator::vibratorInfo);
}
