...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            ...
            auto enable = std::get<LIMITER_ENABLE>(info.param);
            ...
            fillLimiterConfig(cfg, channelIndex, enable, linkGroup, attackTime, releaseTime, ratio,
                              threshold, postGain);
            std::string name =
  ...
  public:
    ...
    void checkExpectedRatio(float threshold, float expectedRatio,
        ...
        if (threshold >= mInputDb) {
            ...
            EXPECT_EQ(expectedRatio, std::round(ratio));
        }
    }
