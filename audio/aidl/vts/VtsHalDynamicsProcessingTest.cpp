...
void fillEngineArchConfig(DynamicsProcessing::EngineArchitecture& cfg,
    ...
    cfg.resolutionPreference = std::get<1>(params);
    cfg.preferredFrameDurationMs = std::get<2>(params);
    cfg.preEqBand.inUse = std::get<3>(params);
    cfg.preEqBand.bandCount = std::get<4>(params);
    cfg.postEqBand.inUse = std::get<5>(params);
    cfg.postEqBand.bandCount = std::get<6>(params);
    cfg.mbcBand.inUse = std::get<7>(params);
    cfg.mbcBand.bandCount = std::get<8>(params);
    cfg.limiterInUse = std::get<9>(params);
}
...
class DynamicsProcessingTestEngineArchitecture
  ...
  public:
    DynamicsProcessingTestEngineArchitecture()
        : DynamicsProcessingTestHelper(std::get<PARAM_INSTANCE_NAME>(GetParam())) {
...
using LimiterConfigTestParams =
        std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor>, int32_t, bool, bool, int32_t,
...
void fillLimiterConfig(DynamicsProcessing::LimiterConfig& cfg,
    ...
    cfg.channel = std::get<1>(params);
    cfg.enable = std::get<2>(params);
    cfg.inUse = std::get<3>(params);
    cfg.linkGroup = std::get<4>(params);
    cfg.attackTimeMs = std::get<5>(params);
    cfg.releaseTimeMs = std::get<6>(params);
    cfg.ratio = std::get<7>(params);
    cfg.thresholdDb = std::get<8>(params);
    cfg.postGainDb = std::get<9>(params);
}
...
class DynamicsProcessingTestLimiterConfig
  ...
  public:
    DynamicsProcessingTestLimiterConfig()
        : DynamicsProcessingTestHelper(std::get<PARAM_INSTANCE_NAME>(GetParam())),
          mLimiterInUseEngine(std::get<10>(GetParam())) {
...
TEST_P(DynamicsProcessingTestLimiterConfig, SetAndGetLimiterConfig) {
    mEngineConfig.limiterInUse = mLimiterInUseEngine;
 ...
 */
using BandChannelConfigTestParams = std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor>,
                                               int32_t, bool, int32_t, bool, int32_t>;

void fillBandChannelConfig(DynamicsProcessing::BandChannelConfig& cfg,
                           const BandChannelConfigTestParams& params) {
    cfg.channel = std::get<1>(params);
    cfg.enablement.inUse = std::get<2>(params);
    cfg.enablement.bandCount = std::get<3>(params);
}

class DynamicsProcessingTestBandChannelConfig
    : public ::testing::TestWithParam<BandChannelConfigTestParams>,
      public DynamicsProcessingTestHelper {
  public:
    DynamicsProcessingTestBandChannelConfig()
        ...
        fillBandChannelConfig(mCfg, GetParam());
    }
    ...
    DynamicsProcessing::BandChannelConfig mCfg;
...
TEST_P(DynamicsProcessingTestBandChannelConfig, SetAndGetPreEqBandChannelConfig) {
    ...
    EXPECT_NO_FATAL_FAILURE(addEngineConfig(mEngineConfig));
    EXPECT_NO_FATAL_FAILURE(addPreEqBandChannelConfig(mCfg));
    SetAndGetDynamicsProcessingParameters();
}
...
TEST_P(DynamicsProcessingTestBandChannelConfig, SetAndGetPostEqBandChannelConfig) {
    ...
    EXPECT_NO_FATAL_FAILURE(addEngineConfig(mEngineConfig));
    EXPECT_NO_FATAL_FAILURE(addPostEqBandChannelConfig(mCfg));
    SetAndGetDynamicsProcessingParameters();
}
...
TEST_P(DynamicsProcessingTestBandChannelConfig, SetAndGetMbcBandChannelConfig) {
    ...
    EXPECT_NO_FATAL_FAILURE(addEngineConfig(mEngineConfig));
    EXPECT_NO_FATAL_FAILURE(addMbcBandChannelConfig(mCfg));
    SetAndGetDynamicsProcessingParameters();
}
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            auto descriptor = std::get<PARAM_INSTANCE_NAME>(info.param).second;
            DynamicsProcessing::BandChannelConfig cfg;
            fillBandChannelConfig(cfg, info.param);
...
void fillEqBandConfig(std::vector<DynamicsProcessing::EqBandConfig>& cfgs,
    ...
    const std::vector<std::pair<int, float>> cutOffFreqs = std::get<3>(params);
    int bandCount = cutOffFreqs.size();
    cfgs.resize(bandCount);
    for (int i = 0; i < bandCount; i++) {
        cfgs[i].channel = std::get<1>(params);
        cfgs[i].band = cutOffFreqs[i].first;
        cfgs[i].enable = std::get<2>(params);
        cfgs[i].cutoffFrequency = cutOffFreqs[i].second;
        cfgs[i].gain = std::get<4>(params);
    }
}
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            ...
            std::string name = "Implementor_" + descriptor.common.implementor + "_name_" +
                               descriptor.common.name + "_UUID_" +
                               descriptor.common.id.uuid.toString() + "_Cfg_";
            for (auto cfg : cfgs) name += cfg.toString();
            name += "_stageInUse_" + stageInUse;
            std::replace_if(
                    name.begin(), name.end(), [](const char c) { return !std::isalnum(c); }, '_');
...
void fillMbcBandConfig(std::vector<DynamicsProcessing::MbcBandConfig>& cfgs,
    ...
    for (int i = 0; i < bandCount; i++) {
        ...
        cfgs[i].gainDb = std::get<4>(params);
        cfgs[i].attackTimeMs = std::get<5>(params);
        cfgs[i].releaseTimeMs = std::get<6>(params);
        cfgs[i].ratio = std::get<7>(params);
        cfgs[i].thresholdDb = std::get<8>(params);
        cfgs[i].kneeWidthDb = std::get<9>(params);
        cfgs[i].noiseGateThresholdDb = std::get<10>(params);
        cfgs[i].expanderRatio = std::get<11>(params);
        cfgs[i].preGainDb = std::get<12>(params);
        cfgs[i].postGainDb = std::get<13>(params);
    }
}

class DynamicsProcessingTestMbcBandConfig
    : public ::testing::TestWithParam<MbcBandConfigTestParams>,
      public DynamicsProcessingTestHelper {
  public:
