...
class DynamicsProcessingTestHelper : public EffectHelper {
    ...
    DynamicsProcessingTestHelper(std::pair<std::shared_ptr<IFactory>, Descriptor> pair,
        ...
        : mAudioChannelLayout(channelLayOut),
          mChannelCount(::aidl::android::hardware::audio::common::getChannelCount(
                  AudioChannelLayout::make<AudioChannelLayout::layoutMask>(mAudioChannelLayout))) {
  ...
  private:
    const int32_t mAudioChannelLayout;
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            ...
            cfg.emplace_back(DynamicsProcessing::LimiterConfig{
                    .channel = std::get<LIMITER_CHANNEL>(info.param),
                    .enable = true,
                    .linkGroup = std::get<LIMITER_LINK_GROUP>(info.param),
                    .attackTimeMs = std::get<LIMITER_ATTACK_TIME>(info.param),
                    .releaseTimeMs = std::get<LIMITER_RELEASE_TIME>(info.param),
                    .ratio = std::get<LIMITER_RATIO>(info.param),
                    .thresholdDb = std::get<LIMITER_THRESHOLD>(info.param),
                    .postGainDb = std::get<LIMITER_POST_GAIN>(info.param),
            });
...
class DynamicsProcessingLimiterConfigDataTest
  ...
  public:
    ...
    void setParamsAndProcess(std::vector<float>& output, bool& allParamsValid) {
        ...
        EXPECT_NO_FATAL_FAILURE(addLimiterConfig(mLimiterConfigList));
        ASSERT_NO_FATAL_FAILURE(SetAndGetDynamicsProcessingParameters());
        isAllParamsValid(allParamsValid);
        if (allParamsValid) {
