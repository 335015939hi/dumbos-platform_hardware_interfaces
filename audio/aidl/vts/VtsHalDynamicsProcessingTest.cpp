...
class DynamicsProcessingTestHelper : public EffectHelper {
  public:
    ...
    void isAllParamsValid(bool& allParamsValid);
...
void DynamicsProcessingTestHelper::isAllParamsValid(bool& allParamsValid) {
    ...
    for (auto& it : mTags) {
        ...
        Descriptor desc;
        ASSERT_STATUS(EX_NONE, mEffect->getDescriptor(&desc));
        if (!isParamInRange(dp, desc.capability.range.get<Range::dynamicsProcessing>())) {
            return;
        }
...
class DynamicsProcessingTestLimiterConfig
  ...
  public:
    DynamicsProcessingTestLimiterConfig()
        ...
        mLimiterConfigList.emplace_back(DynamicsProcessing::LimiterConfig{
                .channel = std::get<LIMITER_CHANNEL>(GetParam()),
                .enable = true,
                .linkGroup = std::get<LIMITER_LINK_GROUP>(GetParam()),
                .attackTimeMs = std::get<LIMITER_ATTACK_TIME>(GetParam()),
                .releaseTimeMs = std::get<LIMITER_RELEASE_TIME>(GetParam()),
                .ratio = std::get<LIMITER_RATIO>(GetParam()),
                .thresholdDb = std::get<LIMITER_THRESHOLD>(GetParam()),
                .postGainDb = std::get<LIMITER_POST_GAIN>(GetParam()),
        });
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            ...
            DynamicsProcessing::LimiterConfig cfg;
            cfg.channel = std::get<LIMITER_CHANNEL>(info.param);
            cfg.enable = true;
            cfg.linkGroup = std::get<LIMITER_LINK_GROUP>(info.param);
            cfg.attackTimeMs = std::get<LIMITER_ATTACK_TIME>(info.param);
            cfg.releaseTimeMs = std::get<LIMITER_RELEASE_TIME>(info.param);
            cfg.ratio = std::get<LIMITER_RATIO>(info.param);
            cfg.thresholdDb = std::get<LIMITER_THRESHOLD>(info.param);
            cfg.postGainDb = std::get<LIMITER_POST_GAIN>(info.param);
            std::string name =
                    "Implementer_" + getPrefix(descriptor) + "_limiterConfig_" + cfg.toString();
            std::replace_if(
                    name.begin(), name.end(), [](const char c) { return !std::isalnum(c); }, '_');
            return name;
        });
...
class DynamicsProcessingLimiterConfigDataTest
  ...
  public:
    DynamicsProcessingLimiterConfigDataTest()
        ...
        mEngineConfigPreset.preferredProcessingDurationMs = 200;
    }
    ...
    void checkExpectedRatio(float threshold, float expectedRatio,
        ...
        if (threshold >= mInputDb) {
            ...
            EXPECT_NEAR(expectedRatio, ratio, expectedRatio * kPercentageTolerance / 100);
        }
    }
    ...
    void setParamsAndProcess(std::vector<float>& output, bool& allParamsValid) {
        ...
        EXPECT_NO_FATAL_FAILURE(addLimiterConfig(mLimiterConfigList));
        ASSERT_NO_FATAL_FAILURE(SetAndGetDynamicsProcessingParameters());
        ASSERT_NO_FATAL_FAILURE(isAllParamsValid(allParamsValid));
        if (allParamsValid) {
    ...
    static constexpr int kStartIndex = 450 * kSamplingFrequency / 1000;  // skip 450ms
    static constexpr float kPercentageTolerance = 0.5;
    std::vector<DynamicsProcessing::LimiterConfig> mLimiterConfigList;
...
TEST_P(DynamicsProcessingLimiterConfigDataTest, IncreasingThresholdDb) {
    std::vector<float> thresholdValues = {-150, -100, -50, -5, 0};
