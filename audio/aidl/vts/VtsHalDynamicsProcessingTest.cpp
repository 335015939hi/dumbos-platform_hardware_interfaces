...
class DynamicsProcessingLimiterLinkerDataTest : public DynamicsProcessingLimiterConfigDataTest {
  public:
    ...
    void calculateExpectedOutputDb(std::vector<float>& expectedOutput) {
        ...
        for (size_t i = 0; i < kThresholdRatioPairValues.size(); i++) {
            float threshold = kThresholdRatioPairValues[i].first;
            float ratio = kThresholdRatioPairValues[i].second;
            ASSERT_NE(ratio, 0);
            expectedOutput.push_back((inputDbValues[i] - threshold) / ratio + threshold);
        }
    }
...
TEST_P(DynamicsProcessingLimiterLinkerDataTest, SameLinkGroupDifferentConfigs) {
    ...
    for (size_t i = 0; i < outputDbValues.size(); i++) {
        EXPECT_NEAR(outputDbValues[i], expectedOutputDb, kToleranceDb);
    }
}
...
TEST_P(DynamicsProcessingLimiterLinkerDataTest, DifferentLinkGroupDifferentConfigs) {
    ...
    for (size_t i = 0; i < outputDbValues.size(); i++) {
        EXPECT_NEAR(outputDbValues[i], expectedOutputDbValues[i], kToleranceDb);
    }
}
