...
class DynamicsProcessingTestHelper : public EffectHelper {
  public:
    ...
    bool isMbcBandChannelConfigValid(
            const std::vector<DynamicsProcessing::BandChannelConfig>& cfgs);
    bool isEqBandConfigValid(const std::vector<DynamicsProcessing::EqBandConfig>& cfgs, bool inUse,
                             int bandCount);
    ...
    bool isBandChannelConfigEqual(const DynamicsProcessing::BandChannelConfig& refCfg,
                                  const DynamicsProcessing::BandChannelConfig& testCfg, bool inUse);
    bool isBandChannelConfigEqual(
            const std::vector<DynamicsProcessing::BandChannelConfig>& refCfgs,
            const std::vector<DynamicsProcessing::BandChannelConfig>& testCfgs, bool inUse);
    bool isBandConfigEqual(const DynamicsProcessing::EqBandConfig& refCfg,
                           const DynamicsProcessing::EqBandConfig& testCfg, bool inUse);
    bool isBandConfigEqual(const std::vector<DynamicsProcessing::EqBandConfig>& refCfgs,
    ...
    // EffectHelper::createParamCommon() by default uses LAYOUT_STEREO
    const int kMaxChannelCount = 2;
...
bool DynamicsProcessingTestHelper::isParamEqual(const DynamicsProcessing::Tag& tag,
    ...
    switch (tag) {
        ...
        case DynamicsProcessing::preEqBand: {
            return isBandConfigEqual(dpRef.get<DynamicsProcessing::preEqBand>(),
                                     dpTest.get<DynamicsProcessing::preEqBand>(),
                                     mEngineConfig.preEqBand.inUse);
        }
        case DynamicsProcessing::postEqBand: {
            return isBandConfigEqual(dpRef.get<DynamicsProcessing::postEqBand>(),
...
bool DynamicsProcessingTestHelper::isLimiterConfigValid(
    ...
    for (auto cfg : cfgs) {
        ...
        if (cfg.inUse) {
            if (cfg.channel < 0 || cfg.channel >= kMaxChannelCount) return false;
...
bool DynamicsProcessingTestHelper::isEqBandConfigValid(
        const std::vector<DynamicsProcessing::EqBandConfig>& cfgs, bool inUse, int bandCount) {
    if (!inUse) return true;
    std::vector<float> freqs(cfgs.size());
    for (auto cfg : cfgs) {
        if (cfg.channel < 0 || cfg.channel >= kMaxChannelCount) return false;
        if (cfg.band < 0 || cfg.band >= bandCount) return false;
        if (cfg.cutoffFrequency < DEFAULT_MIN_FREQUENCY ||
            cfg.cutoffFrequency > DEFAULT_MAX_FREQUENCY) {
            return false;
        }
...
bool DynamicsProcessingTestHelper::isLimiterConfigEqual(
    ...
    for (size_t i = 0; i < refCfgs.size(); i++) {
        ...
        if (j == testCfgs.size() || !isLimiterConfigEqual(refCfgs[i], testCfgs[j])) return false;
    }
    return true;
}
...
bool DynamicsProcessingTestHelper::isBandChannelConfigEqual(
    ...
    for (size_t i = 0; i < refCfgs.size(); i++) {
        ...
        if (j == testCfgs.size() || !isBandChannelConfigEqual(refCfgs[i], testCfgs[j], inUse)) {
            return false;
        }
    }
    return true;
...
bool DynamicsProcessingTestHelper::isBandConfigEqual(
...
bool DynamicsProcessingTestHelper::isBandConfigEqual(
        ...
        const std::vector<DynamicsProcessing::EqBandConfig>& testCfgs, bool inUse) {
    if (!inUse) return true;
    for (size_t i = 0; i < refCfgs.size(); i++) {
        size_t j;
        for (j = 0; j < testCfgs.size(); j++) {
        ...
        if (j == testCfgs.size() || !isBandConfigEqual(refCfgs[i], testCfgs[j], inUse)) {
            return false;
        }
    }
    return true;
