...
class DynamicsProcessingTestHelper : public EffectHelper {
  public:
    DynamicsProcessingTestHelper(std::pair<std::shared_ptr<IFactory>, Descriptor> pair,
        ...
        : kAudioChannelLayout(channelLayOut),
          kChannelCount(::aidl::android::hardware::audio::common::getChannelCount(
                  AudioChannelLayout::make<AudioChannelLayout::layoutMask>(kAudioChannelLayout))) {
    ...
    bool isChannelConfigValid(const std::vector<T>& cfgs) {
        auto& channelCount = kChannelCount;
    ...
    void SetAndGetDynamicsProcessingParameters(bool* allParamsValid = nullptr);
    ...
    static constexpr int kDurationInSec = 1;
    static constexpr int kFrameCount = kSamplingFrequency * kDurationInSec;
  ...
  private:
    const int32_t kAudioChannelLayout;
...
bool DynamicsProcessingTestHelper::isBandConfigValid(const std::vector<T>& cfgs, int bandCount) {
    ...
    for (auto cfg : cfgs) {
        if (cfg.channel < 0 || cfg.channel >= kChannelCount) return false;
...
void DynamicsProcessingTestHelper::SetAndGetDynamicsProcessingParameters(bool* allParamsValid) {
    for (auto& it : mTags) {
        auto& tag = it.first;
        auto& dp = it.second;
...
class DynamicsProcessingTestLimiterConfig
  ...
  public:
    DynamicsProcessingTestLimiterConfig()
        ...
        fillLimiterConfig(mLimiterConfigList, mChannelIndex, true /*enableLimiter*/, mLinkerGroup,
                          mAttackTime, mReleaseTime, mRatio, mThreshold, mPostGain);
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const auto& info) {
            ...
            auto channelIndex = std::get<LIMITER_CHANNEL>(info.param);
            auto linkGroup = std::get<LIMITER_LINK_GROUP>(info.param);
            auto attackTime = std::get<LIMITER_ATTACK_TIME>(info.param);
            auto releaseTime = std::get<LIMITER_RELEASE_TIME>(info.param);
            auto ratio = std::get<LIMITER_RATIO>(info.param);
            auto threshold = std::get<LIMITER_THRESHOLD>(info.param);
            auto postGain = std::get<LIMITER_POST_GAIN>(info.param);
            std::vector<DynamicsProcessing::LimiterConfig> cfg;
            fillLimiterConfig(cfg, channelIndex, true /*enable*/, linkGroup, attackTime,
                              releaseTime, ratio, threshold, postGain);
            std::string name =
...
class DynamicsProcessingLimiterConfigDataTest
  ...
  public:
    DynamicsProcessingLimiterConfigDataTest()
        ...
        mBufferSize = kFrameCount * kChannelCount;
    ...
    float calculateDb(std::vector<float> input, size_t start = 0) {
        float rmse = audio_utils_compute_energy_mono(input.data() + start, AUDIO_FORMAT_PCM_FLOAT,
                                                     input.size() - start);
        float dbVal = 20 * log10(sqrt(rmse / static_cast<int>(input.size() - start)));
        return dbVal;
    }

    void checkExpectedRatio(float threshold, float expectedRatio,
        ...
        if (threshold >= mInputDb) {
            ...
            float ratio = inputOverThreshold / outputOverThreshold;
            ASSERT_EQ(expectedRatio, std::round(ratio));
        }
    }
    ...
    void setParamsAndProcess(std::vector<float>& output, bool* allParamsValid) {
        EXPECT_NO_FATAL_FAILURE(addEngineConfig(mEngineConfigPreset));
        EXPECT_NO_FATAL_FAILURE(addLimiterConfig(mLimiterConfigList));
        ASSERT_NO_FATAL_FAILURE(SetAndGetDynamicsProcessingParameters(allParamsValid));
        if (allParamsValid) {
    ...
    static constexpr int kStartIndex = 500 * kSamplingFrequency / 1000;  // skip 300ms
    static constexpr float kMinTolerance = 0.0001;
    std::vector<DynamicsProcessing::LimiterConfig> mLimiterConfigList;
...
TEST_P(DynamicsProcessingLimiterConfigDataTest, IncreasingThresholdDb) {
    ...
    for (float threshold : thresholdValues) {
        ...
        setParamsAndProcess(output, &allParamsValid);
        if (!allParamsValid) {
            continue;
        }
        EXPECT_NO_FATAL_FAILURE(checkExpectedRatio(threshold, kDefaultRatio, output));
    }
}
...
TEST_P(DynamicsProcessingLimiterConfigDataTest, IncreasingRatio) {
    ...
    for (float ratio : ratioValues) {
        ...
        EXPECT_NO_FATAL_FAILURE(checkExpectedRatio(threshold, ratio, output));
    }
}

TEST_P(DynamicsProcessingLimiterConfigDataTest, LimiterEnableDisable) {
    std::vector<bool> limiterEnabledValues = {true, false};
    std::vector<float> output(mBufferSize);
    std::vector<bool> limiterEnableValues = {true, false};
    for (bool isEnabled : limiterEnableValues) {
        ...
        if (isEnabled) {
            EXPECT_NE(std::round(mInputDb), std::round(calculateDb(output, kStartIndex)));
        } else {
            EXPECT_EQ(std::round(mInputDb), std::round(calculateDb(output, kStartIndex)));
        }
    }
}
...
TEST_P(DynamicsProcessingTestEqBandConfig, SetAndGetPreEqBandConfig) {
    ...
    EXPECT_NO_FATAL_FAILURE(addEngineConfig(mEngineConfigPreset));
    std::vector<DynamicsProcessing::ChannelConfig> cfgs(kChannelCount);
    for (int i = 0; i < kChannelCount; i++) {
        cfgs[i].channel = i;
        cfgs[i].enable = true;
    }
