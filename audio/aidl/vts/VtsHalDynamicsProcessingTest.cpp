...
class DynamicsProcessingLimiterConfigDataTest
  ...
  public:
    ...
    void setLimiterParamsAndProcess(std::vector<float>& input, std::vector<float>& output,
        ...
        mEngineConfigPreset.limiterInUse = isEngineEnabled;
        addEngineConfig(mEngineConfigPreset);
        addLimiterConfig(mLimiterConfigList);
        EXPECT_NO_FATAL_FAILURE(setParamsAndProcess(input, output));
    }

    void testEnableDisableConfiguration(bool isLimiterEnabled, bool isEngineEnabled) {
        ...
        ASSERT_NO_FATAL_FAILURE(setLimiterParamsAndProcess(mInput, output, isEngineEnabled));
        float outputdB = calculateDb(output, kStartIndex);
        if (isAllParamsValid()) {
            if (isLimiterEnabled & isEngineEnabled) {
    ...
    const std::vector<bool> kEnableValues = {true, false};
    std::vector<DynamicsProcessing::LimiterConfig> mLimiterConfigList;
    int mBufferSize;
};
...
TEST_P(DynamicsProcessingLimiterConfigDataTest, EngineConfigEnableDisable) {
    for (bool isEngineEnabled : kEnableValues) {
        ASSERT_NO_FATAL_FAILURE(
                testEnableDisableConfiguration(true /*Limiter Enabled*/, isEngineEnabled));
    }
}
