...
class DynamicsProcessingTestHelper : public EffectHelper {
  public:
    ...
    bool isInputGainEqual(const std::vector<DynamicsProcessing::InputGain>& source,
                          const std::vector<DynamicsProcessing::InputGain>& target);
...
bool DynamicsProcessingTestHelper::isParamEqual(const DynamicsProcessing::Tag& tag,
    ...
    switch (tag) {
        ...
        case DynamicsProcessing::preEq: {
            ...
            return isAidlVectorEqual<DynamicsProcessing::ChannelConfig>(source, target);
        }
        case DynamicsProcessing::postEq: {
            return isAidlVectorEqual<DynamicsProcessing::ChannelConfig>(
        ...
        case DynamicsProcessing::preEqBand: {
            return isAidlVectorEqual<DynamicsProcessing::EqBandConfig>(
        ...
        case DynamicsProcessing::mbcBand: {
            return isAidlVectorEqual<DynamicsProcessing::MbcBandConfig>(
                    dpRef.get<DynamicsProcessing::mbcBand>(),
                    dpTest.get<DynamicsProcessing::mbcBand>());
        }
        case DynamicsProcessing::limiter: {
            return isAidlVectorEqual<DynamicsProcessing::LimiterConfig>(
                    dpRef.get<DynamicsProcessing::limiter>(),
                    dpTest.get<DynamicsProcessing::limiter>());
        }
        case DynamicsProcessing::inputGain: {
            return isInputGainEqual(dpRef.get<DynamicsProcessing::inputGain>(),
...
bool DynamicsProcessingTestHelper::isAidlVectorEqual(const std::vector<T>& source,
    ...
    std::vector<T> tempS = filterEnabledVector<T>(source);
    std::vector<T> tempT = filterEnabledVector<T>(target);

    if (tempS.size() != tempT.size()) return false;
    std::sort(tempS.begin(), tempS.end());
    std::sort(tempT.begin(), tempT.end());
    return tempS == tempT;
}

bool DynamicsProcessingTestHelper::isInputGainEqual(
        const std::vector<DynamicsProcessing::InputGain>& source,
        const std::vector<DynamicsProcessing::InputGain>& target) {
