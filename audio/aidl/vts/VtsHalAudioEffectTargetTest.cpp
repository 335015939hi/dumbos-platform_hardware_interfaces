...
TEST_P(AudioEffectDataPathTest, ConsumeDataInProcessingState) {
    ...
    IEffect::OpenEffectReturn ret;
    ASSERT_NO_FATAL_FAILURE(open(mEffect, common, std::nullopt /* specific */, &ret, EX_NONE));
    std::vector<float> inputBuffer(mInputSizeInFloat), outputBuffer(mOutputSizeInFloat);
    ASSERT_NO_FATAL_FAILURE(
...
TEST_P(AudioEffectDataPathTest, ConsumeDataAfterReopen) {
    ...
    inputBuffer.resize(mInputSizeInFloat);
    outputBuffer.resize(mOutputSizeInFloat);
    ASSERT_NO_FATAL_FAILURE(
            processAndWriteToOutput(inputBuffer, outputBuffer, mEffect, &ret, mVersion));
...
TEST_P(AudioEffectDataPathTest, ConsumeDataMultipleEffects) {
    ...
    std::vector<float> inputBuffer1(kInputFrameCount * mIFrameSize / sizeof(float)),
            outputBuffer1(kOutputFrameCount * mOFrameSize / sizeof(float));
    std::vector<float> inputBuffer2(2 * kInputFrameCount * mIFrameSize / sizeof(float)),
            outputBuffer2(2 * kOutputFrameCount * mOFrameSize / sizeof(float));
