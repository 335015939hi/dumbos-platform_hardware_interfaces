...
TEST_P(AudioEffectTest, CreateAndDestroy) {
    ...
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(destroy, mFactory, effect);
}

TEST_P(AudioEffectTest, OpenAndClose) {
    std::shared_ptr<IEffect> effect;
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(open, effect);
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(destroy, mFactory, effect);
}
...
TEST_P(AudioEffectTest, DoubleOpenAndClose) {
    ...
    HELPER_ASSERT(create, mFactory, effect1, mIdentity);
    HELPER_ASSERT(create, mFactory, effect2, mIdentity);
    HELPER_ASSERT(open, effect1);
    HELPER_ASSERT(open, effect2, 1 /* session */);
    HELPER_ASSERT(close, effect1);
    HELPER_ASSERT(close, effect2);
    HELPER_ASSERT(destroy, mFactory, effect1);
    HELPER_ASSERT(destroy, mFactory, effect2);
}
...
TEST_P(AudioEffectTest, TripleOpenAndClose) {
    ...
    HELPER_ASSERT(create, mFactory, effect1, mIdentity);
    HELPER_ASSERT(create, mFactory, effect2, mIdentity);
    HELPER_ASSERT(create, mFactory, effect3, mIdentity);
    HELPER_ASSERT(open, effect1);
    HELPER_ASSERT(open, effect2, 1 /* session */);
    HELPER_ASSERT(open, effect3, 2 /* session */);
    HELPER_ASSERT(close, effect1);
    HELPER_ASSERT(close, effect2);
    HELPER_ASSERT(close, effect3);
    HELPER_ASSERT(destroy, mFactory, effect1);
    HELPER_ASSERT(destroy, mFactory, effect2);
    HELPER_ASSERT(destroy, mFactory, effect3);
}
...
TEST_P(AudioEffectTest, GetDescritorBeforeOpen) {
    ...
    Descriptor desc;
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(getDescriptor, effect, desc);
...
TEST_P(AudioEffectTest, GetDescritorAfterOpen) {
    ...
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(getDescriptor, effect, beforeOpen);
    HELPER_ASSERT(open, effect);
    HELPER_ASSERT(getDescriptor, effect, afterOpen);
    EXPECT_EQ(beforeOpen.toString(), afterOpen.toString()) << "\n"
                                                           << beforeOpen.toString() << "\n"
                                                           << afterOpen.toString();
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(getDescriptor, effect, afterClose);
...
TEST_P(AudioEffectTest, InitStateAfterCreation) {
    std::shared_ptr<IEffect> effect;
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(expectState, effect, State::INIT);
    HELPER_ASSERT(destroy, mFactory, effect);
}

// An effect instance transfer to IDLE state after IEffect.HELPER_ASSERT(open, ).
TEST_P(AudioEffectTest, IdleStateAfterOpen) {
    std::shared_ptr<IEffect> effect;
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(open, effect);
    HELPER_ASSERT(expectState, effect, State::IDLE);
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(destroy, mFactory, effect);
}
...
TEST_P(AudioEffectTest, ProcessingStateAfterStart) {
    ...
    HELPER_ASSERT(open, effect);
    HELPER_ASSERT(expectState, effect, State::IDLE);
    HELPER_ASSERT(command, effect, CommandId::START);
    HELPER_ASSERT(expectState, effect, State::PROCESSING);
    HELPER_ASSERT(command, effect, CommandId::STOP);
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(destroy, mFactory, effect);
}
...
TEST_P(AudioEffectTest, IdleStateAfterReset) {
    ...
    HELPER_ASSERT(command, effect, CommandId::START);
    HELPER_ASSERT(expectState, effect, State::PROCESSING);
    HELPER_ASSERT(command, effect, CommandId::RESET);
    HELPER_ASSERT(expectState, effect, State::IDLE);
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(destroy, mFactory, effect);
}

// An effect instance transfer to INIT after IEffect.HELPER_ASSERT(close, ).
...
TEST_P(AudioEffectTest, NoCommandAcceptedBeforeOpen) {
    std::shared_ptr<IEffect> effect;
    HELPER_ASSERT(create, mFactory, effect, mIdentity);
    HELPER_ASSERT(command, effect, CommandId::START, EX_ILLEGAL_STATE);
    HELPER_ASSERT(command, effect, CommandId::STOP, EX_ILLEGAL_STATE);
    HELPER_ASSERT(command, effect, CommandId::RESET, EX_ILLEGAL_STATE);
    HELPER_ASSERT(open, effect);
    HELPER_ASSERT(close, effect);
    HELPER_ASSERT(destroy, mFactory, effect);
}
...
TEST_P(AudioEffectTest, CloseProcessingStateEffects) {
    ...
    HELPER_ASSERT(close, effect, EX_ILLEGAL_STATE);
...
TEST_P(AudioEffectTest, DestroyOpenEffects) {
    ...
    HELPER_ASSERT(destroy, mFactory, effect, EX_ILLEGAL_STATE);
}
...
TEST_P(AudioEffectTest, VerifyCommonParametersAfterOpen) {
    ...
    HELPER_ASSERT(open, effect, common, std::nullopt /* specific */, &ret, EX_NONE);
...
TEST_P(AudioEffectTest, ConsumeDataMultipleEffects) {
    ...
    HELPER_ASSERT(open, effect1, common1, std::nullopt /* specific */, &ret1, EX_NONE);
    HELPER_ASSERT(open, effect2, common2, std::nullopt /* specific */, &ret2, EX_NONE);
    HELPER_ASSERT(command, effect1, CommandId::START);
    HELPER_ASSERT(expectState, effect1, State::PROCESSING);
    HELPER_ASSERT(command, effect2, CommandId::START);
    HELPER_ASSERT(expectState, effect2, State::PROCESSING);
    ...
    HELPER_ASSERT(command, effect1, CommandId::STOP);
    HELPER_ASSERT(expectState, effect1, State::IDLE);
    HELPER_ASSERT(close, effect1);
    HELPER_ASSERT(destroy, mFactory, effect1);

    HELPER_ASSERT(command, effect2, CommandId::STOP);
    HELPER_ASSERT(expectState, effect2, State::IDLE);
    HELPER_ASSERT(close, effect2);
    HELPER_ASSERT(destroy, mFactory, effect2);
}
