...
TEST_P(AudioEffectDataPathTest, SetCommonParameterAndReopen) {
    if (int version = EffectFactoryHelper::getHalVersion(mFactory);
        version < kReopenSupportedVersion) {
        GTEST_SKIP() << "Skipping test as effect V" << version << " does not support reopen";
    }
