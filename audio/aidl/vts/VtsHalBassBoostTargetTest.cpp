...
TEST_P(BassBoostDataTest, IncreasingStrength) {
    ...
    ASSERT_NO_FATAL_FAILURE(generateSineWave(testFrequencies, input, 1.0, kSamplingFrequency,
                                             mChannelLayout == AudioChannelLayout::LAYOUT_STEREO));

    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            inputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, input, binOffsets));
    ...
    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            baseMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, baseOutput, binOffsets));
