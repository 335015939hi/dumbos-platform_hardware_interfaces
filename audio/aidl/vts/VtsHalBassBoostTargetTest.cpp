...
TEST_P(BassBoostParamTest, SetAndGetStrength) {
    if (isStrengthValid(mParamStrength)) {
        ASSERT_NO_FATAL_FAILURE(setParameters(mParamStrength, EX_NONE));
        ASSERT_NO_FATAL_FAILURE(validateParameters(mParamStrength));
    } else {
        ASSERT_NO_FATAL_FAILURE(setParameters(mParamStrength, EX_ILLEGAL_ARGUMENT));
    }
}
...
TEST_P(BassBoostDataTest, IncreasingStrength) {
    ...
    if (isStrengthValid(0)) {
        ASSERT_NO_FATAL_FAILURE(setParameters(0, EX_NONE));
    } else {
        GTEST_SKIP() << "Invalid Base Strength\n";
    }
    ...
    for (int strength : mStrengthValues) {
        ...
        ASSERT_NO_FATAL_FAILURE(setParameters(strength, EX_NONE));
