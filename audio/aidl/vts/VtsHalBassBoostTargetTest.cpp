...
class BassBoostEffectHelper : public EffectHelper {
  public:
    ...
    void getFramecounts(int32_t inputBufferLayout) {
    ...
    binder_exception_t isStrengthValid(int strength) {
        auto bb = BassBoost::make<BassBoost::strengthPm>(strength);
        if (isParameterValid<BassBoost, Range::bassBoost>(bb, mDescriptor)) {
            return EX_NONE;
        } else {
    ...
    void setParameters(int strength, binder_exception_t expected) {
        auto param = createBassBoostParam(strength);
        EXPECT_STATUS(expected, mEffect->setParameter(param)) << param.toString();
    }

    void validateParameters(int strength) {
    ...
    static constexpr float kDuration = 2;
    static constexpr int kInputSize = kSamplingFrequency * kDuration;
...
class BassBoostParamTest : public ::testing::TestWithParam<BassBoostParamTestParam>,
  ...
  public:
    ...
    void SetUp() override { SetUpBassBoost(); }
...
TEST_P(BassBoostParamTest, SetAndGetStrength) {
    binder_exception_t expected = isStrengthValid(mParamStrength);
    ASSERT_NO_FATAL_FAILURE(setParameters(mParamStrength, expected));
    if (expected == EX_NONE) {
        ASSERT_NO_FATAL_FAILURE(validateParameters(mParamStrength));
    }
}
...
class BassBoostDataTest : public ::testing::TestWithParam<BassBoostDataTestParam>,
  ...
  public:
    ...
    std::vector<float> calculateMagnitude(std::vector<float>& buffer,
                                          std::vector<int>& binOffsets) {
    ...
    float calculateGainDiff(std::vector<float>& inputMag, std::vector<float>& outputMag) {
    ...
    static constexpr int kNPointFFT = 4096;
...
TEST_P(BassBoostDataTest, IncreasingStrength) {
    ...
    binder_exception_t expected = isStrengthValid(0);
    ASSERT_NO_FATAL_FAILURE(setParameters(0, expected));
    ...
    for (int strength : mStrengthValues) {
        binder_exception_t expected = isStrengthValid(strength);
        // Skipping the further steps for invalid strength values
        if (expected != EX_NONE) {
            continue;
        }

        ASSERT_NO_FATAL_FAILURE(setParameters(strength, expected));
