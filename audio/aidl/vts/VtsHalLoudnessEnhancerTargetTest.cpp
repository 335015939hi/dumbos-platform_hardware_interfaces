...
TEST_P(LoudnessEnhancerParamTest, IncreasingGains) {
    ...
    // Process a reference output buffer with 0 gain
    process(inputBuffer, baseOutput, 0 /* gain */);

    // Compare the outputs for increasing gain
    const std::vector<int> kGainMbValues = {50, 100, std::numeric_limits<int>::max()};
...
TEST_P(LoudnessEnhancerParamTest, MinimumGain) {
    ...
    process(inputBuffer, outputBuffer, std::numeric_limits<int>::min());
