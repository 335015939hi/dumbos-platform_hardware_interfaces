...
static const std::vector<int> kGainMbValues = {kMinGain, -50, -100, 0, 50, 100, kMaxGain};

class LoudnessEnhancerEffectTest : public EffectHelper {
  public:
...
class LoudnessEnhancerParamTest : public ::testing::TestWithParam<LoudnessEnhancerParamTestParam>,
                                  public LoudnessEnhancerEffectTest {
  public:
...
class LoudnessEnhancerDataTest : public ::testing::TestWithParam<LoudnessEnhancerDataTestParam>,
  ...
  public:
    ...
    // Fill inputBuffer with random values between -1 to 1
    void generateInputBuffer(std::vector<float>& buffer) {
        for (size_t i = 0; i < buffer.size(); i++) {
            buffer[i] = (static_cast<float>(std::rand()) / RAND_MAX) * 2 - kMaxAudioSample;
        }
    }
    ...
    void assertGreaterGain(std::vector<float> first, std::vector<float> second) {
        for (size_t i = 0; i < first.size(); i++) {
            if (first[i] != 0) ASSERT_GT(abs(first[i]), abs(second[i]));
        }
    }
};
...
TEST_P(LoudnessEnhancerDataTest, DecreasingGains) {
    ...
    // Compare the outputs for increasing gain
    for (int gain : kDecreasingGains) {
        std::vector<float> outputBuffer(128);

        // Setting the parameters
        if (SetParameters(gain) != EX_NONE) {
            GTEST_SKIP() << "Negative gains not supported.";
        }

        // Add gains to the input buffer
        process(inputBuffer, outputBuffer);

        // Compare the Decrease in the outputBuffer values with baseOutput and update it
        assertGreaterGain(baseOutput, outputBuffer);
        for (size_t i = 0; i < outputBuffer.size(); i++) {
            baseOutput[i] = outputBuffer[i];
        }
    }
...
TEST_P(LoudnessEnhancerDataTest, MinimumGain) {
    ...
    // Validate that outputBuffer has 0 values for INT_MIN gain
    for (size_t i = 0; i < outputBuffer.size(); i++) {
        ASSERT_FLOAT_EQ(outputBuffer[i], 0);
    }
}
...
TEST_P(LoudnessEnhancerDataTest, MaximumGain) {
    ...
    // Validate that outputBuffer reaches to kMaxAudioSample for INT_MAX gain
    for (size_t i = 0; i < outputBuffer.size(); i++) {
        if (outputBuffer[i] != 0) {
            EXPECT_NEAR(kMaxAudioSample, abs(outputBuffer[i]), kAbsError);
        }
    }
}
