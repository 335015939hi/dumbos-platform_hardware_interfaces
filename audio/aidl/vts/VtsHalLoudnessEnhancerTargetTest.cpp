...
class LoudnessEnhancerParamTest : public ::testing::TestWithParam<LoudnessEnhancerParamTestParam>,
  ...
  public:
    ...
    void SetAndGetLoudnessEnhancerParameters() {
        auto functor = [&](const std::shared_ptr<IEffect>& effect) {
            for (auto& it : mTags) {
                ...
                specific.set<Parameter::Specific::loudnessEnhancer>(*le.get());
    ...
    void addGainMbParam(int gainMb) {
        ...
        mTags.push_back(
                {LoudnessEnhancer::gainMb, std::make_unique<LoudnessEnhancer>(std::move(le))});
    }

  private:
    std::vector<std::pair<LoudnessEnhancer::Tag, std::unique_ptr<LoudnessEnhancer>>> mTags;
