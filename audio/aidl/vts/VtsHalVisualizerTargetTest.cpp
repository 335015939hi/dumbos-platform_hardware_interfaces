...
const std::vector<int> kLatencyValues = {Visualizer::MIN_LATENCY - 1, Visualizer::MIN_LATENCY,
...
class VisualizerParamTest : public ::testing::TestWithParam<VisualizerParamTestParam>,
  ...
  public:
    ...
    bool isLatencyInRange(const Visualizer::Capability& cap, int latency) const {
        return (latency >= Visualizer::MIN_LATENCY && latency <= cap.maxLatencyMs);
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<VisualizerParamTest::ParamType>& info) {
            ...
            std::string name = instance.second.uuid.toString() + "_captureSize" + captureSize +
