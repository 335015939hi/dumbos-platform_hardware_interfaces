...
class VisualizerTestHelper : public EffectHelper {
  public:
    VisualizerTestHelper(
            int captureSize = 128, int latency = 0,
            ...
            std::pair<std::shared_ptr<IFactory>, Descriptor> descPair = {})
        ...
        mBufferSizeInFrames = kInputFrameCount * mChannelCount;
    ...
    void SetAndGetParameters(bool& allParamsValid = *(new bool(true))) {
        allParamsValid = true;
        for (auto& it : mCommonTags) {
            ...
            if (expected == EX_ILLEGAL_ARGUMENT) allParamsValid = false;
