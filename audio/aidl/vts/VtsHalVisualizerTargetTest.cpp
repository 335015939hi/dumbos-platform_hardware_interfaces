...
TEST_P(VisualizerDataTest, testScalingModeandMeasurementParameters) {
    SKIP_TEST_IF_DATA_UNSUPPORTED(mDescriptor.common.flags);
    ...
    // These values holds true for a sinewave input of 44100Hz sampling frequency, 1000Hz testing
    // frequency, 256 input frames, stereo channel and captureSize = 128
    constexpr float expectedPowerN = -3.175283;
    const std::map<float, int> maxAudioSampleRmsMap = {
            {0.25f, -1508}, {0.5f, -1110}, {0.75f, -839}, {1.0f, -633}};
    const std::map<float, int> maxAudioSamplePeakMap = {
            {0.25f, -1204}, {0.5f, -602}, {0.75f, -249}, {1.0f, 0}};
    ...
    Parameter::Id idM;
    Visualizer::Id vsIdM;
    vsIdM.set<Visualizer::Id::commonTag>(Visualizer::measurement);
    idM.set<Parameter::Id::visualizerTag>(vsIdM);
    ...
    for (float maxAudioSampleValue : testMaxAudioSampleValueList) {
        ...
        if (allParamsValid) {
            ...
            EXPECT_STATUS(EX_NONE, mEffect->getParameter(idM, &getParam))
                    << " with: " << idM.toString();
            Visualizer::Measurement measurement = getParam.get<Parameter::specific>()
                                                          .get<Parameter::Specific::visualizer>()
                                                          .get<Visualizer::measurement>();
            int currRms = measurement.rms;
            int currPeak = measurement.peak;
            ...
            if (mScalingMode == Visualizer::ScalingMode::NORMALIZED) {
                EXPECT_NEAR(currPowerCsb, expectedPowerN, 0.0001);
            } else {
                float powerI = audio_utils_compute_power_mono(
                        mInputBuffer.data(), AUDIO_FORMAT_PCM_FLOAT, mInputBuffer.size());
                EXPECT_NEAR(currPowerCsb, powerI, 0.5);
            }
            EXPECT_EQ(currRms, maxAudioSampleRmsMap.find(maxAudioSampleValue)->second);
            EXPECT_EQ(currPeak, maxAudioSamplePeakMap.find(maxAudioSampleValue)->second);
        }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Combine(
                ...
                testing::Values(128),
                testing::ValuesIn(VisualizerTestHelper::getScalingModeValues()),
                testing::Values(Visualizer::MeasurementMode::PEAK_RMS), testing::Values(0)),
