...
TEST_P(VisualizerDataTest, testScalingModeParameters) {
    ...
    float refPowerCsbAP = -FLT_MAX, refEnergyCsbAP = -FLT_MAX;
    float refPowerCsbN = -FLT_MAX, refEnergyCsbN = -FLT_MAX;
    int refRmsAP = INT_MIN;
    ...
    for (float maxAudioSampleValue : testMaxAudioSampleValueList) {
        ...
        if (allParamsValid) {
            ...
            float currEnergyCsb = audio_utils_compute_energy_mono(
                    captureBuffer.data(), AUDIO_FORMAT_PCM_8_BIT, mCaptureSize);

            if (mScalingMode == Visualizer::ScalingMode::NORMALIZED) {
                if (maxAudioSampleValue == testMaxAudioSampleValueList.front()) {
                    refPowerCsbN = currPowerCsb;
                    refEnergyCsbN = currEnergyCsb;
                } else {
                    EXPECT_EQ(currPowerCsb, refPowerCsbN);
                    EXPECT_EQ(currEnergyCsb, refEnergyCsbN);
                }
            } else {
                if (currRms > refRmsAP) {
                    EXPECT_GT(currPowerCsb, refPowerCsbAP);
                    EXPECT_GT(currEnergyCsb, refEnergyCsbAP);
                } else {
                    EXPECT_LT(currPowerCsb, refPowerCsbAP);
                    EXPECT_LT(currEnergyCsb, refEnergyCsbAP);
                }
                refPowerCsbAP = currPowerCsb;
                refEnergyCsbAP = currEnergyCsb;
