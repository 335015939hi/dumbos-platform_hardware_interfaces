...
TEST_P(VisualizerDataTest, testScalingModeAndMeasurementModeParameters) {
    SKIP_TEST_IF_DATA_UNSUPPORTED(mDescriptor.common.flags);
    ...
    float kPowerToleranceNormalized = 0.1;
    float kPowerToleranceAsPlayed = 0.5;

    generateSineWave(std::vector<int>{1000}, mInputBuffer, 1.0, mBufferSizeInFrames);
    const float kExpectedPowerNormalized = audio_utils_compute_power_mono(
    ...
    Parameter::Id idMeasurement;
    Visualizer::Id vsidMeasurement;
    vsidMeasurement.set<Visualizer::Id::commonTag>(Visualizer::measurement);
    idMeasurement.set<Parameter::Id::visualizerTag>(vsidMeasurement);
    ...
    for (float maxAudioSampleValue : testMaxAudioSampleValueList) {
        ...
        if (allParamsValid) {
            ...
            EXPECT_STATUS(EX_NONE, mEffect->getParameter(idMeasurement, &getParam))
                    << " with: " << idMeasurement.toString();
            ...
            if (mScalingMode == Visualizer::ScalingMode::NORMALIZED) {
                EXPECT_NEAR(currPowerCsb, kExpectedPowerNormalized, kPowerToleranceNormalized);
            } else {
                float powerI = audio_utils_compute_power_mono(
                        mInputBuffer.data(), AUDIO_FORMAT_PCM_FLOAT, mInputBuffer.size());
                EXPECT_NEAR(currPowerCsb, powerI, kPowerToleranceAsPlayed);
            }
        }
