...
class VisualizerTestHelper : public EffectHelper {
  public:
    ...
    void SetAndGetParameters(bool* allParamsValid = nullptr) {
        if (allParamsValid) *allParamsValid = true;
        for (auto& it : mCommonTags) {
            ...
            if (expected == EX_ILLEGAL_ARGUMENT && allParamsValid) *allParamsValid = false;
...
TEST_P(VisualizerDataTest, testScalingModeParameters) {
    ...
    const std::vector<Visualizer::ScalingMode> scalingModes = {Visualizer::ScalingMode::NORMALIZED,
                                                               Visualizer::ScalingMode::AS_PLAYED};
    ...
    for (Visualizer::ScalingMode scalingMode : scalingModes) {
        for (float maxAudioSampleValue : testMaxAudioSampleValueList) {
            ...
            ASSERT_NO_FATAL_FAILURE(processAndWriteToOutput(
                    mInputBuffer, mOutputBuffer, mEffect, &mOpenEffectReturn, mVersion, 1, false));

            if (allParamsValid) {
                EXPECT_STATUS(EX_NONE, mEffect->getParameter(id, &getParam))
                        << " with: " << id.toString();
                std::vector<uint8_t> captureBuffer =
                        getParam.get<Parameter::specific>()
                                .get<Parameter::Specific::visualizer>()
                                .get<Visualizer::captureSampleBuffer>();
                ASSERT_EQ((size_t)mCaptureSize, captureBuffer.size());
                ...
                if (scalingMode == Visualizer::ScalingMode::NORMALIZED) {
