...
TEST_P(AudioStreamOut, PlaybackRate) {
    ...
    Float minSpeedF, maxSpeedF, minPitchF, maxPitchF;
    ndk::ScopedAStatus status;
    EXPECT_STATUS(kStatuses, status = module.get()->getSupportedPlaybackRateFactors(
                                     &minSpeedF, &maxSpeedF, &minPitchF, &maxPitchF));
    if (status.getExceptionCode() == EX_UNSUPPORTED_OPERATION) {
        GTEST_SKIP() << "Audio playback rate configuration is not supported";
    }
    const float minSpeed = minSpeedF.value, maxSpeed = maxSpeedF.value;
    const float minPitch = minPitchF.value, maxPitch = maxPitchF.value;
    EXPECT_LE(minSpeed, 1.0f);
    EXPECT_GE(maxSpeed, 1.0f);
    EXPECT_LE(minPitch, 1.0f);
    EXPECT_GE(maxPitch, 1.0f);
    ...
    const std::vector<AudioPlaybackRate> validValues = {
            ...
            AudioPlaybackRate{maxSpeed, maxPitch, tsDefault, fbMute},
            AudioPlaybackRate{minSpeed, minPitch, tsDefault, fbMute},
            AudioPlaybackRate{1.0f, 1.0f, tsVoice, fbMute},
            AudioPlaybackRate{1.0f, 1.0f, tsVoice, fbFail},
            AudioPlaybackRate{maxSpeed, maxPitch, tsVoice, fbMute},
            AudioPlaybackRate{minSpeed, minPitch, tsVoice, fbMute},
            // Out of range speed / pitch values must not be rejected if the fallback mode is "mute"
            AudioPlaybackRate{maxSpeed * 2, maxPitch * 2, tsDefault, fbMute},
            AudioPlaybackRate{minSpeed / 2, minPitch / 2, tsDefault, fbMute},
            AudioPlaybackRate{maxSpeed * 2, maxPitch * 2, tsVoice, fbMute},
            AudioPlaybackRate{minSpeed / 2, minPitch / 2, tsVoice, fbMute},
    };
    const std::vector<AudioPlaybackRate> invalidValues = {
            AudioPlaybackRate{maxSpeed, maxPitch * 2, tsDefault, fbFail},
            AudioPlaybackRate{maxSpeed * 2, maxPitch, tsDefault, fbFail},
            AudioPlaybackRate{minSpeed, minPitch / 2, tsDefault, fbFail},
            AudioPlaybackRate{minSpeed / 2, minPitch, tsDefault, fbFail},
            AudioPlaybackRate{maxSpeed, maxPitch * 2, tsVoice, fbFail},
            AudioPlaybackRate{maxSpeed * 2, maxPitch, tsVoice, fbFail},
            AudioPlaybackRate{minSpeed, minPitch / 2, tsVoice, fbFail},
            AudioPlaybackRate{minSpeed / 2, minPitch, tsVoice, fbFail},
