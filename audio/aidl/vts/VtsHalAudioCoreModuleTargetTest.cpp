  ...
  public:
    ...
    void SetUpPortConfigAnyMixPort(IModule* module, ModuleConfig* moduleConfig,
        ...
        for (const auto& mixPort : mixPorts) {
            mSkipTestReason = "";
            ASSERT_NO_FATAL_FAILURE(SetUpPortConfig(module, moduleConfig, mixPort, connectedOnly));
    ...
    void SetUpPortConfig(IModule* module, ModuleConfig* moduleConfig,
                         const AudioPort& initialMixPort, bool connectedOnly,
                         const std::optional<AudioPortConfig>& mixPortConfig = {}) {
    ...
    void SetUpForAnyMixPort(IModule* module, ModuleConfig* moduleConfig,
    ...
    void SetUpForDevicePort(IModule* module, ModuleConfig* moduleConfig,
    ...
    void SetUpForMixPort(IModule* module, ModuleConfig* moduleConfig, const AudioPort& mixPort,
                         bool connectedOnly = false) {
        ASSERT_NO_FATAL_FAILURE(SetUpPortConfig(module, moduleConfig, mixPort, connectedOnly));
        if (!mSkipTestReason.empty()) return;
        ASSERT_NO_FATAL_FAILURE(SetUpStream(module));
    }
    void SetUpForPortsPair(IModule* module, ModuleConfig* moduleConfig, const AudioPort& mixPort,
                           const AudioPort& devicePort) {
        ASSERT_NO_FATAL_FAILURE(SetUpPortConfig(module, moduleConfig, mixPort, devicePort));
        if (!mSkipTestReason.empty()) return;
        ASSERT_NO_FATAL_FAILURE(SetUpStream(module));
    }
    void SetUpForMixPortConfig(IModule* module, ModuleConfig* moduleConfig,
        ...
        ASSERT_NO_FATAL_FAILURE(
                SetUpPortConfig(module, moduleConfig, *mixPortIt, connectedOnly, mixPortConfig));
        if (!mSkipTestReason.empty()) return;
        ASSERT_NO_FATAL_FAILURE(SetUpStream(module));
    }
...
class AudioStream : public AudioCoreModule {
  public:
    ...
    void GetStreamCommon() {
        ...
        ASSERT_NO_FATAL_FAILURE(stream.SetUpForAnyMixPort(module.get(), moduleConfig.get()));
    ...
    void OpenAllConfigs() {
        ...
        for (const auto& portConfig : allPortConfigs) {
            StreamFixture<Stream> stream;
            ASSERT_NO_FATAL_FAILURE(
                    stream.SetUpForMixPortConfig(module.get(), moduleConfig.get(), portConfig));
        }
    }
    ...
    void OpenOverMaxCount() {
        ...
        for (const auto& port : ports) {
            ...
            for (size_t i = 0; i <= maxStreamCount; ++i) {
                ASSERT_NO_FATAL_FAILURE(streams[i].SetUpPortConfig(
    ...
    void HwGainHwVolume() {
        ...
        for (const auto& port : ports) {
            ...
            StreamFixture<Stream> stream;
            ASSERT_NO_FATAL_FAILURE(
                    stream.SetUpForMixPort(module.get(), moduleConfig.get(), port, connectedOnly));
    ...
    void OpenTwiceSamePortConfigImpl(const AudioPortConfig& portConfig) {
        ...
        ASSERT_NO_FATAL_FAILURE(
                stream1.SetUpForMixPortConfig(module.get(), moduleConfig.get(), portConfig));
...
TEST_P(AudioStreamIn, ActiveMicrophones) {
    ...
    for (const auto& port : ports) {
        ...
        ASSERT_NO_FATAL_FAILURE(stream.SetUpForPortsPair(module.get(), moduleConfig.get(), port,
                                                         micDevicePorts[0]));
        if (!stream.skipTestReason().empty()) continue;
...
TEST_P(AudioStreamOut, RequireOffloadInfo) {
    ...
    ASSERT_NO_FATAL_FAILURE(stream.SetUpPortConfig(module.get(), moduleConfig.get(),
                                                   *offloadMixPorts.begin(), connectedOnly));
    if (auto reason = stream.skipTestReason(); !reason.empty()) {
        GTEST_SKIP() << reason;
    }
    const auto portConfig = stream.getPortConfig();
...
TEST_P(AudioStreamOut, RequireAsyncCallback) {
    ...
    StreamFixture<IStreamOut> stream;
    ASSERT_NO_FATAL_FAILURE(stream.SetUpPortConfig(module.get(), moduleConfig.get(),
                                                   *nonBlockingMixPorts.begin(), connectedOnly));
    if (auto reason = stream.skipTestReason(); !reason.empty()) {
        GTEST_SKIP() << reason;
    }
    const auto portConfig = stream.getPortConfig();
...
class WithRemoteSubmix {
  ...
  private:
    void SetUp(IModule* module, ModuleConfig* moduleConfig, const AudioPort& devicePort) {
        ...
        ASSERT_NO_FATAL_FAILURE(mStream->SetUpForDevicePort(module, moduleConfig, devicePort));
