...
class WithDevicePortConnectedState {
  public:
    ...
    ~WithDevicePortConnectedState() {
        if (mModule != nullptr) {
            EXPECT_IS_OK_FOR_KNOWN_TRANSACTION(mModule->prepareToDisconnectExternalDevice(getId()))
...
TEST_P(AudioCoreModule, TryConnectMissingDevice) {
    ...
    for (const auto& port : ports) {
        ...
        if (status.isOk()) {
            EXPECT_IS_OK_FOR_KNOWN_TRANSACTION(
...
TEST_P(AudioCoreModule, ConnectDisconnectExternalDeviceInvalidPorts) {
    ...
    for (const auto portId : GetNonExistentIds(portIds)) {
        ...
        EXPECT_STATUS_FOR_KNOWN_TRANSACTION(EX_ILLEGAL_ARGUMENT,
    ...
    for (const auto& port : ports) {
        if (port.ext.getTag() != AudioPortExt::Tag::device) {
            ...
            if (devicePort.device.type.connection.empty()) {
                ...
                EXPECT_STATUS_FOR_KNOWN_TRANSACTION(
