...
TEST_P(AudioCoreModule, AddRemoveEffectInvalidArguments) {
    ...
    bool isSupported = addEffectStatus.getExceptionCode() != EX_UNSUPPORTED_OPERATION;
    if (isSupported) {
        ...
        ADD_FAILURE() << "addEffect and removeEffect must be either supported or not supported "
                      << "together";
        isSupported = true;
    }
    if (!isSupported) {
