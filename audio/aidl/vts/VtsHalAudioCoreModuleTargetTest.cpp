...
class WithRemoteSubmix {
  public:
    ...
    ~WithRemoteSubmix() {
        ...
        if (mPatch != nullptr) delete (mPatch);
        if (mConnectedPort != nullptr) delete (mConnectedPort);
    }
    ...
    void SetUpPortConnection() {
        ...
        mConnectedPort = new WithDevicePortConnectedState(port.value());
    ...
    void SetupPatch() {
        ...
        mPatch = new WithAudioPatch(IOTraits<Stream>::is_input, portConfig.value(),
                                    devicePortConfig);
    ...
    void SetUp(IModule* module, ModuleConfig* moduleConfig) {
        ...
        // open stream
        mStream = new WithStream<Stream>(mPatch->getPortConfig(IOTraits<Stream>::is_input));
        ASSERT_NO_FATAL_FAILURE(
  ...
  private:
    ...
    WithDevicePortConnectedState* mConnectedPort = nullptr;
    WithAudioPatch* mPatch = nullptr;
    WithStream<Stream>* mStream = nullptr;
};
