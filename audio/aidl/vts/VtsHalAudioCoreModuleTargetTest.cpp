...
std::shared_ptr<StateSequence> makeAsyncDrainCommands(bool isInput) {
    ...
    if (isInput) {
        d->makeNode(
                State::STANDBY, kStartCommand,
                d->makeNode(
                        State::IDLE, kBurstCommand,
                        d->makeNode(State::ACTIVE, kDrainInCommand,
                                    d->makeNode(State::DRAINING, kStartCommand,
                                                d->makeNode(State::ACTIVE, kDrainInCommand,
                                                            d->makeFinalNode(State::DRAINING))))));
    } else {
        StateDag::Node draining = d->makeNode(State::DRAINING, kBurstCommand,
                                              d->makeNode(State::TRANSFERRING, kDrainOutAllCommand,
                                                          d->makeFinalNode(State::DRAINING)));
        StateDag::Node idle =
                d->makeNode(State::IDLE, kBurstCommand,
                            d->makeNode(State::TRANSFERRING, kDrainOutAllCommand, draining));
...
std::shared_ptr<StateSequence> makeDrainOutCommands(bool isSync) {
    using State = StreamDescriptor::State;
    auto d = std::make_unique<StateDag>();
    StateDag::Node active = d->makeNode(State::ACTIVE, kDrainOutAllCommand,
                                        d->makeNode(State::DRAINING,
                                                    isSync ? TransitionTrigger(kGetStatusCommand)
                                                           : TransitionTrigger(kDrainReadyEvent),
                                                    d->makeFinalNode(State::IDLE)));
    StateDag::Node idle = d->makeNode(State::IDLE, kBurstCommand, active);
    if (!isSync) {
        idle.children().push_back(d->makeNode(State::TRANSFERRING, kTransferReadyEvent, active));
    }
...
std::shared_ptr<StateSequence> makeDrainPauseOutCommands(bool isSync) {
    using State = StreamDescriptor::State;
    auto d = std::make_unique<StateDag>();
    StateDag::Node last = d->makeFinalNode(isSync ? State::PAUSED : State::TRANSFER_PAUSED);
    StateDag::Node draining = d->makeNode(
            State::DRAINING, kPauseCommand,
            d->makeNode(State::DRAIN_PAUSED, kStartCommand,
                        d->makeNode(State::DRAINING, kPauseCommand,
                                    d->makeNode(State::DRAIN_PAUSED, kBurstCommand, last))));
...
std::shared_ptr<StateSequence> makeStandbyCommands(bool isInput, bool isSync) {
    ...
    if (isInput) {
        d->makeNode(
                State::STANDBY, kStartCommand,
                d->makeNode(
                        State::IDLE, kStandbyCommand,
                        d->makeNode(
                                State::STANDBY, kStartCommand,
                                d->makeNode(
                                        State::IDLE, kBurstCommand,
                                        d->makeNode(
                                                State::ACTIVE, kPauseCommand,
                                                d->makeNode(
                                                        State::PAUSED, kFlushCommand,
                                                        d->makeNode(
                                                                State::STANDBY, kStartCommand,
                                                                d->makeNode(
                                                                        State::IDLE, kBurstCommand,
        ...
        StateDag::Node idle2 = d->makeNode(State::IDLE, kStandbyCommand,
                                           d->makeNode(State::STANDBY, kStartCommand, idle3));
        StateDag::Node active = d->makeNode(State::ACTIVE, kPauseCommand,
                                            d->makeNode(State::PAUSED, kFlushCommand, idle2));
        StateDag::Node idle = d->makeNode(State::IDLE, kBurstCommand, active);
        if (!isSync) {
            idle3.children().push_back(d->makeFinalNode(State::TRANSFERRING));
            StateDag::Node transferring =
                    d->makeNode(State::TRANSFERRING, kPauseCommand,
                                d->makeNode(State::TRANSFER_PAUSED, kFlushCommand, idle2));
            idle.children().push_back(transferring);
        }
        d->makeNode(State::STANDBY, kStartCommand,
                    d->makeNode(State::IDLE, kStandbyCommand,
                                d->makeNode(State::STANDBY, kStartCommand, idle)));
    }
    return std::make_shared<StateSequenceFollower>(std::move(d));
}
    ...
    if (isInput) {
        d->makeNode(
                ...
                d->makeNode(
                        ...
                        d->makeNode(
                                State::ACTIVE, kPauseCommand,
                                d->makeNode(State::PAUSED, kBurstCommand,
                                            d->makeNode(State::ACTIVE, kPauseCommand,
                                                        d->makeNode(State::PAUSED, kFlushCommand,
                                                                    d->makeFinalNode(
                                                                            State::STANDBY)))))));
    } else {
        StateDag::Node idle = d->makeNode(
                ...
                d->makeNode(
                        State::ACTIVE, kPauseCommand,
                        d->makeNode(
                                State::PAUSED, kStartCommand,
                                d->makeNode(
                                        State::ACTIVE, kPauseCommand,
                                        d->makeNode(
                                                State::PAUSED, kBurstCommand,
                                                d->makeNode(
                                                        State::PAUSED, kStartCommand,
                                                        d->makeNode(State::ACTIVE, kPauseCommand,
                                                                    d->makeFinalNode(
                                                                            State::PAUSED))))))));
        if (!isSync) {
            idle.children().push_back(d->makeNode(
                    State::TRANSFERRING, kPauseCommand,
                    d->makeNode(
                            State::TRANSFER_PAUSED, kStartCommand,
                            d->makeNode(
                                    State::TRANSFERRING, kPauseCommand,
                                    d->makeNode(State::TRANSFER_PAUSED, kDrainOutAllCommand,
                                                d->makeNode(State::DRAIN_PAUSED, kBurstCommand,
                                                            d->makeFinalNode(
    ...
    if (isInput) {
        d->makeNode(State::STANDBY, kStartCommand,
                    d->makeNode(State::IDLE, kBurstCommand,
                                d->makeNode(State::ACTIVE, kPauseCommand,
                                            d->makeNode(State::PAUSED, kFlushCommand,
                                                        d->makeFinalNode(State::STANDBY)))));
    } else {
        StateDag::Node last = d->makeFinalNode(State::IDLE);
        StateDag::Node idle =
                d->makeNode(State::IDLE, kBurstCommand,
                            d->makeNode(State::ACTIVE, kPauseCommand,
                                        d->makeNode(State::PAUSED, kFlushCommand, last)));
        if (!isSync) {
            idle.children().push_back(
                    d->makeNode(State::TRANSFERRING, kPauseCommand,
                                d->makeNode(State::TRANSFER_PAUSED, kFlushCommand, last)));
        }
        d->makeNode(State::STANDBY, kStartCommand, idle);
    }
    return std::make_shared<StateSequenceFollower>(std::move(d));
...
std::shared_ptr<StateSequence> makeDrainPauseFlushOutCommands(bool isSync) {
    ...
    StateDag::Node draining = d->makeNode(
            State::DRAINING, kPauseCommand,
            d->makeNode(State::DRAIN_PAUSED, kFlushCommand, d->makeFinalNode(State::IDLE)));
    StateDag::Node idle = d->makeNode(State::IDLE, kBurstCommand,
                                      d->makeNode(State::ACTIVE, kDrainOutAllCommand, draining));
    if (!isSync) {
        idle.children().push_back(d->makeNode(State::TRANSFERRING, kDrainOutAllCommand, draining));
    }
    d->makeNode(State::STANDBY, kStartCommand, idle);
