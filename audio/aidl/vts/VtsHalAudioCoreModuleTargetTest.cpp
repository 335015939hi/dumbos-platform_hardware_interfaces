...
class StreamContext {
  public:
    ...
    explicit StreamContext(const StreamDescriptor& descriptor)
        ...
        if (isMmapped()) {
            mSharedMemory = (int8_t *)mmap(nullptr, getBufferSizeBytes(),
    ...
    void checkIsValid() const {
        ...
        if (isMmapped()) {
            EXPECT_TRUE(mSharedMemory != nullptr);
        } else {
            if (mDataMQ != nullptr) {
...
class StreamLogicDefaultDriver : public StreamLogicDriver {
  public:
    ...
    bool hasPositionIncrease() const {
        if (mIsMmap) {
            return mObservablePositionIncrease && mHardwarePositionIncrease;
        } else {
            return mObservablePositionIncrease;
        }
    }
    bool hasRetrogradePosition() const {
        if (mIsMmap) {
            return mRetrogradeObservablePosition || mRetrogradeHardwarePosition;
        } else {
            return mRetrogradeObservablePosition;
        }
    }
