...
class WithRemoteSubmix {
  public:
    ...
    AudioDeviceAddress getAudioDeviceAddress() { return mAddress.value(); }
    /* Get mix port config for stream and setup patch for it. */
    void SetupPatch(AudioPort connectedPort) {
        const auto portConfig =
    ...
    void SetUp(IModule* module, ModuleConfig* moduleConfig, AudioPort connectedPort) {
        mModule = module;
        mModuleConfig = moduleConfig;
...
TEST_P(AudioModuleRemoteSubmix, OpenInputMultipleTimes) {
    ...
    auto port = WithRemoteSubmix<IStreamIn>::getRemoteSubmixAudioPort(
            moduleConfig.get(), streamOut.getAudioDeviceAddress());
    ASSERT_TRUE(port.has_value()) << "Device AudioPort for remote submix not found";
    ...
    // write something to ouptut stream
    ASSERT_NO_FATAL_FAILURE(streamOut.sendBurstCommands());
