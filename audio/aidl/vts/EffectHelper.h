...
class EffectHelper {
  public:
    void create(std::shared_ptr<IFactory> factory, std::shared_ptr<IEffect>& effect,
        ...
        isSpatializer = id.type == getEffectTypeUuidSpatializer();
        kDescriptor = desc;
    }
    ...
    Parameter::Common createParamCommon(int session = 0, int ioHandle = -1, int iSampleRate = 48000,
        ...
        if (isSpatializer && isRangeValid<Range::spatializer>(Spatializer::supportedChannelLayout,
                                                              kDescriptor.capability)) {
            const auto layoutRange = getRange<Range::spatializer, Range::SpatializerRange>(
                    kDescriptor.capability, Spatializer::supportedChannelLayout);
