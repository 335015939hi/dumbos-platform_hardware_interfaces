...
// Use this macro to propagate fatal asserts to test case
#define HELPER_ASSERT(functor, ...)                      \
    do {                                                 \
        ASSERT_NO_FATAL_FAILURE((functor)(__VA_ARGS__)); \
    } while (0)

#define HELPER_EXPECT(functor, ...)                      \
    do {                                                 \
        EXPECT_NO_FATAL_FAILURE((functor)(__VA_ARGS__)); \
    } while (0)

class EffectHelper {
  public:
    static inline void create(std::shared_ptr<IFactory> factory, std::shared_ptr<IEffect>& effect,
    ...
    static inline void destroy(std::shared_ptr<IFactory> factory, std::shared_ptr<IEffect> effect,
    ...
    static inline void open(std::shared_ptr<IEffect> effect, const Parameter::Common& common,
    ...
    static inline void open(std::shared_ptr<IEffect> effect, int session = 0,
                            binder_status_t status = EX_NONE) {
        ASSERT_NE(effect, nullptr);
    ...
    static inline void close(std::shared_ptr<IEffect> effect, binder_status_t status = EX_NONE) {
        if (effect) {
            EXPECT_STATUS(status, effect->close());
        }
    }
    static inline void getDescriptor(std::shared_ptr<IEffect> effect, Descriptor& desc,
                                     binder_status_t status = EX_NONE) {
        ASSERT_NE(effect, nullptr);
        EXPECT_STATUS(status, effect->getDescriptor(&desc));
    }
    static inline void expectState(std::shared_ptr<IEffect> effect, State expectState,
                                   binder_status_t status = EX_NONE) {
        ASSERT_NE(effect, nullptr);
        State state;
    ...
    static inline void command(std::shared_ptr<IEffect> effect, CommandId command,
                               binder_status_t status = EX_NONE) {
        ASSERT_NE(effect, nullptr);
        EXPECT_STATUS(status, effect->command(command));
    }
    static inline void allocateInputData(const Parameter::Common common,
                                         std::unique_ptr<DataMQ>& mq, std::vector<float>& buffer) {
    ...
    static inline void writeToFmq(std::unique_ptr<DataMQ>& mq, const std::vector<float>& buffer) {
    ...
    static inline void readFromFmq(std::unique_ptr<StatusMQ>& statusMq, size_t statusNum,
