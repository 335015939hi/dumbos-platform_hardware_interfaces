...
class EffectHelper {
  public:
    ...
    void generateSineWave(const std::vector<int>& testFrequencies, std::vector<float>& input,
        ...
        ASSERT_TRUE((channelLayout == AudioChannelLayout::LAYOUT_MONO) ||
                    (channelLayout == AudioChannelLayout::LAYOUT_STEREO));
    ...
    // a >= 5, b >=0, c >= 0.
    bool IsFftInputSizeValid(size_t n) {
        if (n == 0) {
            return false;
        }
    ...
    void calculateMagnitudeMono(std::vector<float>& bufferMag, const std::vector<float>& buffer,
        ...
        ASSERT_EQ(bufferMag.size(), binOffsets.size());
    ...
    // Use FFT transform to convert the buffer to frequency domain
    // Compute its magnitude at binOffsets
    void calculateMagnitudeStereo(std::pair<std::vector<float>, std::vector<float>>& bufferMag,
    ...
    void calculateAndVerifyMagnitude(std::vector<float>& mag, int channelLayout,
                                     const std::vector<float>& buffer,
                                     const std::vector<int>& binOffsets,
                                     const int nPointFFT = kNPointFFT) {
        ASSERT_TRUE((channelLayout == AudioChannelLayout::LAYOUT_MONO) ||
