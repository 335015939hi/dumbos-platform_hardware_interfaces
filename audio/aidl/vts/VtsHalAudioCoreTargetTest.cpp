...
TEST_P(AudioCoreTelephony, UpdateAudioMode) {
    if (telephony == nullptr) {
        GTEST_SKIP() << "Telephony is not supported";
    }
    ...
    for (const auto mode : supportedModes) {
        EXPECT_IS_OK(telephony->updateAudioMode(mode)) << toString(mode);
        unsupportedModes.erase(mode);
    }
    for (const auto mode : unsupportedModes) {
        EXPECT_STATUS(EX_UNSUPPORTED_OPERATION, telephony->updateAudioMode(mode)) << toString(mode);
    }
}
