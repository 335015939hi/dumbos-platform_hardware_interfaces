...
class VirtualizerProcessTest : public ::testing::TestWithParam<VirtualizerProcessTestParam>,
  ...
  public:
    ...
    float computeRMSE(const std::vector<float>& buffer) {
        float sum = 0;
        for (size_t i = 0; i < buffer.size(); i++) {
...
TEST_P(VirtualizerProcessTest, IncreasingStrength) {
    ...
    std::vector<int> strengths = {0, 250, 500, 750, 1000};
    ...
    for (int strength : strengths) {
        ...
        float inputRmse = computeRMSE(input);
        float outputRmse = computeRMSE(output);
        ...
        if (inputRmse != 0) {
            ...
            if (strength != 0) {
                ...
                // Zero Strength gives compressed output
                ASSERT_LE(outputRmse, inputRmse);
            }
        }
    }
}
