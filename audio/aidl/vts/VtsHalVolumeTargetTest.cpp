  ...
  public:
    ...
    void SetAndGetParameters() {
        for (auto& it : mTags) {
            ...
            const bool validCap = isCapabilityInRange(desc);
            // only set and get parameter if capability is valid
            ASSERT_TRUE(validCap);
            ...
            // only get if parameter and capability is in range and set success
            if (expected == EX_NONE) {
                Parameter getParam;
                Parameter::Id id;
                Volume::Id volId;
    ...
    bool isCapabilityInRange(const Descriptor& desc) {
