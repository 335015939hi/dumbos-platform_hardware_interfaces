...
const std::vector<int> kLevelValues = {Volume::MIN_LEVEL - 1, Volume::MIN_LEVEL, -100,
                                       Volume::MAX_LEVEL, Volume::MAX_LEVEL + 1};
...
class VolumeParamTest : public ::testing::TestWithParam<VolumeParamTestParam>, public EffectHelper {
  public:
    ...
    Parameter::Specific getDefaultParamSpecific() {
        Volume vol = Volume::make<Volume::levelDb>(Volume::MIN_LEVEL);
    ...
    bool isLevelInRange(const Volume::Capability& cap, int level) const {
        return level >= Volume::MIN_LEVEL && level <= Volume::MAX_LEVEL && level <= cap.maxLevel;
    }
