...
class VolumeDataTest : public ::testing::TestWithParam<VolumeDataTestParam>,
  ...
  public:
    ...
    std::vector<int> calcutalatePercentageDiff(const std::vector<float>& outputMag) {
    ...
    // Convert Decible value to Percentage
    ...
    static constexpr size_t offeset = kSamplingFrequency * kTransitionDuration / 1000;
...
TEST_P(VolumeDataTest, ApplyLevelMuteUnmute) {
    ...
    diffs = calcutalatePercentageDiff(outputMag);
    ...
    std::vector<float> subOutputMute(output.begin() + offeset, output.end());
    ...
    std::vector<float> subOutputUnmute(output.begin() + offeset, output.end());
...
TEST_P(VolumeDataTest, DecreasingLevels) {
    ...
    baseDiffs = calcutalatePercentageDiff(outputMag);
