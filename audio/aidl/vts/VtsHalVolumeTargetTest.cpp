...
class VolumeDataTest : public ::testing::TestWithParam<VolumeDataTestParam>,
  ...
  public:
    ...
    void SetUp() override {
        ...
        ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
                mInputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, mInput,
                mBinOffsets));
...
TEST_P(VolumeDataTest, ApplyLevelMuteUnmute) {
    ...
    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            outputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, output, mBinOffsets));
    ...
    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            outputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, subOutputMute,
            mBinOffsets));
    ...
    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            outputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, subOutputUnmute,
            mBinOffsets));
...
TEST_P(VolumeDataTest, DecreasingLevels) {
    ...
    ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
            outputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, baseOutput,
            mBinOffsets));
    ...
    for (int level : decreasingLevels) {
        ...
        ASSERT_NO_FATAL_FAILURE(calculateAndVerifyMagnitude(
                outputMag, mChannelLayout == AudioChannelLayout::LAYOUT_STEREO, output,
                mBinOffsets));
