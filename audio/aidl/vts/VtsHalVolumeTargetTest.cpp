...
const std::vector<int> kLevelValues = {Volume::MIN_STRENGTH - 1, Volume::MIN_STRENGTH, -100,
                                       Volume::MAX_STRENGTH, Volume::MAX_STRENGTH + 1};
...
class VolumeParamTest : public ::testing::TestWithParam<VolumeParamTestParam>, public EffectHelper {
  public:
    ...
    Parameter::Specific getDefaultParamSpecific() {
        Volume vol = Volume::make<Volume::levelDb>(Volume::MIN_STRENGTH);
    ...
    bool isLevelInRange(const Volume::Capability& cap, int level) const {
        return level >= Volume::MIN_STRENGTH && level <= Volume::MAX_STRENGTH &&
               level <= cap.maxLevel;
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<VolumeParamTest::ParamType>& info) {
            ...
            std::string name = instance.second.uuid.toString() + "_level" + level + "_mute" + mute;
            std::replace_if(
