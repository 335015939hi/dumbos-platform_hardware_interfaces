...
using VolumeParamTestParam =
        std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor::Identity>, int, bool>;

const std::vector<int> kLevelValues = {Volume::MIN_STRENGTH_DB - 1, Volume::MIN_STRENGTH_DB, -100,
                                       Volume::MAX_STRENGTH_DB, Volume::MAX_STRENGTH_DB + 1};
...
class VolumeParamTest : public ::testing::TestWithParam<VolumeParamTestParam>, public EffectHelper {
  public:
    ...
    Parameter::Specific getDefaultParamSpecific() {
        Volume vol = Volume::make<Volume::levelDb>(Volume::MIN_STRENGTH_DB);
    ...
    bool isLevelInRange(const Volume::Capability& cap, int level) const {
        return level >= Volume::MIN_STRENGTH_DB && level <= Volume::MAX_STRENGTH_DB &&
               level <= cap.maxLevel;
    }
...
INSTANTIATE_TEST_SUITE_P(
        ...
        [](const testing::TestParamInfo<VolumeParamTest::ParamType>& info) {
            ...
            std::string name = address.str() + "_level" + level + "_mute" + mute;
            std::replace_if(
