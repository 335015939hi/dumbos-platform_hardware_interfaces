...
static const std::vector<Downmix::Type> kTypeValues = {Downmix::Type::STRIP, Downmix::Type::FOLD};
...
class DownmixEffectHelper : public EffectHelper {
  public:
    ...
    void validateParameters(Downmix::Type type) {
        ...
        Downmix::Tag tag(Downmix::type);
        leId.set<Downmix::Id::commonTag>(tag);
        id.set<Parameter::Id::downmixTag>(leId);
    ...
    void setDataTestParams(int32_t layoutType) {
        if (log2(layoutType) >=
            ChannelMix<AUDIO_CHANNEL_OUT_STEREO>::MAX_INPUT_CHANNELS_SUPPORTED) {
        ...
        mInputChannelCount = ::aidl::android::hardware::audio::common::getChannelCount(
    ...
    static constexpr float kOutputChannelCount = 2;
};
...
class DownmixParamTest : public ::testing::TestWithParam<DownmixParamTestParam>,
  ...
  public:
    ...
    Downmix::Type mParamType;
};

TEST_P(DownmixParamTest, SetAndGetType) {
    setParameters(mParamType);
    validateParameters(mParamType);
}

enum FoldParamName { FOLD_INSTANCE_NAME, FOLD_INPUT_LAYOUT, FOLD_TEST_CHANNEL };

using DownmixDataTestParamFold =
        std::tuple<std::pair<std::shared_ptr<IFactory>, Descriptor>, int32_t, int32_t>;

class DownmixFoldDataTest : public ::testing::TestWithParam<DownmixDataTestParamFold>,
                            public DownmixEffectHelper {
  public:
    DownmixFoldDataTest()
        : mInputChannelLayout(std::get<FOLD_INPUT_LAYOUT>(GetParam())),
          mChannel(std::get<FOLD_TEST_CHANNEL>(GetParam())) {
    ...
    void generateInputBuffer(size_t position) {
        for (size_t i = position; i < mInputBuffer.size(); i += mInputChannelCount) {
            mInputBuffer[i] =
                    ((static_cast<float>(std::rand()) / RAND_MAX) * 2 - 1) * kMaxDownmixSample;
        }
    }

    std::vector<int32_t> getInputChannelLayouts() {
        std::vector<int32_t> supportedChannels;
        int32_t channelLayout = 1;
        while (channelLayout <= AudioChannelLayout::CHANNEL_FRONT_WIDE_RIGHT) {
            if (mInputChannelLayout & channelLayout) {
                supportedChannels.push_back(channelLayout);
            }
            channelLayout = channelLayout << 1;
        }
...
TEST_P(DownmixFoldDataTest, DownmixProcessData) {
    ...
    setParameters(Downmix::Type::FOLD);

    if ((mChannel & mInputChannelLayout) == mChannel) {
        // Get all the channels from input layout
        std::vector<int32_t> supportedChannels = getInputChannelLayouts();

        // Get the channel position
        auto iterator = std::find(supportedChannels.begin(), supportedChannels.end(), mChannel);
        size_t position = iterator - supportedChannels.begin();

        generateInputBuffer(position);
        ASSERT_NO_FATAL_FAILURE(
                processAndWriteToOutput(mInputBuffer, mOutputBuffer, mEffect, &mOpenEffectReturn));
        validateOutput(mChannel, position);
    } else {
        GTEST_SKIP() << "Channel not supported \n";
    }
}
...
class DownmixStripDataTest : public ::testing::TestWithParam<DownmixStripDataTestParam>,
  ...
  public:
    ...
    void validateOutput() {
        for (size_t i = 0, j = 0; j < mOutputBufferSize;
...
TEST_P(DownmixStripDataTest, DownmixProcessData) {
    ...
    setParameters(Downmix::Type::STRIP);
...
INSTANTIATE_TEST_SUITE_P(
        ...
        ::testing::Combine(testing::ValuesIn(EffectFactoryHelper::getAllEffectDescriptors(
                                   IFactory::descriptor, getEffectTypeUuidDownmix())),
                           testing::ValuesIn(kLayoutValues), testing::ValuesIn(kChannels)),
        [](const testing::TestParamInfo<DownmixFoldDataTest::ParamType>& info) {
            ...
            std::string channel = std::to_string(std::get<FOLD_TEST_CHANNEL>(info.param));
            std::string name =
                    getPrefix(descriptor) + "_fold" + "_layout" + layout + "_channel" + channel;
            std::replace_if(
                    name.begin(), name.end(), [](const char c) { return !std::isalnum(c); }, '_');
            return name;
        });
