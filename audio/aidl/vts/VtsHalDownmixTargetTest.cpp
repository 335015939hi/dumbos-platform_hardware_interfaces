...
class DownmixEffectHelper : public EffectHelper {
  public:
    ...
    Parameter createDownmixParam(Downmix::Type type) {
        auto dm = Downmix::make<Downmix::type>(type);
    ...
    void generateInputBuffer(size_t position, bool isStrip) {
        ...
        if (isStrip)
            ...
            increament = mInputChannelCount;

        for (size_t i = position; i < mInputBuffer.size(); i += increament) {
            mInputBuffer[i] =
...
TEST_P(DownmixFoldDataTest, DownmixProcessData) {
    ...
    if ((mChannel & mInputChannelLayout) == mChannel) {
        ...
        size_t position =
                std::distance(supportedChannels.begin(), supportedChannels.find(mChannel));
