...
ndk::ScopedAStatus Module::getMmapPolicyInfos(AudioMMapPolicyType /*mmapPolicyType*/,
    ...
    for (const auto& route : getConfig().routes) {
        if (mmapSinks.find(route.sinkPortId) != mmapSinks.end()) {
            ...
            if (std::find_if(route.sourcePortIds.begin(), route.sourcePortIds.end(),
                             [&mmapSources](int sourcePortId) {
                                 return mmapSources.find(sourcePortId) != mmapSources.end();
