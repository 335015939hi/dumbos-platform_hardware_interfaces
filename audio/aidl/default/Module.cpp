...
ndk::ScopedAStatus Module::setModuleDebug(
    ...
    LOG(DEBUG) << __func__ << " " << mType << ": " << mType << ": old flags:" << mDebug.toString()
    ...
    if (mDebug.simulateDeviceConnections != in_debug.simulateDeviceConnections &&
        ...
        LOG(ERROR) << __func__ << " " << mType << ": " << mType
    ...
    if (in_debug.streamTransientStateDelayMs < 0) {
        LOG(ERROR) << __func__ << " " << mType << ": " << mType
                   << ": streamTransientStateDelayMs is negative: "
...
ndk::ScopedAStatus Module::setAudioPortConfigImpl(
    ...
    if (portId == 0) {
        LOG(ERROR) << __func__ << " " << mType << ": requested port config does not specify portId";
        return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
    }
    auto& ports = getConfig().ports;
    auto portIt = findById<AudioPort>(ports, portId);
        ...
        *out_suggested = *existing;
        ...
        if (fillPortConfig(*portIt, &newConfig)) {
            *out_suggested = newConfig;
            ...
            LOG(ERROR) << __func__ << " " << mType << ": unable generate a default config for port "
                       << portId;
            return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
        }
    }
    ...
    if (!(out_suggested->format.value() == AudioFormatDescription{} && allowDynamicConfig) &&
        ...
        LOG(ERROR) << __func__ << " " << mType << ": port " << portId << " does not support format "
...
ndk::ScopedAStatus Module::setMasterMute(bool in_mute) {
    ...
    if (result.isOk()) {
        ...
        LOG(ERROR) << __func__ << " " << mType << ": failed calling onMasterMuteChanged(" << in_mute
                   << "), error=" << result;
