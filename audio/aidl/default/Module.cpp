...
ndk::ScopedAStatus Module::updateStreamsConnectedState(const AudioPatch& oldPatch,
     ...
     *      'b': handled only in disconnected step with empty devices because 'b' is only present 
     *           in oldConnections.
     *      'c': handled only in connected step with {E,F,G} devices because 'c' is only present 
     *           in newConnections.
     *      'd': handled only in connected step with {K,L,M} devices because 'd' is also present 
     *           in newConnections and it is ignored in disconnected step.
     *      'e': handled only in connected step with {N,P} devices because 'e' is also present 
     *           in newConnections and it is ignored in disconnected step. please note that there
     *           is exclusive disconnection for device {O}.
     *      'f': handled only in connected step with {Q,R,S} devices because 'f' is also present 
     *           in newConnections and it is ignored in disconnected step. Even though stream is 
     *           already connected with {Q,R} devices and connection happens with {Q,R,S}.
     *      'g': handled only in connected step with {U,V,W} devices because 'g' is also present 
     *           in newConnections and it is ignored in disconnected step. There is no exclusive 
