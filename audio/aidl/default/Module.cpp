...
ndk::ScopedAStatus Module::updateStreamsConnectedState(const AudioPatch& oldPatch,
     ...
     *
     * Expecting routings:
     *      'a': is ignored both in disconnection logic and connection logic,
     *           due to same devices both in oldConnections and newConnections.
     *      'b': is ignored in disconnection logic because 'b' is present in newConnections,
     *           and proceed to connection logic with devices {A}.
     *      'c': same as 'b'. only in connection logic with devices {X,Y,Z}
     *      'd': same as 'b'. only in connection logic with devices {G,H,I,J}.
     *      'e': handled in disconection logic only, because 'e' is only present in oldConnections.
     *      'f': handled in connection logic only, because 'f' is only present in newConnections.
