...
ndk::ScopedAStatus EffectImpl::close() {
    ...
    notifyEventFlag(kEventFlagNotEmpty);
...
void EffectImpl::notifyEventFlag(uint32_t flag) {
    if (!mEventFlag) {
...
void EffectImpl::process() {
    ...
    if (!mEventFlag || ::android::OK != mEventFlag->wait(kEventFlagNotEmpty, &efState)) {
        LOG(ERROR) << getEffectName() << __func__ << ": StatusEventFlag invalid - " << mEventFlag;
    }
