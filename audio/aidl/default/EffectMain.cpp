...
static inline std::string config_file_path() {
    if (__builtin_available(android AAPEXSUPPORT_API, *)) {
        ...
        if (AApexInfo_create(&apexInfo) == AAPEXINFO_OK) {
            ...
            std::string candidatePath("/apex/" + apexName + "/etc/" + kDefaultConfigName);
        ...
        LOG(ERROR) << __func__ << "libapexsupport is not supported in " << __ANDROID_API__;
    }
    LOG(ERROR) << __func__ << ": Uable to resolve config file path in APEX";
    return {};
}
