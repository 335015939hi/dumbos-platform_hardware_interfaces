...
const std::pair<int, int> EqualizerSw::getBandRange(
...
ndk::ScopedAStatus EqualizerSw::setSpecificParameter(const Parameter::Specific& specific) {
    ...
    switch (tag) {
        case Equalizer::bandLevels: {
            ...
            const auto& range = getBandRange(bandLevels);
            if (bandLevels.size() >= NUM_OF_BANDS || range.first < 0 ||
                range.second >= NUM_OF_BANDS) {
                LOG(ERROR) << " bandLevels " << bandLevels.size() << "minIndex " << range.first
                           << "maxIndex " << range.second << " illegal ";
                return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                        "ExceedMaxBandNum");
            }
        ...
        default: {
            LOG(ERROR) << __func__ << " unsupported eq param: " << toString(tag);
            return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                    "ParamNotSupported");
        }
    }
