...
ndk::ScopedAStatus EqualizerSw::open(const Parameter::Common& common,
    ...
    mStatusMQ = std::make_unique<StatusMQ>(1, true /*configureEventFlagWord*/);
    mInputMQ = std::make_unique<DataMQ>(input.frameCount * inputFrameSize);
    mOutputMQ = std::make_unique<DataMQ>(output.frameCount * outputFrameSize);
