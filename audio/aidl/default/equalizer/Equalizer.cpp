...
ndk::ScopedAStatus EqualizerSw::setSpecificParameter(const Parameter::Specific& specific) {
    ...
    if (tag < static_cast<Equalizer::Tag>(Equalizer::SetParameterRange::MIN) ||
        tag > static_cast<Equalizer::Tag>(Equalizer::SetParameterRange::MAX)) {
        LOG(ERROR) << " tag " << static_cast<int>(tag) << " out of SetParameterRange";
        return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                "EffectNotSupported");
    }
    ...
    switch (tag) {
        case Equalizer::bandLevels: {
            auto& bands = eqParam.get<Equalizer::bandLevels>();
...
ndk::ScopedAStatus EqualizerSw::getSpecificParameter(Parameter::Specific::Id id,
    ...
    if (eqTag < static_cast<Equalizer::Tag>(Equalizer::GetParameterRange::MIN) ||
        eqTag > static_cast<Equalizer::Tag>(Equalizer::GetParameterRange::MAX)) {
        LOG(ERROR) << " tag " << static_cast<int>(eqTag) << " out of GetParameterRange";
        return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                "EffectNotSupported");
    }
    switch (eqTag) {
        case Equalizer::bandCapability: {
            eqParam.set<Equalizer::bandCapability>(mBandCaps);
            break;
        }
        case Equalizer::bandLevels: {
