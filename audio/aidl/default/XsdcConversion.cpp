...
ConversionResult<AudioHalCapCriterionType> convertCapCriterionTypeToAidl(
    ...
    aidlCapCriterionType.numericalValue = VALUE_OR_RETURN(
            (convertWrappedCollectionToAidl<eng_xsd::ValuesType, eng_xsd::ValueType, int64_t>(
                    xsdcCriterionType.getValues(), &eng_xsd::ValuesType::getValue,
                    &convertCriterionTypeNumericalValueToAidl)));

    std::vector<std::optional<AudioDeviceDescription>> resultAidlAndroidMappedValueVec;
    bool validate = true;
        ...
        for (eng_xsd::ValuesType xsdcValues : xsdcCriterionType.getValues()) {
            ...
            for (const eng_xsd::ValueType& xsdcValue : xsdcValues.getValue()) {
                if (xsdcValue.hasAndroid_type()) {
                    if (aidlCapCriterionType.name !=
                                AudioHalCapCriterionType::AVAILABLE_OUTPUT_DEVICES_TYPE &&
                        aidlCapCriterionType.name !=
                                AudioHalCapCriterionType::AVAILABLE_INPUT_DEVICES_TYPE) {
                        LOG(ERROR) << __func__ << " android type only expected on device criterion"
                                   << ", found for " << aidlCapCriterionType.name;
                        return unexpected(BAD_VALUE);
                    }
                    ...
                    resultAidlAndroidMappedValueVec.push_back(std::optional<AudioDeviceDescription>(
    ...
    aidlCapCriterionType.androidMappedValue =
            validate ? std::optional<std::vector<std::optional<AudioDeviceDescription>>>(
                               resultAidlAndroidMappedValueVec)
                     : std::nullopt;
