...
ConversionResult<AudioPolicyForcedConfig> convertForcedConfigToAidl(
    ...
    const auto it = std::find_if(kValidAudioPolicyForcedConfig.begin(),
            kValidAudioPolicyForcedConfig.end(), [&](const auto& config) {
        return toString(config) == xsdcForcedConfigCriterionType; });
...
ConversionResult<eng_xsd::CriterionTypeType> getCriterionTypeByName(const std::string& name,
...
convertCapCriteriaCollectionToAidl(const std::vector<eng_xsd::CriteriaType>& xsdcCriteriaVec,
        const std::vector<eng_xsd::CriterionTypesType>& xsdcCriterionTypesVec) {
    ...
    for (const auto& xsdCriteria : xsdcCriteriaVec) {
        for (const auto& xsdcCriterion : xsdCriteria.getCriterion()) {
            resultAidlCriterionVec.push_back(std::optional<AudioHalCapCriterionV2>(std::move(
                    VALUE_OR_FATAL(convertCapCriterionV2ToAidl(
                            xsdcCriterion, xsdcCriterionTypesVec)))));
        }
    }
    return resultAidlCriterionVec;
}

ConversionResult<std::vector<std::optional<AudioDeviceDescription>>> convertDevicesToAidl(
    ...
    std::vector<std::optional<AudioDeviceDescription>> aidlDevices;
    for (eng_xsd::ValuesType xsdcValues : xsdcDeviceCriterionType.getValues()) {
        ...
        for (const eng_xsd::ValueType& xsdcValue : xsdcValues.getValue()) {
            ...
            aidlDevices.push_back(std::optional<AudioDeviceDescription>(std::move(VALUE_OR_RETURN(
...
ConversionResult<std::vector<std::optional<AudioDeviceAddress>>> convertDeviceAddressesToAidl(
        const eng_xsd::CriterionTypeType& xsdcDeviceAddressesCriterionType) {
    if (xsdcDeviceAddressesCriterionType.getValues().empty()) {
        LOG(ERROR) << __func__ << " no values provided";
        return unexpected(BAD_VALUE);
    }
    std::vector<std::optional<AudioDeviceAddress>> aidlDeviceAddresses;
    for (eng_xsd::ValuesType xsdcValues : xsdcDeviceAddressesCriterionType.getValues()) {
        ...
        for (const eng_xsd::ValueType& xsdcValue : xsdcValues.getValue()) {
            aidlDeviceAddresses.push_back(std::optional<AudioDeviceAddress>(std::move(
                    AudioDeviceAddress::make<AudioDeviceAddress::Tag::id>(
                            xsdcValue.getLiteral()))));
        }
    }
...
ConversionResult<AudioHalCapCriterionV2> convertCapCriterionV2ToAidl(
    ...
    if (iequals(xsdcCriterion.getName(), toString(Tag::availableInputDevices))) {
        return AudioHalCapCriterionV2::make<Tag::availableInputDevices>(VALUE_OR_RETURN(
                convertDevicesToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::availableOutputDevices))) {
        return AudioHalCapCriterionV2::make<Tag::availableOutputDevices>(VALUE_OR_RETURN(
                convertDevicesToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::availableInputDevicesAddresses))) {
        return AudioHalCapCriterionV2::make<Tag::availableInputDevicesAddresses>(VALUE_OR_RETURN(
                convertDeviceAddressesToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::availableOutputDevicesAddresses))) {
        return AudioHalCapCriterionV2::make<Tag::availableOutputDevicesAddresses>(VALUE_OR_RETURN(
                convertDeviceAddressesToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::telephonyMode))) {
        return AudioHalCapCriterionV2::make<Tag::telephonyMode>(std::make_optional<>(std::move(
                VALUE_OR_RETURN(convertTelephonyModesToAidl(xsdcCriterionType)))));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForCommunication))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForCommunication>(
                AudioPolicyForceUse::COMMUNICATION,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForMedia))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForMedia>(
                AudioPolicyForceUse::MEDIA,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForRecord))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForRecord>(
                AudioPolicyForceUse::RECORD,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForDock))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForDock>(
                AudioPolicyForceUse::DOCK,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForSystem))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForSystem>(
                AudioPolicyForceUse::SYSTEM,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForHdmiSystemAudio))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForHdmiSystemAudio>(
                AudioPolicyForceUse::HDMI_SYSTEM_AUDIO,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForEncodedSurround))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForEncodedSurround>(
                AudioPolicyForceUse::ENCODED_SURROUND,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
    if (iequals(xsdcCriterion.getName(), toString(Tag::forceUseForVibrateRinging))) {
        return AudioHalCapCriterionV2::make<Tag::forceUseForVibrateRinging>(
                AudioPolicyForceUse::VIBRATE_RINGING,
                VALUE_OR_RETURN(convertForcedConfigsToAidl(xsdcCriterionType)));
    }
