...
ConversionResult<AudioPolicyForceUse::DeviceCategory> convertForceUseDeviceCategoryToAidl(
        const std::string& xsdcForcedConfigCriterionType) {
    const auto it = std::find_if(
            enum_range<AudioPolicyForceUse::DeviceCategory>().begin(),
            enum_range<AudioPolicyForceUse::DeviceCategory>().end(),
            [&](const auto v) { return toString(v) == xsdcForcedConfigCriterionType; });
    if (it == enum_range<AudioPolicyForceUse::DeviceCategory>().end()) {
...
ConversionResult<AudioPolicyForceUse::DockType> convertForceUseDockTypeToAidl(
        const std::string& xsdcForcedConfigCriterionType) {
    const auto it = std::find_if(
            enum_range<AudioPolicyForceUse::DockType>().begin(),
            enum_range<AudioPolicyForceUse::DockType>().end(), [&](const auto v) {
                return toString(v) + (v != AudioPolicyForceUse::DockType::NONE ? "_DOCK" : "") ==
                       xsdcForcedConfigCriterionType;
            });
    if (it == enum_range<AudioPolicyForceUse::DockType>().end()) {
        LOG(ERROR) << __func__ << " invalid forced config " << xsdcForcedConfigCriterionType;
        return unexpected(BAD_VALUE);
    }
    return *it;
...
ConversionResult<AudioPolicyForceUse::EncodedSurroundConfig> convertForceUseEncodedSurroundToAidl(
        const std::string& xsdcForcedConfigCriterionType) {
    const auto it = std::find_if(
            enum_range<AudioPolicyForceUse::EncodedSurroundConfig>().begin(),
...
ConversionResult<AudioPolicyForceUse> convertForceUseToAidl(const std::string& xsdcCriterionName,
    ...
    if (!fastcmp<strncmp>(xsdcCriterionName.c_str(), kXsdcForceConfigForCommunication,
        ...
        const auto deviceCategory =
                VALUE_OR_RETURN(convertForceUseDeviceCategoryToAidl(xsdcCriterionValue));
    ...
    if (!fasticmp<strncmp>(xsdcCriterionName.c_str(), kXsdcForceConfigForDock,
        ...
        const auto dockType = VALUE_OR_RETURN(convertForceUseDockTypeToAidl(xsdcCriterionValue));
    ...
    if (!fasticmp<strncmp>(xsdcCriterionName.c_str(), kXsdcForceConfigForEncodedSurround,
        ...
        const auto encodedSurround =
                VALUE_OR_RETURN(convertForceUseEncodedSurroundToAidl(xsdcCriterionValue));
