...
ConversionResult<AudioDevice> createAudioDevice(
        ...
        const std::unordered_set<std::string>& xAttachedDeviceSet) {
    AudioDevice device = {
...
ConversionResult<AudioPortExt> createAudioPortExt(
    ...
    AudioPortDeviceExt deviceExt = {
            .device = VALUE_OR_FATAL(createAudioDevice(xDevicePort, xAttachedDeviceSet)),
...
ConversionResult<AudioPort> convertDevicePortToAidl(
        const ap_xsd::DevicePorts::DevicePort& xDevicePort,
        const std::unordered_set<std::string>& xAttachedDeviceSet,
        const std::string& xDefaultOutputDevice, int32_t& nextPortId) {
    return AudioPort{
            .id = nextPortId++,
            ...
            .ext = VALUE_OR_FATAL(
                    createAudioPortExt(xDevicePort, xAttachedDeviceSet, xDefaultOutputDevice))};
}
    ...
    if (!xDevicePortsVec.empty()) {
        ...
        for (const ap_xsd::DevicePorts& xDevicePortsType : xDevicePortsVec) {
            for (const ap_xsd::DevicePorts::DevicePort& xDevicePort :
                ...
                audioPortVec.push_back(VALUE_OR_FATAL(convertDevicePortToAidl(
                        xDevicePort, xAttachedDeviceSet, xDefaultOutputDevice, nextPortId)));
            }
        }
    }
    return audioPortVec;
