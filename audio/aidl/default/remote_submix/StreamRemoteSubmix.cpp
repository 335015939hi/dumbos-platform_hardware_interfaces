...
::android::status_t DriverRemoteSubmix::prepareToClose() {
    if (!mIsInput) {
        auto prepareToCloseTask = std::async(std::launch::async, [this] {
            setCurrentThreadName("Create prepareToCloseTask");

            // TODO : Lock can be acquired only while shutdown call.
            std::lock_guard guard(mLock);
