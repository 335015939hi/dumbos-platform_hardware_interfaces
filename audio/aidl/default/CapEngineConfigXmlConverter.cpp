...
ConversionResult<AudioHalCapRule::CriterionRule> convertCriterionRuleToAidl(
    ...
    if (iequals(criterionName, toString(Tag::availableInputDevices))) {
        ...
        rule.criterionTypeValue = VALUE_OR_RETURN(convertDeviceTypeToAidl(
                gLegacyInputDevicePrefix + criterionValue));
        ...
        rule.criterionTypeValue = VALUE_OR_RETURN(convertDeviceTypeToAidl(
                gLegacyOutputDevicePrefix + criterionValue));
    ...
    } else if (iequals(criterionName, toString(Tag::forceUseForCommunication))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForCommunication>(
                AudioPolicyForceUse::COMMUNICATION);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForMedia))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForMedia>(
                AudioPolicyForceUse::MEDIA);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForRecord))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForRecord>(
                AudioPolicyForceUse::RECORD);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForDock))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForDock>(
                AudioPolicyForceUse::DOCK);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForSystem))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForSystem>(
                AudioPolicyForceUse::SYSTEM);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForHdmiSystemAudio))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForHdmiSystemAudio>(
                AudioPolicyForceUse::HDMI_SYSTEM_AUDIO);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForEncodedSurround))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForEncodedSurround>(
                AudioPolicyForceUse::ENCODED_SURROUND);
        rule.criterionTypeValue = VALUE_OR_RETURN(convertForcedConfigToAidl(criterionValue));
    } else if (iequals(criterionName, toString(Tag::forceUseForVibrateRinging))) {
        rule.criterion = AudioHalCapCriterionV2::make<Tag::forceUseForVibrateRinging>(
    ...
    if (xsdcCompoundRule.hasSelectionCriterionRule_optional()) {
        ...
        for (const auto& xsdcRule : xsdcCompoundRule.getSelectionCriterionRule_optional()) {
            criterionRules.push_back(std::make_optional<AudioHalCapRule::CriterionRule>(
                    std::move(VALUE_OR_FATAL(convertCriterionRuleToAidl(xsdcRule)))));
        }
...
ConversionResult<AudioHalCapConfiguration> CapEngineConfigXmlConverter::convertConfigurationToAidl(
    ...
    if (xsdcConfiguration.hasCompoundRule()) {
        ...
        aidlCapConfiguration.rule = VALUE_OR_FATAL(
                convertRule(xsdcConfiguration.getCompoundRule()[0]));
        aidlCapConfiguration.parameterSettings = VALUE_OR_FATAL(
                convertSettingToAidl(xsdcSettingConfiguration));
    }
...
CapEngineConfigXmlConverter::convertConfigurationsToAidl(
    ...
    for (const auto& xsdcConfigurations : xsdcConfigurationsVec) {
        for (const auto& xsdcConfiguration : xsdcConfigurations.getConfiguration()) {
            auto xsdcSettingConfiguration = VALUE_OR_FATAL(getConfigurationByName(
                    xsdcConfiguration.getName(), xsdcSettingsVec));
            aidlConfigurations.push_back(VALUE_OR_FATAL(convertConfigurationToAidl(
                    xsdcConfiguration, xsdcSettingConfiguration)));
        }
    }
