...
ConversionResult<AudioHalCapRule> convertRule(
        const eng_xsd::CompoundRuleType& xsdcCompoundRule) {
    ...
    for (const auto& childXsdcCoumpoundRule : xsdcCompoundRule.getCompoundRule_optional()) {
        if (childXsdcCoumpoundRule.hasCompoundRule_optional()) {
            nestedRules.push_back(std::make_optional<AudioHalCapRule>(std::move(VALUE_OR_FATAL(
                    convertRule(childXsdcCoumpoundRule)))));
