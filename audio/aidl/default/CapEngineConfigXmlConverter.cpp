...
ConversionResult<std::string> CapEngineConfigXmlConverter::convertConfigurableElementToAidl(
    ...
    std::string aidlConfigurableElement;
    aidlConfigurableElement = xsdcConfigurableElement.getPath();
    return aidlConfigurableElement;
}
ConversionResult<AudioHalCapSetting> CapEngineConfigXmlConverter::convertSettingToAidl(
    ...
    for (const auto& configurableElement : xsdcSetting.getConfigurableElement()) {
        if (configurableElement.hasBooleanParameter_optional()) {
            const auto* param = configurableElement.getFirstBooleanParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasIntegerParameter_optional()) {
            const auto* param = configurableElement.getFirstIntegerParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasEnumParameter_optional()) {
            const auto* param = configurableElement.getFirstEnumParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasFixedPointParameter_optional()) {
            const auto* param = configurableElement.getFirstFloatingPointParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasFloatingPointParameter_optional()) {
            const auto* param = configurableElement.getFirstIntegerParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        }  else if (configurableElement.hasBitParameter_optional()) {
            const auto* param = configurableElement.getFirstBitParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasBitParameterBlock_optional()) {
            const auto* param = configurableElement.getFirstBitParameterBlock_optional();
            if (param->hasBitParameter()) {
                const auto* bitParam = param->getFirstBitParameter();
                aidlCapSetting.parameterSettings.push_back(
                        {configurableElement.getPath(), bitParam->getValue()});
            } else {
                return unexpected(BAD_VALUE);
            }
        } else if (configurableElement.hasStringParameter_optional()) {
            const auto* param = configurableElement.getFirstStringParameter_optional();
            aidlCapSetting.parameterSettings.push_back(
                    {configurableElement.getPath(), param->getValue()});
        } else if (configurableElement.hasParameterBlock_optional()) {
            return unexpected(BAD_VALUE);
        }
    }
...
ConversionResult<AudioHalCapDomain> CapEngineConfigXmlConverter::convertConfigurableDomainToAidl(
    ...
    if (xsdcConfigurableDomain.hasConfigurableElements()) {
        aidlConfigurableDomain.parameters = VALUE_OR_FATAL(
...
void CapEngineConfigXmlConverter::init() {
    if (getXsdcConfig()->hasConfigurableDomain()) {
        std::vector<AudioHalCapDomain> domains = VALUE_OR_FATAL(
                (convertCollectionToAidl<eng_xsd::ConfigurableDomainType, AudioHalCapDomain>(
                        getXsdcConfig()->getConfigurableDomain(),
                        std::bind(&CapEngineConfigXmlConverter::convertConfigurableDomainToAidl,
                                  this, std::placeholders::_1))));
        std::vector<std::optional<AudioHalCapDomain>> optDomains {};
        for (auto &domain: domains) {
            optDomains.push_back(std::optional<AudioHalCapDomain>{std::move(domain)});
        }
        mAidlCapDomains = std::make_optional(std::move(optDomains));
    } else {
