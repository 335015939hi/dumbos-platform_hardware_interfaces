...
void Factory::openEffectLibrary(const AudioUuid& impl, const std::string& libName) {
    static std::unordered_set<std::string> libNameSet;  // to avoid duplicate lib dlopen
    if (libNameSet.count(libName)) {
        return;
    }
...
void Factory::createIdentityWithConfig(const EffectConfig::LibraryUuid& configLib,
    ...
    if (auto path = libMap.find(libName); path != libMap.end()) {
        ...
        id.type = typeUuid;
        id.uuid = configLib.implUuid;
        LOG(DEBUG) << __func__ << ": type " << id.type.toString() << "\nimpl "
                   << id.uuid.toString();
        if (proxyUuid.has_value()) {
            id.proxy = proxyUuid.value();
            LOG(DEBUG) << __func__ << ": proxy " << id.proxy->toString();
        }
