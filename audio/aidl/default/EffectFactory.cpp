...
Factory::Factory() {
    ...
    std::shared_ptr<void> libHandle{dlopen("libequalizer.so", RTLD_LAZY), [](void* handle) {
                                        if (handle) {
                                            if (dlclose(handle)) {
                                                LOG(ERROR) << "dlclose failed " << dlerror();
                                            } else {
                                                LOG(DEBUG) << " dlclose handle " << handle;
                                            }
                                        }
                                    }};
    if (!libHandle) {
...
Factory::~Factory() {
    auto count = mEffectUuidMap.size();
    if (count) {
    ...
    if (in_impl_uuid == EqualizerSwImplUUID) {
        if (mEffectLibMap.count(in_impl_uuid)) {
            ...
            if (nullptr == lib.second) {
                ...
                auto dlInterface = std::make_shared<struct effect_interface_s>(intf);
...
ndk::ScopedAStatus Factory::destroyEffectImpl(const std::shared_ptr<IEffect>& in_handle) {
    ...
    if (in_handle && mEffectUuidMap.count(wpHandle)) {
        auto& uuid = mEffectUuidMap[wpHandle];
        if (mEffectLibMap.count(uuid)) {
            auto& libInterface = mEffectLibMap[uuid].second;
            RETURN_IF_BINDER_EXCEPTION(libInterface->destroyEffectFunc(in_handle));
            mEffectUuidMap.erase(wpHandle);
            return ndk::ScopedAStatus::ok();
        } else {
...
void Factory::cleanupEffectMap() {
    for (auto it = mEffectUuidMap.begin(); it != mEffectUuidMap.end();) {
        auto erased = it++;
        if (nullptr == erased->first.lock()) {
            LOG(ERROR) << __func__ << " erase expired instance UUID " << erased->second.toString();
            mEffectUuidMap.erase(erased);
        }
    }
}
