...
IEffect::Status SpatializerSwContext::process(float* in, float* out, int samples) {
    ...
    // spatializer only support stereo output
    ...
    const auto frameSize = getInputFrameSize();
    RETURN_VALUE_IF(frameSize == 0, status, "invalidFrameSize");
    int iFrames = samples * sizeof(float) / frameSize;
    samples = iFrames * frameSize / sizeof(float);  // align samples with frameSize
    while (iFrames--) {
        out[0] = in[0];
        out[1] = in[1];
