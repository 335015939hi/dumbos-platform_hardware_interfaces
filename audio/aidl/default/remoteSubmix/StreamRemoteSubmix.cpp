...
bool DriverRemoteSubmix::compareConfigs() {
    ...
    if (mChannelLayout != config.mChannelLayout) {
        LOG(ERROR) << __func__
                   << ": channel count mismatch, stream channels = " << mChannelLayout.toString()
                   << " pipe config channels = " << config.mChannelLayout.toString();
        return false;
    }
    if (mSampleRate != config.mSampleRate) {
        LOG(ERROR) << __func__ << ": sample rate mismatch, stream sample rate = " << mSampleRate
                   << " pipe config sample rate = " << config.mSampleRate;
        return false;
    }
    if (mFormat != config.mFormat) {
        LOG(ERROR) << __func__ << ": format mismatch, stream format = " << mFormat.toString()
                   << " pipe config format = " << config.mFormat.toString();
        return false;
    }
    return true;
}
...
::android::status_t DriverRemoteSubmix::createPipe() {
    ...
    int channelCount = getChannelCount(mChannelLayout);
    audio_format_t audioFormat =
            VALUE_OR_RETURN_STATUS(aidl2legacy_AudioFormatDescription_audio_format_t(mFormat));
    const ::android::NBAIO_Format format =
            ::android::Format_from_SR_C(mSampleRate, channelCount, audioFormat);
    const ::android::NBAIO_Format offers[1] = {format};
    size_t numCounterOffers = 0;

    size_t pipeSizeInFrames = getPipeSizeInFrames();
    LOG(VERBOSE) << __func__ << ": creating pipe, rate : " << mSampleRate
    ...
    LOG(VERBOSE) << __func__ << ": Pipe frame size : " << mSubmixRoute->config.mPipeFrameSize
...
::android::status_t DriverRemoteSubmix::init() {
    {
        ...
        if (submixRouteItr == mSubmixRoutes.end()) {
            // Initialize the pipe.
            mSubmixRoute = std::make_shared<SubmixRoute>();
            if (::android::OK != createPipe()) {
                LOG(ERROR) << __func__ << ": create pipe failed";
                return mStatus;
            }
            mSubmixRoutes.push_back(mSubmixRoute);
        } else {
    ...
    mSubmixRoute->config.mChannelLayout = mChannelLayout;
    mStatus = ::android::OK;
    return mStatus;
}
...
::android::status_t DriverRemoteSubmix::close() {
    ...
    std::lock_guard routeGuard(mSubmixRoute->mLock);
    if (!mSubmixRoute->mInputOpen && !mSubmixRoute->mOutputOpen) {
        std::lock_guard guard(mLock);
        auto submixRouteItr = findPtrById<std::shared_ptr<SubmixRoute>>(mSubmixRoutes, mPortId);
...
::android::status_t DriverRemoteSubmix::outWrite(void* buffer, size_t frameCount,
    ...
    sp<MonoPipe> sink = mSubmixRoute->sink;
    {
        ...
        if (!shouldBlock && availableToWrite < frameCount) {
            ...
            const size_t flushBufferSizeFrames = sizeof(flushBuffer) / mFrameSizeBytes;
...
::android::status_t DriverRemoteSubmix::inRead(void* buffer, size_t frameCount,
    ...
    if (readError) {
        const size_t delayUs =
                static_cast<size_t>(std::roundf(frameCount * kMicrosPerSecond / mSampleRate));
        usleep(delayUs);
        memset(buffer, 0, mFrameSizeBytes * frameCount);
        *actualFrameCount = frameCount;
        return ::android::OK;
    }
    ...
    while ((remainingFrames > 0) && (attempts < MAX_READ_ATTEMPTS)) {
        ...
        size_t readFrames = remainingFrames;
        ...
        if (framesRead > 0) {
            ...
            buff += framesRead * mFrameSizeBytes;
    ...
    if (remainingFrames > 0) {
        const size_t remainingBytes = remainingFrames * mFrameSizeBytes;
        LOG(VERBOSE) << __func__ << ":  clearing remaining_frames = " << remainingFrames;
        memset(((char*)buffer) + (mFrameSizeBytes * frameCount) - remainingBytes, 0,
               remainingBytes);
    }
    ...
    auto recordDurationUs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::steady_clock::now() - mSubmixRoute->mRecordStartTime);
