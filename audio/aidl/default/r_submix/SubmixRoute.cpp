...
std::mutex SubmixRoute::sSubmixRoutesLock;
std::map<AudioDeviceAddress, std::shared_ptr<SubmixRoute>> SubmixRoute::sSubmixRoutes;
...
std::shared_ptr<SubmixRoute> SubmixRoute::findOrCreateRoute(const AudioDeviceAddress& deviceAddress,
    ...
    auto routeItr = sSubmixRoutes.find(deviceAddress);
    ...
    sSubmixRoutes.emplace(deviceAddress, route);
    return route;
}
...
void SubmixRoute::removeRoute(const AudioDeviceAddress& deviceAddress) {
    std::lock_guard guard(sSubmixRoutesLock);
    sSubmixRoutes.erase(deviceAddress);
}
