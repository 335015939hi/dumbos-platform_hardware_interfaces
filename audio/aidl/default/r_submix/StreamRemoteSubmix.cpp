...
size_t DriverRemoteSubmix::getPipeSizeInFrames() {
    return kDefaultPipeSizeInFrames *
           ((float)mStreamConfig.sampleRate / r_submix::kDefaultSampleRateHz);
}
...
::android::status_t DriverRemoteSubmix::createPipe() {
    ...
    const int channelCount = getChannelCount(mStreamConfig.channelLayout);
    ...
    const size_t pipeSizeInFrames = getPipeSizeInFrames();
...
::android::status_t DriverRemoteSubmix::init() {
    std::shared_ptr<SubmixRoute> route;
    {
...
::android::status_t DriverRemoteSubmix::outWrite(void* buffer, size_t frameCount,
    ...
    const bool shouldBlock = mCurrentRoute->inputOpen || (mCurrentRoute->inputStandby &&
                                                          (mCurrentRoute->readCounterFrames != 0));
...
::android::status_t DriverRemoteSubmix::inRead(void* buffer, size_t frameCount,
    ...
    long readCounterFrames;
    {
        ...
        source.clear();
        recordStartTime = mCurrentRoute->recordStartTime;
        readCounterFrames = mCurrentRoute->readCounterFrames;
    }
    ...
    auto recordDurationUs = std::chrono::steady_clock::now() - recordStartTime;
