...
void EngineConfigXmlConverter::initProductStrategyMap() {
    productStrategyMap = {
            ...
            {"STRATEGY_ENFORCED_AUDIBLE",
             static_cast<int>(AudioProductStrategyType::ENFORCED_AUDIBLE)},
...
int EngineConfigXmlConverter::convertProductStrategyNameToAidl(
        std::string xsdcProductStrategyName) {
    if (productStrategyMap.find(xsdcProductStrategyName) == productStrategyMap.end()) {
        productStrategyMap[xsdcProductStrategyName] = latest_vendor_strategy++;
    }
    return productStrategyMap.at(xsdcProductStrategyName);
}

AudioAttributes EngineConfigXmlConverter::convertAudioAttributesToAidl(
        const xsd::AttributesType& xsdcAudioAttributes) {
    // TODO: check if AttributesRef can be removed from schema:
    // 1. They aren't used in any of the example XMLs => vendors probably aren't using them.
    // 2. While a single attributes reference can contain a vector of attributes,
    // it seems that we only parse the first one in EngineConfig.cpp, so even now,
    // they are not working as intended.
    // 3. Writing code to make them WAI adds unnecessary complexity.
    // For now, only using the first Attribute in the AttributesRef vector, as this
    // is the current behavior, but prefer to remove from schema.
    ...
    if (xsdcAudioAttributes.hasFlags()) {
        ...
        for (const xsd::FlagType& xsdcFlagType : xsdcFlagTypeVec) {
            // TODO: add VTS test to ensure AUDIO_FLAG_NONE can't be used in conjunction with other
            // flags
...
AudioHalAttributesGroup EngineConfigXmlConverter::convertAttributesGroupToAidl(
    ...
    static const int STREAM_TYPE_ENUM_OFFSET =
    ...
    aidlAttributesGroup.streamType = static_cast<AudioStreamType>(
            static_cast<int>(xsdcAttributesGroup.getStreamType()) - STREAM_TYPE_ENUM_OFFSET);
    ...
    if (xsdcAttributesGroup.hasAttributes_optional()) {
        ...
        aidlAttributesGroup.attributes.push_back(std::move(convertAudioAttributesToAidl(
                xsd::AttributesType(xsdcAttributesGroup.getContentType_optional(),
...
AudioHalProductStrategy EngineConfigXmlConverter::convertProductStrategyToAidl(
    ...
    aidlProductStrategy.productStrategyId =
            convertProductStrategyNameToAidl(xsdcProductStrategy.getName());
...
void EngineConfigXmlConverter::init() {
    ...
    if (mXsdcConfig->hasProductStrategies()) {
    ...
    if (mXsdcConfig->hasVolumeGroups()) {
        mAidlEngineConfig.volumeGroups = convertWrappedCollectionToAidl<
                ...
                mXsdcConfig->getVolumeGroups(), &xsd::VolumeGroupsType::getVolumeGroup,
                std::bind(&EngineConfigXmlConverter::convertVolumeGroupToAidl, this,
                          std::placeholders::_1));
    }
    if (mXsdcConfig->hasCriteria() && mXsdcConfig->hasCriterion_types()) {
        ...
        capSpecificConfig.criteria =
                convertWrappedCollectionToAidl<xsd::CriteriaType, xsd::CriterionType,
                        ...
                        mXsdcConfig->getCriteria(), &xsd::CriteriaType::getCriterion,
        ...
        capSpecificConfig.criterionTypes =
                convertWrappedCollectionToAidl<xsd::CriterionTypesType, xsd::CriterionTypeType,
                        ...
                        mXsdcConfig->getCriterion_types(),
