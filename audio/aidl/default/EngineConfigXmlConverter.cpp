...
static const int kInvalidPointIndicator = -1;
...
int EngineConfigXmlConverter::convertProductStrategyNameToAidl(
    ...
    if (mProductStrategyMap.find(xsdcProductStrategyName) == mProductStrategyMap.end()) {
        mProductStrategyMap[xsdcProductStrategyName] = latest_vendor_strategy++;
    }
    return mProductStrategyMap.at(xsdcProductStrategyName);
}
