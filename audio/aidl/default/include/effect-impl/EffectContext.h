...
class EffectContext {
  public:
    ...
    size_t availableToRead() { return mWorkBuffer.size(); };
    size_t availableToWrite() { return mWorkBuffer.size(); };
  ...
  private:
    ...
    // work buffer set by effect instances, the access and update should be in same thread
