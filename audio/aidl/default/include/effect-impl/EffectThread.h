...
class EffectThread {
  public:
    ...
    /**
     * The default process() implementation of EffectThread doesn't need to lock. It will only
access the FMQ and mWorkBuffer in  EffectContext, since they will only be changed in EffectImpl
IEffect::open() (in this case EffectThread just created and not running yet) and
IEffect::command(CommandId::RESET) (in this case EffectThread already stopped).
  ...
  private:
    ...
    std::shared_ptr<EffectContext> mThreadContext;
    std::thread mThread;
    int mPriority;
    std::string mName;
};
