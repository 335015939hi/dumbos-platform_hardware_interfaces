...
class EngineConfigXmlConverter
    : public XmlConverter<::android::audio::policy::engine::configuration::Configuration> {
  public:
    EngineConfigXmlConverter(const std::string& configFilePath)
        : XmlConverter(configFilePath, &::android::audio::policy::engine::configuration::read) {
        init();
    }
    static inline const std::string DEFAULT_CONFIG_FILE_NAME =
  ...
  private:
    ...
    int convertProductStrategyNameToAidl(std::string xsdcProductStrategyName);
    ...
    std::unordered_map<std::string, int> productStrategyMap;
    int latest_vendor_strategy = ::aidl::android::media::audio::common::AudioHalProductStrategy::
