...
class EngineConfigXmlConverter {
  public:
    explicit EngineConfigXmlConverter(const std::string& configFilePath)
        : converter(configFilePath, &::android::audio::policy::engine::configuration::read) {
        init();
    }

    std::string getError() const { return converter.getError(); }
    ::android::status_t getStatus() const { return converter.getStatus(); }

    ::aidl::android::media::audio::common::AudioHalEngineConfig& getAidlEngineConfig();

  private:
    XmlConverter<::android::audio::policy::engine::configuration::Configuration> converter;
    const std::optional<::android::audio::policy::engine::configuration::Configuration>&
    getXsdcConfig() {
        return converter.getXsdcConfig();
    }
    void init();
