...
class DriverBluetooth : public DriverInterface {
  public:
    ...
    ::android::status_t setConnectedDevices(
            const std::vector<AudioDevice>& connectedDevices) override;
  ...
  private:
    ...
    AudioFormatDescription mFormat;
    ...
    std::vector<AudioDevice> mConnectedDevices GUARDED_BY(mLock);
    std::vector<std::shared_ptr<BluetoothAudioPort>> mBtDeviceProxies GUARDED_BY(mLock);
