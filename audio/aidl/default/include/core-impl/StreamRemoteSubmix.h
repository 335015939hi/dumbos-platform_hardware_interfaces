...
struct SubmixRoute {
    ...
    sp<MonoPipe> sink GUARDED_BY(lock);
    sp<MonoPipeReader> source GUARDED_BY(lock);

    // how many frames have been requested to be read since standby
    long readCounterFrames GUARDED_BY(lock) = 0;
    int inputRefCount GUARDED_BY(lock) = 0;
    int readErrorCount GUARDED_BY(lock) = 0;
    ...
    std::chrono::time_point<std::chrono::steady_clock> recordStartTime GUARDED_BY(lock);
};
