...
void StreamAlsa::inputIoThread(size_t idx) {
    ...
    while (mIoThreadIsRunning) {
        if (proxy_read_with_retries(mAlsaDeviceProxies[idx].get(), &buffer[0], bufferSize,
                                    mReadWriteRetries) == 0) {
            ...
            while (bufferFramesWritten < mBufferSizeFrames) {
                ...
                if (framesWrittenOrError >= 0) {
                    ...
                    LOG(WARNING) << "Error while writing into the pipe: " << framesWrittenOrError;
                }
            }
        }
    }
...
void StreamAlsa::outputIoThread(size_t idx) {
    ...
    while (mIoThreadIsRunning) {
        ...
        if (framesRead > 0) {
            proxy_write_with_retries(mAlsaDeviceProxies[idx].get(), &buffer[0],
