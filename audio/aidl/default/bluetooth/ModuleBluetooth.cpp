...
ndk::ScopedAStatus ModuleBluetooth::setAudioPortConfig(const AudioPortConfig& in_requested,
    ...
    if (in_requested.ext.getTag() == AudioPortExt::device) {
        ...
        mLastDevicePortId = out_suggested->portId;
        return ndk::ScopedAStatus::ok();
    } else if (in_requested.id == 0 && in_requested.ext.getTag() == AudioPortExt::mix) {
        // This relies on the fact that during stream opening the mix port config is initialized
        // after the device port config, and these calls to 'setAudioPortConfig' are on the same
        // client thread.
        CachedProxy proxy;
        auto proxyIt = mProxies.find(mLastDevicePortId);
        if (proxyIt == mProxies.end()) {
            auto& ports = getConfig().ports;
            if (auto portsIt = findById<AudioPort>(ports, mLastDevicePortId);
                portsIt != ports.end()) {
                RETURN_STATUS_IF_ERROR(createProxy(*portsIt, portsIt->id, proxy));
            } else {
        ...
        return Module::setAudioPortConfigImpl(
                in_requested,
                [&proxy](const AudioPort& port, AudioPortConfig* out_config) {
                    const auto pcmConfig = proxy.pcmConfig;
                    LOG(DEBUG) << "setAudioPortConfig: suggesting port config from "
                               << pcmConfig.toString();
                    out_config->format = AudioFormatDescription{
                            .type = AudioFormatType::PCM,
                            .pcm = pcmTypeFromBitsPerSample(pcmConfig.bitsPerSample)};
                    out_config->channelMask = channelLayoutFromChannelMode(pcmConfig.channelMode);
                    out_config->sampleRate = Int{.value = pcmConfig.sampleRateHz};
                    out_config->flags = port.flags;
                    out_config->ext = port.ext;
                    return true;
                },
                out_suggested, _aidl_return);
    }
    return Module::setAudioPortConfig(in_requested, out_suggested, _aidl_return);
}
