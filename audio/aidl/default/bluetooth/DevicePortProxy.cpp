...
bool BluetoothAudioPortAidl::getPresentationPosition(
    ...
    if (retval) {
        uint64_t delay_ns = presentation_position.remoteDeviceAudioDelayNanos;
        uint64_t bytes = presentation_position.transmittedOctets;
        timespec timestamp = {.tv_sec = static_cast<__kernel_old_time_t>(
