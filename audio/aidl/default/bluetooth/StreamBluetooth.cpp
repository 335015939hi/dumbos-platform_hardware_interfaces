...
::android::status_t DriverBluetooth::standby() {
    ...
    for (auto proxy : btDeviceProxies) {
        ...
        if (proxy->GetState() == BluetoothStreamState::SUSPENDING) {
        ...
        if (!stateTransitionTimeOut(proxy, BluetoothStreamState::STARTING)) {
            LOG(ERROR) << __func__ << ": state=" << proxy->GetState() << " NOT ready to standby";
            return -EBUSY;
        }

        if (proxy->GetState() == BluetoothStreamState::STARTED) {
            status = (proxy->Suspend() ? 0 : -EIO);
        }

        /* Give some time to suspend */
        if (status == ::android::OK &&
            !stateTransitionTimeOut(proxy, BluetoothStreamState::SUSPENDING)) {
            LOG(ERROR) << __func__ << ": state=" << proxy->GetState() << " NOT ready to by standby";
            return -EBUSY;
        }
