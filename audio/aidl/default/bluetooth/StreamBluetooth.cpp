...
const int kBluetoothStatePollTimeOutMs = 1;
...
::android::status_t DriverBluetooth::transfer(void* buffer, size_t frameCount,
    ...
    for (auto proxy : btDeviceProxies) {
        ...
        if (!stateTransitionTimeOut(proxy, lock, BluetoothStreamState::SUSPENDING)) {
        ...
        if (!stateTransitionTimeOut(proxy, lock, BluetoothStreamState::STARTING)) {
            LOG(ERROR) << __func__ << ": state = " << proxy->getState()
