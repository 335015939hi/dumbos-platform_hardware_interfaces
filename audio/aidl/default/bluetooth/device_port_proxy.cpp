...
bool BluetoothAudioPortAidl::SetUp(AudioDeviceDescription description) {
  if (!init_session_type(description)) return false;
...
bool BluetoothAudioPortAidl::init_session_type(AudioDeviceDescription description) {
#define GET_DEVICE_DESC_CONNECTION(x)  AudioDeviceDescription::CONNECTION_##x
  if (description.connection == GET_DEVICE_DESC_CONNECTION(BT_A2DP) && 
      ...
      LOG(VERBOSE)
          ...
          << StringPrintf("%s", description.toString().c_str()) << ")";
      session_type_ = SessionType::A2DP_SOFTWARE_ENCODING_DATAPATH;
  } else if (description.connection == GET_DEVICE_DESC_CONNECTION(WIRELESS) && 
      description.type == AudioDeviceType::OUT_HEARING_AID) {
      LOG(VERBOSE) << __func__
                   << ": device=AUDIO_DEVICE_OUT_HEARING_AID (MEDIA/VOICE) ("
                   << StringPrintf("%s", description.toString().c_str()) << ")";
      session_type_ = SessionType::HEARING_AID_SOFTWARE_ENCODING_DATAPATH;
  } else if (description.connection == GET_DEVICE_DESC_CONNECTION(BT_LE) && 
  ...
  if (!BluetoothAudioSessionControl::IsSessionReady(session_type_)) {
    LOG(ERROR) << __func__ << ": device=" << StringPrintf("%s", description.toString().c_str())
...
bool BluetoothAudioPortAidlOut::LoadAudioConfig(
    PcmConfiguration* audio_cfg) const {
  if (!in_use()) {
...
void BluetoothAudioPortAidl::UpdateSourceMetadata(
    const SourceMetadata* source_metadata) const {
  if (!in_use()) {
    LOG(ERROR) << __func__ << ": BluetoothAudioPortAidl is not in use";
    return;
  }
  LOG(DEBUG) << __func__ << ": session_type=" << toString(session_type_)
             << ", cookie=" << StringPrintf("%#hx", cookie_)
             << ", state=" << state_ << ", " << source_metadata->tracks.size()
             << " track(s)";
...
void BluetoothAudioPortAidl::UpdateSinkMetadata(
    const SinkMetadata* sink_metadata) const {
  if (!in_use()) {
    LOG(ERROR) << __func__ << ": BluetoothAudioPortAidl is not in use";
    return;
  }
  LOG(DEBUG) << __func__ << ": session_type=" << toString(session_type_)
             << ", cookie=" << StringPrintf("%#hx", cookie_)
             << ", state=" << state_ << ", " << sink_metadata->tracks.size()
             << " track(s)";
