...
const AcousticEchoCanceler::Capability AcousticEchoCancelerSw::kCapability = {
        .maxEchoDelayMs = 500, .supportMobileMode = false};
...
ndk::ScopedAStatus AcousticEchoCancelerSw::setParameterSpecific(
    ...
    switch (tag) {
        case AcousticEchoCanceler::echoDelayMs: {
            RETURN_IF(mContext->setEchoDelay(param.get<AcousticEchoCanceler::echoDelayMs>()) !=
                              RetCode::SUCCESS,
...
ndk::ScopedAStatus AcousticEchoCancelerSw::getParameterAcousticEchoCanceler(
    ...
    switch (tag) {
        case AcousticEchoCanceler::echoDelayMs: {
            param.set<AcousticEchoCanceler::echoDelayMs>(mContext->getEchoDelay());
            break;
        }
        case AcousticEchoCanceler::mobileMode: {
...
RetCode AcousticEchoCancelerSwContext::setEchoDelay(int echoDelayMs) {
    if (echoDelayMs < 0 || echoDelayMs > AcousticEchoCancelerSw::kCapability.maxEchoDelayMs) {
        LOG(DEBUG) << __func__ << " illegal delay " << echoDelayMs;
