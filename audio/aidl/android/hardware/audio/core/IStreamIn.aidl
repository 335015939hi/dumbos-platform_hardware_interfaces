...
interface IStreamIn {
    ...
    /**
     * Indicate which microphones are active for this stream.
     *
     * The returned array contains unique ids ('MicrophoneInfo.id' field values)
     * for the microphones which are active for this stream. The list of all
     * available microphones is provided by the IModule.getMicrophones method.
     *
     * @return The vector with unique ids of microphones
     * @throws EX_ILLEGAL_STATE If the stream has already been closed.
     * @throws EX_UNSUPPORTED_OPERATION If the information is unavailable.
     */
    @utf8InCpp String[] getActiveMicrophonesIds();
    ...
    // const float MIC_FIELD_DIMENSION_WIDE_ANGLE = -1.0f;
    // const float MIC_FIELD_DIMENSION_NO_ZOOM = 0.0f;
    // const float MIC_FIELD_DIMENSION_MAX_ZOOM = 1.0f;
    /**
