...
parcelable StreamDescriptor {
    ...
    enum State {
        /**
         * The initial state of the stream (entered after opening). When the
         * stream is in this state, it is assumed that all intermediate audio
         * buffers are empty. The HAL module must not account for any under- or
         * overruns as the client is not expected to perform audio I/O. Audio
         * hardware may still be using power in this state.
         */
        IDLE = 1,
        /**
         ...
         * considered as an under- or overrun.
         */
        READY = 3,
        /**
        ...
        ERROR = 4,
        /**
         * This state applies to output streams only. This is an intermediate
         * state in which the audio output has been put on hold. This state is
         * similar to the IDLE state, except that the buffers still contain
         * audio data, thus the audio pipeline is ready to continue seamlessly.
         */
        PAUSED = 64,
        /**
         * This state applies only to output streams opened in non-blocking
         * mode.  This is a transit state which the stream takes after the
         * control thread returns control to the client for the BURST
         * operation. On leaving this state, the control thread notifies the
         * client of the I/O completion via IStreamCallback interface.
         */
        TRANSFER = 65,
        /**
         * This state applies only to output streams opened in non-blocking
         * mode.  This is a transit state which the stream takes after the
         * control thread returns control to the client for the DRAIN operation.
         *
         * While the stream remains in this state, the HAL module must send
         * periodic updates on the remaining playback time via
         * IStreamCallback.onDrainUpdate method. The stream transfers to the
         * IDLE state once intermediate buffers are empty.
         */
        DRAIN = 66,
    }
    ...
    enum CommandCode {
        /**
         * The START command is used to draw the stream out of IDLE or STANDBY
         * state and prepare it for audio I/O. The 'fmqByteCount' field must
         * always be set to 0.
         */
        START = 1,
        /**
        ...
        BURST = 2,
        /**
         * The DRAIN command is used to return the stream back to the IDLE
         * state.  For the MMap no IRQ mode this command means that exchange of
         * audio via the 'AudioBuffer.mmap' shared buffer must cease until
         * receiving the START command. For output streams, all remaining data
         * from audio buffers is sent to hardware for playback (for discarding
         * the data, use the sequence of PAUSE and FLUSH commands). The
         * 'fmqByteCount' field must always be set to 0.
         */
        DRAIN = 3,
        /**
         * The STANDBY command is used to hint the HAL module that the client is
         * not going to perform audio I/O for some time, thus the HAL module can
         * put any connected hardware into standby mode to save power. The
         * 'fmqByteCount' field must always be set to 0.
         *
         * It's left on the discretion of the HAL implementation to assess all
         * the necessary conditions that could prevent hardware from being
         * suspended, and ignore the call in this case. Inability to suspend the
         * hardware is not considered as an error, in this case the stream
        ...
        PAUSE = 5,
        /**
         * This command is used to discard all audio data remaining in buffers
         * after pausing, and bring the stream back to the IDLE state. The
         * 'fmqByteCount' field must always be set to 0.
         *
         ...
         * current latency value, if possible. The FLUSH command does not take
         * the stream out of standby.
         */
        FLUSH = 6,
    }
