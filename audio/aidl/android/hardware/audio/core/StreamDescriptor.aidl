 ...
 * the state machine enters after replying to the 'burst' command. The client
 * gets unblocked, but actual audio delivery to / from the observer is not
 ...
 * (this can take a long time). For output streams, asynchronous transfer can be
 * paused, and it's another new state: 'TRANSFER_PAUSED'. It differs from
 * 'PAUSED' by the fact that no new writes are allowed. Please see
 * 'stream-in-async-sm.gv' and 'stream-out-async-sm.gv' files for details. Below
 * is the table summary for asynchronous only-states:
 *
 *  Producer | Buffer state | Consumer | Applies | State
 *  active?  |              | active?  | to      |
 * ==========|==============|==========|=========|==============================
