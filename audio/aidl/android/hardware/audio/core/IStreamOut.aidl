...
interface IStreamOut {
    ...
    // const float MIN_AUDIO_DESCRIPTION_MIX_LEVEL = -Inf;
    // const float MAX_AUDIO_DESCRIPTION_MIX_LEVEL = 48.0f;
    /**
     ...
     * @throws EX_UNSUPPORTED_OPERATION If setting of this parameter is not supported.
     */
    void setDualMonoMode(DualMonoMode mode);

    /**
     * Retrieve supported latency modes.
     *
     * Indicates which latency modes are currently supported on this output
     * stream. If the transport protocol (e.g Bluetooth A2DP) used by this
     * output stream to reach the output device supports variable latency modes,
     * the HAL indicates which modes are currently supported. The client can
     * then call setLatencyMode() with one of the supported modes to select the
     ...
     *
     * @return modes Currently supported latency modes.
     * @throws EX_ILLEGAL_STATE If the stream is closed.
     * @throws EX_UNSUPPORTED_OPERATION If the information is unavailable.
     */
    LatencyMode[] getRecommendedLatencyModes();
    /**
     ...
     * @throws EX_UNSUPPORTED_OPERATION If the information is unavailable.
     */
    PlaybackRate getPlaybackRateParameters();
    /**
     ...
     * the 'IModule.getSupportedPlaybackRateFactors' method.
     *
     ...
     *                                  is not supported.
     */
    void setPlaybackRateParameters(in PlaybackRate playbackRate);
     ...
     * to unity gain, 0.0f corresponds to full mute. The returned array specifies
     * attenuation for each output channel of the stream.
     *
     ...
     *
     * The valid range for attenuation is [0.0f, 1.0f], where 1.0f corresponds
     * to unity gain, 0.0f corresponds to full mute.
     *
     * Support of hardware volume control is optional.
     *
