 ...
 * one offload implementation), so effect process can be seamlessly switch between implementations
 * in same proxy depending on the configuration and/or use case. If the optional proxy UUID is
 * specified in Descriptor.Identity, then client must consider the effect instance as part of effect
 * proxy.
 */
@VintfStability
