 ...
 * limitation. Effect must return EX_ILLEGAL_ARGUMENT for any out-of-range parameter setting.
 *
 * If an effect implementation need to define the valid range for a certain parameter, it can
 ...
 * Range::AcousticEchoCanceler kRange = MAKE_RANGE(AcousticEchoCanceler, echoDelayUs, 0, 500)};
 *
 ...
 * indicate no paramter from effect client can be accepted for setting. The effect implementation
 * must rerun EX_ILLEGAL_ARGUMENT if receive any setParameter call for get only parameter.
 * For a capability definition, the effect implementation must define a range with min == max, to
 * indicate this is a fixed capability which shouldn't set by client.
 * For example, the Equalizer presets capability can be defined with:
 * const std::vector<Equalizer::Preset> kPresets = {
 *       {0, "Normal"},      {1, "Classical"}, {2, "Dance"}, {3, "Flat"}, {4, "Folk"},
 *       {5, "Heavy Metal"}, {6, "Hip Hop"},   {7, "Jazz"},  {8, "Pop"},  {9, "Rock"}};
 * const Range::EqualizerRange kRanges =
 *      MAKE_RANGE(Equalizer, presets, EqualizerSw::kPresets, EqualizerSw::kPresets);
 *
 * from effect client can be accepted for setting. The effect implementation must rerun
 * EX_ILLEGAL_ARGUMENT if receive any setParameter call for get only parameter.
 *
 */
@VintfStability
union Range {
