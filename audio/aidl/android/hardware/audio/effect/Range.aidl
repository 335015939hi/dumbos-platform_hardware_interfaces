 ...
 * indicate no parameter from effect client can be accepted for setting. The effect implementation
 ...
 *
 * The effect implementation must rerun EX_ILLEGAL_ARGUMENT if:
...
union Range {
     ...
     * Each element of the vector represents the min and max range of a paramter, so the size of
