...
int main(int /* argc */, char* /* argv */ []) {
    ...
    const std::vector<InterfacesList> optionalInterfaceSharedLibs = {
        {
    ...
    for (const auto& listIter : optionalInterfaceSharedLibs) {
        auto iter = listIter.begin();
        const std::string& interfaceLoaderFuncName = *iter++;
        for (; iter != listIter.end(); iter++) {
            auto library = *iter;
            if (registerExternalServiceImplementation(library, interfaceLoaderFuncName)) {
                ALOGI("%s() from %s success", interfaceLoaderFuncName.c_str(), library.c_str());
            } else {
                ALOGW("%s() from %s failed", interfaceLoaderFuncName.c_str(), library.c_str());
            }
        }
    }
