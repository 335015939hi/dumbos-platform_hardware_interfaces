...
bool logFromPipe(int fd, android::base::LogSeverity severity) {
    bool logged = false;
    std::string line;
    char buf;
...
TEST_P(ProcessTimedRunner, RunProcess) {
    int childOutPipe[2], childErrorPipe[2];
    ASSERT_NE(-1, pipe(childOutPipe));
    ASSERT_NE(-1, pipe(childErrorPipe));
    pid_t pid = fork();
    ASSERT_GE(pid, 0);
    ...
    if (pid == 0) {
        ...
        dup2(childOutPipe[1], STDOUT_FILENO);
        dup2(childErrorPipe[1], STDERR_FILENO);
    ...
    if (WIFEXITED(status)) {
        ...
        // We do not expect the child to output much, thus it should not be blocked
        // on writing into pipes, and we don't have to read while waiting for its completion.
        logFromPipe(childOutPipe[0], android::base::LogSeverity::DEBUG);
        EXPECT_FALSE(logFromPipe(childErrorPipe[0], android::base::LogSeverity::ERROR))
                << "See logcat for child error output";
        close(childOutPipe[0]);
        close(childErrorPipe[0]);
    } else {
