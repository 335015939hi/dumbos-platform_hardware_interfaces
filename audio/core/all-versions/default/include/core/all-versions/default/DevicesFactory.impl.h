...
Return<void> DevicesFactory::openDevice(const char* moduleName, wp<DeviceItf>* cache,
    ...
    if (result == nullptr) {  // device not found in the cache
        int halStatus = loadAudioInterface(moduleName, &halDevice);
        if (halStatus == OK) {
            result = new DeviceImpl(halDevice);
            *cache = result;
        } else if (halStatus == -EINVAL) {
            retval = Result::NOT_INITIALIZED;  // The device could not be initialized
        } else {
            retval = Result::INVALID_ARGUMENTS;  // The device was not found
        }
    }
