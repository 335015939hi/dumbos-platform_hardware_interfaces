...
Return<void> DevicesFactory::openPrimaryDevice(Callback _hidl_cb) {
    return openDevice<PrimaryDevice>(AUDIO_HARDWARE_MODULE_ID_PRIMARY, &mPrimaryDeviceCache,  _hidl_cb);
}

template <class DeviceImpl, class DeviceItf, class Callback>
Return<void> DevicesFactory::openDevice(const char* moduleName, wp<DeviceItf>* cache, Callback _hidl_cb) {
