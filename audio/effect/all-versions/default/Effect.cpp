...
Effect::~Effect() {
    ...
    if (EventFlag* evFlag = mEfGroup.load(std::memory_order_relaxed)) {
...
Return<void> Effect::prepareForProcessing(prepareForProcessing_cb _hidl_cb) {
    ...
    EventFlag* evFlag = mEfGroup.load(std::memory_order_relaxed);
    status = EventFlag::createEventFlag(tempStatusMQ->getEventFlagWord(), &evFlag);
    if (status != OK || !mEfGroup.load(std::memory_order_relaxed)) {
    ...
    mProcessThread = new ProcessThread(&mStopProcessThread, mHandle, &mHalInBufferPtr,
                                       &mHalOutBufferPtr, tempStatusMQ.get(), 
                                       mEfGroup.load(std::memory_order_relaxed), this);
...
std::tuple<Result, effect_handle_t> Effect::closeImpl() {
    ...
    if (mEfGroup.load(std::memory_order_relaxed)) {
        mEfGroup.load(std::memory_order_relaxed)->wake(static_cast<uint32_t>(
