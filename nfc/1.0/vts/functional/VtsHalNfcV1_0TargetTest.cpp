...
class NfcHidlTest : public ::testing::VtsHalHidlTargetTestBase {
 public:
  virtual void SetUp() override {
    ...
    EXPECT_EQ(NfcEvent::OPEN_CPLT, res.second->last_event_);
    EXPECT_EQ(NfcStatus::OK, res.second->last_status_);
  }
  ...
  virtual void TearDown() override {
    ...
    auto res = nfc_cb_->WaitForCallback(kCallbackNameSendEvent);
    EXPECT_TRUE(res.first);
    EXPECT_EQ(NfcEvent::CLOSE_CPLT, res.second->last_event_);
    EXPECT_EQ(NfcStatus::OK, res.second->last_status_);
  }
...
TEST_F(NfcHidlTest, WriteCoreReset) {
  ...
  EXPECT_TRUE(res.first);
  EXPECT_EQ(6ul, res.second->last_data_.size());
  EXPECT_EQ((int)NfcStatus::OK, res.second->last_data_[3]);
  EXPECT_GE(VERSION, res.second->last_data_[4]);
  EXPECT_EQ(0ul, res.second->last_data_[5]);
}
...
TEST_F(NfcHidlTest, WriteCoreResetConfigReset) {
  ...
  EXPECT_EQ((int)NfcStatus::OK, res.second->last_data_[3]);
  EXPECT_GE(VERSION, res.second->last_data_[4]);
  EXPECT_EQ(1ul, res.second->last_data_[5]);
}
...
TEST_F(NfcHidlTest, WriteInvalidCommand) {
  ...
  auto res = nfc_cb_->WaitForCallback(kCallbackNameSendData);
  EXPECT_TRUE(res.first);
  EXPECT_EQ(4ul, res.second->last_data_.size());
  EXPECT_EQ(SYNTAX_ERROR, res.second->last_data_[3]);
}
...
TEST_F(NfcHidlTest, WriteInvalidAndThenValidCommand) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    auto res = nfc_cb_->WaitForCallback(kCallbackNameSendData);
    EXPECT_TRUE(res.first);
    EXPECT_EQ(5ul, res.second->last_data_.size());
    EXPECT_EQ(0x60, res.second->last_data_[0]);
    EXPECT_EQ(0x08, res.second->last_data_[1]);
    EXPECT_EQ(0x02, res.second->last_data_[2]);
    EXPECT_EQ(SYNTAX_ERROR, res.second->last_data_[3]);
  }
  ...
  auto res = nfc_cb_->WaitForCallback(kCallbackNameSendData);
  EXPECT_TRUE(res.first);
  EXPECT_EQ(7ul, res.second->last_data_.size());
  EXPECT_EQ((int)NfcStatus::OK, res.second->last_data_[3]);
}
/*
 * Bandwidth:
...
TEST_F(NfcHidlTest, Bandwidth) {
  ...
  EXPECT_EQ(7ul, res.second->last_data_.size());
  EXPECT_EQ((int)NfcStatus::OK, res.second->last_data_[3]);
  uint8_t conn_id = res.second->last_data_[6];
  uint32_t max_payload_size = res.second->last_data_[4];

  for (int loops = 0; loops < NUMBER_LOOPS; loops++) {
    res.second->last_data_.resize(0);
    ...
    NfcData credits_ntf = res1.second->last_data_;
    NfcData received_data = res2.second->last_data_;
    ...
    if (received_data.size() != data.size()) {
      credits_ntf = res2.second->last_data_;
      received_data = res1.second->last_data_;
    }
...
TEST_F(NfcHidlTest, CoreInitialized) {
  ...
  for (int i = 0; i <= 6; i++) {
    ...
    auto res = nfc_cb_->WaitForCallback(kCallbackNameSendEvent);
    EXPECT_TRUE(res.first);
    EXPECT_EQ(NfcEvent::POST_INIT_CPLT, res.second->last_event_);
  }
}
