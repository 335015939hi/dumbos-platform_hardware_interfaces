...
import android.hardware.nfc.NfcData;
...
interface INfc {
     ...
     *
     * If open completes successfully, the controller should be ready to perform NCI
     ...
     * If open() returns ok(), the NCI stack will wait for a NfcEvent.OPEN_CPLT
     * callback before continuing.
     * If a NfcEvent::OPEN_CPLT callback received with status NfcStatus::OK, the controller
     * should be ready to perform  NCI initialization - ie accept CORE_RESET and subsequent
     ...
     * Associated resources such as clientCallback are released.
     * The clientCallback reference from open() should be invalid after close().
     * If close() returns ok(), the NCI stack will wait for a NfcEvent.CLOSE_CPLT
     * callback before continuing.
     * Returns an error if close may be called more than once.
     * Calls to any other method which expect a callback after this will return
     * a service-specific error NfcStatus::FAILED.
     * HAL will call close() automatically if the client drops the reference to the HAL or
     * crashes.
     */
     ...
     *
     * If coreInitialized() returns ok(), the NCI stack will wait for a
     * NfcEvent.POST_INIT_CPLT before continuing.
     * If coreInitialized() returns an error, the NCI stack will continue immediately.
     *
     ...
     * resources.
     *
     * If powerCycle() returns ok(), the NCI stack will wait for a NfcEvent.OPEN_CPLT
     * before continuing.
     *
     ...
     *
     * If preDiscover() returns ok(), the NCI stack will wait for a
     * NfcEvent.PREDISCOVER_CPLT before continuing.
     *
     * preDiscover() must be called after open() registers the clientCallback
     * or return a service-specific error NfcStatus::FAILED directly.
     *
     * If preDiscover() reports an error, the NCI stack will start RF discovery immediately.
     */
    void preDiscover();
     ...
     *
     * @param NfcData for the NCI data writes to the NFCC.
     *
     * @return number of bytes written to the NFCC.
     */
    int write(in NfcData data);

    /**
     * Set the logging flag for NFC HAL to enable it's verbose logging.
     * Do nothing if verbose logging does not supported, but NFC HAL should store the flag state
     * anyway.
