...
static int get_board_api_level() {
    ...
    EXPECT_NE(board_api_level, -1) << "Could not find board API level.";
...
TEST_P(NfcBehaviorChanges, SetPassiveObserverTech_individualTechnologies) {
    ...
    // TODO assert that querying observe mode returns the same values.
...
TEST_P(NfcBehaviorChanges, SetPassiveObserverTech_allOnAndOff) {
    ...
    tNFC_STATUS status = nfaSetPassiveObserverTech(0xF);
    ASSERT_EQ(status, NFA_STATUS_OK);

    status = nfaSetPassiveObserverTech(0x0);
    ASSERT_EQ(status, NFA_STATUS_OK);
}
