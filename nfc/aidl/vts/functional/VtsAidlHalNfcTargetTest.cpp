 ...
 * controlGranted() should fail immediately
 */
TEST_P(NfcAidl, ControlGrantedAfterClose) {
    std::promise<void> open_cb_promise;
    std::promise<void> close_cb_promise;
    auto open_cb_future = open_cb_promise.get_future();
    auto close_cb_future = close_cb_promise.get_future();
    ...
    // controlGranted should fail
    LOG(INFO) << "controlGranted";
    EXPECT_TRUE(!(infc_->controlGranted().isOk()));
}
