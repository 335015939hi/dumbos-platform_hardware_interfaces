...
using ::aidl::android::hardware::nfc::NfcData;
...
struct Nfc : public BnNfc {
  public:
    ...
    ::ndk::ScopedAStatus write(const NfcData& aidl_data, int32_t* _aidl_return) override;
    ...
    static void dataCallback(uint16_t data_len, uint8_t* p_data) {
        NfcData aidl_data;
        aidl_data.data.assign(p_data, p_data + data_len);
        if (mCallback != nullptr) {
            auto ret = mCallback->sendData(aidl_data);
            if (!ret.isOk()) {
