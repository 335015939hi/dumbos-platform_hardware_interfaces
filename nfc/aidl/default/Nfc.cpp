...
::ndk::ScopedAStatus Nfc::write(const NfcData& aidl_data, int32_t* _aidl_return) {
    LOG(INFO) << "write";
    if (Nfc::mCallback == nullptr) {
        LOG(ERROR) << __func__ << "mCallback null";
        return ndk::ScopedAStatus::fromServiceSpecificError(
    ...
    std::vector<uint8_t> copy = aidl_data.data;
    *_aidl_return = Vendor_hal_write(copy.size(), &copy[0]);
    return ndk::ScopedAStatus::ok();
}
