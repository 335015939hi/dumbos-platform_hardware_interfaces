...
using ::android::hardware::nfc::V1_2::ExtendedNfcConfig;
...
TEST_F(NfcHidlTest, GetExtendedConfig) {
    nfc_->getConfig_1_2([](ExtendedNfcConfig config) {
        if (config.offHostRouteUicc != 0) {
            EXPECT_GE(config.offHostRouteUicc, MIN_OFFHOST_ROUTE_ID);
            EXPECT_LE(config.offHostRouteUicc, MAX_OFFHOST_ROUTE_ID);
        }
        if (config.offHostRouteEse != 0) {
            EXPECT_GE(config.offHostRouteEse, MIN_OFFHOST_ROUTE_ID);
            EXPECT_LE(config.offHostRouteEse, MAX_OFFHOST_ROUTE_ID);
        }
