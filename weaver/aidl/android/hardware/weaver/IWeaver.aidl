...
interface IWeaver {
     ...
     * @return read status is OK if the value was successfully read, INCORRECT_KEY if
     *         the key does not match the key in the slot, THROTTLE if
     *         throttling is active or FAILED if the read was unsuccessful for
     *         another reason.
     *         the value read and the timeout to wait before making the next request.
     *         If the status is OK, value is set to the value in the slot and timeout is 0.
     *         Otherwise, value is empty and timeout is set accordingly.
     */
     ...
     * @return is OK if the write was successfully completed.
     */
