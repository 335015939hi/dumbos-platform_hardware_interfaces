...
#include <aidl/android/hardware/confirmationui/MessageSize.h>
#include <aidl/android/hardware/confirmationui/TestKeyBits.h>
...
template <typename... Data> vector<uint8_t> testHMAC(const Data&... data) {
...
template <typename T> auto toBytes(const T& v) -> const uint8_t (&)[sizeof(T)] {
...
TEST_P(ConfirmationUIAidlTest, UserOkTest) {
    ...
    string prompt_text(test_prompt);
    ...
    ASSERT_TRUE(
        confirmator_->deliverSecureInputEvent(makeTestToken(TestModeCommands::OK_EVENT)).isOk());
...
TEST_P(ConfirmationUIAidlTest, MalformedUTF8Test1) {
    ...
    string prompt_text(test_prompt);
    vector<uint8_t> extra(test_extra, test_extra + 3);
    // For any Malformed UTF8 strings: the binder transaction fails with EINVAL (-22)
    // which is mapped to STATUS_BAD_VALUE
    auto result = confirmator_->promptUserConfirmation(conf_cb, prompt_text, extra, "en", {});
    ASSERT_EQ(STATUS_BAD_VALUE, getReturnCode(result));
}
