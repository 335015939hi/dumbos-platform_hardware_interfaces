    ...
    void Initialize(std::string allocatorService) {
        ...
        if (mIAllocatorVersion >= 2) {
            ...
            if (status.isOk()) {
                ...
                // getIMapperLibrarySuffix is allowed to return UNKNOWN_TRANSACTION to signal
                // that IMapper5 isn't available. Other errors would represent actual failures.
                ASSERT_EQ(STATUS_UNKNOWN_TRANSACTION, status.getStatus())
                        << "getIMapperLibrarySuffix failed " << status.getMessage();
            }
        }
