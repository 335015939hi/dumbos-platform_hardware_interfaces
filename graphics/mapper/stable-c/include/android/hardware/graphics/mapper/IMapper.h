...
/**
 * Although this is a "C" interface, this class is shared with the IAllocator AIDL HAL and
 * thus it is important that it is defined in a single location. The type is marked `@FixedSize`
 * to ensure that it doesn't change sizes, but all the fields are static_assert'd to be the right
 * size in the right place.
 */

using AIMapper_BufferDescriptorInfo =
        aidl::android::hardware::graphics::allocator::BufferDescriptorInfo;

static_assert(sizeof(AIMapper_BufferDescriptorInfo) == 160);
static_assert(std::is_standard_layout<AIMapper_BufferDescriptorInfo>::value);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, name) == 0);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, width) == 128);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, height) == 132);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, layerCount) == 136);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, format) == 140);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, usage) == 144);
static_assert(offsetof(AIMapper_BufferDescriptorInfo, reservedSize) == 152);
 ...
 * All functions must not be null to be a valid implementation except for any
 * specifically marked optional, which may be nullptr to indicate no availability
 */
typedef struct AIMapperV5 {
     ...
     */
    AIMapper_Error (*_Nonnull validateBufferSize)(
            buffer_handle_t _Nonnull buffer, AIMapper_BufferDescriptorInfo* _Nonnull description,
            uint32_t stride);
     ...
     * @param outData Pointer to a buffer in which to store the result of the get() call
     * @param outDataSize How large the outData buffer is
     * @return The number of bytes written to `outData` or which would have been written
     *         if `outDataSize` was large enough.
    ...
    AIMapper_Error (*_Nonnull setMetadata)(buffer_handle_t _Nonnull buffer,
                                           AIMapper_MetadataType metadataType,
                                           void* _Nonnull metadata, size_t metadataSize);
     ...
     *         if `outDataSize` was large enough.
     *         A negative value indicates an error, which may be
     *         - `BAD_VALUE` if any of the specified BufferDescriptorInfo attributes are invalid.
     *         - `UNSUPPORTED` when any of the description attributes are unsupported or
     *            if the metadataType is unknown/unsupported. This should also be returned if the
     *            requested metadata is not defined until a buffer has been allocated.
     */
    int32_t (*_Nonnull getFromBufferDescriptorInfo)(
            AIMapper_BufferDescriptorInfo* _Nonnull description,
            const AIMapper_MetadataType* _Nonnull metadataType, void* _Nonnull outData,
            size_t outDataSize);
    ...
    AIMapper_Error (*_Nonnull listSupportedMetadataTypes)(
            AIMapper_MetadataTypeDescription* _Nullable* _Nonnull outDescriptionList,
