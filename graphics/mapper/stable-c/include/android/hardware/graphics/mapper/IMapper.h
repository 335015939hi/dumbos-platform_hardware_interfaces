...
typedef struct AIMapperV5 {
     ...
     * and setting buffer metadata as an generic buffer, vendors can use the standard
     ...
     * @param outData Pointer to a buffer in which to store the result of the get() call; if null,
     *                the computed output size or error must still be returned.
     * @param outDataSize How large the outData buffer is. If outData is null this must be 0.
    ...
    int32_t (*_Nonnull getMetadata)(buffer_handle_t _Nonnull buffer,
                                    AIMapper_MetadataType metadataType, void* _Nullable outData,
                                    size_t outDataSize);
    ...
    int32_t (*_Nonnull getStandardMetadata)(buffer_handle_t _Nonnull buffer,
                                            int64_t standardMetadataType, void* _Nullable outData,
                                            size_t outDataSize);
