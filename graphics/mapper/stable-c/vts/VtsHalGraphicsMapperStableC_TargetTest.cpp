...
class GraphicsTestsBase {
  ...
  public:
    ...
    AIMapper_Error setStandardMetadata(buffer_handle_t bufferHandle,
        ...
        using Value = typename StandardMetadata<T>::value;
        int32_t sizeRequred = Value::encode(value, nullptr, 0);
        if (sizeRequred < 0) {
            EXPECT_GE(sizeRequred, 0) << "Failed to calculate required size";
            return static_cast<AIMapper_Error>(-sizeRequred);
        }
        std::vector<uint8_t> buffer;
        buffer.resize(sizeRequred);
        sizeRequred = Value::encode(value, buffer.data(), buffer.size());
        if (sizeRequred < 0 || sizeRequred > buffer.size()) {
            ADD_FAILURE() << "Failed to encode with calculated size " << sizeRequred
                          << "; buffer size" << buffer.size();
            return static_cast<AIMapper_Error>(-sizeRequred);
        }
        return mapper()->v5.setStandardMetadata(bufferHandle, static_cast<int64_t>(T),
                                                buffer.data(), sizeRequred);
    }
