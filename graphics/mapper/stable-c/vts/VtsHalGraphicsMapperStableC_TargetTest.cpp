...
class GraphicsTestsBase {
  ...
  public:
    ...
    std::unique_ptr<BufferAllocation> allocateMultiView(const BufferDescriptorInfo* descriptors,
                                                        size_t count, int baseViewIndex) {
        ...
        if (mIAllocatorVersion < 3) {
            return allocate(descriptors[0]);
        }

        // Convert the C-array to the container required by the AIDL call.
        std::vector<BufferDescriptorInfo> vec(descriptors, descriptors + count);

        AllocationResult result;
        ::ndk::ScopedAStatus status =
                mAllocator->allocateMultiView(vec, static_cast<int32_t>(baseViewIndex), &result);
        if (!status.isOk()) {
            status_t error = status.getExceptionCode();
            if (error == EX_SERVICE_SPECIFIC) {
                error = status.getServiceSpecificError();
...
TEST_P(GraphicsMapperStableCTests, GetBaseView) {
    ...
    BufferDescriptorInfo descs[] = {desc};
    auto buffer = allocateMultiView(descs, 1, 0);
