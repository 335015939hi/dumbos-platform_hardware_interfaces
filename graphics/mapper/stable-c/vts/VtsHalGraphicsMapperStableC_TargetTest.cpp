...
class GraphicsTestsBase {
  ...
  public:
    ...
    YCbCr getAndroidYCbCr_10bit(const native_handle_t *bufferHandle,
                                uint8_t *data) {
      YCbCr yCbCr_10bit;
      auto decodeResult =
          getStandardMetadata<StandardMetadataType::PLANE_LAYOUTS>(
              bufferHandle);
      ...
      for (const auto &planeLayout : planeLayouts) {
        for (const auto &planeLayoutComponent : planeLayout.components) {
          if (!gralloc4::isStandardPlaneLayoutComponentType(
                  planeLayoutComponent.type)) {
            continue;
          }
          ...
          auto type = static_cast<PlaneLayoutComponentType>(
              planeLayoutComponent.type.value);
          switch (type) {
          case PlaneLayoutComponentType::Y:
            // For specs refer:
          ...
          case PlaneLayoutComponentType::CB:
          case PlaneLayoutComponentType::CR:
            sampleIncrementInBytes =
                bitsToBytes(planeLayout.sampleIncrementInBits);
            EXPECT_EQ(4, sampleIncrementInBytes);

            if (yCbCr_10bit.yCbCr.cstride == 0 &&
                yCbCr_10bit.yCbCr.chroma_step == 0) {
            ...
            if (yCbCr_10bit.horizontalSubSampling == 0 &&
              ...
              yCbCr_10bit.horizontalSubSampling =
