...
class GraphicsTestsBase {
  ...
  protected:
    void Initialize(std::shared_ptr<IAllocator> allocator) {
        ...
        mIMapperHALVersion = (int32_t*)dlsym(so, "ANDROID_HAL_STABLEC_VERSION");
    }
...
TEST_P(GraphicsMapperStableCTests, VersionChecks) {
    ASSERT_NE(nullptr, getHalVersion()) << "Resolving ANDROID_HAL_STABLEC_VERSION symbol failed";
    int32_t halVersion = *getHalVersion();
    EXPECT_EQ(halVersion, AIMAPPER_VERSION_5) << "Unrecognized ANDROID_HAL_STABLEC_VERSION";
    EXPECT_EQ(mapper()->version, AIMAPPER_VERSION_5) << "Unrecognized AIMapper::version";
    EXPECT_EQ(halVersion, mapper()->version)
            << "AIMapper version & ANDROID_HAL_STABLEC_VERSION don't agree";
}
