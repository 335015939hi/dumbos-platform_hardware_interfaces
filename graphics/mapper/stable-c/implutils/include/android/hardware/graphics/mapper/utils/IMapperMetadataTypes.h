...
struct MetadataValue<std::string_view> {
    ...
    [[nodiscard]] static std::optional<std::string_view> decode(const void* _Nonnull metadata,
                                                                size_t metadataSize) {
        auto reader = MetadataReader{metadata, metadataSize};
        auto result = reader.readString();
        return reader.ok() ? std::optional<std::string_view>{result} : std::nullopt;
    }
};
...
struct MetadataValue<std::vector<uint8_t>> {
    ...
    using DecodeResult = std::optional<std::vector<uint8_t>>;
    [[nodiscard]] static DecodeResult decode(const void* _Nonnull metadata, size_t metadataSize) {
        MetadataReader reader{metadata, metadataSize};
