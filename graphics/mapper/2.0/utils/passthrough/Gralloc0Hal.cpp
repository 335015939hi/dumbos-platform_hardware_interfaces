...
uint64_t Gralloc0Hal::getValidBufferUsageMask() const {
...
Error Gralloc0Hal::createDescriptor(const IMapper::BufferDescriptorInfo& descriptorInfo,
...
Error Gralloc0Hal::importBuffer(const native_handle_t* rawHandle,
...
Error Gralloc0Hal::freeBuffer(native_handle_t* bufferHandle) {
...
void Gralloc0Hal::waitFenceFd(const base::unique_fd& fenceFd, const char* logname) {
    if (fenceFd < 0) {
        return;
    }
...
Error Gralloc0Hal::lockBuffer(const native_handle_t* bufferHandle, uint64_t cpuUsage,
    ...
    if (mMinor >= 3 && mModule->lockAsync) {
        result =
            mModule->lockAsync(mModule, bufferHandle, cpuUsage, accessRegion.left, accessRegion.top,
                               accessRegion.width, accessRegion.height, &data, fenceFd.release());
    } else {
...
Error Gralloc0Hal::unlockBuffer(const native_handle_t* bufferHandle, base::unique_fd* outFenceFd) {
    int result;
    int fenceFd = -1;
