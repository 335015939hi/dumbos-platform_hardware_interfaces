...
// MapperImpl implements IMapper on top of MapperHal
template <typename IMAPPER, typename MAPPER_HAL>
class MapperImpl : public IMAPPER {
   public:
    bool init(std::unique_ptr<MAPPER_HAL> hal) {
        mHal = std::move(hal);
        return true;
    }
    ...
    Return<void> lock(void* buffer, uint64_t cpuUsage, const V2_0::IMapper::Rect& accessRegion,
        ...
        void* data = nullptr;
        error = mHal->lockBuffer(bufferHandle, cpuUsage, accessRegion, std::move(fenceFd), &data);
        hidl_cb(error, data);
        return Void();
    }
    ...
    Return<void> lockYCbCr(void* buffer, uint64_t cpuUsage, const V2_0::IMapper::Rect& accessRegion,
        ...
        YCbCrLayout layout{};
        error = mHal->lockBuffer(bufferHandle, cpuUsage, accessRegion, std::move(fenceFd), &layout);
    ...
    Return<void> unlock(void* buffer, IMapper::unlock_cb hidl_cb) override {
        ...
        base::unique_fd fenceFd;
        Error error = mHal->unlockBuffer(bufferHandle, &fenceFd);
        if (error != Error::NONE) {
            hidl_cb(error, nullptr);
            return Void();
        }
   ...
   protected:
    ...
    std::unique_ptr<MAPPER_HAL> mHal;
};
