...
class MapperHal {
   public:
    ...
    // lock a buffer
    virtual Error lockBuffer(const native_handle_t* bufferHandle, uint64_t cpuUsage,
    ...
    // unlock a buffer
    virtual Error unlockBuffer(const native_handle_t* bufferHandle,
                               base::unique_fd* outFenceFd) = 0;
};
