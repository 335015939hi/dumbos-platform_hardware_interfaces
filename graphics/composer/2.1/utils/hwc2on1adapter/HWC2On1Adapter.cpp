...
class HWC2On1Adapter::Callbacks : public hwc_procs_t {
   public:
    ...
    static void vsyncHook(const hwc_procs_t* procs, int display, int64_t timestamp) {
        auto callbacks = static_cast<const Callbacks*>(procs);
...
void HWC2On1Adapter::doGetCapabilities(uint32_t* outCount, int32_t* outCapabilities) {
...
hwc2_function_pointer_t HWC2On1Adapter::doGetFunction(FunctionDescriptor descriptor) {
    switch (descriptor) {
        // Device functions
        ...
        case FunctionDescriptor::AcceptDisplayChanges:
            return asFP<HWC2_PFN_ACCEPT_DISPLAY_CHANGES>(
                displayHook<decltype(&Display::acceptChanges), &Display::acceptChanges>);
        case FunctionDescriptor::CreateLayer:
            return asFP<HWC2_PFN_CREATE_LAYER>(
                displayHook<decltype(&Display::createLayer), &Display::createLayer, hwc2_layer_t*>);
        case FunctionDescriptor::DestroyLayer:
            return asFP<HWC2_PFN_DESTROY_LAYER>(displayHook<decltype(&Display::destroyLayer),
                                                            &Display::destroyLayer, hwc2_layer_t>);
        case FunctionDescriptor::GetActiveConfig:
            return asFP<HWC2_PFN_GET_ACTIVE_CONFIG>(
                displayHook<decltype(&Display::getActiveConfig), &Display::getActiveConfig,
                            hwc2_config_t*>);
        case FunctionDescriptor::GetChangedCompositionTypes:
            return asFP<HWC2_PFN_GET_CHANGED_COMPOSITION_TYPES>(
                displayHook<decltype(&Display::getChangedCompositionTypes),
                            &Display::getChangedCompositionTypes, uint32_t*, hwc2_layer_t*,
                            int32_t*>);
        case FunctionDescriptor::GetColorModes:
            return asFP<HWC2_PFN_GET_COLOR_MODES>(
                displayHook<decltype(&Display::getColorModes), &Display::getColorModes, uint32_t*,
                            int32_t*>);
        ...
        case FunctionDescriptor::SetOutputBuffer:
            return asFP<HWC2_PFN_SET_OUTPUT_BUFFER>(
                displayHook<decltype(&Display::setOutputBuffer), &Display::setOutputBuffer,
        ...
        case FunctionDescriptor::ValidateDisplay:
            return asFP<HWC2_PFN_VALIDATE_DISPLAY>(
                displayHook<decltype(&Display::validate), &Display::validate, uint32_t*,
                            uint32_t*>);
        case FunctionDescriptor::GetClientTargetSupport:
            return asFP<HWC2_PFN_GET_CLIENT_TARGET_SUPPORT>(
                displayHook<decltype(&Display::getClientTargetSupport),
                            &Display::getClientTargetSupport, uint32_t, uint32_t, int32_t,
                            int32_t>);

        // Layer functions
        case FunctionDescriptor::SetCursorPosition:
            return asFP<HWC2_PFN_SET_CURSOR_POSITION>(
                layerHook<decltype(&Layer::setCursorPosition), &Layer::setCursorPosition, int32_t,
                          int32_t>);
        case FunctionDescriptor::SetLayerBuffer:
            return asFP<HWC2_PFN_SET_LAYER_BUFFER>(
                layerHook<decltype(&Layer::setBuffer), &Layer::setBuffer, buffer_handle_t,
                          int32_t>);
        case FunctionDescriptor::SetLayerSurfaceDamage:
            return asFP<HWC2_PFN_SET_LAYER_SURFACE_DAMAGE>(
                layerHook<decltype(&Layer::setSurfaceDamage), &Layer::setSurfaceDamage,
                          hwc_region_t>);
        ...
        case FunctionDescriptor::SetLayerDisplayFrame:
            return asFP<HWC2_PFN_SET_LAYER_DISPLAY_FRAME>(
                layerHook<decltype(&Layer::setDisplayFrame), &Layer::setDisplayFrame, hwc_rect_t>);
        case FunctionDescriptor::SetLayerPlaneAlpha:
            return asFP<HWC2_PFN_SET_LAYER_PLANE_ALPHA>(
                layerHook<decltype(&Layer::setPlaneAlpha), &Layer::setPlaneAlpha, float>);
        case FunctionDescriptor::SetLayerSidebandStream:
            return asFP<HWC2_PFN_SET_LAYER_SIDEBAND_STREAM>(
                layerHook<decltype(&Layer::setSidebandStream), &Layer::setSidebandStream,
                          const native_handle_t*>);
        case FunctionDescriptor::SetLayerSourceCrop:
            return asFP<HWC2_PFN_SET_LAYER_SOURCE_CROP>(
                layerHook<decltype(&Layer::setSourceCrop), &Layer::setSourceCrop, hwc_frect_t>);
        ...
        case FunctionDescriptor::SetLayerVisibleRegion:
            return asFP<HWC2_PFN_SET_LAYER_VISIBLE_REGION>(
                layerHook<decltype(&Layer::setVisibleRegion), &Layer::setVisibleRegion,
                          hwc_region_t>);
        ...
        default:
            ALOGE("doGetFunction: Unknown function descriptor: %d (%s)",
                  static_cast<int32_t>(descriptor), to_string(descriptor).c_str());
            return nullptr;
    }
}
...
Error HWC2On1Adapter::createVirtualDisplay(uint32_t width, uint32_t height,
    ...
    mHwc1VirtualDisplay =
        std::make_shared<HWC2On1Adapter::Display>(*this, HWC2::DisplayType::Virtual);
...
void HWC2On1Adapter::dump(uint32_t* outSize, char* outBuffer) {
    ...
    std::unique_lock<std::recursive_timed_mutex> lock(mStateMutex, std::defer_lock);
    ...
    if (mHwc1Device->dump) {
        ...
        mHwc1Device->dump(mHwc1Device, hwc1Dump.data(), static_cast<int>(hwc1Dump.size() - 1));
...
Error HWC2On1Adapter::registerCallback(Callback descriptor, hwc2_callback_data_t callbackData,
    ...
    ALOGV("registerCallback(%s, %p, %p)", to_string(descriptor).c_str(), callbackData, pointer);
    ...
    if (descriptor == Callback::Refresh) {
        ...
        for (auto pending : mPendingHotplugs) {
            auto hwc1DisplayId = pending.first;
            if (mHwc1DisplayMap.count(hwc1DisplayId) == 0) {
                ALOGE("hwc1Hotplug: Couldn't find display for HWC1 id %d", hwc1DisplayId);
                continue;
            }
            auto displayId = mHwc1DisplayMap[hwc1DisplayId];
...
Error HWC2On1Adapter::Display::acceptChanges() {
    ...
    for (auto& change : mChanges->getTypeChanges()) {
        ...
        if (mDevice.mLayers.count(layerId) == 0) {
            ...
            ALOGW("Cannot accept change for unknown layer (%" PRIu64 ")", layerId);
            continue;
        }
...
Error HWC2On1Adapter::Display::destroyLayer(hwc2_layer_t layerId) {
    ...
    if (mapLayer == mDevice.mLayers.end()) {
        ALOGV("[%" PRIu64 "] destroyLayer(%" PRIu64 ") failed: no such layer", mId, layerId);
...
Error HWC2On1Adapter::Display::getActiveConfig(hwc2_config_t* outConfig) {
    ...
    if (!mActiveConfig) {
        ALOGV("[%" PRIu64 "] getActiveConfig --> %s", mId, to_string(Error::BadConfig).c_str());
...
Error HWC2On1Adapter::Display::getAttribute(hwc2_config_t configId, Attribute attribute,
                                            int32_t* outValue) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);

    if (configId > mConfigs.size() || !mConfigs[configId]->isOnDisplay(*this)) {
        ALOGV("[%" PRIu64 "] getAttribute failed: bad config (%u)", mId, configId);
        return Error::BadConfig;
    }
    *outValue = mConfigs[configId]->getAttribute(attribute);
    ALOGV("[%" PRIu64 "] getAttribute(%u, %s) --> %d", mId, configId, to_string(attribute).c_str(),
          *outValue);
    return Error::None;
}

Error HWC2On1Adapter::Display::getChangedCompositionTypes(uint32_t* outNumElements,
                                                          hwc2_layer_t* outLayers,
                                                          int32_t* outTypes) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);

    if (!mChanges) {
        ALOGE("[%" PRIu64 "] getChangedCompositionTypes failed: not validated", mId);
        return Error::NotValidated;
    }
    ...
    for (const auto& element : mChanges->getTypeChanges()) {
        ...
        ALOGV("Adding %" PRIu64 " %s", layerId, to_string(element.second).c_str());
...
Error HWC2On1Adapter::Display::getColorModes(uint32_t* outNumModes, int32_t* outModes) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
    ...
    uint32_t numModes = std::min(*outNumModes, static_cast<uint32_t>(mColorModes.size()));
    std::copy_n(mColorModes.cbegin(), numModes, outModes);
    *outNumModes = numModes;
    return Error::None;
}

Error HWC2On1Adapter::Display::getConfigs(uint32_t* outNumConfigs, hwc2_config_t* outConfigs) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
...
Error HWC2On1Adapter::Display::getHdrCapabilities(uint32_t* outNumTypes, int32_t* /*outTypes*/,
...
Error HWC2On1Adapter::Display::getReleaseFences(uint32_t* outNumElements, hwc2_layer_t* outLayers,
                                                int32_t* outFences) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
...
Error HWC2On1Adapter::Display::getRequests(int32_t* outDisplayRequests, uint32_t* outNumElements,
...
Error HWC2On1Adapter::Display::present(int32_t* outRetireFence) {
    ...
    *outRetireFence = mRetireFence.get()->dup();
    ALOGV("[%" PRIu64 "] present returning retire fence %d", mId, *outRetireFence);
...
Error HWC2On1Adapter::Display::setActiveConfig(hwc2_config_t configId) {
    ...
    if (mDevice.mHwc1MinorVersion >= 4) {
        ...
        int intError = mDevice.mHwc1Device->setActiveConfig(mDevice.mHwc1Device, mHwc1Id,
                                                            static_cast<int>(hwc1Id));
        if (intError != 0) {
            ALOGE("setActiveConfig: Failed to set active config on HWC1 (%d)", intError);
            return Error::BadConfig;
        }
...
Error HWC2On1Adapter::Display::setClientTarget(buffer_handle_t target, int32_t acquireFence,
...
Error HWC2On1Adapter::Display::setColorMode(android_color_mode_t mode) {
    ...
    int intError = mDevice.mHwc1Device->setActiveConfig(mDevice.mHwc1Device, mHwc1Id, hwc1Config);
    if (intError != 0) {
...
Error HWC2On1Adapter::Display::setColorTransform(android_color_transform_t hint) {
    ...
    ALOGV("%" PRIu64 "] setColorTransform(%d)", mId, static_cast<int32_t>(hint));
    mHasColorTransform = (hint != HAL_COLOR_TRANSFORM_IDENTITY);
    return Error::None;
}

Error HWC2On1Adapter::Display::setOutputBuffer(buffer_handle_t buffer, int32_t releaseFence) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
...
static int getHwc1PowerMode(PowerMode mode) {
    switch (mode) {
        ...
        case PowerMode::DozeSuspend:
            return HWC_POWER_MODE_DOZE_SUSPEND;
        case PowerMode::Doze:
            return HWC_POWER_MODE_DOZE;
        case PowerMode::On:
...
Error HWC2On1Adapter::Display::setPowerMode(PowerMode mode) {
    ...
    int error = 0;
    if (mDevice.mHwc1MinorVersion < 4) {
        error = mDevice.mHwc1Device->blank(mDevice.mHwc1Device, mHwc1Id, mode == PowerMode::Off);
    } else {
        error =
            mDevice.mHwc1Device->setPowerMode(mDevice.mHwc1Device, mHwc1Id, getHwc1PowerMode(mode));
    }
    ALOGE_IF(error != 0, "setPowerMode: Failed to set power mode on HWC1 (%d)", error);
...
Error HWC2On1Adapter::Display::setVsyncEnabled(Vsync enable) {
    ...
    int error = mDevice.mHwc1Device->eventControl(mDevice.mHwc1Device, mHwc1Id, HWC_EVENT_VSYNC,
                                                  enable == Vsync::Enable);
    ALOGE_IF(error != 0, "setVsyncEnabled: Failed to set vsync on HWC1 (%d)", error);

    mVsyncEnabled = enable;
    return Error::None;
}

Error HWC2On1Adapter::Display::validate(uint32_t* outNumTypes, uint32_t* outNumRequests) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
    ...
    *outNumRequests = mChanges->getNumLayerRequests();
    ALOGV("[%" PRIu64 "] validate --> %u types, %u requests", mId, *outNumTypes, *outNumRequests);
    for (auto request : mChanges->getTypeChanges()) {
        ALOGV("Layer %" PRIu64 " --> %s", request.first, to_string(request.second).c_str());
    }
...
Error HWC2On1Adapter::Display::updateLayerZ(hwc2_layer_t layerId, uint32_t z) {
    ...
    if (!layerOnDisplay) {
        ALOGE("[%" PRIu64 "] updateLayerZ failed to find layer on display", mId);
        return Error::BadLayer;
    }
...
static constexpr bool attributesMatch() {
    bool match = (attribute == ATTRIBUTES_WITH_COLOR[ATTRIBUTE_MAP_WITH_COLOR[attribute]]);
    if (attribute == HWC_DISPLAY_COLOR_TRANSFORM) {
        return match;
    }

    return match && (attribute == ATTRIBUTES_WITHOUT_COLOR[ATTRIBUTE_MAP_WITHOUT_COLOR[attribute]]);
}
...
static_assert(attributesMatch<HWC_DISPLAY_COLOR_TRANSFORM>(), "Tables out of sync");
...
void HWC2On1Adapter::Display::populateConfigs() {
    ...
    mDevice.mHwc1Device->getDisplayConfigs(mDevice.mHwc1Device, mHwc1Id, configs, &numConfigs);
    ...
    for (size_t c = 0; c < numConfigs; ++c) {
        ...
        auto result = mDevice.mHwc1Device->getDisplayAttributes(
            mDevice.mHwc1Device, mHwc1Id, hwc1ConfigId, ATTRIBUTES_WITH_COLOR, values);
        if (result != 0) {
            mDevice.mHwc1Device->getDisplayAttributes(mDevice.mHwc1Device, mHwc1Id, hwc1ConfigId,
                                                      ATTRIBUTES_WITHOUT_COLOR, values);
            hasColor = false;
        }

        auto attributeMap = hasColor ? ATTRIBUTE_MAP_WITH_COLOR : ATTRIBUTE_MAP_WITHOUT_COLOR;

        newConfig->setAttribute(Attribute::VsyncPeriod,
                                values[attributeMap[HWC_DISPLAY_VSYNC_PERIOD]]);
        newConfig->setAttribute(Attribute::Width, values[attributeMap[HWC_DISPLAY_WIDTH]]);
        newConfig->setAttribute(Attribute::Height, values[attributeMap[HWC_DISPLAY_HEIGHT]]);
        newConfig->setAttribute(Attribute::DpiX, values[attributeMap[HWC_DISPLAY_DPI_X]]);
        ...
        if (hasColor) {
            ...
            // both HWC1 and 2.
            newConfig->setAttribute(ColorMode, values[attributeMap[HWC_DISPLAY_COLOR_TRANSFORM]]);
        }
        ...
        for (auto& existingConfig : mConfigs) {
            if (existingConfig->merge(*newConfig)) {
                ALOGV("Merged config %d with existing config %u: %s", hwc1ConfigId,
                      existingConfig->getId(), existingConfig->toString().c_str());
                newConfig.reset();
                break;
            }
        }
        ...
        if (newConfig) {
            ...
            ALOGV("Found new config %u: %s", newConfig->getId(), newConfig->toString().c_str());
...
Error HWC2On1Adapter::Display::set(hwc_display_contents_1& hwcContents) {
    ...
    if (clientTargetLayer.compositionType == HWC_FRAMEBUFFER_TARGET) {
        ...
        ALOGE("[%" PRIu64 "] set: last HWC layer wasn't FRAMEBUFFER_TARGET", mId);
    }
...
void HWC2On1Adapter::Display::addReleaseFences(const hwc_display_contents_1_t& hwcContents) {
    std::unique_lock<std::recursive_mutex> lock(mStateMutex);
    ...
    for (size_t hwc1Id = 0; hwc1Id < numLayers; ++hwc1Id) {
        ...
        if (mHwc1LayerMap.count(hwc1Id) == 0) {
            if (receivedLayer.compositionType != HWC_FRAMEBUFFER_TARGET) {
                ALOGE(
                    "addReleaseFences: HWC1 layer %zd doesn't have a"
        ...
        Layer& layer = *mHwc1LayerMap[hwc1Id];
        ALOGV("Adding release fence %d to layer %" PRIu64, receivedLayer.releaseFenceFd,
              layer.getId());
...
static std::string approximateFloatString(float f) {
    ...
    auto bytesWritten = snprintf(buffer, BUFFER_SIZE, "%s%.1f", approximate ? "~" : "", f);
...
static std::string to_string(const hwc_layer_1_t& hwcLayer, int32_t hwc1MinorVersion) {
    const char* fill = "          ";

    std::stringstream output;

    output << "  Composition: " << hwc1CompositionString(hwcLayer.compositionType);
    ...
    if (hwcLayer.compositionType == HWC_BACKGROUND) {
        ...
        output << "  Buffer: " << hwcLayer.handle << "/" << hwcLayer.acquireFenceFd << '\n';
    }
...
std::string HWC2On1Adapter::Display::dump() const {
    ...
    output << "    " << mConfigs.size() << " Config" << (mConfigs.size() == 1 ? "" : "s")
           << " (* active)\n";
    for (const auto& config : mConfigs) {
...
void HWC2On1Adapter::Display::Config::setAttribute(HWC2::Attribute attribute, int32_t value) {
...
Error HWC2On1Adapter::Display::Config::getColorModeForHwc1Id(uint32_t id,
...
bool HWC2On1Adapter::Display::Config::merge(const Config& other) {
    auto attributes = {HWC2::Attribute::Width, HWC2::Attribute::Height,
                       HWC2::Attribute::VsyncPeriod, HWC2::Attribute::DpiX, HWC2::Attribute::DpiY};
    ...
    if (mHwc1Ids.count(otherColorMode) != 0) {
        ALOGE(
            "Attempted to merge two configs (%u and %u) which appear to be "
            "identical",
            mHwc1Ids.at(otherColorMode), other.mHwc1Ids.at(otherColorMode));
        return false;
    }
...
std::string HWC2On1Adapter::Display::Config::toString(bool splitLine) const {
    ...
    char buffer[BUFFER_SIZE] = {};
    auto writtenBytes =
        snprintf(buffer, BUFFER_SIZE, "%u x %u", mAttributes.at(HWC2::Attribute::Width),
    ...
    if (mAttributes.count(HWC2::Attribute::DpiX) != 0 &&
        ...
        std::memset(buffer, 0, BUFFER_SIZE);
        writtenBytes = snprintf(buffer, BUFFER_SIZE, ", DPI: %.1f x %.1f",
    ...
    if (splitLine) {
        ...
        writtenBytes = snprintf(buffer, BUFFER_SIZE, ", HWC1 ID/Color transform:");
    }
    output.append(buffer, writtenBytes);
    ...
    for (const auto& id : mHwc1Ids) {
        ...
        if (colorMode == mDisplay.mActiveColorMode) {
            writtenBytes = snprintf(buffer, BUFFER_SIZE, " [%u/%d]", hwc1Id, colorMode);
        } else {
            writtenBytes = snprintf(buffer, BUFFER_SIZE, " %u/%d", hwc1Id, colorMode);
        }
        output.append(buffer, writtenBytes);
...
std::shared_ptr<const HWC2On1Adapter::Display::Config> HWC2On1Adapter::Display::getConfig(
    hwc2_config_t configId) const {
    if (configId > mConfigs.size() || !mConfigs[configId]->isOnDisplay(*this)) {
        return nullptr;
    }
...
void HWC2On1Adapter::Display::populateColorModes() {
    ...
    for (const auto& config : mConfigs) {
        ...
        std::set_intersection(mColorModes.cbegin(), mColorModes.cend(), configModes.cbegin(),
                              configModes.cend(),
...
void HWC2On1Adapter::Display::initializeActiveConfig() {
    ...
    auto activeConfig = mDevice.mHwc1Device->getActiveConfig(mDevice.mHwc1Device, mHwc1Id);
    ...
    if (!mActiveConfig) {
        ALOGV(
            "Unable to find active HWC1 config %u, defaulting to "
            "config 0",
            activeConfig);
        mActiveConfig = mConfigs[0];
        mActiveColorMode = HAL_COLOR_MODE_NATIVE;
...
void HWC2On1Adapter::Display::allocateRequestedContents() {
    ...
    size_t size = sizeof(hwc_display_contents_1_t) + sizeof(hwc_layer_1_t) * numLayers +
...
void HWC2On1Adapter::Display::updateLayerRequests(const hwc_layer_1_t& hwc1Layer,
                                                  const Layer& layer) {
    if ((hwc1Layer.hints & HWC_HINT_CLEAR_FB) != 0) {
        mChanges->addLayerRequest(layer.getId(), LayerRequest::ClearClientTarget);
    }
}
...
Error HWC2On1Adapter::Layer::setBuffer(buffer_handle_t buffer, int32_t acquireFence) {
...
Error HWC2On1Adapter::Layer::setVisibleRegion(hwc_region_t visible) {
    if ((getNumVisibleRegions() != visible.numRects) ||
        !std::equal(mVisibleRegion.begin(), mVisibleRegion.end(), visible.rects, compareRects)) {
...
void HWC2On1Adapter::Layer::applyState(hwc_layer_1_t& hwc1Layer) {
    ...
    switch (mCompositionType) {
        case Composition::SolidColor:
            applySolidColorState(hwc1Layer);
            break;
        case Composition::Sideband:
            applySidebandState(hwc1Layer);
            break;
        default:
...
std::string HWC2On1Adapter::Layer::dump() const {
    ...
    if (mCompositionType == HWC2::Composition::SolidColor) {
        ...
        output << "  Buffer: " << mBuffer.getBuffer() << "/" << mBuffer.getFence() << '\n';
        ...
        if (mPlaneAlpha != 1.0f) {
            output << "  Alpha: " << alphaString(mPlaneAlpha) << '\n';
        } else {
            output << '\n';
        }
...
void HWC2On1Adapter::Layer::applyCommonState(hwc_layer_1_t& hwc1Layer) {
    ...
    if (minorVersion < 2) {
        ...
        hwc1Layer.planeAlpha = static_cast<uint8_t>(255.0f * pendingAlpha + 0.5f);
    }

    if (minorVersion < 3) {
        auto pending = mSourceCrop;
        hwc1Layer.sourceCropi.left = static_cast<int32_t>(std::ceil(pending.left));
        hwc1Layer.sourceCropi.top = static_cast<int32_t>(std::ceil(pending.top));
        hwc1Layer.sourceCropi.right = static_cast<int32_t>(std::floor(pending.right));
        hwc1Layer.sourceCropi.bottom = static_cast<int32_t>(std::floor(pending.bottom));
    } else {
...
void HWC2On1Adapter::Layer::applyCompositionType(hwc_layer_1_t& hwc1Layer) {
    ...
    ALOGV("Layer %" PRIu64 " %s set to %d", mId, to_string(mCompositionType).c_str(),
...
void HWC2On1Adapter::populateCapabilities() {
    if (mHwc1MinorVersion >= 3U) {
        ...
        auto result = mHwc1Device->query(mHwc1Device, HWC_DISPLAY_TYPES_SUPPORTED, &supportedTypes);
    ...
    if (mHwc1MinorVersion >= 1U) {
        ...
        auto result = mHwc1Device->query(mHwc1Device, HWC_BACKGROUND_LAYER_SUPPORTED,
...
std::tuple<HWC2On1Adapter::Layer*, Error> HWC2On1Adapter::getLayer(hwc2_display_t displayId,
...
bool HWC2On1Adapter::prepareAllDisplays() {
    ...
    ALOGV("Calling HWC1 prepare");
    {
        ...
        mHwc1Device->prepare(mHwc1Device, mHwc1Contents.size(), mHwc1Contents.data());
    }
...
void dumpHWC1Message(hwc_composer_device_1* device, size_t numDisplays,
    ...
    while (displayId < numDisplays) {
        ...
        for (size_t layerId = 0; layerId < display->numHwLayers; layerId++) {
            ...
            ALOGV("      sourceCropf        : %f, %f, %f, %f", layer.sourceCropf.left,
                  layer.sourceCropf.top, layer.sourceCropf.right, layer.sourceCropf.bottom);
            ALOGV("      displayFrame       : %d, %d, %d, %d", layer.displayFrame.left,
                  layer.displayFrame.left, layer.displayFrame.left, layer.displayFrame.left);
            hwc_region_t& visReg = layer.visibleRegionScreen;
            ALOGV("      visibleRegionScreen: #0x%08zx[@0x%p]", visReg.numRects, visReg.rects);
            for (size_t visRegId = 0; visRegId < visReg.numRects; visRegId++) {
                if (layer.visibleRegionScreen.rects == nullptr) {
                    ALOGV("        null");
                } else {
                    ALOGV("        visibleRegionScreen[%zu] %d, %d, %d, %d", visRegId,
                          visReg.rects[visRegId].left, visReg.rects[visRegId].top,
                          visReg.rects[visRegId].right, visReg.rects[visRegId].bottom);
                }
            }
            ...
            if (getMinorVersion(device) < 5) continue;
            ALOGV("      surfaceDamage      : #0x%08zx[@0x%p]", layer.surfaceDamage.numRects,
                  layer.surfaceDamage.rects);
            for (size_t sdId = 0; sdId < layer.surfaceDamage.numRects; sdId++) {
                if (layer.surfaceDamage.rects == nullptr) {
                    ALOGV("      null");
                } else {
                    ALOGV("      surfaceDamage[%zu] %d, %d, %d, %d", sdId,
                          layer.surfaceDamage.rects[sdId].left, layer.surfaceDamage.rects[sdId].top,
...
Error HWC2On1Adapter::setAllDisplays() {
    ...
    ALOGV("Calling HWC1 set");
    {
        ...
        mHwc1Device->set(mHwc1Device, mHwc1Contents.size(), mHwc1Contents.data());
    }
    ...
    for (size_t hwc1Id = 0; hwc1Id < mHwc1Contents.size(); ++hwc1Id) {
        ...
        ALOGV("setAllDisplays: Adding retire fence %d to display %zd", retireFenceFd, hwc1Id);
...
void HWC2On1Adapter::hwc1Hotplug(int hwc1DisplayId, int connected) {
    ...
    if (mHwc1DisplayMap.count(hwc1DisplayId) == 0) {
        ...
        auto display =
            std::make_shared<HWC2On1Adapter::Display>(*this, HWC2::DisplayType::Physical);
        ...
        if (connected != 0) {
            ALOGW(
                "hwc1Hotplug: Received connect for previously connected "
                "display");
            return;
        }
    ...
    auto hwc2Connected =
        (connected == 0) ? HWC2::Connection::Disconnected : HWC2::Connection::Connected;
