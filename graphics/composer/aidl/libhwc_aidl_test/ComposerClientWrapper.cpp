...
ScopedAStatus ComposerClientWrapper::setActiveConfig(DisplayWrapper* displayWrapper,
                                                     int32_t config) {
    auto status = mComposerClient->setActiveConfig(displayWrapper->getDisplayId(), config);
    if (!status.isOk()) {
        return status;
    }
    return updateDisplayProperties(displayWrapper, config);
}

ScopedAStatus ComposerClientWrapper::setPeakRefreshRateConfig(DisplayWrapper* displayWrapper) {
    const auto displayId = displayWrapper->getDisplayId();
    auto [activeStatus, activeConfig] = getActiveConfig(displayId);
    EXPECT_TRUE(activeStatus.isOk());
    auto peakDisplayConfig = displayWrapper->getDisplayConfig(activeConfig);
    auto peakConfig = activeConfig;

    const auto displayConfigs = displayWrapper->getDisplayConfigs();
    ...
    return setActiveConfig(displayWrapper, peakConfig);
}
...
ComposerClientWrapper::setActiveConfigWithConstraints(
        DisplayWrapper* displayWrapper, int32_t config,
        const VsyncPeriodChangeConstraints& constraints) {
    VsyncPeriodChangeTimeline outTimeline;
    auto status = mComposerClient->setActiveConfigWithConstraints(
            displayWrapper->getDisplayId(), config, constraints, &outTimeline);
    if (!status.isOk()) {
        return {std::move(status), outTimeline};
    }
    return {updateDisplayProperties(displayWrapper, config), outTimeline};
}
...
std::pair<ScopedAStatus, std::vector<DisplayWrapper>> ComposerClientWrapper::getDisplays() {
    while (true) {
        ...
        std::vector<DisplayWrapper> vtsDisplays;
        vtsDisplays.reserve(displays.size());
        for (int64_t display : displays) {
            auto displayWrapper = DisplayWrapper{display};
            if (getDisplayConfigurationSupported()) {
                auto [status, configs] = getDisplayConfigurations(display);
                if (!status.isOk()) {
                ...
                addDisplayConfigs(&displayWrapper, configs);
            } else {
                ...
                for (int config : configs) {
                    status = addDisplayConfigLegacy(&displayWrapper, config);
                    if (!status.isOk()) {
            ...
            auto status = updateDisplayProperties(&displayWrapper, activeConfig.second);
            if (!status.isOk()) {
            ...
            vtsDisplays.emplace_back(displayWrapper);
...
void ComposerClientWrapper::addDisplayConfigs(DisplayWrapper* displayWrapper,
                                              const std::vector<DisplayConfiguration>& configs) {
    for (const auto& config : configs) {
        displayWrapper->addDisplayConfig(
                config.configId, {config.vsyncPeriod, config.configGroup, config.vrrConfig});
    }
}

ScopedAStatus ComposerClientWrapper::addDisplayConfigLegacy(DisplayWrapper* displayWrapper,
                                                            int32_t config) {
    const auto vsyncPeriod = getDisplayAttribute(displayWrapper->getDisplayId(), config,
                                                 DisplayAttribute::VSYNC_PERIOD);
    const auto configGroup = getDisplayAttribute(displayWrapper->getDisplayId(), config,
                                                 DisplayAttribute::CONFIG_GROUP);
    if (vsyncPeriod.first.isOk() && configGroup.first.isOk()) {
        displayWrapper->addDisplayConfig(config, {vsyncPeriod.second, configGroup.second});
        return ScopedAStatus::ok();
    }
...
ScopedAStatus ComposerClientWrapper::updateDisplayProperties(DisplayWrapper* displayWrapper,
                                                             int32_t config) {
    if (getDisplayConfigurationSupported()) {
        auto [status, configs] = getDisplayConfigurations(displayWrapper->getDisplayId());
        if (status.isOk()) {
            for (const auto& displayConfig : configs) {
                if (displayConfig.configId == config) {
                    displayWrapper->setDimensions(displayConfig.width, displayConfig.height);
                    return ScopedAStatus::ok();
                }
            }
        }
        ...
        const auto width = getDisplayAttribute(displayWrapper->getDisplayId(), config,
                                               DisplayAttribute::WIDTH);
        const auto height = getDisplayAttribute(displayWrapper->getDisplayId(), config,
                                                DisplayAttribute::HEIGHT);
        if (width.first.isOk() && height.first.isOk()) {
            displayWrapper->setDimensions(width.second, height.second);
            return ScopedAStatus::ok();
        }
