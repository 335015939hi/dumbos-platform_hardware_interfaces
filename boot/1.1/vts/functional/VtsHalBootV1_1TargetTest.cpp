...
using ::android::hardware::boot::V1_0::MergeStatus;
 ...
 */
TEST_P(BootHidlTest, GetMergeStatus) {
    EXPECT_OK(boot->GetMergeStatus([](auto success, auto) { EXPECT_TRUE(success); }));
}

/**
 * Ensure merge status can be set to arbitrary value.
 */
TEST_P(BootHidlTest, SetMergeStatus) {
    for (const auto value : hidl_enum_range<MergeStatus>()) {
        EXPECT_TRUE(boot->SetMergeStatus(value).withDefault(false));
        EXPECT_OK(boot->GetMergeStatus([](auto success, auto new_value) {
            EXPECT_TRUE(success);
