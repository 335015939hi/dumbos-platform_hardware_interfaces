...
ScopedAStatus BootControl::isSlotBootable(
        int32_t in_slot, ::aidl::android::hardware::boot::BoolResult* _aidl_return) {
    if (!impl_.IsValidSlot(in_slot)) {
        *_aidl_return = BoolResult::INVALID_SLOT;
        return ScopedAStatus::ok();
    }
    *_aidl_return = impl_.IsSlotBootable(in_slot) ? BoolResult::TRUE : BoolResult::FALSE;
    return ScopedAStatus::ok();
}

ScopedAStatus BootControl::isSlotMarkedSuccessful(
        int32_t in_slot, ::aidl::android::hardware::boot::BoolResult* _aidl_return) {
    if (!impl_.IsValidSlot(in_slot)) {
        *_aidl_return = BoolResult::INVALID_SLOT;
        return ScopedAStatus::ok();
    }
    *_aidl_return = impl_.IsSlotMarkedSuccessful(in_slot) ? BoolResult::TRUE : BoolResult::FALSE;
    return ScopedAStatus::ok();
}

ScopedAStatus BootControl::markBootSuccessful(
        ::aidl::android::hardware::boot::CommandResult* _aidl_return) {
    if (impl_.MarkBootSuccessful()) {
        _aidl_return->success = true;
        _aidl_return->errMsg = "Success";
    } else {
        _aidl_return->success = false;
        _aidl_return->errMsg = "Operation failed";
    }
    return ScopedAStatus::ok();
}

ScopedAStatus BootControl::setActiveBootSlot(
        int32_t in_slot, ::aidl::android::hardware::boot::CommandResult* _aidl_return) {
    _aidl_return->success = false;
    if (!impl_.IsValidSlot(in_slot)) {
        _aidl_return->errMsg = "Invalid slot";
        return ScopedAStatus::ok();
    }
...
ScopedAStatus BootControl::setSlotAsUnbootable(
        int32_t in_slot, ::aidl::android::hardware::boot::CommandResult* _aidl_return) {
    if (!impl_.IsValidSlot(in_slot)) {
        _aidl_return->errMsg = "Invalid slot";
        return ScopedAStatus::ok();
