...
BootControlClientAidl::BootControlClientAidl(std::shared_ptr<IBootControl> module)
    : module_(module),
      boot_control_death_recipient(AIBinder_DeathRecipient_new(onBootControlServiceDied)) {
    binder_status_t status =
            AIBinder_linkToDeath(module->asBinder().get(), boot_control_death_recipient, nullptr);
    if (status != STATUS_OK) {
        LOG(ERROR) << "Could not link to binder death";
    }
}
BootControlVersion BootControlClientAidl::GetVersion() const {
    return BootControlVersion::BOOTCTL_AIDL;
}

BootControlClientAidl::~BootControlClientAidl() {
...
int32_t BootControlClientAidl::GetNumSlots() const {
    int32_t ret = -1;
    LOG_NDK_STATUS(module_->getNumberSlots(&ret));
    return ret;
}

int32_t BootControlClientAidl::GetCurrentSlot() const {
    int32_t ret = -1;
    LOG_NDK_STATUS(module_->getCurrentSlot(&ret));
    return ret;
}
MergeStatus BootControlClientAidl::getSnapshotMergeStatus() const {
    MergeStatus status = MergeStatus::UNKNOWN;
    LOG_NDK_STATUS(module_->getSnapshotMergeStatus(&status));
    return status;
}
std::string BootControlClientAidl::GetSuffix(int32_t slot) const {
    std::string ret;
    const auto status = module_->getSuffix(slot, &ret);
    if (!status.isOk()) {
        LOG(ERROR) << __FUNCTION__ << "(" << slot << ")" << " failed " << status.getDescription();
        return {};
    }
    return ret;
}

std::optional<bool> BootControlClientAidl::IsSlotBootable(int32_t slot) const {
    bool ret = false;
...
CommandResult BootControlClientAidl::MarkSlotUnbootable(int32_t slot) {
...
CommandResult BootControlClientAidl::SetActiveBootSlot(int slot) {
...
int BootControlClientAidl::GetActiveBootSlot() const {
    int ret = -1;
...
std::optional<bool> BootControlClientAidl::IsSlotMarkedSuccessful(int slot) const {
    bool ret = false;
...
CommandResult BootControlClientAidl::MarkBootSuccessful() {
...
CommandResult BootControlClientAidl::SetSnapshotMergeStatus(
        aidl::android::hardware::boot::MergeStatus merge_status) {
...
BootControlClientHIDL::BootControlClientHIDL(android::sp<V1_0::IBootControl> module_v1,
...
BootControlVersion BootControlClientHIDL::GetVersion() const {
...
int32_t BootControlClientHIDL::GetNumSlots() const {
...
int32_t BootControlClientHIDL::GetCurrentSlot() const {
...
std::string BootControlClientHIDL::GetSuffix(int32_t slot) const {
    std::string suffix;
    const auto ret = module_v1_->getSuffix(
            slot, [&](const ::android::hardware::hidl_string& slotSuffix) { suffix = slotSuffix; });
    if (!ret.isOk()) {
...
std::optional<bool> BootControlClientHIDL::IsSlotBootable(int32_t slot) const {
...
CommandResult BootControlClientHIDL::MarkSlotUnbootable(int32_t slot) {
    CommandResult result;
    const auto ret = module_v1_->setSlotAsUnbootable(slot, [&](const V1_0::CommandResult& error) {
...
CommandResult BootControlClientHIDL::SetActiveBootSlot(int32_t slot) {
    CommandResult result;
...
CommandResult BootControlClientHIDL::MarkBootSuccessful() {
    CommandResult result;
...
std::optional<bool> BootControlClientHIDL::IsSlotMarkedSuccessful(int32_t slot) const {
...
MergeStatus BootControlClientHIDL::getSnapshotMergeStatus() const {
...
CommandResult BootControlClientHIDL::SetSnapshotMergeStatus(MergeStatus merge_status) {
    if (module_v1_1_ == nullptr) {
...
int32_t BootControlClientHIDL::GetActiveBootSlot() const {
    if (module_v1_2_ == nullptr) {
