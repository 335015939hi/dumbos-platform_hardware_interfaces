...
class BootControlClientAidl final : public BootControlClient {
  ...
  public:
    BootControlClientAidl(std::shared_ptr<IBootControl> module) : module_(module) {
        boot_control_death_recipient = AIBinder_DeathRecipient_new(onBootCootControlServiceDied);
    ...
    void onBootCootControlServiceDied(void*) const { LOG(ERROR) << "Boot Control Service Died"; }
