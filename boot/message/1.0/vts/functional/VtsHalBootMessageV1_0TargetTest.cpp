...
TEST_P(BootMessageHidlTest, writeBootloaderMessage) {
    // Write the message.
    Message message = {
            "command",
            "message1\nmessage2\n",
...
TEST_P(BootMessageHidlTest, writeBootloaderMessage_size_check) {
    // Verify the HAL implementation support the minimal size requirements.
    Message message = {
...
TEST_P(BootMessageHidlTest, updateBootloaderMessageRecoveryOptionsEmpty) {
    // Write empty vector.
    std::vector<std::string> options;
    CommandResult write_status;
    boot_->updateBootloaderMessageWithRecoveryOptions(
...
TEST_P(BootMessageHidlTest, updateBootloaderMessageRecoveryOptionsLong) {
    // Write the long message up to the size requirement.
    std::vector<std::string> options;
    for (int i = 0; i < 69; i++) {
...
TEST_P(BootMessageHidlTest, writeWipePackage) {
