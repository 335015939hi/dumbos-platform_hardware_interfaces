...
vector<uint8_t> authToken2AidlVec(const HardwareAuthToken& token) {
    static_assert(1 + sizeof(token.challenge) + sizeof(token.userId) +
    ...
    *pos++ = 0;  // Version byte
    ...
    auto timestamp = htonq(static_cast<uint64_t>(token.timestamp));
...
HardwareAuthToken vector2AuthToken(const vector<uint8_t>& buffer) {
    ...
    token.timestamp = ntohq(static_cast<uint64_t>(token.timestamp>);
