...
struct choose_serializer<android::hardware::keymaster::TypedTag<tag_type, tag>, Tail...> {
    static OutStreams& serialize(OutStreams& out, const KeyParameter& param) {
        if (param.tag == tag) {
            return android::hardware::keymaster::serialize(TypedTag<tag_type, tag>(), out, param);
        } else {
...
struct choose_deserializer<TypedTag<tag_type, tag>, Tail...> {
    static InStreams& deserialize(InStreams& in, KeyParameter* param) {
        if (param->tag == tag) {
            return android::hardware::keymaster::deserialize(TypedTag<tag_type, tag>(), in, param);
        } else {
...
AuthorizationSetBuilder& AuthorizationSetBuilder::BlockMode(
        std::initializer_list<android::hardware::keymaster::BlockMode> blockModes) {
...
AuthorizationSetBuilder& AuthorizationSetBuilder::Digest(
        std::vector<android::hardware::keymaster::Digest> digests) {
