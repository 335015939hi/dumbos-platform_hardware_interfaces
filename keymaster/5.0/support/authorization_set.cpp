...
#include <aidl/android/hardware/keymaster/Algorithm.h>
#include <aidl/android/hardware/keymaster/BlockMode.h>
#include <aidl/android/hardware/keymaster/Digest.h>
#include <aidl/android/hardware/keymaster/KeyParameter.h>
#include <aidl/android/hardware/keymaster/KeyPurpose.h>
...
struct choose_serializer<::aidl::android::hardware::keymaster::TypedTag<tag_type, tag>, Tail...> {
    static OutStreams& serialize(OutStreams& out, const KeyParameter& param) {
        if (param.tag == tag) {
            return serialize(::aidl::android::hardware::keymaster::TypedTag<tag_type, tag>(), out,
                             param);
        } else {
...
struct choose_deserializer<TypedTag<tag_type, tag>, Tail...> {
    static InStreams& deserialize(InStreams& in, KeyParameter* param) {
        if (param->tag == tag) {
            return deserialize(TypedTag<tag_type, tag>(), in, param);
        } else {
...
AuthorizationSetBuilder& AuthorizationSetBuilder::BlockMode(
        std::initializer_list<::aidl::android::hardware::keymaster::BlockMode> blockModes) {
...
AuthorizationSetBuilder& AuthorizationSetBuilder::Digest(
        std::vector<::aidl::android::hardware::keymaster::Digest> digests) {
