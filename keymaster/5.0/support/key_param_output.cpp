...
// TODO(seleneh) revert this when aidl union support is added.
/*
ostream& operator<<(ostream& os, const KeyParameter& param) {
    ...
    switch (typeFromTag(param.tag)) {
        ...
        case TagType::ENUM:
            switch (param.tag) {
                case Tag::ALGORITHM:
                    return os << param.algorithm;
                case Tag::BLOCK_MODE:
                    return os << param.blockMode;
                case Tag::PADDING:
                    return os << param.paddingMode;
                case Tag::DIGEST:
                    return os << param.digest;
                case Tag::EC_CURVE:
                    return os << (int)param.ecCurve;
                case Tag::ORIGIN:
                    return os << param.origin;
                case Tag::BLOB_USAGE_REQUIREMENTS:
                    return os << (int)param.f.keyBlobUsageRequirements;
                case Tag::PURPOSE:
                    return os << param.purpose;
                default:
                    return os << " UNKNOWN ENUM " << param.integer;
            }
        case TagType::ULONG_REP:
        case TagType::ULONG:
            return os << param.longInteger;
        case TagType::DATE:
            return os << param.dateTime;
        case TagType::BOOL:
            return os << "true";
        case TagType::BIGNUM:
            os << " Bignum: ";
