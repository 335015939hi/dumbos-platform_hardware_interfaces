...
std::pair<ErrorCode, attestation_record> parse_attestation_record(const uint8_t* asn1_key_desc,
    ...
    if (!record.get()) return std::make_pair(ErrorCode::UNKNOWN_ERROR, parsed_record);
    ...
    if (error != ErrorCode::OK) return std::make_pair(error, parsed_record);
    error = extract_auth_list(record->tee_enforced, &(parsed_record.hw_enforced));
    if (error != ErrorCode::OK) return std::make_pair(error, parsed_record);

    return std::make_pair(error, parsed_record);
}
