...
class KeyMaster4AttestationFuzzer {
  ...
  private:
    ...
    X509* parseCertificateBlob(const hidl_vec<uint8_t>& blob);
...
X509* KeyMaster4AttestationFuzzer::parseCertificateBlob(const hidl_vec<uint8_t>& blob) {
    const uint8_t* p = blob.data();
    return d2i_X509(nullptr, &p, blob.size());
}

ASN1_OCTET_STRING* KeyMaster4AttestationFuzzer::getAttestationRecord(X509* certificate) {
...
void KeyMaster4AttestationFuzzer::invokeAttestationRecord() {
    ...
    attestKey(keyBlob,
              AuthorizationSetBuilder()
                      .Authorization(TAG_ATTESTATION_CHALLENGE, toHidlVec(challenge))
                      .Authorization(TAG_ATTESTATION_APPLICATION_ID, toHidlVec(attestationId)),
              &certificateChain);
