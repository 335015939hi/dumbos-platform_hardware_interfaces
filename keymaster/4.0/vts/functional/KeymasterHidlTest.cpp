...
void KeymasterHidlTest::CheckCreationDateTime(const AuthorizationSet& sw_enforced) {
    ASSERT_TRUE(sw_enforced.Contains(TAG_CREATION_DATETIME));
    for (int i = 0; i < sw_enforced.size(); i++) {
        if (sw_enforced[i].tag == TAG_CREATION_DATETIME) {
            auto now = std::chrono::system_clock::now().time_since_epoch();
            auto reported_time = std::chrono::milliseconds(sw_enforced[i].f.dateTime);
            auto time_dif =
                    std::chrono::duration_cast<std::chrono::milliseconds>(now - reported_time);
            // The time doesn't need to be exact, and time to generate the key needs to be taken
            // into account. Ten seconds is a reasonable closeness.
            EXPECT_TRUE((time_dif <= 10s) && (time_dif >= -10s));
        }
    }
}
