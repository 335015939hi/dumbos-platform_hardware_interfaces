...
bool verify_attestation_record(const string& challenge, const string& app_id,
    ...
    for (int i = 0; i < att_tee_enforced.size(); i++) {
        if (att_tee_enforced[i].tag == TAG_BOOT_PATCHLEVEL) {
            // This should be replaced with proper validation. Additionally, this will fail after
            // the year 9999
            EXPECT_GE(att_tee_enforced[i].f.integer, 20000000);
            EXPECT_LE(att_tee_enforced[i].f.integer, 100000000);
        }
    }
    ...
    if (att_tee_enforced.Contains(TAG_ALGORITHM, Algorithm::EC)) {
        // For ECDSA keys, either an EC_CURVE or a KEY_SIZE can be specified, but one must be.
        EXPECT_TRUE(att_tee_enforced.Contains(TAG_EC_CURVE) ||
                    att_tee_enforced.Contains(TAG_KEY_SIZE));
    }
