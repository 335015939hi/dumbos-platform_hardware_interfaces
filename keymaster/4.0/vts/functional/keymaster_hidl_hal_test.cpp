...
TEST_F(NewKeyGenerationTest, ReturnUnknownTag) {
    HidlBuf key_blob;
    KeyCharacteristics key_characteristics;
    typedef typename Tag2TypedTag<static_cast<Tag>((3 << 28) | 0xffff)>::type TAG_UNKNOWN_t;
    TAG_UNKNOWN_t TAG_UNKNOWN;
    ASSERT_EQ(ErrorCode::OK, GenerateKey(AuthorizationSetBuilder()
                                             .RsaSigningKey(2048, 3)
                                             .Digest(Digest::NONE)
                                             .Padding(PaddingMode::NONE)
                                             .Authorization(TAG_UNKNOWN, 3),
                                         &key_blob, &key_characteristics));
    CheckCharacteristics(key_blob, key_characteristics);
    AuthorizationSet unknown_param = key_characteristics.hardwareEnforced;
    EXPECT_TRUE(unknown_param.Contains(TAG_UNKNOWN));
    EXPECT_TRUE(unknown_param.Contains(TAG_UNKNOWN, 3));
}
