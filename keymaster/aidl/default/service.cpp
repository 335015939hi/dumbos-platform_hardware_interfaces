...
using android::hardware::keymaster::V5_0::SecurityLevel;

int main() {
    // identity config 0 thread, old keymaster uses 1 ???
    ABinderProcess_setThreadPoolMaxThreadCount(0);
    std::shared_ptr<IKeymaster5Device> km5 =
            ndk::SharedRefBase::make<IKeymaster5Device>(SecurityLevel::SOFTWARE);

    // auto keymaster = ::keymaster::V4_0::ng::CreateKeymasterDevice(SecurityLevel::SOFTWARE);
    // auto status = keymaster->registerAsService();

    const std::string instance = std::string() + IKeymaster5Device::descriptor + "/default";
