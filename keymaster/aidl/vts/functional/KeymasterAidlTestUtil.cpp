...
ErrorCode KeymasterAidlTestUtil::GenerateKey(const AuthorizationSet& key_desc, HidlBuf* key_blob,
...
ErrorCode KeymasterAidlTestUtil::ExportKey(KeyFormat format, const HidlBuf& key_blob,
...
ErrorCode KeymasterAidlTestUtil::ExportKey(KeyFormat format, HidlBuf* key_material) {
    HidlBuf client_id, app_data;
    return ExportKey(format, key_blob_, client_id, app_data, key_material);
}

ErrorCode KeymasterAidlTestUtil::DeleteKey(HidlBuf* key_blob, bool keep_key_blob) {
...
void KeymasterAidlTestUtil::CheckedDeleteKey(HidlBuf* key_blob, bool keep_key_blob) {
...
void KeymasterAidlTestUtil::CheckGetCharacteristics(const HidlBuf& key_blob,
                                                    const HidlBuf& client_id,
...
ErrorCode KeymasterAidlTestUtil::GetCharacteristics(const HidlBuf& key_blob,
                                                    const HidlBuf& client_id,
                                                    const HidlBuf& app_data,
                                                    KeyCharacteristics* key_characteristics) {
...
ErrorCode KeymasterAidlTestUtil::GetDebugInfo(DebugInfo* debug_info) {
    EXPECT_TRUE(keymaster_
                        ->getDebugInfo([&](const DebugInfo& hidl_debug_info) {
...
ErrorCode KeymasterAidlTestUtil::Begin(KeyPurpose purpose, const HidlBuf& key_blob,
...
ErrorCode KeymasterAidlTestUtil::Update(OperationHandle op_handle,
                                        const AuthorizationSet& in_params, const string& input,
...
ErrorCode KeymasterAidlTestUtil::Finish(OperationHandle op_handle,
                                        const AuthorizationSet& in_params, const string& input,
    ...
    ErrorCode error;
    EXPECT_TRUE(keymaster_
                        ->finish(op_handle, in_params.hidl_data(), HidlBuf(input),
                                 HidlBuf(signature), HardwareAuthToken(), VerificationToken(),
                                 [&](ErrorCode hidl_error,
                                     const hidl_vec<KeyParameter>& hidl_out_params,
                                     const HidlBuf& hidl_output) {
                                     error = hidl_error;
...
ErrorCode KeymasterAidlTestUtil::Abort(OperationHandle op_handle) {
    SCOPED_TRACE("Abort");
...
ErrorCode KeymasterAidlTestUtil::AttestKey(const HidlBuf& key_blob,
...
ErrorCode KeymasterAidlTestUtil::AttestKey(const AuthorizationSet& attest_params,
                                           hidl_vec<hidl_vec<uint8_t>>* cert_chain) {
    SCOPED_TRACE("AttestKey");
    return AttestKey(key_blob_, attest_params, cert_chain);
}

string KeymasterAidlTestUtil::ProcessMessage(const HidlBuf& key_blob, KeyPurpose operation,
...
string KeymasterAidlTestUtil::SignMessage(const HidlBuf& key_blob, const string& message,
...
void KeymasterAidlTestUtil::VerifyMessage(const HidlBuf& key_blob, const string& message,
...
string KeymasterAidlTestUtil::EncryptMessage(const HidlBuf& key_blob, const string& message,
                                             const AuthorizationSet& in_params,
                                             AuthorizationSet* out_params) {
...
string KeymasterAidlTestUtil::DecryptMessage(const HidlBuf& key_blob, const string& ciphertext,
                                             const AuthorizationSet& params) {
...
std::pair<ErrorCode, HidlBuf> KeymasterAidlTestUtil::UpgradeKey(const HidlBuf& key_blob) {
...
std::vector<uint32_t> KeymasterAidlTestUtil::ValidKeySizes(Algorithm algorithm) {
    switch (algorithm) {
        case Algorithm::RSA:
            switch (SecLevel()) {
...
std::vector<uint32_t> KeymasterAidlTestUtil::InvalidKeySizes(Algorithm algorithm) {
...
std::vector<EcCurve> KeymasterAidlTestUtil::ValidCurves() {
...
std::vector<EcCurve> KeymasterAidlTestUtil::InvalidCurves() {
    if (SecLevel() == SecurityLevel::TRUSTED_ENVIRONMENT) return {};
    CHECK(SecLevel() == SecurityLevel::STRONGBOX);
    return {EcCurve::P_224, EcCurve::P_384, EcCurve::P_521};
}

std::vector<Digest> KeymasterAidlTestUtil::ValidDigests(bool withNone, bool withMD5) {
    switch (SecLevel()) {
        case SecurityLevel::SOFTWARE:
        case SecurityLevel::TRUSTED_ENVIRONMENT:
            if (withNone) {
...
std::vector<Digest> KeymasterAidlTestUtil::InvalidDigests() {
    return {};
}
