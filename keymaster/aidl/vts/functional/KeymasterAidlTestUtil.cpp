...
ErrorCode KeymasterAidlTestUtil::ImportWrappedKey(string wrapped_key, string wrapping_key,
    ...
    if (!result.isOk()) {
        ...
        // old cold overwrite the old key blob in importkey to wrap key output.
...
ErrorCode KeymasterAidlTestUtil::Update(const string& input, string* out, int32_t* input_consumed) {
    SCOPED_TRACE("Update");
    AuthorizationSet out_params;
    ErrorCode result =
...
ErrorCode KeymasterAidlTestUtil::Finish(const AuthorizationSet& in_params, const string& input,
    ...
    if (result.isOk()) {
        output->append(oPut.begin(), oPut.end());
        *out_params = oParams;
        // ??? old hidl can pass AuthorizationSet* as vector<KeyParameter>*, but
        // aidl can't compile and needs explicate conversion, one is a class and
        // other is a vector , not sure why it
        // worked in hidl.
    }
...
ErrorCode KeymasterAidlTestUtil::Abort(sp<IKeymasterOperation>& op) {
    SCOPED_TRACE("Abort");
...
std::pair<ErrorCode, vector<uint8_t>> KeymasterAidlTestUtil::UpgradeKey(
