...
#include <android/hardware/keymaster/IKeymasterDevice.h>
...
class KeymasterAidlTestBase : public ::testing::TestWithParam<string> {
  public:
    void SetUp() override;
    ...
    ErrorCode Abort();
    ErrorCode Abort(sp<IKeymasterOperation>& op);
    void AbortIfNeeded();
    ...
    sp<IKeymasterOperation> op_;
    vector<Certificate> certChain_;
    vector<uint8_t> key_blob_;
    KeyCharacteristics key_characteristics_;

    static vector<string> build_params() {
        // vector<string> params = {"keymaster 5.0 vts tests"};
        auto params = android::getAidlHalInstanceNames(IKeymasterDevice::descriptor);
        return params;
    }
...
#define INSTANTIATE_KEYMASTER_AIDL_TEST(name)                                          \
    INSTANTIATE_TEST_SUITE_P(PerInstance, name,                                        \
                             testing::ValuesIn(KeymasterAidlTestBase::build_params()), \
