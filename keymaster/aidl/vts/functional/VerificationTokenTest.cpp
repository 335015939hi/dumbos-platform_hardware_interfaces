...
class VerificationTokenTest : public KeymasterAidlTestBase {
  protected:
    ...
    VerifyAuthorizationResult verifyAuthorization(uint64_t operationHandle,
        ...
        Status err;
        err = keymaster().verifyAuthorization(operationHandle,               //
                                              paramsToVerify.vector_data(),  //
                                              authToken,                     //
                                              &result.token);

        result.error = static_cast<ErrorCode>(err.serviceSpecificErrorCode());
        return result;
    }
...
TEST_P(VerificationTokenTest, TestCreation) {
    auto result1 = verifyAuthorization(1 /* operation handle */,
                                       AuthorizationSet(), /* parametersToVerify */
                                       HardwareAuthToken());
    ...
    auto result2 = verifyAuthorization(
            2 /* operation handle */, AuthorizationSet(), /* parametersToVerify */
            HardwareAuthToken());                         // ASSERT_TRUE(result2.callSuccessful);
...
TEST_P(VerificationTokenTest, MacChangesOnChangingTimestamp) {
    ...
    auto result2 = verifyAuthorization(0 /* operation handle */,
                                       AuthorizationSet(), /* parametersToVerify */
                                       HardwareAuthToken());
...
INSTANTIATE_KEYMASTER_AIDL_TEST(VerificationTokenTest);
