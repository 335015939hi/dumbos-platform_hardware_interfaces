...
ErrorCode KeymasterAidlTestBase::GenerateKey(const AuthorizationSet& key_desc,
    ...
    ALOGE("GENERATE key returns keyblob size %uu, soft size %u", (unsigned int)keyBlob->size(),
          (unsigned int)keyChar->softwareEnforced.size());
...
ErrorCode KeymasterAidlTestBase::Begin(KeyPurpose purpose, const vector<uint8_t>& key_blob,
    ...
    Status result;
    BeginResult out;
    ALOGE("BEGIN received keyblobk size %u", (unsigned int)key_blob.size());
...
string KeymasterAidlTestBase::SignMessage(const string& message, const AuthorizationSet& params) {
    ...
    /*
SCOPED_TRACE("temp test");

    SCOPED_TRACE("Begin");
    Status result;
    BeginResult out;
    AuthorizationSet out_params;
    result = keymaster_->begin(KeyPurpose::SIGN, key_blob_, params.vector_data(),
HardwareAuthToken(), &out); EXPECT_TRUE(result.isOk()) << "error code returned is "
                               << result.serviceSpecificErrorCode() << endl;
    ...
    AuthorizationSet finish_params;
    AuthorizationSet finish_out_params;
    EXPECT_EQ(ErrorCode::OK, Finish(finish_params, "", unused,
