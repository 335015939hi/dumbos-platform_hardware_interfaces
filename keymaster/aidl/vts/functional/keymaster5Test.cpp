...
#define LOG_TAG "keymaster_5_test"
...
#include <android/hardware/keymaster/KeyFormat.h>

#include <keymasterV5_0/attestation_record.h>
#include <keymasterV5_0/key_param_output.h>
#include <keymasterV5_0/openssl_utils.h>

#include "KeymasterAidlTestUtil.h"
...
using android::hardware::keymaster::KeyCharacteristics;
using android::hardware::keymaster::KeyFormat;

namespace keymaster {

bool operator==(const keymaster::AuthorizationSet& a, const keymaster::AuthorizationSet& b) {
    return a.size() == b.size() && std::equal(a.begin(), a.end(), b.begin());
}

// Aidl auto defines it's own operator== for KeyCharacteristics which does not
// sort, and operator== cannot be overloaded again.  So we need a different name.
...
/* TODO(seleneh) fix attestation related later
bool verify_chain(const vector<vector<uint8_t>>& chain, const std::string& msg,
                  const std::string& signature) {
    {
        ...
        EXPECT_TRUE((result = result &&
                              EVP_DigestVerifyUpdate(&md_ctx_verify, msg.c_str(), msg.size())));
...
bool verify_attestation_record(const string& challenge, const string& /* app_id */,
    ...
    // TODO(seleneh) fix key tags should not bee seen bool
    // expected_sw_enforced.push_back(TAG_ATTESTATION_APPLICATION_ID, app_id.data(), app_id.size());
...
class NewKeyGenerationTest : public KeymasterAidlTestUtil {
  protected:
...
INSTANTIATE_KEYMASTER_AIDL_TEST(NewKeyGenerationTest);

typedef KeymasterAidlTestUtil SigningOperationsTest;
...
TEST_P(SigningOperationsTest, RsaUseRequiresCorrectAppIdAppData) {
    ASSERT_EQ(ErrorCode::OK,
              GenerateKey(AuthorizationSetBuilder()
                                  ...
                                  .Digest(Digest::NONE)
                                  .Padding(PaddingMode::NONE)
                          // TODO(seleneh) fix keymaster_tag thinking all is a BOOL
                          //                      .Authorization(TAG_APPLICATION_ID, "clientid", 8)
                          //                      .Authorization(TAG_APPLICATION_DATA, "appdata", 7)
                          ));
...
INSTANTIATE_KEYMASTER_AIDL_TEST(SigningOperationsTest);

typedef KeymasterAidlTestUtil VerificationOperationsTest;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(VerificationOperationsTest);

typedef KeymasterAidlTestUtil ExportKeyTest;
 ...
 */
// TODO(seleneh) confirm there is no point in porting these ExportKey tests to GenerateKey
// check result
#if 0
...
TEST_P(ExportKeyTest, RsaCorruptedKeyBlob) {
    ...
    for (size_t i = 0; i < key_blob_.size(); ++i) {
        vector<uint8_t> corrupted(key_blob_);
        ++corrupted[i];

        vector<uint8_t> export_data;
        EXPECT_EQ(ErrorCode::INVALID_KEY_BLOB,
                  ExportKey(KeyFormat::X509, corrupted, AidlBuf(), AidlBuf(), &export_data))
...
INSTANTIATE_KEYMASTER_AIDL_TEST(ExportKeyTest);

class ImportKeyTest : public KeymasterAidlTestUtil {
  public:
    template <TagType tag_type, Tag tag, typename ValueT>
...
INSTANTIATE_KEYMASTER_AIDL_TEST(ImportKeyTest);
...
class ImportWrappedKeyTest : public KeymasterAidlTestUtil {};
...
INSTANTIATE_KEYMASTER_AIDL_TEST(ImportWrappedKeyTest);

typedef KeymasterAidlTestUtil EncryptionOperationsTest;
 ...
 */
#if 0  // TODO(seleneh) fix ExportKey
...
INSTANTIATE_KEYMASTER_AIDL_TEST(EncryptionOperationsTest);

typedef KeymasterAidlTestUtil MaxOperationsTest;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(MaxOperationsTest);

typedef KeymasterAidlTestUtil AddEntropyTest;
...
TEST_P(AddEntropyTest, AddEntropy) {
    string data = "foo";
    EXPECT_TRUE(keymaster().addRngEntropy(vector<uint8_t>(data.begin(), data.end())).isOk());
}
...
TEST_P(AddEntropyTest, AddEmptyEntropy) {
    EXPECT_TRUE(keymaster().addRngEntropy(AidlBuf()).isOk());
}
...
TEST_P(AddEntropyTest, AddLargeEntropy) {
    EXPECT_TRUE(keymaster().addRngEntropy(AidlBuf(string(2 * 1024, 'a'))).isOk());
}

INSTANTIATE_KEYMASTER_AIDL_TEST(AddEntropyTest);

typedef KeymasterAidlTestUtil AttestationTest;
 ...
 */
#if 0  // TODO(seleneh) fix AttestKey
TEST_P(AttestationTest, RsaAttestation) {
    ...
    vector<vector<uint8_t>> cert_chain;
    ASSERT_EQ(ErrorCode::OK,
              AttestKey(AuthorizationSetBuilder()
                                .Authorization(TAG_ATTESTATION_CHALLENGE, "challenge", 9)
                                .Authorization(TAG_ATTESTATION_APPLICATION_ID, "foo", 3),
                        &cert_chain));
...
TEST_P(AttestationTest, AttestationApplicationIDLengthProperlyEncoded) {
    ...
    for (uint32_t length : app_id_lengths) {
        ...
        ASSERT_EQ(ErrorCode::OK,
                  AttestKey(AuthorizationSetBuilder()
                                    .Authorization(TAG_ATTESTATION_CHALLENGE, "challenge", 9)
                                    .Authorization(TAG_ATTESTATION_APPLICATION_ID, AidlBuf(app_id)),
                            &cert_chain));
        EXPECT_GE(cert_chain.size(), 2U);
...
INSTANTIATE_KEYMASTER_AIDL_TEST(AttestationTest);
#endif

typedef KeymasterAidlTestUtil KeyDeletionTest;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(KeyDeletionTest);

using UpgradeKeyTest = KeymasterAidlTestUtil;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(UpgradeKeyTest);

using ClearOperationsTest = KeymasterAidlTestUtil;
...
TEST_P(ClearOperationsTest, TooManyOperations) {
    ...
    sp<IKeymasterOperation> op_handles[max_operations];
    AuthorizationSet out_params;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(ClearOperationsTest);

typedef KeymasterAidlTestUtil TransportLimitTest;
...
INSTANTIATE_KEYMASTER_AIDL_TEST(TransportLimitTest);
