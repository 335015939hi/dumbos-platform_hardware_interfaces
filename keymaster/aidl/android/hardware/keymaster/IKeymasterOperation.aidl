...
interface IKeymasterOperation {
     ...
     * finish().
     *
     * @param operationHandle The operation handle returned by begin().  ???? no such param
     *
     ...
     *        calls to update(), but only until input data has been provided.  ??? what does this mean
     *
     ...
     *        fields to zero and the MAC must be an empty vector.   ??? doesn't exist
     *
     ...
     *
     * @return int inputConsumed Amount of data that was consumed by update().  If this is less than the
     ...
     *
     * @return out variable outParams Output parameters, used to return additional data from the operation.
     *
     * @return out variable output The output data, if any.
     */
    int update(in KeyParameter[] inParams, in byte[] input,
