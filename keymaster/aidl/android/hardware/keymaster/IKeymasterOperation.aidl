...
@VintfStability
interface IKeymasterOperation {
    /**
     * Retrieves the operations "challenge value", an unpredictable, unique value that is added to
     * tokens related to this operation, to show the relationship and "freshness".
     *
     * The generated challenge value must not change over the life of the operation, and every
     * operation must get a unique (probabilistic uniqueness is fine) value.
     */
    long getOperationChallenge();
     ...
     * unless the operation cannot accept any more; if more than zero bytes are provided and zero        * bytes are consumed, callers must consider this an error and abort the operation.
     *
     ...
     * updates, and update() must accept the data this way as well.  If the caller provides more         * data to sign than can be used (length of data exceeds RSA key size), update() must return
     ...
     *
     * @return out variable outParams Output parameters, used to return additional data from the
     * operation.
     *
     * @return out variable output The output data, if any.
     */
    int update(in KeyParameter[] inParams,
               in byte[] input,
               in VerificationToken inVerificationToken,
     ...
     */
    void finish(in KeyParameter[] inParams, in byte[] input, in byte[] inSignature,
                in HardwareAuthToken authToken, in VerificationToken inVerificationToken,
                out KeyParameter[] outParams, out byte[] output);
