...
import android.hardware.keymaster.Certificate;
import android.hardware.keymaster.HardwareAuthToken;
import android.hardware.keymaster.IKeymasterOperation;
import android.hardware.keymaster.KeyCharacteristics;
import android.hardware.keymaster.KeyFormat;
import android.hardware.keymaster.KeyParameter;
import android.hardware.keymaster.KeymasterHardwareInfo;
...
@VintfStability interface IKeymasterDevice {
     ...
     */
    void getHardwareInfo(out KeymasterHardwareInfo info);
     ...
     */
    VerificationToken verifyAuthorization(in IKeymasterOperation operation,
                                          in KeyParameter[] parametersToVerify,
                                          /* in KeymasterCapability capability, ????  all capability replace by token ? */
     ...
     *         method below.
     *
     * @return outCertChain The attestation certificate, and additional certificates back to the root
    ...
    void generateKey(in KeyParameter[] keyParams, out byte[] generatedKeyBlob,
                     out KeyCharacteristics generatedKeyCharacteristics, out Certificate[] outCertChain);
    ...
    void importKey(in KeyParameter[] inKeyParams, in KeyFormat inKeyFormat,
                   ...
                   out KeyCharacteristics outImportedKeyCharacteristics, out Certificate[] outCertChain);
    ...
    void importWrappedKey(in byte[] inWrappedKeyData, in byte[] inWrappingKeyBlob,
                          ...
                          in KeyParameter[] inUnwrappingParams, in long inPasswordSid,
                          in long inBiometricSid, out byte[] outImportedKeyBlob,
    ...
    KeyCharacteristics getKeyCharacteristics(
            in byte[] inKeyBlob, in byte[] inClientId, in byte[] inAppData);
     ...
     */
    byte[] exportKey(in KeyFormat inKeyFormat, in byte[] inKeyBlob,
                              in byte[] inClientId, in byte[] inAppData);
     ...
     *         The certificates must be DER-encoded.
     */
    Certificate[] attestKey(in byte[] inKeyToAttest, in KeyParameter[] inAttestParams);
     ...
     *         finish() or abort().
     */
    IKeymasterOperation begin(in KeyPurpose inPurpose, in byte[] inKeyBlob,
               in KeyParameter[] inParams, /* in KeymasterCapability inCapability, */
	       in HardwareAuthToken inAuthToken, /*  ???? replace capability with token??? */
               out KeyParameter[] outParams);
}
