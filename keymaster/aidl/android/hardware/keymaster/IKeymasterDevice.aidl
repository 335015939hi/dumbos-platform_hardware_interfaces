...
import android.hardware.keymaster.BeginResult;
...
@VintfStability interface IKeymasterDevice {
     ...
     *
     * @param the challenge returned by StrongBox's keymaster's begin().
     *
     ...
     * parameters correctly in the hardware-enforced and software-enforced lists.
     *
     ...
     *
     *         The rule that IKeymasterDevice implementations must use for deciding whether a
     ...
     * use an old key blob with exportKey(), attestKey() or begin() must
     ...
     *         finish() or abort().
     */
    void begin(in KeyPurpose inPurpose,
               in byte[] inKeyBlob,
               in KeyParameter[] inParams,
               in HardwareAuthToken inAuthToken,
               out BeginResult result);
}
