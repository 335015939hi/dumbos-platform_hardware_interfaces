...
class FactoryLoader {
  public:
    ...
    bool enumeratePlugins(vector<CasPluginDescriptor>* results);
  ...
  private:
    ...
    KeyedVector<String8, sp<SharedLibrary>> mLibraryPathToOpenLibraryMap;

    bool loadFactoryForSchemeFromPath(const String8& path, int32_t CA_system_id,
                                      sp<SharedLibrary>* library, T** factory);

    bool queryPluginsFromPath(const String8& path, vector<CasPluginDescriptor>* results);
...
template <class T>
bool FactoryLoader<T>::enumeratePlugins(vector<CasPluginDescriptor>* results) {
...
bool FactoryLoader<T>::queryPluginsFromPath(const String8& path,
                                            vector<CasPluginDescriptor>* results) {
    closeFactory();

    vector<android::CasPluginDescriptor> descriptors;
    ...
    for (auto it = descriptors.begin(); it != descriptors.end(); it++) {
        results->push_back(
                CasPluginDescriptor{.caSystemId = it->CA_system_id, .name = it->name.c_str()});
    }
    return true;
}
