...
class MediaCasAidlTest : public testing::TestWithParam<std::string> {
  ...
  protected:
    ...
    ::testing::AssertionResult descrambleTestInputBuffer(
            ...
            uint8_t*& inMemory);
    ::testing::AssertionResult descrambleTestOobInput(const shared_ptr<IDescrambler>& descrambler,
...
::testing::AssertionResult MediaCasAidlTest::openCasSession(std::vector<uint8_t>* sessionId,
                                                            ...
                                                            ScramblingMode mode) {
    return ::testing::AssertionResult(mMediaCas->openSession(intent, mode, sessionId).isOk());
}
...
::testing::AssertionResult MediaCasAidlTest::descrambleTestInputBuffer(
    ...
    SharedBuffer srcBuffer = {.heapbase.fd = ::ndk::ScopedFileDescriptor(std::move(fd)),
                              .heapbase.size = size,
                              .offset = 0};

    SharedBuffer dupBuffer = {
            .heapbase.fd = ::ndk::ScopedFileDescriptor(dupFd), .heapbase.size = size, .offset = 0};

    DestinationBuffer dstBuffer;
    dstBuffer.type = BufferType::SHARED_MEMORY;
    dstBuffer.nonsecureMemory = std::move(dupBuffer);
...
::testing::AssertionResult MediaCasAidlTest::descrambleTestOobInput(
        const shared_ptr<IDescrambler>& descrambler, ScopedAStatus& descrambleStatus,
        const OobInputTestParams& params) {
    std::vector<SubSample> subSample(params.subSamples, params.subSamples + params.numSubSamples);

    int size = params.imemSizeActual;
    auto fd = ashmem_create_region("vts-cas", size);
    if (fd < 0) {
        ALOGE("ashmem_create_region failed");
        return ::testing::AssertionFailure();
    ...
    SharedBuffer srcBuffer = {
            .heapbase.fd = ::ndk::ScopedFileDescriptor(std::move(fd)),
            .heapbase.size = params.imemSize,
            .offset = params.imemOffset,
    };

    SharedBuffer dupBuffer = {.heapbase.fd = ::ndk::ScopedFileDescriptor(dupFd),
                              .heapbase.size = params.imemSize,
                              .offset = params.imemOffset};
...
TEST_P(MediaCasAidlTest, TestInvalidSystemIdFails) {
    ...
    shared_ptr<MediaCasListener> casListener = ::ndk::SharedRefBase::make<MediaCasListener>();
...
TEST_P(MediaCasAidlTest, TestClearKeySessionClosedAfterRelease) {
    ...
    if (mDescrambler != nullptr) {
        ...
        EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_SESSION_NOT_OPENED),
...
TEST_P(MediaCasAidlTest, TestClearKeyErrors) {
    ...
    EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_NO_LICENSE),
    ...
    EXPECT_FALSE(returnStatus.isOk());
    EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_NOT_PROVISIONED),
              returnStatus.getServiceSpecificError());
    ...
    returnStatus = mMediaCas->processEcm(sessionId, ecm);
    EXPECT_FALSE(returnStatus.isOk());
    EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_UNKNOWN),
              returnStatus.getServiceSpecificError());
    ...
    returnStatus = mMediaCas->processEcm(sessionId, ecm);
    EXPECT_FALSE(returnStatus.isOk());
    EXPECT_EQ(static_cast<int32_t>(Status::BAD_VALUE), returnStatus.getServiceSpecificError());
    ...
    if (mDescrambler != nullptr) {
        ...
        EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_DECRYPT_UNIT_NOT_INITIALIZED),
        ...
        ASSERT_FALSE(descrambleTestInputBuffer(mDescrambler, descrambleStatus, sharedBuffer));
        EXPECT_EQ(static_cast<int32_t>(Status::ERROR_CAS_DECRYPT),
                  descrambleStatus.getServiceSpecificError());
