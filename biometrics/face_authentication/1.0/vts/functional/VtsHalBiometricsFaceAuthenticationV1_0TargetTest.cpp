...
class FaceHidlTest : public ::testing::VtsHalHidlTargetTestBase {
   public:
    virtual void SetUp() override {
        ...
        Return<RequestStatus> res = mService->setActiveGroup(kUserId, kTmpDir);
...
TEST_F(FaceHidlTest, SetActiveGroupTest) {
    // Create an active group
    Return<RequestStatus> res = mService->setActiveGroup(2, kTmpDir);
    ASSERT_EQ(RequestStatus::SYS_OK, static_cast<RequestStatus>(res));

    // Reset active group
    res = mService->setActiveGroup(kUserId, kTmpDir);
    ASSERT_EQ(RequestStatus::SYS_OK, static_cast<RequestStatus>(res));
}

// Active group should fail to set to an unwritable location.
TEST_F(FaceHidlTest, SetActiveGroupUnwritableTest) {
...
TEST_F(FaceHidlTest, SetActiveGroupNullTest) {
