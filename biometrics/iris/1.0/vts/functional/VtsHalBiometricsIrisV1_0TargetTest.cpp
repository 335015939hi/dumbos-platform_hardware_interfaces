...
#define LOG_TAG "iris_hidl_hal_test"

#include <VtsHalHidlTargetTestBase.h>
#include <android-base/logging.h>
#include <android/hardware/biometrics/iris/1.0/IBiometricsIris.h>
#include <android/hardware/biometrics/iris/1.0/IBiometricsIrisClientCallback.h>
...
using android::hardware::biometrics::iris::V1_0::IBiometricsIris;
using android::hardware::biometrics::iris::V1_0::IBiometricsIrisClientCallback;
using android::hardware::biometrics::iris::V1_0::IrisAcquiredInfo;
using android::hardware::biometrics::iris::V1_0::IrisError;
using android::hardware::biometrics::iris::V1_0::RequestStatus;
...
static const std::string kTmpDir = "/data/system/users/0/irisdata/";
...
class IrisCallbackBase : public IBiometricsIrisClientCallback {
   public:
    // implement methods of IBiometricsIrisClientCallback
    virtual Return<void> onEnrollResult(uint64_t, uint32_t, uint32_t, uint32_t) override {
        ALOGD("Enroll callback called.");
        return Return<void>();
    }

    virtual Return<void> onAcquired(uint64_t, IrisAcquiredInfo, int32_t) override {
    ...
    virtual Return<void> onError(uint64_t, IrisError, int32_t) override {
        ALOGD("Error callback called.");
        EXPECT_TRUE(false);  // fail any test that triggers an error
        return Return<void>();
    }

    virtual Return<void> onRemoved(uint64_t, uint32_t, uint32_t, uint32_t) override {
        ALOGD("Removed callback called.");
        return Return<void>();
    }

    virtual Return<void> onEnumerate(uint64_t, uint32_t, uint32_t, uint32_t) override {
        ALOGD("Enumerate callback called.");
        return Return<void>();
    }
};

class EnumerateCallback : public IrisCallbackBase {
   public:
    virtual Return<void> onEnumerate(uint64_t deviceId, uint32_t irisId, uint32_t groupId,
...
class ErrorCallback : public IrisCallbackBase {
   public:
    ErrorCallback(bool filterErrors = false, IrisError errorType = IrisError::ERROR_NO_ERROR) {
        this->filterErrors = filterErrors;
        this->errorType = errorType;
    }

    virtual Return<void> onError(uint64_t deviceId, IrisError error, int32_t vendorCode) override {
...
class RemoveCallback : public IrisCallbackBase {
   public:
    RemoveCallback(uint32_t groupId) { this->removeGroupId = groupId; }
...
class IrisHidlTest : public ::testing::VtsHalHidlTargetTestBase {
   public:
    virtual void SetUp() override {
        mService = ::testing::VtsHalHidlTargetTestBase::getService<IBiometricsIris>();
        ASSERT_FALSE(mService == nullptr);

        // Create an active group
        // Iris service can only write to /data/system/users/*/irisdata/ due to
    ...
    sp<IBiometricsIris> mService;
};

// The service should be reachable.
TEST_F(IrisHidlTest, ConnectTest) {
    sp<IrisCallbackBase> cb = new IrisCallbackBase();
...
TEST_F(IrisHidlTest, ConnectNullTest) {
...
TEST_F(IrisHidlTest, PreEnrollTest) {
...
TEST_F(IrisHidlTest, EnrollInvalidHatTest) {
    ...
    ASSERT_NE(IrisError::ERROR_NO_ERROR, cb->error);
}

// Enroll with an invalid (null) HAT should fail.
TEST_F(IrisHidlTest, EnrollNullTest) {
    sp<ErrorCallback> cb = new ErrorCallback();
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0UL, static_cast<uint64_t>(rc));
...
TEST_F(IrisHidlTest, PostEnrollTest) {
    sp<IrisCallbackBase> cb = new IrisCallbackBase();
    Return<uint64_t> rc = mService->setNotify(cb);
...
TEST_F(IrisHidlTest, GetAuthenticatorIdTest) {
    Return<uint64_t> res = mService->getAuthenticatorId();
    EXPECT_NE(0UL, static_cast<uint64_t>(res));
}

// Enumerate should always trigger onEnumerated(id=0, rem=0) when there are no irises
TEST_F(IrisHidlTest, EnumerateTest) {
...
// Remove should accept 0 to delete all irises
// At least one callback with "remaining=0" should occur.
TEST_F(IrisHidlTest, RemoveAllIrisesTest) {
    // Register callback
    sp<RemoveCallback> cb = new RemoveCallback(kGroupId);
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0UL, static_cast<uint64_t>(rc));
...
TEST_F(IrisHidlTest, SetActiveGroupTest) {
    // Create an active group
...
TEST_F(IrisHidlTest, SetActiveGroupUnwritableTest) {
...
TEST_F(IrisHidlTest, SetActiveGroupNullTest) {
...
// the IDLE state.
TEST_F(IrisHidlTest, CancelTest) {
    sp<ErrorCallback> cb = new ErrorCallback(true, IrisError::ERROR_CANCELED);
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0UL, static_cast<uint64_t>(rc));
    ...
    ASSERT_EQ(IrisError::ERROR_CANCELED, cb->error);
}

// A call to cancel should succeed during enroll.
TEST_F(IrisHidlTest, CancelEnrollTest) {
    Return<RequestStatus> res = mService->setActiveGroup(kGroupId, kTmpDir);
    ASSERT_EQ(RequestStatus::SYS_OK, static_cast<RequestStatus>(res));
...
TEST_F(IrisHidlTest, CancelAuthTest) {
    sp<ErrorCallback> cb = new ErrorCallback(true, IrisError::ERROR_CANCELED);
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0U, static_cast<uint64_t>(rc));
...
// A call to cancel should succeed during authentication.
TEST_F(IrisHidlTest, CancelRemoveTest) {
    sp<ErrorCallback> cb = new ErrorCallback(true, IrisError::ERROR_CANCELED);
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0U, static_cast<uint64_t>(rc));
...
// A call to cancel should succeed during authentication.
TEST_F(IrisHidlTest, CancelRemoveAllTest) {
    sp<ErrorCallback> cb = new ErrorCallback(true, IrisError::ERROR_CANCELED);
    Return<uint64_t> rc = mService->setNotify(cb);
    ASSERT_NE(0U, static_cast<uint64_t>(rc));
