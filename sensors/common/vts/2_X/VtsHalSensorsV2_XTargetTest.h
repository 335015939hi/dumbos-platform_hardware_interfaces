...
void SensorsHidlTest::runFlushTest(const std::vector<SensorInfoType>& sensors, bool activateSensor,
    ...
    int sensorsPerGroup = 10;
    for (int g = 0; g < sensors.size(); g += sensorsPerGroup) {
        std::vector<SensorInfoType> sensorGroup(sensors.begin() + g,
                        sensors.begin() + std::min(g + sensorsPerGroup, (int)sensors.size()));
...
<<<<<<< HEAD   (b3d5fe Fix force setting environment variable error)
...
=======
>>>>>>> CHANGE (bec0be Fix VtsHalSensorsV2_0TargetTest)
    getEnvironment()->unregisterCallback();
}
