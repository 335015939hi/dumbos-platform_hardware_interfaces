...
TEST_P(SensorsAidlTest, InjectSensorEventData) {
    std::vector<SensorInfo> sensors_all = getInjectEventSensors();
    std::vector<SensorInfo> sensors_3d;

    for (const auto& s : sensors_all) {
        if ((s.type == SensorType::ACCELEROMETER) || (s.type == SensorType::GYROSCOPE) ||
            (s.type == SensorType::MAGNETIC_FIELD)) {
            sensors_3d.push_back(s);
        }
    }

    // Here we only test for the sensors with vec3 data type
    if (sensors_3d.size() == 0) {
        return;
    }
    ...
    callback.waitForEvents(sensors_3d, std::chrono::milliseconds(1000) /* timeout */);
    getEnvironment()->unregisterCallback();
