...
ExternalCameraConfig ExternalCameraConfig::loadFromCfg(const char* cfgPath) {
    ...
    if (depthFpsList == nullptr) {
        ALOGI("%s: no depth fps list specified", __FUNCTION__);
    } else {
...
bool ExternalCameraConfig::updateFpsList(tinyxml2::XMLElement* fpsList,
    ...
    while (row != nullptr) {
        ...
        if (limit.size.width <= prevLimit.size.width ||
            ...
            ALOGE(
                "%s: Depth FPS limit list must have increasing size and decreasing fps!"
...
ExternalCameraConfig::ExternalCameraConfig() :
    ...
    depthFpsLimits.push_back({/*Size*/ {640, 480}, /*FPS upper bound*/ 30.0});
    depthFpsLimits.push_back({/*Size*/ {1280, 720}, /*FPS upper bound*/ 7.5});
    depthFpsLimits.push_back({/*Size*/ {1920, 1080}, /*FPS upper bound*/ 5.0});
    minStreamSize = {0, 0};
}
