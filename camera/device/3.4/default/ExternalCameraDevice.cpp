...
const std::array<uint32_t, /*size*/ 3> kSupportedFourCCs{
    {V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_Z16, V4L2_PIX_FMT_UYVY}};  // double braces required in C++11
...
status_t ExternalCameraDevice::initOutputCharskeysByFormat(
    ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata,
    std::array<int, SIZE>& halFormats, int scaler_stream_config_tag, int stream_configuration,
    ...
    for (const auto& supportedFormat : mSupportedFormats) {
        for (const auto& format : halFormats) {
            ...
            streamConfigurations.push_back(scaler_stream_config_tag);
        }
    ...
    UPDATE(stream_configuration, streamConfigurations.data(), streamConfigurations.size());

    UPDATE(min_frame_duration, minFrameDurations.data(), minFrameDurations.size());

    UPDATE(stall_duration, stallDurations.data(), stallDurations.size());
...
status_t ExternalCameraDevice::initOutputCharsKeys(
    ...
    for (const auto& supportedFormat : mSupportedFormats) {
        switch (supportedFormat.fourcc) {
            ...
            case V4L2_PIX_FMT_UYVY:
                ...
                break;
            default:
                ALOGW("%s: no supported formats available!", __FUNCTION__);
        }
    }

    if (hasDepth) {
        initOutputCharskeysByFormat(metadata, halDepthFormats,
    ...
    if (hasColor) {
        initOutputCharskeysByFormat(
            metadata, halFormats, ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT,
            ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS,
