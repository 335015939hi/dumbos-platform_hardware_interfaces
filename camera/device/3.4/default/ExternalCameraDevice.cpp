...
status_t ExternalCameraDevice::initAvailableCapabilities(
    ...
    for (const auto& fmt : mSupportedFormats) {
        switch (fmt.fourcc) {
            ...
            default: hasColor = true;
        }
    }

    if (hasDepth) {
        const uint8_t availableCapabilities[] = {ANDROID_REQUEST_AVAILABLE_CAPABILITIES_DEPTH_OUTPUT};
        UPDATE(ANDROID_REQUEST_AVAILABLE_CAPABILITIES, availableCapabilities,
            ARRAY_SIZE(availableCapabilities));
    }
    if (hasColor) {
        const uint8_t availableCapabilities[] = {ANDROID_REQUEST_AVAILABLE_CAPABILITIES_BACKWARD_COMPATIBLE};
        UPDATE(ANDROID_REQUEST_AVAILABLE_CAPABILITIES, availableCapabilities,
            ARRAY_SIZE(availableCapabilities));
    }
...
status_t ExternalCameraDevice::initOutputCharskeysByFormat(
    ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata,
    std::array<int, SIZE>& halFormats, int streamConfigTag, int streamConfiguration,
    int minFrameDuration, int stallDuration) {
    if (mSupportedFormats.empty()) {
        ALOGE("%s: Init supported format list failed", __FUNCTION__);
        return UNKNOWN_ERROR;
    }
