...
bool CameraAidlTest::isReadoutTimestampSupported(const camera_metadata_t* staticMeta) {
    ...
    if (rc != 0) {
        ALOGI("isReadoutTimestampSupported: Failed to find ANDROID_SENSOR_READOUT_TIMESTAMP");
        return true;
    }
    if (readoutTimestampEntry.count == 1 && !readoutTimestampEntry.data.u8[0]) {
        ALOGI("isReadoutTimestampSupported: readout timestamp not supported");
        return false;
    }
    ALOGI("isReadoutTimestampSupported: readout timestamp supported");
    return true;
}
