...
Status CameraAidlTest::getMandatoryConcurrentStreams(const camera_metadata_t* staticMeta,
    ...
    if (isDepthOnly(staticMeta)) {
        ...
        if (availableY16Sizes.empty()) {
            ALOGE("%s: Y16 format is mandatory for depth-only camera", __FUNCTION__);
    ...
    if (availableYuvSizes.empty() || availableJpegSize.empty()) {
        ALOGE("%s: YUV and JPEG format is mandatory for color camera", __FUNCTION__);
        return Status::OPERATION_NOT_SUPPORTED;
    }
...
Size CameraAidlTest::getPreferredSize(Size preferredMax, const std::vector<Size>& sizes) {
    ...
    for (const auto& size : sizes | std::views::drop(1)) {
        int checkWh = size.width * size.height;
