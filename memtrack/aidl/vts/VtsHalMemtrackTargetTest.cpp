...
TEST_P(MemtrackAidl, GetMemoryInvalidPid) {
    int pid = -1;
...
TEST_P(MemtrackAidl, GetMemoryInvalidType) {
    int pid = 1;
...
TEST_P(MemtrackAidl, GetGpuDevices) {
    std::vector<std::string> gpu_devices;

    auto status = memtrack_->getGpuDevices(&gpu_devices);

    EXPECT_TRUE(status.isOk());
}

INSTANTIATE_TEST_SUITE_P(, MemtrackAidl,
