...
TEST_F(RenderscriptHidlTest, ElementTypeAllocationCreate) {
    ...
    Allocation allocation = context->allocationCreateTyped(type, AllocationMipmapControl::NONE, (int)((uint32_t)AllocationUsageType::ALL & ~(uint32_t)AllocationUsageType::OEM), (Ptr)nullptr);
...
TEST_F(RenderscriptHidlTest, Simple1DCopyTest) {
    Element element = context->elementCreate(DataType::FLOAT_32, DataKind::USER, false, 1); // float1
    Type type = context->typeCreate(element, 128, 0, 0, false, false, YuvFormat::YUV_NONE); // 128 x float1
    Allocation allocation = context->allocationCreateTyped(type, AllocationMipmapControl::NONE, (int)AllocationUsageType::SCRIPT, (Ptr)nullptr); // 128 x float1
    ...
    context->allocation1DRead(allocation, 0, 0, (uint32_t)dataOut.size(), (Ptr)dataOut.data(), (Size)dataOut.size()*sizeof(float));
    bool same = std::all_of(dataOut.begin(), dataOut.end(), [](float x){ static int val = 0; return x == (float)val++; });
    EXPECT_EQ(true, same);
}

TEST_F(RenderscriptHidlTest, Simple2DCopyTest) {
    Element element = context->elementCreate(DataType::FLOAT_32, DataKind::USER, false, 1); // float1
    Type type = context->typeCreate(element, 128, 128, 0, false, false, YuvFormat::YUV_NONE); // 128 x 128 x float1
    Allocation allocation = context->allocationCreateTyped(type, AllocationMipmapControl::NONE, (int)AllocationUsageType::SCRIPT, (Ptr)nullptr); // 128 x 128 x float1
    std::vector<float> dataIn(128*128), dataOut(128*128);
    std::generate(dataIn.begin(), dataIn.end(), [](){ static int val = 0; return (float)val++; });
    hidl_vec<uint8_t> _data;
    _data.setToExternal((uint8_t*)dataIn.data(), dataIn.size()*sizeof(float));
    context->allocation2DWrite(allocation, 0, 0, 0, AllocationCubemapFace::POSITIVE_X, 128, 128, _data, 0);
    context->allocation2DRead(allocation, 0, 0, 0, AllocationCubemapFace::POSITIVE_X, 128, 128, (Ptr)dataOut.data(), (Size)dataOut.size()*sizeof(float), 0);
    bool same = std::all_of(dataOut.begin(), dataOut.end(), [](float x){ static int val = 0; return x == (float)val++; });
    EXPECT_EQ(true, same);
}
...
TEST_F(RenderscriptHidlTest, Simple3DCopyTest) {
    Element element = context->elementCreate(DataType::FLOAT_32, DataKind::USER, false, 1); // float1
    Type type = context->typeCreate(element, 32, 32, 32, false, false, YuvFormat::YUV_NONE); // 32 x 32 x 32 x float1
    Allocation allocation = context->allocationCreateTyped(type, AllocationMipmapControl::NONE, (int)AllocationUsageType::SCRIPT, (Ptr)nullptr); // 32 x 32 x 32 x float1
    std::vector<float> dataIn(32*32*32), dataOut(32*32*32);
    std::generate(dataIn.begin(), dataIn.end(), [](){ static int val = 0; return (float)val++; });
    hidl_vec<uint8_t> _data;
    _data.setToExternal((uint8_t*)dataIn.data(), dataIn.size()*sizeof(float));
    context->allocation3DWrite(allocation, 0, 0, 0, 0, 32, 32, 32, _data, 0);
    context->allocation3DRead(allocation, 0, 0, 0, 0, 32, 32, 32, (Ptr)dataOut.data(), (Size)dataOut.size()*sizeof(float), 0);
    bool same = std::all_of(dataOut.begin(), dataOut.end(), [](float x){ static int val = 0; return x == (float)val++; });
    EXPECT_EQ(true, same);
}
...
TEST_F(RenderscriptHidlTest, SimpleBitmapTest) {
    Element element = context->elementCreate(DataType::FLOAT_32, DataKind::USER, false, 1); // float1
    Type type = context->typeCreate(element, 512, 512, 0, false, false, YuvFormat::YUV_NONE); // 512 x 512 x float1
    std::vector<float> dataIn(512*512), dataOut1(512*512), dataOut2(512*512);
    std::generate(dataIn.begin(), dataIn.end(), [](){ static int val = 0; return (float)val++; });
    hidl_vec<uint8_t> _data;
    _data.setToExternal((uint8_t*)dataIn.data(), dataIn.size()*sizeof(float));
    Allocation allocation = context->allocationCreateFromBitmap(type, AllocationMipmapControl::NONE, _data, (int)AllocationUsageType::SCRIPT); // 512 x 512 x float1
    EXPECT_NE(allocation, Allocation(0));
    context->allocationCopyToBitmap(allocation, (Ptr)dataOut1.data(), (Size)dataOut1.size()*sizeof(float));
    bool same1 = std::all_of(dataOut1.begin(), dataOut1.end(), [](float x){ static int val = 0; return x == (float)val++; });
    EXPECT_EQ(true, same1);
    context->allocationRead(allocation, (Ptr)dataOut2.data(), (Size)dataOut2.size()*sizeof(float));
    bool same2 = std::all_of(dataOut2.begin(), dataOut2.end(), [](float x){ static int val = 0; return x == (float)val++; });
...
if (allocation == Allocation(0)) {
    ...
    context->contextPeekMessage([&](MessageToClientType _type, Size _size, uint32_t _subID){ messageType = _type; size = (uint32_t)_size; subID = _subID; });
    std::vector<char> message(size+2, '\0');
    context->contextGetMessage(message.data(), message.size()+1, [&](MessageToClientType _type, Size _size){ messageType = _type; size = (uint32_t)_size; });
