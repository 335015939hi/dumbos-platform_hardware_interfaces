 ...
 * the Allocation using allocation1DWrite. Then dataOut is then overridden with
 * allocation1DRead.
 *
 ...
 * into the Allocation using allocation2DWrite. Then dataOut is then overridden
 * with allocation2DRead.
 *
 ...
 * float vector dataIn & dataOut. dataIn is pre-populated with data, and copied
 * into the Allocation using allocation3DWrite. Then dataOut is then overridden
 * with allocation3DRead.
 *
 ...
 * allocationCopyToBitmap. Then dataOut is then overridden with allocationRead.
 *
 ...
 * allocationCopy2DRange. Then dataOut is then overridden with allocationRead.
 *
 * Calls: elementCreate, typeCreate, allocationCreateFromBitmap,
 ...
 * allocationCopy3DRange. Then dataOut is then overridden with allocationRead.
 *
 * Calls: elementCreate, typeCreate, allocationCreateTyped, allocation3DWrite,
 ...
 * The Allocation is pre-populated with dataIn. Then dataOut is then overridden
 * with allocationRead on the AllocationAdapter.
 *
 * Calls: elementCreate, typeCreate, allocationCreateFromBitmap,
 ...
 * Expect: dataIn & dataOut are the same.
 *
 * Current, this test is valid and should pass, but a bug in the RenderScript HIDL
 * HAL causes the test to fail. This should be fixed by 3/17.
 * TODO(butlermichael)
 */
/*
...
TEST_F(RenderscriptHidlTest, SimpleCubemapTest) {
    ...
    Allocation allocation = context->allocationCubeCreateFromBitmap(type,
        AllocationMipmapControl::NONE, _data, (int)AllocationUsageType::SCRIPT);
...
/*
 * This test creates a complex element type (uint8_t, uint32_t) out of known elements. It then
 * verifies the element structure was created correctly. Finally, the test creates a 128-wide,
 * 1-dimension allocation of this type and transfers memory to and from this structure.
 *
