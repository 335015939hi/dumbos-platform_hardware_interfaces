...
int main() {
    ...
    // TODO(b/162553096) Remove this sleep once this bug is closed. On mixed
    // bitness devices we run this multiple times in quick succession,
    // registering the same interface with servicemanager. If we register the
    // interface before servicemanager can clean up the older one with
    // binderDied(), it will end up cleaning up this one!
    sleep(1);
