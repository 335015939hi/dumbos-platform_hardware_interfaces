...
interface ISecureStorage {
     ...
     *
     * Allows subsequent changes (and/or atomic segments) to be added to the same commit as
     ...
     */
    IFile openFile(in String filePath, in FileProperties fileProperties, in OpenOptions options);
     ...
     *       does not acknowledge
     */
    void deleteFile(in String filePath, in FileProperties fileProperties, in DeleteOptions options);
     ...
     *       @destPath
     *   - ERR_UNSUPPORTED_PROPERTIES this rename is part of an atomic segment on a filesystem that
     * does not support multi-file atomics.
     *   - ERR_FS_* if the filesystem has been tampered with in a way that @options.readIntegrity
     *       does not acknowledge
     */
     ...
     *       acknowledge
     */
    IDir openDir(in String path, in FileProperties fileProperties, in ReadIntegrity readIntegrity);
}
