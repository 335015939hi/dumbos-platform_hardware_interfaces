...
interface IHwCryptoKey {
     ...
     *
     * AGREEMENT_HMAC_KEY:
     *      This is a key equivalent to the one currently derived by KeyMint from the Key Agreement
     *      Key that can be used by authenticators. It is expected that HwCrypto will participate on
     *      the shared secret negotiation and create that key. It will then share it with KeyMint
     *      and the other authenticators. See ISharedSecret.aidl for more information.
     */
    enum KeySlot {
        SHARED_HMAC_KEY,
    }
     ...
     *                          <code>IHwCryptoKey::keyTokenImport</code>.
     *
     * Return:
     ...
     *                      client. Keys to be imported are represented by a token created using
     *                      <code>IOpaqueKey::getShareableToken</code>.
     *
     * @requested_key:
