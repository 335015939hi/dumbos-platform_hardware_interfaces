...
namespace {
static constexpr SessionType session_types[4] = {
  SessionType::UNKNOWN, SessionType::A2DP_SOFTWARE_ENCODING_DATAPATH,
  SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH,
  SessionType::HEARING_AID_SOFTWARE_ENCODING_DATAPATH};
static constexpr SampleRate sample_rates[2] = {
  SampleRate::RATE_UNKNOWN, SampleRate::RATE_44100};
static constexpr BitsPerSample bits_per_samples[2] = {
  BitsPerSample::BITS_UNKNOWN, BitsPerSample::BITS_16};
static constexpr ChannelMode channel_modes[2] = {
  ChannelMode::UNKNOWN, ChannelMode::STEREO};
}  // namespace
...
TEST_F(BluetoothAudioHidlTest, OpenProviderBySession) {
  LOG(INFO) << "Test opening BluetoothAudioProvider with "
               "a (maybe invalid) SessionType";
  for (auto session_type : session_types) {
    EXPECT_TRUE(providers_factory->openProvider(
        session_type,
        [&](Status status,
 ...
 public:
  ...
  class BluetoothAudioPort
   ...
   public:
    ...
    Return<void> updateMetadata(
        const SourceMetadata &sourceMetadata __unused) { return Void(); }
  };
...
TEST_F(BluetoothAudioA2dpHidlTest, StartAndEndA2dpSession) {
  ...
  if (is_offloading) {
    // parameters used when offloading and test for default config only
    ...
    CodecConfiguration::EncodedDataConfiguration::CodecSpecific::SbcData sbc_data = {
  ...
  EXPECT_TRUE(audio_provider->startSession(
    ...
    for (auto bits_per_sample : bits_per_samples) {
      ...
      for (auto channel_mode : channel_modes) {
        ...
        if (sample_rate != SampleRate::RATE_UNKNOWN &&
          ...
          continue;
        // encodedDataConfiguration stays as invalid also
        codec_config.encodedDataConfiguration = {};
        std::unique_ptr<DataMQ> tempDataMQ;
        EXPECT_TRUE(audio_provider->startSession(
            audio_port, codec_config,
...
class BluetoothAudioHearingAidHidlTest : public BluetoothAudioHidlTest {
 public:
  ...
  class BluetoothAudioPort
   ...
   public:
    BluetoothAudioPort(BluetoothAudioHearingAidHidlTest &parent)
        : parent_(parent) {}
    virtual ~BluetoothAudioPort() = default;
...
TEST_F(BluetoothAudioHearingAidHidlTest, OpenHearingAidProvider) {
  LOG(INFO) << "Test the openProvider for "
               "HEARING_AID_SOFTWARE_ENCODING_DATAPATH (called in SetUp)";
}
...
TEST_F(BluetoothAudioHearingAidHidlTest,
    ...
    for (auto bits_per_sample : bits_per_samples) {
      ...
      for (auto channel_mode : channel_modes) {
        ...
        if (sample_rate != SampleRate::RATE_UNKNOWN &&
            ...
            channel_mode != ChannelMode::UNKNOWN)
          continue;
        // ignore encodedDataConfiguration when software encoding
        codec_config.encodedDataConfiguration = {};
        std::unique_ptr<DataMQ> tempDataMQ;
        EXPECT_TRUE(audio_provider->startSession(
            audio_port, codec_config,
