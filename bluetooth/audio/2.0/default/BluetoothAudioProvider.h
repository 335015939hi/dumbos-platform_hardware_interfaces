...
class BluetoothAudioProvider : public IBluetoothAudioProvider {
  public:
    ...
    virtual void updateStreamTracksMetadata(const struct source_metadata *source_metadata);
  ...
  protected:
    ...
    CodecConfiguration mCodecConfig;
    ...
    struct PortStateCback {
      ...
      std::function<void(const uint16_t&)> session_ended_cb_;
    };
    std::unordered_map<uint16_t, struct PortStateCback*> mObservers;
