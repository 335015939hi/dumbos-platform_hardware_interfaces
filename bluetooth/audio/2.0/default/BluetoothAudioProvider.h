...
class BluetoothAudioProvider : public IBluetoothAudioProvider {
  public:
    ...
    int registerControlResultCback(std::function<void(const SessionType, const BluetoothAudioStatus&)>&,
                                   std::function<void(const SessionType)>&);
    Return<void> unregisterControlResultCback(const int& key);
