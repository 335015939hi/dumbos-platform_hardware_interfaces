...
Return<void> BluetoothAudioProvidersFactory::getOffloadCodecCapabilities(
  ...
  auto retval = BluetoothAudioStatus::FAILURE;
  hidl_vec<CodecCapability> codec_capabilities = hidl_vec<CodecCapability>(0);
  if (sessionType == SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    std::vector<CodecCapability> db_codec_capabilities =
        android::bluetooth::audio::GetOffloadCodecCapabilities(sessionType);
    if (db_codec_capabilities.size()) {
      codec_capabilities.resize(db_codec_capabilities.size());
      for (int i = 0; i < db_codec_capabilities.size(); ++i) {
        codec_capabilities[i] = db_codec_capabilities[i];
      }
      retval = BluetoothAudioStatus::SUCCESS;
    }
  }
  LOG(INFO) << __func__ << " - SessionType=" << toString(sessionType)
            << ", status=" << toString(retval) << " supports "
            << codec_capabilities.size() << " codecs";
  _hidl_cb(retval, codec_capabilities);
  return Void();
}
