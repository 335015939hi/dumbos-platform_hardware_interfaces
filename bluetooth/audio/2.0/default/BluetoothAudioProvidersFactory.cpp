...
Return<void> BluetoothAudioProvidersFactory::getOffloadCodecCapabilities(
  ...
  std::vector<CodecCapability> db_codec_caps =
      android::bluetooth::audio::GetOffloadCodecCapabilities(
          SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH);
  if (!db_codec_caps.size()) {
    _hidl_cb(BluetoothAudioStatus::FAILURE, codec_caps);
    return Void();
  }
  codec_caps.resize(db_codec_caps.size());
  int i = 0;
  for (auto codec_cap : db_codec_caps) {
    codec_caps[i++] = codec_cap;
  }
