...
Return<void> BluetoothAudioProvider::startSession(
  ...
  ALOGI("%s - SessionType=%s(0x%02hhx), PcmConfig=[%s], "
        "EncodedConfig=[{Codec=0x%08x, MTU=0x%04x}]", __func__,
        toString(mSessionType).c_str(), mSessionType,
  ...
  if (has_session_) {
    for (auto& observer : mObservers) {
      ...
      ALOGD("%s - status=%s(0x%02hhx) for SessionType=%s(0x%02hhx), ctrl_key=0x%04hx",
            ...
            toString(mSessionType).c_str(), mSessionType, cb->ctrl_key);
  ...
  if (has_session_) {
    for (auto& observer : mObservers) {
      ...
      ALOGD("%s - notify to SessionType=%s(0x%02hhx), ctrl_key=0x%04hx",
            __func__, toString(mSessionType).c_str(), mSessionType, cb->ctrl_key);
...
void BluetoothAudioProvider::updateStreamTracksMetadata(const struct source_metadata *source_metadata) {
  if (has_session_ && stack_iface_) {
    ...
    ALOGD("%s - SessionType=%s(0x%02hhx), %zu track(s)", __func__,
          toString(mSessionType).c_str(), mSessionType, track_count);
    ...
    while (track_count && track) {
       ...
       ALOGV("%s - SessionType=%s(0x%02hhx), usage=%d, content=%d, gain=%f",
             __func__, toString(mSessionType).c_str(), mSessionType,
...
Return<void> BluetoothAudioProvider::unregisterControlResultCback(uint16_t& ctrl_key) {
  if (mSessionType != OBSERVERS_GET_SESSION_TYPE(ctrl_key)) {
