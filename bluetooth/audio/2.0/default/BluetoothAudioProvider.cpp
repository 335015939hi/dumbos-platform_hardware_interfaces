...
BluetoothAudioProvider::BluetoothAudioProvider()
  ...
  ALOGI("%s - size of audio data path %d byte(s)", __func__,
        BUFFER_SIZE * DOUBLE_BUFFER);
...
Return<void> BluetoothAudioProvider::startSession(
  ...
  if (!observers_.empty()) {
    ...
    ALOGI("%s - SessionType=%s(0x%02hhx) has NO observer", __func__,
          toString(session_type_).c_str(), session_type_);
  }
...
Return<void> BluetoothAudioProvider::streamStarted(
  ...
  if (has_session_) {
    ...
    if (!observers_.empty()) {
      for (auto &observer : observers_) {
        shared_ptr<struct PortStateCback> cb = observer.second;
        ALOGD("%s - status=%s(0x%02hhx) for SessionType=%s(0x%02hhx), "
              "ctrl_key=0x%04hx", __func__, toString(status).c_str(), status,
...
void BluetoothAudioProvider::updateStreamTracksMetadata(
  ...
  if (has_session_ && stack_iface_) {
    ...
    ALOGI("%s - SessionType=%s(0x%02hhx), %zu track(s)", __func__,
...
uint16_t BluetoothAudioProvider::registerControlResultCback(
  ...
  if ((ctrl_key & ~OBSERVERS_SESSION_TYPE_MASK_) >= OBSERVERS_SIZE_) {
    ALOGE("%s - provider has %zu observers already", __func__,
          observers_.size());
    return OBSERVERS_CTRL_KEY_UNDEF;
  }
