...
#define RTP_FRAME_SIZE   (PCM_FRAME_SIZE * 128)
#endif /* RTP_FRAME_SIZE */

#define BUFFER_SIZE      (RTP_FRAME_SIZE * 14)
...
BluetoothAudioProvider::BluetoothAudioProvider()
    : mSessionType(SessionType::UNKNOWN), has_session_(false),
...
Return<void> BluetoothAudioProvider::startSession(
  ...
  ALOGI("%s - SessionType=0x%02hhx, PcmConfig=[%s], EncodedConfig="
        "[{Codec=0x%08x, MTU=0x%04x}]", __func__, mSessionType,
        toString(mCodecConfig.pcmDataConfiguration).c_str(),
        mCodecConfig.encodedDataConfiguration.codecType,
        mCodecConfig.encodedDataConfiguration.peerMtu);
...
Return<void> BluetoothAudioProvider::streamStarted(BluetoothAudioStatus status) {
  ...
  if (has_session_) {
    for (auto& observer : mObservers) {
      ...
      ALOGI("%s - status=0x%02hhx for SessionType=0x%02hhx, ctrl_key=0x%04hx",
            __func__, status, mSessionType, cb->ctrl_key);
      cb->ctrl_res_cb_(cb->ctrl_key, status);
    }
  } else {
    ALOGW("%s - provider has no session: status=0x%02hhx", __func__, status);
  }
...
Return<void> BluetoothAudioProvider::endSession() {
  ...
  if (has_session_) {
    for (auto& observer : mObservers) {
      struct PortStateCback *cb = observer.second;
      ALOGI("%s - notify to SessionType=0x%02hhx, ctrl_key=0x%04hx",
            __func__, mSessionType, cb->ctrl_key);
  ...
  mCodecConfig.pcmDataConfiguration = kInvalidPcmConfiguration;
...
const DataMQ::Descriptor* BluetoothAudioProvider::getStreamDataFMQ() {
  if (has_session_) {
    if (mDataMQ != nullptr && mDataMQ->isValid()) {
      ...
      ALOGW("%s - SessionType=0x%02hhx, mDataMQ=%p invalid", __func__,
            mSessionType, (mDataMQ ? mDataMQ.get() : nullptr));
    }
  } else {
    ALOGW("%s - Provider has NO session", __func__);
  }
...
const PcmDataConfiguration& BluetoothAudioProvider::getStreamPcmDataConfig(void) {
  PcmDataConfiguration& pcmConfig = mCodecConfig.pcmDataConfiguration;;
  if (!has_session_) {
    ALOGW("%s - Provider has NO session", __func__);
...
void BluetoothAudioProvider::updateStreamTracksMetadata(const struct source_metadata *source_metadata) {
  if (has_session_ && stack_iface_) {
    ...
    ALOGD("%s - SessionType=0x%02hhx, %zu track(s)", __func__, mSessionType,
          track_count);
    sourceMetadata.tracks.resize(track_count);
    halMetadata = sourceMetadata.tracks.data();
    while (track_count && track) {
       halMetadata->usage = track->usage;
       halMetadata->contentType = track->content_type;
       halMetadata->gain = track->gain;
       ALOGV("%s - SessionType=0x%02hhx, usage=%d, content=%d, gain=%f",
             __func__, mSessionType, halMetadata->usage,
             halMetadata->contentType, halMetadata->gain);
       --track_count;
       ++track;
       ++halMetadata;
    }
    stack_iface_->updateMetadata(sourceMetadata);
  } else
    ALOGW("%s - provider has NO session", __func__);
}
...
uint16_t BluetoothAudioProvider::registerControlResultCback(
  ...
  uint16_t ctrl_key = (static_cast<uint16_t>(mSessionType) << OBSERVERS_SESSION_TYPE_OFFSET_);
  ...
  while (ctrl_key < OBSERVERS_SIZE_) {
    if (mObservers.find(ctrl_key) == mObservers.end())
      break;
    ++ctrl_key;
  }

  if ((ctrl_key & ~OBSERVERS_SESSION_TYPE_MASK_) >= OBSERVERS_SIZE_) {
    ALOGE("%s - provider has no resouce", __func__);
    return OBSERVERS_CTRL_KEY_UNDEF;
  }
  ...
  mObservers[cb->ctrl_key] = cb;

  return ctrl_key;
}

Return<void> BluetoothAudioProvider::unregisterControlResultCback(uint16_t& ctrl_key) {
  if (mSessionType != (SessionType)OBSERVERS_GET_SESSION_TYPE(ctrl_key)) {
    ALOGW("%s - wrong SessionType=0x%02hhx:0x%02hhx", __func__, mSessionType,
          (SessionType)OBSERVERS_GET_SESSION_TYPE(ctrl_key));
    return Void();
  }
  if (mObservers.find(ctrl_key) != mObservers.end()) {
    auto observer = mObservers.find(ctrl_key);
    struct PortStateCback *cb = observer->second;
    delete cb;
    mObservers.erase(ctrl_key);
