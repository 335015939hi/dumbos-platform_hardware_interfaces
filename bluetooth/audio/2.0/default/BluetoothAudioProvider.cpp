...
bool BluetoothAudioProvider::getStreamPresentationPosition(
  ...
  if (!retval) {
    if (remote_delay_report) *remote_delay_report = {};
...
Return<void> BluetoothAudioProvider::unregisterControlResultCback(
    uint16_t* ctrl_key) {
  if (!ctrl_key) {
    LOG(WARNING) << __func__ << " - ctrl_key is null";
    return Void();
  } else if (session_type_ != OBSERVERS_GET_SESSION_TYPE(*ctrl_key)) {
