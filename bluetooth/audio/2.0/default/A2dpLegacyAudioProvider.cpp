...
Return<void> A2dpLegacyAudioProvider::startSession(
  ...
  switch (pcmConfig.sampleRate) {
    ...
    default:
      ALOGW("%s - Unsupported PCM Configuration=%s, CodecType=%s(0x%08x)",
            __func__, toString(pcmConfig).c_str(),
            toString(codecConfig.encodedDataConfiguration.codecType).c_str(),
            codecConfig.encodedDataConfiguration.codecType);
...
void A2dpLegacyAudioProvider::updateStreamTracksMetadata(
  ...
  if (has_session_ && stack_iface_) {
    ALOGD("%s - %zu track(s) is no needed for SessionType=%s(0x%02hhx)",
          __func__, source_metadata->track_count,
