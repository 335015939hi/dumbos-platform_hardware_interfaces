...
Return<void> A2dpLegacyAudioProvider::streamStarted(BluetoothAudioStatus status) {
   ...
   */
  if (ctrl_res_cb_)
    ctrl_res_cb_(SessionType::A2DP_SOFTWARE_ENCODING_DATAPATH, status);
  else
...
Return<void> A2dpLegacyAudioProvider::streamSuspended(BluetoothAudioStatus status) {
...
Return<void> A2dpLegacyAudioProvider::endSession() {

  ALOGI("%s", __func__);

  if (session_ended_cb_)
    session_ended_cb_(SessionType::A2DP_SOFTWARE_ENCODING_DATAPATH);
  else
