...
typedef ::android::hardware::bluetooth::audio::V2_0::CodecConfiguration::EncodedDataConfiguration EncodedDataConfiguration;
...
Return<void> A2dpLegacyAudioProvider::startSession(
   ...
   * aptX HD: mSampleRate:(44100|48000), mBitsPerSample:(24), mChannelMode:(STEREO)
   * LDAC: mSampleRate:(44100|48000|88200|96000), mBitsPerSample:(16|24|32), mChannelMode:(STEREO)
   */
...
#endif
    default:
      ALOGW("%s - Unsupported CodecConfiguration=%s", __func__,
            toString(codecConfig).c_str());
  ...
  switch (pcmConfig.bitsPerSample) {
    ...
    case BitsPerSample::BITS_24:
      ...
      if (codecConfig.encodedDataConfiguration.codecType == CodecType::APTX_HD
  ...
  switch (pcmConfig.channelMode) {
    ...
    case ChannelMode::STEREO:
      // assume those channel are supported by all codecs
      break;
    default:
      ALOGW("%s - Unsupported CodecConfiguration=%s", __func__,
            toString(codecConfig).c_str());
...
void A2dpLegacyAudioProvider::updateStreamTracksMetadata(const struct source_metadata *source_metadata) {
  if (has_session_ && stack_iface_) {
    ALOGI("%s - Metadata is no needed for SessionType=%s(0x%02hhx)", __func__,
          toString(mSessionType).c_str(), mSessionType);
  } else
    ALOGW("%s - provider has NO session", __func__);

  // [SEG_BT] FIXME: for test only and remember to remove after HearingAids added
