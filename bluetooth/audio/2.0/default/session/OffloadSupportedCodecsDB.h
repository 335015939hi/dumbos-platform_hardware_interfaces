...
extern const std::vector<CodecCapability> kDefaultOffloadA2dpCodecCapabilities;

inline const std::vector<CodecCapability> GetOffloadCodecCapabilities(
    const SessionType& session_type) {
  if (session_type == SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    return kDefaultOffloadA2dpCodecCapabilities;
  }
