...
using ::android::hardware::bluetooth::audio::V2_0::implementation::
...
bool bt_audio_get_presentation_position(const SessionType& sessionType,
  ...
  BluetoothAudioProvider* provider =
      BluetoothAudioProvidersFactory::getProvider(sessionType);
  if (provider != nullptr) {
    return provider->getStreamPresentationPosition(
        remote_delay_report, total_bytes_readed, data_position);
  }
  return false;
}
...
uint16_t bt_audio_set_port_ctrl_result_cb(
  ...
  if (provider != nullptr && ctrl_res_cb != nullptr &&
    ...
    return provider->registerControlResultCback(ctrl_res_cb,
                                                session_changed_cb);
...
void bt_audio_reset_port_ctrl_result_cb(const SessionType& sessionType,
  ...
  BluetoothAudioProvider* provider =
      BluetoothAudioProvidersFactory::getProvider(sessionType);
  if (provider != nullptr) provider->unregisterControlResultCback(ctrl_key);
}
