...
int bt_audio_set_port_ctrl_result_cb(const SessionType& sessionType,
  ...
  if (provider != nullptr && res_cb != nullptr && end_cb != nullptr)
    return provider->registerControlResultCback(res_cb, end_cb);
  return -1;
}

void bt_audio_reset_port_ctrl_result_cb(const SessionType& sessionType, const int& key) {
  BluetoothAudioProvider* provider = BluetoothAudioProvidersFactory::getProvider(sessionType);
  if (provider != nullptr)
    provider->unregisterControlResultCback(key);
}
