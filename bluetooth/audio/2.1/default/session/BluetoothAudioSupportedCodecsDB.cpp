...
static const PcmParameters_2_1 kDefaultSoftwarePcmCapabilities = {
...
std::vector<PcmParameters_2_1> GetSoftwarePcmCapabilities() {
  return std::vector<PcmParameters_2_1>(1, kDefaultSoftwarePcmCapabilities);
}

std::vector<CodecCapabilities> GetOffloadCodecCapabilities(
    const SessionType_2_1& session_type) {
  if (session_type != SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
...
bool IsOffloadCodecConfigurationValid(const SessionType_2_1& session_type,
  ...
  if (session_type != SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    LOG(ERROR) << __func__
               << ": Invalid SessionType_2_1=" << toString(session_type);
    return false;
