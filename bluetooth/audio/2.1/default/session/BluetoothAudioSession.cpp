...
const CodecConfiguration BluetoothAudioSession::kInvalidCodecConfiguration = {
    ...
    .config = {}};
AudioConfiguration_2_1 BluetoothAudioSession::invalidSoftwareAudioConfiguration_2_1 =
    {};
AudioConfiguration_2_1 BluetoothAudioSession::invalidOffloadAudioConfiguration_2_1 = {};
...
BluetoothAudioSession::BluetoothAudioSession(const SessionType_2_1& session_type)
    : session_type_(session_type), stack_iface_(nullptr), mDataMQ(nullptr) {
  invalidSoftwareAudioConfiguration_2_1.pcmConfig(kInvalidPcmParameters_2_1);
  invalidOffloadAudioConfiguration_2_1.codecConfig(kInvalidCodecConfiguration);
}
...
void BluetoothAudioSession::OnSessionStarted(
  ...
  if (stack_iface == nullptr) {
    ...
    LOG(ERROR) << __func__ << " - SessionType=" << toString(session_type_)
               << ", AudioConfiguration_2_1=" << toString(audio_config)
               << " Invalid";
    ...
    audio_config_ =
        (session_type_ == SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH
             ? kInvalidOffloadAudioConfiguration_2_1
             : kInvalidSoftwareAudioConfiguration_2_1);
  } else {
...
void BluetoothAudioSession::OnSessionEnded() {
  ...
  LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_);
  audio_config_ = (session_type_ == SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH
                       ? kInvalidOffloadAudioConfiguration_2_1
                       : kInvalidSoftwareAudioConfiguration_2_1);
...
const AudioConfiguration_2_1& BluetoothAudioSession::GetAudioConfig() {
  std::lock_guard<std::recursive_mutex> guard(mutex_);
  if (IsSessionReady()) {
    return audio_config_;
  } else if (session_type_ == SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    return kInvalidOffloadAudioConfiguration_2_1;
  } else {
    return kInvalidSoftwareAudioConfiguration_2_1;
  }
}
...
void BluetoothAudioSession::UpdateTracksMetadata(
  ...
  LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_) << ", "
            << track_count << " track(s)";
  if (session_type_ == SessionType_2_1::A2DP_SOFTWARE_ENCODING_DATAPATH ||
      session_type_ == SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    return;
  }
