...
class BluetoothAudioSessionReport {
 public:
  ...
  static void OnSessionStarted(const SessionType_2_1& session_type,
  ...
  // inform registered bluetooth_audio outputs
  static void OnSessionEnded(const SessionType_2_1& session_type) {
    std::shared_ptr<BluetoothAudioSession> session_ptr =
        BluetoothAudioSessionInstance::GetSessionInstance(session_type);
    if (session_ptr != nullptr) {
  ...
  static void ReportControlStatus(const SessionType_2_1& session_type,
