...
Return<void> BluetoothAudioProvidersFactory::openProvider_2_1(
    const SessionType_2_1 sessionType, openProvider_cb _hidl_cb) {
  LOG(INFO) << __func__ << " - SessionType_2_1=" << toString(sessionType);
  BluetoothAudioStatus status = BluetoothAudioStatus::SUCCESS;
  BluetoothAudioProvider* provider = nullptr;
  switch (sessionType) {
    case SessionType_2_1::A2DP_SOFTWARE_ENCODING_DATAPATH:
      provider = &a2dp_software_provider_instance_;
      break;
    case SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH:
      provider = &a2dp_offload_provider_instance_;
      break;
    case SessionType_2_1::HEARING_AID_SOFTWARE_ENCODING_DATAPATH:
      provider = &hearing_aid_provider_instance_;
      break;
    case SessionType_2_1::LE_AUDIO_SOFTWARE_ENCODING_DATAPATH:
  ...
  if (provider == nullptr || !provider->isValid(sessionType)) {
    provider = nullptr;
    status = BluetoothAudioStatus::FAILURE;
    LOG(ERROR) << __func__ << " - SessionType_2_1=" << toString(sessionType)
...
Return<void> BluetoothAudioProvidersFactory::getProviderCapabilities_2_1(
    const SessionType_2_1 sessionType, getProviderCapabilities_cb _hidl_cb) {
  hidl_vec<AudioCapabilities> audio_capabilities =
      hidl_vec<AudioCapabilities>(0);
  if (sessionType == SessionType_2_1::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
  ...
  } else if (sessionType != SessionType_2_1::UNKNOWN) {
    std::vector<PcmParameters_2_1> db_pcm_capabilities =
