...
using ::android::hardware::bluetooth::audio::V2_2::LeAudioCodecCapabilities;
...
std::vector<hardware::bluetooth::audio::V2_2::LeAudioCodecCapabilities>
...
std::vector<Lc3Parameters> supportedLc3CapabilityList = {
    kLc3Capability_48_4, kLc3Capability_16_2, kLc3Capability_16_1};

static AudioLocation stereo = static_cast<AudioLocation>(
    AudioLocation::FRONT_LEFT | AudioLocation::FRONT_RIGHT);
static AudioLocation mono = AudioLocation::UNKNOWN;

// Stores the supported setting of audio location, connected device, and the
// channel count for each device
std::vector<std::tuple<AudioLocation, uint8_t, uint8_t>>
    supportedDeviceSetting = {std::make_tuple(stereo, 2, 1),
                              std::make_tuple(stereo, 1, 2),
                              std::make_tuple(mono, 1, 1)};
    ...
    for (auto [audioLocation, deviceCnt, channelCount] :
      ...
      for (auto capability : supportedLc3CapabilityList) {
        ...
        LeAudioCodecCapability lc3MonoCapability =
            composeLc3Capability(mono, 1, 1, capability);
