...
static std::vector<LeAudioCodecCapabilities> kDefaultOffloadLeAudioCapabilities;
...
std::vector<AudioLocation> supportedAudioLocation = {
...
std::vector<LeAudioCodecCapabilities> GetLeAudioOffloadCodecCapabilities(
  ...
  if (kDefaultOffloadLeAudioCapabilities.empty()) {
    for (auto capability : supportedLc3CapabilityList) {
      for (auto audioLocation : supportedAudioLocation) {
        kDefaultOffloadLeAudioCapabilities.push_back(
            {.codecType = CodecType::LC3,
             .mode = LeAudioMode::UNICAST,
             .supportedChannel = audioLocation,
             // for mono case, channel count should be 1.
             .channelCount = static_cast<uint32_t>(
                 std::bitset<8>((int)audioLocation).count() ?: 1),
