...
static std::vector<LeAudioCodecCapabilitiesSetting>
    kDefaultLeAudioCodecCapabilities;

std::vector<LeAudioCodecCapabilitiesSetting>
BluetoothLeAudioCodecsProvider::GetLeAudioCodecCapabilities() {
  if (!kDefaultLeAudioCodecCapabilities.empty()) {
    return kDefaultLeAudioCodecCapabilities;
  }
...
AudioLocation BluetoothLeAudioCodecsProvider::GetAudioLocation(
    const std::string& audio_location_str) {
  if (audio_location_str == "STEREO") {
    return kStereoAudio;
  } else if (audio_location_str == "MONO") {
    return kMonoAudio;
  }
...
CodecType BluetoothLeAudioCodecsProvider::GetCodecType(
    const std::string& codec_type_str) {
  if (codec_type_str == "LC3") {
...
bool BluetoothLeAudioCodecsProvider::IsValidStrategyConfiguration(
  ...
  if (strategy_configuration.getAudioLocation() == "STEREO") {
  ...
  } else if (strategy_configuration.getAudioLocation() == "MONO") {
