...
inline AudioConfiguration to_aidl_audio_config(
  ...
  AudioConfiguration aidl_audio_config;
  if (audio_config.getDiscriminator() ==
      AudioConfig_2_1::hidl_discriminator::pcmConfig) {
    aidl_audio_config.set<AudioConfiguration::pcmConfig>(
        to_aidl_pcm_config(audio_config.pcmConfig()));
  } else if (audio_config.getDiscriminator() ==
             AudioConfig_2_1::hidl_discriminator::codecConfig) {
    aidl_audio_config.set<AudioConfiguration::a2dpConfig>(
        to_aidl_codec_config(audio_config.codecConfig()));
  }
  return aidl_audio_config;
}
