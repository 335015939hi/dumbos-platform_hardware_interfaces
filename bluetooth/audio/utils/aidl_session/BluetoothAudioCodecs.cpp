...
static const PcmCapabilities kDefaultSoftwarePcmCapabilities = {
    ...
    .channelMode =
        static_cast<ChannelMode>(static_cast<int8_t>(ChannelMode::MONO) |
                                 static_cast<int8_t>(ChannelMode::STEREO)),
...
static const SbcCapabilities kDefaultOffloadSbcCapability = {
    .sampleRateHz = {44100},
    .channelMode = static_cast<SbcChannelMode>(
        static_cast<int8_t>(SbcChannelMode::MONO) |
        static_cast<uint8_t>(SbcChannelMode::JOINT_STEREO)),
...
static const LdacCapabilities kDefaultOffloadLdacCapability = {
    ...
    .channelMode = static_cast<LdacChannelMode>(
        static_cast<int8_t>(LdacChannelMode::DUAL) |
        static_cast<int8_t>(LdacChannelMode::STEREO)),
...
template <class T>
bool BluetoothAudioCodecs::ContainedInBitmask(const T& bitmask,
                                              const T& target) {
  uint32_t bitmask_flat = static_cast<uint32_t>(bitmask);
  uint32_t target_flat = static_cast<uint32_t>(target);

  // Check if target only contains 1 bit
  if (target_flat == 0 || ((target_flat & (target_flat - 1)) != 0)) {
    return false;
  }
  return ((bitmask_flat & target_flat) > 0);
}
...
bool BluetoothAudioCodecs::IsOffloadSbcConfigurationValid(
  ...
  if (ContainedInVector(kDefaultOffloadSbcCapability.sampleRateHz,
      ...
      ContainedInBitmask(kDefaultOffloadSbcCapability.channelMode,
                         sbc_data.channelMode) &&
      ContainedInBitmask(kDefaultOffloadSbcCapability.allocMethod,
...
bool BluetoothAudioCodecs::IsOffloadAacConfigurationValid(
  ...
  if (ContainedInVector(kDefaultOffloadAacCapability.sampleRateHz,
      ...
      ContainedInBitmask(kDefaultOffloadAacCapability.channelMode,
                         aac_data.channelMode) &&
      ContainedInBitmask(kDefaultOffloadAacCapability.objectType,
                         aac_data.objectType) &&
...
bool BluetoothAudioCodecs::IsOffloadLdacConfigurationValid(
  ...
  if (ContainedInVector(kDefaultOffloadLdacCapability.sampleRateHz,
      ...
      ContainedInBitmask(kDefaultOffloadLdacCapability.channelMode,
                         ldac_data.channelMode) &&
      ContainedInBitmask(kDefaultOffloadLdacCapability.qualityIndex,
...
bool BluetoothAudioCodecs::IsOffloadAptxConfigurationValid(
  ...
  if (ContainedInVector(kDefaultOffloadAptxCapability.sampleRateHz,
      ...
      ContainedInBitmask(kDefaultOffloadAptxCapability.channelMode,
...
bool BluetoothAudioCodecs::IsOffloadAptxHdConfigurationValid(
  ...
  if (ContainedInVector(kDefaultOffloadAptxHdCapability.sampleRateHz,
      ...
      ContainedInVector(kDefaultOffloadAptxHdCapability.bitsPerSample,
                        aptx_data.bitsPerSample) &&
      ContainedInBitmask(kDefaultOffloadAptxHdCapability.channelMode,
                         aptx_data.channelMode)) {
    return true;
  }
  LOG(WARNING) << __func__
...
bool BluetoothAudioCodecs::IsSoftwarePcmConfigurationValid(
  ...
  if (ContainedInVector(kDefaultSoftwarePcmCapabilities.sampleRateHz,
      ...
      ContainedInBitmask(kDefaultSoftwarePcmCapabilities.channelMode,
...
UnicastCapability composeUnicastLc3Capability(
  ...
  UnicastCapability ucast_capability{.codecType = CodecType::LC3,
                                     .supportedChannel = audioLocation,
                                     .deviceCount = deviceCnt,
                                     .channelCountPerDevice = channelCount};
  ucast_capability.leAudioCodecCapabilities
      .set<UnicastCapability::LeAudioCodecCapabilities::lc3Capabilities>(
          capability);
  return ucast_capability;
}
