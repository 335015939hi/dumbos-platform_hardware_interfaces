...
static const std::vector<Lc3Capabilities> supportedAptxAdaptiveLeCapabilityList = {
  ...
  if (kDefaultOffloadLeAudioCapabilities.empty()) {
    ...
    for (auto [audioLocation, deviceCnt, channelCount] :
      ...
      for (auto capability : supportedAptxAdaptiveLeCapabilityList) {
        for (auto codec_type : {CodecType::APTX_ADAPTIVE_LEA, CodecType::APTX_ADAPTIVE_LEX}) {
          if (session_type ==
              SessionType::LE_AUDIO_HARDWARE_OFFLOAD_ENCODING_DATAPATH) {
