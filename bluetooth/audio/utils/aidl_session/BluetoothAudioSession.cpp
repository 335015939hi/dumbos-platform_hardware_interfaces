 ...
 *
 ***/
std::string getLatencyModesStr(const std::vector<LatencyMode> &latencies) {
...
void BluetoothAudioSession::OnSessionStarted(
  ...
  if (stack_iface == nullptr) {
    ...
    LOG(INFO) << __func__ << " - SessionType=" << toString(session_type_)
              << ", AudioConfiguration=" << audio_config.toString()
              << ", LatencyModes =" << getLatencyModesStr(latency_modes_);
    ReportSessionStatus();
  }
}
...
std::vector<LatencyMode> BluetoothAudioSession::GetSupportedLatencyModes() {
  ...
  LOG(INFO) << __func__ << toString(session_type_) <<
                " - Supported LatencyModes =" << 
                getLatencyModesStr(supported_latency_modes);
  LOG(INFO) << __func__ << " - All LatencyModes =" << getLatencyModesStr(latency_modes_);
