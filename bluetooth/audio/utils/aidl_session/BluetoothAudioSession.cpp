...
BluetoothAudioSession::BluetoothAudioSession(const SessionType& session_type)
  ...
  invalidOffloadAudioConfiguration.set<AudioConfiguration::codecConfig>(
...
void BluetoothAudioSession::OnSessionStarted(
  ...
  if (stack_iface == nullptr) {
    ...
    if (session_type_ == SessionType::A2DP_HARDWARE_OFFLOAD_ENCODING_DATAPATH) {
      ParcelableUtils::Update(audio_config_, invalidOffloadAudioConfiguration);
    } else {
      ParcelableUtils::Update(audio_config_, invalidSoftwareAudioConfiguration);
    }
  } else {
...
void BluetoothAudioSession::ReportAudioConfigChanged(
  ...
  std::lock_guard<std::recursive_mutex> guard(mutex_);
  ParcelableUtils::Update(audio_config_, audio_config);
  if (observers_.empty()) {
...
bool BluetoothAudioSession::IsAidlAvailable() {
  if (is_aidl_available) return is_aidl_available;
  is_aidl_available =
