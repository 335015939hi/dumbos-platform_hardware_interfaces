  ...
  void GetProviderCapabilitiesHelper(const SessionType& session_type) {
    ...
    switch (session_type) {
      ...
      case SessionType::LE_AUDIO_BROADCAST_HARDWARE_OFFLOAD_ENCODING_DATAPATH: {
        bool offload_enable =
            property_get_bool("ro.bluetooth.leaudio_offload.supported",
                              false) &&
            !property_get_bool("persist.bluetooth.leaudio_offload.disabled",
                               true);
        ASSERT_TRUE(offload_enable && !temp_provider_capabilities_.empty() ||
                    !offload_enable && temp_provider_capabilities_.empty());
        for (auto audio_capability : temp_provider_capabilities_) {
          ASSERT_EQ(audio_capability.getTag(),
