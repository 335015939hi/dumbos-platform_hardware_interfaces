...
<<<<<<< PATCH SET (2489e6 Add VTS for HFP sessions)
=======
using aidl::android::hardware::bluetooth::audio::CodecInfo;
>>>>>>> BASE      (956a28 Add HFP to BluetoothAudioSession)
...
class BluetoothAudioProviderHfpSoftwareEncodingAidl
 ...
 public:
  ...
  bool canOpenSession(int32_t sample_rate, int8_t bits_per_sample,
                      ChannelMode channel_mode) {
    PcmConfiguration pcm_config{
        .sampleRateHz = sample_rate,
        .channelMode = channel_mode,
    ...
    if (aidl_retval.isOk() != is_codec_config_valid) return false;
    if (is_codec_config_valid && !data_mq.isValid()) return false;
    return true;
  }
};
  ...
  for (auto sample_rate : hfp_sample_rates_) {
    for (auto bits_per_sample : hfp_bits_per_samples_) {
      for (auto channel_mode : hfp_channel_modes_) {
        EXPECT_TRUE(canOpenSession(sample_rate, bits_per_sample, channel_mode));
        EXPECT_TRUE(audio_provider_->endSession().isOk());
      }
    }
  }
...
class BluetoothAudioProviderHfpHardwareAidl
 ...
 public:
  virtual void SetUp() override {
    BluetoothAudioProviderFactoryAidl::SetUp();
    GetProviderCapabilitiesHelper(SessionType::HFP_HARDWARE_OFFLOAD_DATAPATH);
  ...
  bool canOpenSession(CodecId codec_id, int connection_handle, bool nrec,
...
TEST_P(BluetoothAudioProviderHfpHardwareAidl,
  ...
  if (!IsOffloadSupported()) return;
  // Try to open with a sample configuration
  EXPECT_TRUE(canOpenSession(CodecId::Core::CVSD, 6, false, true));
  EXPECT_TRUE(audio_provider_->endSession().isOk());
}
