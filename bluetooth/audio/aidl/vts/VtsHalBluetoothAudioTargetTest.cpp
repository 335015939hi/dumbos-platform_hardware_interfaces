...
static constexpr CodecType a2dp_codec_types[] = {
    CodecType::UNKNOWN, CodecType::SBC,  CodecType::AAC, CodecType::APTX,
    CodecType::APTX_HD, CodecType::LDAC, CodecType::LC3};
...
template <typename T>
std::vector<T> extract_values_from_bitmkask(T bitmasks, uint32_t bitfield,
                                            bool supported) {
  std::vector<T> retval;
  if (!supported) {
  ...
  while (test_bit <= static_cast<uint32_t>(bitmasks) && test_bit <= bitfield) {
    if ((bitfield & test_bit)) {
      if ((!(static_cast<uint32_t>(bitmasks) & test_bit) && !supported) ||
          ((static_cast<uint32_t>(bitmasks) & test_bit) && supported)) {
  ...
  for (auto sample_rate : a2dp_sample_rates) {
    for (auto bits_per_sample : a2dp_bits_per_samples) {
      for (auto channel_mode : a2dp_channel_modes) {
        ...
        std::unique_ptr<DataMQ> data_mq(new DataMQ(mq_desc));

        EXPECT_EQ(aidl_retval.isOk(), is_codec_config_valid);
        if (is_codec_config_valid) {
          EXPECT_TRUE(data_mq != nullptr && data_mq->isValid());
        }
...
class BluetoothAudioProviderA2dpHardwareAidl
 ...
 public:
  ...
  GetLc3CodecSpecificSupportedList(bool supported) {
    ...
    if (temp_codec_capabilities_.codecType != CodecType::LC3) {
      return lc3_codec_specifics;
    }
    // parse the capability
    auto& lc3_capability =
...
TEST_P(BluetoothAudioProviderLeAudioOutputHardwareAidl,
  ...
  UnicastConfiguration unicast_config = {
      .peerDelay = 0,
  };

  for (auto& lc3_config : lc3_codec_configs) {
    unicast_config.leAudioCodecConfig.set<LeAudioCodecConfiguration::lc3Config>(
        lc3_config);
    le_audio_config.modeConfig
        .set<LeAudioConfiguration::LeAudioModeConfig::unicastConfig>(
            unicast_config);
    DataMQDesc mq_desc;
    auto aidl_retval = audio_provider_->startSession(
