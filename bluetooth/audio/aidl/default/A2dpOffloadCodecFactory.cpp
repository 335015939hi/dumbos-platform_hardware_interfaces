...
A2dpOffloadCodecFactory::A2dpOffloadCodecFactory() :
  ...
  if (kEnableAac)
    ranked_codecs_.push_back(A2dpOffloadCodecAac::GetInstance());
  if (kEnableSbc)
    ranked_codecs_.push_back(A2dpOffloadCodecSbc::GetInstance());
}

const A2dpOffloadCodec* A2dpOffloadCodecFactory::GetCodec(CodecId id) const {

  auto codec = std::find_if(
    begin(ranked_codecs_), end(ranked_codecs_),
...
bool A2dpOffloadCodecFactory::GetConfiguration(
  ...
  auto hinted_codec = std::find_if(
    begin(ranked_codecs_), end(ranked_codecs_),
    [&](auto c){ return hint.codecId == c->info.id; });

  if (hinted_codec != end(ranked_codecs_))
    codecs.push_back(*hinted_codec);

  std::copy_if(
    begin(ranked_codecs_), end(ranked_codecs_), std::back_inserter(codecs),
    [&](auto c){ return c != *hinted_codec; });

  for (auto codec: codecs) {

    auto rc = std::find_if(
      begin(remote_capabilities), end(remote_capabilities),
      [&](auto& rc__){ return codec->info.id == rc__.id; });

    if ((rc == end(remote_capabilities)) ||
        !codec->BuildConfiguration(
            rc->capabilities, hint.codecParameters,
            &configuration->configuration))
      continue;

    configuration->id = codec->info.id;
    A2dpStatus status = codec->ParseConfiguration(
