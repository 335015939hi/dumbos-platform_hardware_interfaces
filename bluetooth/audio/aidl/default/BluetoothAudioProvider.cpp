...
ndk::ScopedAStatus BluetoothAudioProvider::startSession(
  ...
  std::vector<LatencyMode> free{LatencyMode::FREE};
  return startSessionWithLatencyModes(
      host_if, audio_config, free, _aidl_return);
}
ndk::ScopedAStatus BluetoothAudioProvider::startSessionWithLatencyModes(
    const std::shared_ptr<IBluetoothAudioPort>& host_if,
    const AudioConfiguration& audio_config,
