...
bool LeAudioOffloadAudioProvider::filterMatchedSamplingFreq(
  ...
  if (p != freq_to_support_bitmask_map.end()) {
    if (capability_freq.bitmask & p->second) {
      capability_freq.bitmask &= p->second;
      return true;
    }
  }
  return false;
...
bool LeAudioOffloadAudioProvider::filterMatchedFrameDuration(
  ...
  if (p != fduration_to_support_fduration_map.end())
    if (capability_fduration.bitmask & p->second) {
      capability_fduration.bitmask &= p->second;
      return true;
    }
  return false;
}
...
bool LeAudioOffloadAudioProvider::isCapabilitiesMatchedCodecConfiguration(
  ...
  for (auto& codec_capability : codec_capabilities) {
    ...
    if (cfg == cfg_tag_map.end()) {
      LOG(DEBUG) << __func__
                 << ": Tag not found: " << int32_t(codec_capability.getTag());
      return false;
    }
    ...
    if (codec_capability.getTag() ==
      ...
      if (!isMatchedAudioChannel(
        ...
        LOG(DEBUG) << __func__ << ": unmatched audioChannelAllocation";
        return false;
      }
      ...
      if (!isMatchedCodecFramesPerSDU(
              ...
              codec_capability.get<CodecSpecificCapabilitiesLtv::Tag::
                                       supportedMaxCodecFramesPerSDU>())) {
        LOG(DEBUG) << __func__ << ": unmatched codecFrameBlocksPerSDU";
        return false;
      }
    } else if (codec_capability.getTag() == CodecSpecificCapabilitiesLtv::Tag::
      ...
      if (!isMatchedOctetsPerCodecFrame(
              ...
              codec_capability.get<CodecSpecificCapabilitiesLtv::Tag::
                                       supportedOctetsPerCodecFrame>())) {
        LOG(DEBUG) << __func__ << ": unmatched octetsPerCodecFrame";
        return false;
      }
    }
  }
...
bool LeAudioOffloadAudioProvider::isMatchedAseConfiguration(
  ...
  if (requirement_cfg.codecId.has_value()) {
    ...
    if (!isMatchedValidCodec(setting_cfg.codecId.value(),
      ...
      LOG(INFO) << __func__ << ": Codec not matched";
      return false;
    }
  }
  ...
  if (requirement_cfg.targetLatency ==
    ...
    LOG(INFO) << __func__ << ": target latency not matched: cfg = "
  ...
  for (auto requirement_cfg : requirement_cfg.codecConfiguration) {
    ...
    if (cfg == cfg_tag_map.end()) {
      LOG(INFO) << __func__ << ": required " << int(requirement_cfg.getTag())
...
bool LeAudioOffloadAudioProvider::isMatchedBISConfiguration(
  ...
  if (!isMatchedValidCodec(bis_cfg.codecId, capabilities.codecId)) {
    LOG(DEBUG) << __func__ << ": Not matched codec, settings codec = "
               << bis_cfg.codecId.toString()
               << ", cap codec = " << capabilities.codecId.toString();
    return false;
  }
  if (!isCapabilitiesMatchedCodecConfiguration(
          bis_cfg.codecConfiguration, capabilities.codecSpecificCapabilities)) {
    LOG(DEBUG) << __func__ << ": Unmatched configuration with capability";
    return false;
  }
  return true;
}
...
void LeAudioOffloadAudioProvider::filterRequirementAseDirectionConfiguration(
  ...
  for (auto& requirement : requirements) {
    ...
    for (auto direction_configuration : direction_configurations.value()) {
      ...
      LOG(INFO) << __func__ << ": matched a configuraton";
      valid_direction_configurations.push_back(direction_configuration);
      break;
    }
  }
...
LeAudioOffloadAudioProvider::getCapabilitiesMatchedAseConfigurationSettings(
  ...
  if (valid_direction_configuration.empty()) {
    LOG(WARNING) << __func__
                 << ": No valid configuration matched with capability";
    return std::nullopt;
  }
...
LeAudioOffloadAudioProvider::getRequirementMatchedAseConfigurationSettings(
  ...
  if (requirement.sinkAseRequirement.has_value()) {
    LOG(INFO) << __func__ << ": Try to match sink";
    ...
    filterRequirementAseDirectionConfiguration(
        ...
        filtered_setting.sinkAseConfiguration.value());

    if (!filtered_setting.sinkAseConfiguration.value().empty()) {
      LOG(INFO) << __func__ << ": Matched something:";
      for (auto setting : filtered_setting.sinkAseConfiguration.value()) {
        LOG(INFO) << __func__ << ": " << setting.value().toString();
      }
    }

    if (filtered_setting.sinkAseConfiguration.value().empty() ||
        (filtered_setting.sinkAseConfiguration.value().size() !=
         requirement.sinkAseRequirement.value().size())) {
      LOG(DEBUG) << __func__ << ": No match for sink";
      filtered_setting.sinkAseConfiguration = std::nullopt;
    }
  }
  if (requirement.sourceAseRequirement.has_value()) {
    LOG(INFO) << __func__ << ": Try to match source";
    filtered_setting.sourceAseConfiguration =
        std::vector<std::optional<AseDirectionConfiguration>>();
    filterRequirementAseDirectionConfiguration(
        setting.sourceAseConfiguration,
        requirement.sourceAseRequirement.value(),
        filtered_setting.sourceAseConfiguration.value());

    if (!filtered_setting.sourceAseConfiguration.value().empty()) {
      LOG(INFO) << __func__ << ": Matched something:";
      for (auto setting : filtered_setting.sourceAseConfiguration.value()) {
        LOG(INFO) << __func__ << ": " << setting.value().toString();
      }
    }

    if (filtered_setting.sourceAseConfiguration.value().empty() ||
        (filtered_setting.sourceAseConfiguration.value().size() !=
         requirement.sourceAseRequirement.value().size())) {
      LOG(DEBUG) << __func__ << ": No match for source";
      filtered_setting.sourceAseConfiguration = std::nullopt;
    }
  }

  // Ignore when the direction requirement has no valid result
  if ((requirement.sourceAseRequirement.has_value() &&
       !filtered_setting.sourceAseConfiguration.has_value()) ||
      (requirement.sinkAseRequirement.has_value() &&
       !filtered_setting.sinkAseConfiguration.has_value()))
    return std::nullopt;
...
ndk::ScopedAStatus LeAudioOffloadAudioProvider::getLeAudioAseConfiguration(
  ...
  if (!in_remoteSinkAudioCapabilities.has_value() &&
    ...
    LOG(WARNING) << __func__
                 << ": Source and sink capabilities cannot be both nullopt";
    return ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_ARGUMENT);
  }
  ...
  LOG(INFO) << __func__
            << ": Some setting matched, start matching requirement...";
  ...
  for (auto& requirement : in_requirements) {
    ...
    // TODO: Debug print for that specific context only
...
bool LeAudioOffloadAudioProvider::isMatchedQosRequirement(
  ...
  if (setting_qos.retransmissionNum !=
    ...
    LOG(INFO) << __func__ << ": retransmissionNum don't match, setting = "
              << setting_qos.retransmissionNum
              << ", req = " << requirement_qos.preferredRetransmissionNum;
    return false;
  }
  if (setting_qos.maxTransportLatencyMs >
      requirement_qos.maxTransportLatencyMs) {
    LOG(INFO) << __func__ << ": maxTransportLatencyMs too small, setting = "
              << setting_qos.maxTransportLatencyMs
              << ", req = " << requirement_qos.maxTransportLatencyMs;
    return false;
  }
...
LeAudioOffloadAudioProvider::getLeAudioBroadcastConfiguration(
  ...
  for (auto& setting : broadcast_settings) {
    LOG(INFO) << __func__
              << ": Local broadcast settings: " << setting.toString();
    for (auto& capability : in_remoteSinkAudioCapabilities.value()) {
      if (!capability.has_value()) continue;
