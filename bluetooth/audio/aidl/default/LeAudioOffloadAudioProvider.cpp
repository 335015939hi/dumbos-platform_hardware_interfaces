...
LeAudioOffloadAudioProvider::
  ...
  std::vector<IBluetoothAudioProvider::LeAudioBroadcastSubgroupConfiguration>
      filter_subgroup;
  for (auto& p : setting.subgroupsConfigurations) {
    std::vector<IBluetoothAudioProvider::LeAudioSubgroupBisConfiguration>
        filtered_bis_cfg;
    for (auto& q : p.bisConfigurations)
      if (isMatchedBISConfiguration(q.bisConfiguration, capabilities)) {
        filtered_bis_cfg.push_back(q);
      }
...
LeAudioOffloadAudioProvider::getLeAudioBroadcastConfiguration(
  ...
  if (filtered_settings.empty()) return ndk::ScopedAStatus::ok();
