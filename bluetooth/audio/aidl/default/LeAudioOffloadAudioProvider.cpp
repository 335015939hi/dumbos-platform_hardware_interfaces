...
void LeAudioOffloadAudioProvider::getBroadcastSettings() {
  ...
  for (auto& codec_info : db_codec_info) {
    ...
    // Logic are copied from broadcast_configuration_provider.h
    // These default settings are lc3_mono_16_2_2
...
LeAudioOffloadAudioProvider::getLeAudioBroadcastConfiguration(
  ...
  return_setting.retransmitionNum = 4;
  return_setting.maxTransportLatencyMs = 60;
  return_setting.sduIntervalUs = 10000;
  return_setting.maxSduOctets = 40;

  // Populate other settings base on context
  auto context =
      in_requirement.subgroupConfigurationRequirements[0].audioContext;
