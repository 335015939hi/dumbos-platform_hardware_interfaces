...
constexpr A2dpBits::Range kObjectType(kObjectTypeMpeg2AacLc,
                                      kObjectTypeMpeg4AacEldV2);
constexpr A2dpBits::Range kSamplingFrequency(kSamplingFrequency8000,
...
bool A2dpOffloadCodecAac::BuildConfiguration(
    ...
    const CodecParameters& hint, std::vector<uint8_t>* configuration) const {
  auto& a2dp_info = info_.transport.get<CodecInfo::Transport::Tag::a2dp>();
  ...
  config.set(kBitrate, hint.maxBitrate > 0 && bitrate
                           ? std::min(hint.maxBitrate, bitrate)
                       : hint.maxBitrate > 0 ? hint.maxBitrate
                                             : bitrate);
