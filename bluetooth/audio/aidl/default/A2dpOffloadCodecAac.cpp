...
enum : int {
  kObjectTypeMpeg2AacLc       =  0,
  kObjectTypeMpeg4AacLc       =  1,
  kObjectTypeMpeg4AacLtp      =  2,
  kObjectTypeMpeg4AacScalable =  3,
  kObjectTypeMpeg4AacHeV1     =  4,
  kObjectTypeMpeg4AacHeV2     =  5,
  kObjectTypeMpeg4AacEldV2    =  6,

  kDrcEnable                  =  7,

  kSamplingFrequency8000      =  8,
  kSamplingFrequency11025     =  9,
  kSamplingFrequency12000     = 10,
  kSamplingFrequency16000     = 11,
  kSamplingFrequency22050     = 12,
  kSamplingFrequency24000     = 13,
  kSamplingFrequency32000     = 14,
  kSamplingFrequency44100     = 15,
  kSamplingFrequency48000     = 16,
  kSamplingFrequency64000     = 17,
  kSamplingFrequency88200     = 18,
  kSamplingFrequency96000     = 19,
  ...
  kCapabilitiesSize           = 48/8,
};

constexpr A2dpBits::Range kObjectType(kObjectTypeMpeg2AacLc, kObjectTypeMpeg4AacEldV2);
constexpr A2dpBits::Range kSamplingFrequency(kSamplingFrequency8000, kSamplingFrequency96000);
constexpr A2dpBits::Range kChannels(kChannels1, kChannels71);
constexpr A2dpBits::Range kBitrate(kBitrateFirstBit, kBitrateLastBit);
...
static AacParameters::ObjectType GetObjectTypeEnum(int object_type) {
  switch (object_type) {
    case kObjectTypeMpeg2AacLc: return AacParameters::ObjectType::MPEG2_AAC_LC;
    case kObjectTypeMpeg4AacLc:
    default: return AacParameters::ObjectType::MPEG4_AAC_LC;
  }
}

static int GetSamplingFrequencyBit(int32_t sampling_frequency) {
  switch (sampling_frequency) {
    case  8000: return kSamplingFrequency8000 ;
    case 11025: return kSamplingFrequency11025;
    case 12000: return kSamplingFrequency12000;
    case 16000: return kSamplingFrequency16000;
    case 22050: return kSamplingFrequency22050;
    case 24000: return kSamplingFrequency24000;
    ...
    case 64000: return kSamplingFrequency64000;
    case 88200: return kSamplingFrequency88200;
    case 96000: return kSamplingFrequency96000;
    default: return -1;
  }
}
...
static int32_t GetSamplingFrequencyValue(int sampling_frequency) {
  switch (sampling_frequency) {
    case kSamplingFrequency8000 : return  8000;
    case kSamplingFrequency11025: return 11025;
    case kSamplingFrequency12000: return 12000;
    case kSamplingFrequency16000: return 16000;
    case kSamplingFrequency22050: return 22050;
    case kSamplingFrequency24000: return 24000;
    ...
    case kSamplingFrequency64000: return 64000;
    case kSamplingFrequency88200: return 88200;
    case kSamplingFrequency96000: return 96000;
    default: return 0;
  }
}
...
static int GetChannelsBit(ChannelMode channel_mode) {
  switch (channel_mode) {
    case ChannelMode::MONO:   return kChannels1;
    case ChannelMode::STEREO: return kChannels2;
    default: return -1;
  }
}
...
static ChannelMode GetChannelModeEnum(int channel_mode) {
  switch (channel_mode) {
    case kChannels1: return ChannelMode::MONO;
    case kChannels2: return ChannelMode::STEREO;
  ...
  info_({ .id = CodecId(CodecId::A2dp::AAC), .name = "AAC" })
{
  ...
  for (auto v: {  8000, 11025, 12000, 16000, 22050, 24000,
  ...
  for (auto v: { ChannelMode::MONO, ChannelMode::STEREO })
    if (capabilities.get(GetChannelsBit(v)))
  ...
  std::vector<uint8_t>* configuration) const
{
  ...
  if (sf_hint >= 0 && lcaps.get(sf_hint) && rcaps.get(sf_hint))
    config.set(sf_hint);
  else if (lcaps.get(kSamplingFrequency96000) && rcaps.get(kSamplingFrequency96000))
    config.set(kSamplingFrequency96000);
  else if (lcaps.get(kSamplingFrequency88200) && rcaps.get(kSamplingFrequency88200))
  ...
  auto ch_hint = GetChannelsBit(hint.channelMode);
  ...
  if (hint.minBitrate == 0)
    config.set(kVbrSupported, lcaps.get(kVbrSupported) && rcaps.get(kVbrSupported));

  int32_t bitrate = lcaps.get(kBitrate);

  config.set(kBitrate,
    hint.maxBitrate > 0 && bitrate ? std::min(hint.maxBitrate, bitrate) :
    hint.maxBitrate > 0 ? hint.maxBitrate : bitrate);
