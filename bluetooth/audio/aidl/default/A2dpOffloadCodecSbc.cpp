...
enum {
  kSamplingFrequency16000   =  0,
  kSamplingFrequency32000   =  1,
  kSamplingFrequency44100   =  2,
  kSamplingFrequency48000   =  3,

  kChannelModeMono          =  4,
  kChannelModeDualChannel   =  5,
  kChannelModeStereo        =  6,
  kChannelModeJointStereo   =  7,
  ...
  kAllocationMethodSnr      = 14,
  kAllocationMethodLoudness = 15,

  kMinimumBitpoolFirstBit   = 16,
  kMinimumBitpoolLastBit    = 23,

  kMaximumBitpoolFirstBit   = 24,
  kMaximumBitpoolLastBit    = 31,

  kCapabilitiesSize         = 32/8,
};

constexpr A2dpBits::Range kSamplingFrequency(kSamplingFrequency16000, kSamplingFrequency48000);
constexpr A2dpBits::Range kChannelMode(kChannelModeMono, kChannelModeJointStereo);
constexpr A2dpBits::Range kBlockLength(kBlockLength4, kBlockLength16);
constexpr A2dpBits::Range kSubbands(kSubbands8, kSubbands4);
constexpr A2dpBits::Range kAllocationMethod(kAllocationMethodSnr, kAllocationMethodLoudness);
constexpr A2dpBits::Range kMinimumBitpool(kMinimumBitpoolFirstBit, kMinimumBitpoolLastBit);
constexpr A2dpBits::Range kMaximumBitpool(kMaximumBitpoolFirstBit, kMaximumBitpoolLastBit);
...
static int GetSamplingFrequencyBit(int32_t sampling_frequency) {
  switch (sampling_frequency) {
    case 16000: return kSamplingFrequency16000;
    case 32000: return kSamplingFrequency32000;
    case 44100: return kSamplingFrequency44100;
    case 48000: return kSamplingFrequency48000;
    default: return -1;
  }
}
...
static int32_t GetSamplingFrequencyValue(int sampling_frequency) {
  switch (sampling_frequency) {
    case kSamplingFrequency16000: return 16000;
    case kSamplingFrequency32000: return 32000;
    case kSamplingFrequency44100: return 44100;
    case kSamplingFrequency48000: return 48000;
    default: return 0;
  }
}
...
static int GetChannelModeBit(ChannelMode channel_mode) {
  switch (channel_mode) {
    case ChannelMode::STEREO:   return kChannelModeJointStereo | kChannelModeStereo;
    case ChannelMode::DUALMONO: return kChannelModeDualChannel;
    case ChannelMode::MONO:     return kChannelModeMono;
    default: return -1;
  }
}
...
static ChannelMode GetChannelModeEnum(int channel_mode) {
  switch (channel_mode) {
    case kChannelModeMono:        return ChannelMode::MONO;
    case kChannelModeDualChannel: return ChannelMode::DUALMONO;
    case kChannelModeStereo:
    case kChannelModeJointStereo: return ChannelMode::STEREO;
    default: return ChannelMode::UNKNOWN;
  }
}

static int32_t GetBlockLengthValue(int block_length) {
  switch (block_length) {
    case kBlockLength4:  return  4;
    case kBlockLength8:  return  8;
    case kBlockLength12: return 12;
    case kBlockLength16: return 16;
    default: return 0;
  }
}
...
static SbcParameters::AllocationMethod GetAllocationMethodEnum(int allocation_method) {
  switch (allocation_method) {
    case kAllocationMethodSnr: return SbcParameters::AllocationMethod::SNR;
    case kAllocationMethodLoudness:
    default: return SbcParameters::AllocationMethod::LOUDNESS;
  }
}

static int32_t GetSamplingFrequencyValue(const A2dpBits& configuration) {
  return GetSamplingFrequencyValue(configuration.find_active_bit(kSamplingFrequency));
}
  ...
  info_({ .id = CodecId(CodecId::A2dp::SBC), .name = "SBC" })
{
  ...
  for (auto v: { ChannelMode::MONO, ChannelMode::DUALMONO, ChannelMode::STEREO })
    if (capabilities.get(GetChannelModeBit(v)))
      channel_modes.push_back(v);
  ...
  SbcParameters *sbc_parameters) const
{
  ...
  if (!lcaps.get(sampling_frequency)) return A2dpStatus::NOT_SUPPORTED_SAMPLING_FREQUENCY;
  ...
  if (!lcaps.get(allocation_method)) return A2dpStatus::NOT_SUPPORTED_ALLOCATION_METHOD;

  /* --- Check Bitpool --- */

  uint8_t min_bitpool = config.get(kMinimumBitpool);
  if (min_bitpool < 2 || min_bitpool > 250) return A2dpStatus::INVALID_MINIMUM_BITPOOL_VALUE;
  if (min_bitpool < lcaps.get(kMinimumBitpool)) return A2dpStatus::NOT_SUPPORTED_MINIMUM_BITPOOL_VALUE;

  uint8_t max_bitpool = config.get(kMaximumBitpool);
  if (max_bitpool < 2 || max_bitpool > 250) return A2dpStatus::INVALID_MAXIMUM_BITPOOL_VALUE;
  if (max_bitpool > lcaps.get(kMaximumBitpool)) return A2dpStatus::NOT_SUPPORTED_MAXIMUM_BITPOOL_VALUE;

  /* --- Return --- */

  codec_parameters->channelMode = GetChannelModeEnum(channel_mode);
  codec_parameters->samplingFrequencyHz = GetSamplingFrequencyValue(sampling_frequency);
...
bool A2dpOffloadCodecSbc::BuildConfiguration(
  ...
  const CodecParameters& hint,
  std::vector<uint8_t>* configuration) const
{
  auto& a2dp_info = info.transport.get<CodecInfo::Transport::Tag::a2dp>();

  if (remote_capabilities.size() != a2dp_info.capabilities.size())
    return false;
  ...
  auto sf_hint = GetSamplingFrequencyBit(hint.samplingFrequencyHz);

  if (sf_hint >= 0 && lcaps.get(sf_hint) && rcaps.get(sf_hint)) 
    config.set(sf_hint);
  else if (lcaps.get(kSamplingFrequency44100) && rcaps.get(kSamplingFrequency44100))
    config.set(kSamplingFrequency44100);
  else if (lcaps.get(kSamplingFrequency48000) && rcaps.get(kSamplingFrequency48000))
  ...
  auto cm_hint = GetChannelModeBit(hint.channelMode);
  ...
  else if (lcaps.get(kChannelModeDualChannel) && rcaps.get(kChannelModeDualChannel))
  ...
  if (lcaps.get(kAllocationMethodLoudness) && rcaps.get(kAllocationMethodLoudness))
  ...
  if (min_bitpool < 2 || min_bitpool > 250 ||
      max_bitpool < 2 || max_bitpool > 250 || min_bitpool > max_bitpool) {
    min_bitpool =   2;
    max_bitpool = 250;
  }
  ...
  min_bitpool = std::max(min_bitpool, GetBitpool(*configuration, hint.minBitrate));
  if (hint.maxBitrate && hint.maxBitrate >= hint.minBitrate)
    max_bitpool = std::min(max_bitpool, GetBitpool(*configuration, hint.maxBitrate));
