...
static int get_bt_dev_sco_usb_alt() {
  bool vendor_bt_dev_sco_usb_alt =
      ::android::base::GetIntProperty("ro.vendor.sco_usb_alt_set", 0);
  if (vendor_bt_dev_sco_usb_alt != -1)
      ALOGD("bt device interface is usb and alternate setting is %d", vendor_bt_dev_interface_usb);
  ...
  if (!sco_connection_handles.empty()) {
    switch(get_bt_dev_sco_usb_alt()) {
        ...
        case 1:
            ...
            break;
        default:
            ALOGW("%s: unknown usb alternate setting", __func__);
            break;
    }
