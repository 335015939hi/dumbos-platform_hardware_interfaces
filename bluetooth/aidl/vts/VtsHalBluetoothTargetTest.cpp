...
class BluetoothAidlTest : public ::testing::TestWithParam<std::string> {
 public:
  virtual void SetUp() override {
    ...
    auto timeout_status = future.wait_for(WAIT_FOR_INIT_TIMEOUT);
...
void BluetoothAidlTest::wait_for_event(bool timeout_is_error = true) {
  if (timeout_is_error) {
    EXPECT_TRUE(event_queue.waitWithTimeout(WAIT_FOR_HCI_EVENT_TIMEOUT));
  } else {
    event_queue.wait();
  }
...
void BluetoothAidlTest::wait_for_command_complete_event(
  ...
  wait_for_event();
  std::vector<uint8_t> event;
  EXPECT_TRUE(event_queue.pop(event));  // should these all be asserts
...
void BluetoothAidlTest::sendAndCheckSCO(int num_packets, size_t size,
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    EXPECT_TRUE(
        sco_queue.tryPopWithTimeout(sco_loopback, WAIT_FOR_SCO_DATA_TIMEOUT));
...
void BluetoothAidlTest::sendAndCheckACL(int num_packets, size_t size,
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    EXPECT_TRUE(
        acl_queue.tryPopWithTimeout(acl_loopback, WAIT_FOR_ACL_DATA_TIMEOUT));
