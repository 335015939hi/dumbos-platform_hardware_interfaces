...
class BluetoothAidlTest : public ::testing::TestWithParam<std::string> {
 public:
  ...
  void handle_sco_loopback_pkt(std::vector<uint8_t>& ScoLoopback,
    size_t size);
...
void BluetoothAidlTest::sendAndCheckSco(int num_packets, size_t size,
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    handle_sco_loopback_pkt(sco_loopback, size);
...
void BluetoothAidlTest::handle_sco_loopback_pkt(std::vector<uint8_t>& ScoLoopback,
        size_t size) {
    std::vector<uint8_t> sco_packet_whole;
    sco_packet_whole.assign(ScoLoopback.begin(), ScoLoopback.end());
    while (size + 3 > sco_packet_whole.size()) {
        std::vector<uint8_t> sco_loopback;
        ASSERT_TRUE(
        sco_queue.tryPopWithTimeout(sco_loopback, kWaitForScoDataTimeout));
        sco_packet_whole.insert(sco_packet_whole.end(), sco_loopback.begin() + 3,
            sco_loopback.end());
    }
    ScoLoopback.assign(sco_packet_whole.begin(), sco_packet_whole.end());
    ScoLoopback[2] = size;
}
