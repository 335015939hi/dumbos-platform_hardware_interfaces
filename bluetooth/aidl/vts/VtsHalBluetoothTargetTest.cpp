...
#define WAIT_FOR_DEFAULT_TIMEOUT std::chrono::milliseconds(1000)
...
class BluetoothAidlTest : public ::testing::TestWithParam<std::string> {
 public:
  virtual void SetUp() override {
    ...
    bluetooth = IBluetoothHci::fromBinder(
    ...
    // Lambda function
    auto onBinderDeath = [](void* /*cookie*/) { FAIL(); };

    bluetooth_hci_death_recipient = AIBinder_DeathRecipient_new(onBinderDeath);
    ASSERT_NE(bluetooth_hci_death_recipient, nullptr);
    ASSERT_EQ(STATUS_OK,
              AIBinder_linkToDeath(bluetooth->asBinder().get(),
    ...
    auto result = bluetooth->initialize(bluetooth_cb);
    bool isOk = result.isOk();
    ASSERT_TRUE(isOk);
    ASSERT_TRUE(bluetooth_cb->waitForBluetoothCallback(WAIT_FOR_INIT_TIMEOUT)
                    .no_timeout);
  ...
  struct WaitForBluetoothCallbackResult {
    bool no_timeout;
  };
  ...
  class BluetoothHciCallbacks
   ...
   public:
    ...
    ndk::ScopedAStatus initializationComplete(Status status) {
      // since promises are meant to only be used once, we reset it each
      // callback
      empty_promise_ = std::promise<void>();
      parent_.initialized = (status == Status::SUCCESS);
      ALOGV("%s (status = %d)", __func__, static_cast<int>(status));
      empty_promise_.set_value();
    ...
    WaitForBluetoothCallbackResult waitForBluetoothCallback(
        std::chrono::milliseconds timeout = WAIT_FOR_DEFAULT_TIMEOUT) {
      auto future = empty_promise_.get_future();
      auto timeout_status = future.wait_for(timeout);
      EXPECT_EQ(timeout_status, std::future_status::ready);
      return {.no_timeout = timeout_status == std::future_status::ready};
    }

   private:
    std::promise<void> empty_promise_;
  };
  ...
  std::queue<std::vector<uint8_t>> iso_queue;
...
void BluetoothAidlTest::handle_no_ops() {
  ...
  while (acl_queue.size() > 0) {
    std::vector<uint8_t> acl_packet = acl_queue.front();
...
void BluetoothAidlTest::wait_for_event(bool timeout_is_error = true) {
  ...
  do {
    bool no_timeout =
        bluetooth_cb->waitForBluetoothCallback(WAIT_FOR_HCI_EVENT_TIMEOUT)
            .no_timeout;
    ASSERT_TRUE(no_timeout || !timeout_is_error);
    if (no_timeout && timeout_is_error) {
      ASSERT_LT(static_cast<size_t>(0), event_queue.size());
    }
    if (event_queue.size() == 0) {
      // waitForBluetoothCallback timed out.
      return;
    }
    handle_no_ops();
  } while (event_queue.size() == 0);
...
void BluetoothAidlTest::sendAndCheckHCI(int num_packets) {
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    for (size_t i = 0; i < compare_length; i++)
      ASSERT_EQ(cmd[i], event[EVENT_FIRST_PAYLOAD_BYTE + i]);
  }
...
void BluetoothAidlTest::sendAndCheckSCO(int num_packets, size_t size,
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    ASSERT_TRUE(
        bluetooth_cb->waitForBluetoothCallback(WAIT_FOR_SCO_DATA_TIMEOUT)
            .no_timeout);
    std::vector<uint8_t> sco_loopback = sco_queue.front();
    sco_queue.pop();
...
void BluetoothAidlTest::sendAndCheckACL(int num_packets, size_t size,
  ...
  for (int n = 0; n < num_packets; n++) {
    ...
    ASSERT_TRUE(
        bluetooth_cb->waitForBluetoothCallback(WAIT_FOR_ACL_DATA_TIMEOUT)
            .no_timeout);
    std::vector<uint8_t> acl_loopback = acl_queue.front();
    acl_queue.pop();
...
void BluetoothAidlTest::enterLoopbackMode(std::vector<uint16_t>* sco_handles,
                                          std::vector<uint16_t>* acl_handles) {
  std::vector<uint8_t> cmd = COMMAND_HCI_WRITE_LOOPBACK_MODE_LOCAL;
  bluetooth->sendHciCommand(cmd);
...
TEST_P(BluetoothAidlTest, LoopbackModeSinglePackets) {
  setBufferSizes();
...
TEST_P(BluetoothAidlTest, LoopbackModeBandwidth) {
  setBufferSizes();
