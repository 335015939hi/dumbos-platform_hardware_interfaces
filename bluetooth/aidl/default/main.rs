...
use bluetoothhcihalproxy::{ HciHalProxy, CInterface };
use log;
use std::panic;

const SERVICE_IDENTIFIER: &str = "android.hardware.bluetooth";
...
fn main() {
    ...
    if binder::add_service(
            &format!("{}/default", SERVICE_IDENTIFIER),
            IBluetoothHci::BnBluetoothHci::new_binder(
                HciHalProxy::new(new_hal()),
                binder::BinderFeatures::default(),
            ).as_binder()).is_ok() {
        binder::ProcessState::join_thread_pool();
    } else {
        log::info!("Could not register as a service!");
    }
}
