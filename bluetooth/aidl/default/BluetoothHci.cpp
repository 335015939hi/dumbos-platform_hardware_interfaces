...
class Hal {
 public:
  ...
  static void Initialize(void *instance, const struct hal_callbacks *callbacks) {
    static_cast<Hal *>(instance)->Initialize(callbacks);
  }

  static void Close(void *instance) {
    static_cast<Hal *>(instance)->Close();
  }

  static void SendCommand(void *instance, const uint8_t *data, size_t len) {
    static_cast<Hal *>(instance)->SendCommand(std::vector<uint8_t>(data, data + len));
  }

  static void SendAcl(void *instance, const uint8_t *data, size_t len) {
      static_cast<Hal *>(instance)->SendAcl(std::vector<uint8_t>(data, data + len));
  }

  static void SendSco(void *instance, const uint8_t *data, size_t len) {
      static_cast<Hal *>(instance)->SendSco(std::vector<uint8_t>(data, data + len));
  }

  static void SendIso(void *instance, const uint8_t *data, size_t len) {
      static_cast<Hal *>(instance)->SendIso(std::vector<uint8_t>(data, data + len));
  }
 ...
 private:
  ...
  void Initialize(const struct hal_callbacks *callbacks) {
    ...
    if (old_state != HalState::READY) {
      ...
      Close();
      mCallbacks->initialization_complete(mCallbacks->handle, STATUS_ALREADY_INITIALIZED);
      return;
    }
    ...
    if (mFd < 0) {
      ...
      if (mFd < 0) {
        ...
        mCallbacks->initialization_complete(mCallbacks->handle, STATUS_UNABLE_TO_OPEN_INTERFACE);
        return;
      }
    }
    ...
    mH4 = std::make_shared<H4Protocol>(
        ...
        [this](const std::vector<uint8_t>& raw_acl) {
          mCallbacks->acl_received(mCallbacks->handle, raw_acl.data(), raw_acl.size());
        },
        [this](const std::vector<uint8_t>& raw_sco) {
          mCallbacks->sco_received(mCallbacks->handle, raw_sco.data(), raw_sco.size());
        },
        [this](const std::vector<uint8_t>& raw_event) {
          mCallbacks->event_received(mCallbacks->handle, raw_event.data(), raw_event.size());
        },
        [this](const std::vector<uint8_t>& raw_iso) {
          mCallbacks->iso_received(mCallbacks->handle, raw_iso.data(), raw_iso.size());
        },
        [this]() {
  ...
  void SendCommand(const std::vector<uint8_t> &data) { send(PacketType::COMMAND, data); }
  void SendAcl(const std::vector<uint8_t> &data) { send(PacketType::ACL_DATA, data); }
  void SendSco(const std::vector<uint8_t> &data) { send(PacketType::SCO_DATA, data); }
  void SendIso(const std::vector<uint8_t> &data) { send(PacketType::ISO_DATA, data); }
