...
size_t VendorInterface::Send(const uint8_t* data, size_t length) {
  ...
  if (!firmware_configured_) {
    ALOGE("%s queueing command", __func__);
  ...
  ALOGE("%s sending queued command", __func__);
  SendPrivate(queued_data_.data(), queued_data_.size());
  queued_data_.resize(0);

  ALOGE("%s done sending queued command", __func__);
...
void VendorInterface::OnDataReady(int fd) {
  switch (hci_parser_state_) {
    case HCI_IDLE: {
      ...
      // workaround(s)
      assert(hci_packet_type >= HCI_PACKET_TYPE_ACL_DATA &&
             hci_packet_type <= HCI_PACKET_TYPE_EVENT);
      if (!(hci_packet_type_ >= HCI_PACKET_TYPE_ACL_DATA &&
            hci_packet_type_ <= HCI_PACKET_TYPE_EVENT))
        ALOGE("%s: bad type %d (from %d)", __func__,
              static_cast<int>(hci_packet_type_), static_cast<int>(buffer[0]));
      else
    ...
    case HCI_TYPE_READY: {
      ...
      if (bytes_read != hci_packet_bytes_remaining_)
        ALOGE("%s: bytes read (%d) != bytes remaining (%d)", __func__,
              static_cast<int>(hci_packet_bytes_read_),
              static_cast<int>(hci_packet_bytes_remaining_));
