...
enum {
  VENDOR_STATE_CLOSED = 1, /*Only Initialize() is valid */
  VENDOR_STATE_OPENING,    /*Nothing valid, Caller must wait for initialize_complete_cb_ */
  VENDOR_STATE_OPENED,     /*All calls except Initialize() valid */
} ;

uint8_t vstate = VENDOR_STATE_CLOSED;
...
bool VendorInterface::Initialize(
  ...
  if (vstate != VENDOR_STATE_CLOSED) {
    ALOGI("%s: VendorInterface is open/opening !", __func__);
    return true;
  }
  guard.unlock();

  ALOGI("%s: VendorInterface::Initialize", __func__);
  return vendor_interface.Open(initialize_complete_cb, event_cb, acl_cb,
                                  sco_cb, iso_cb);
}
...
VendorInterface* VendorInterface::get() { return &vendor_interface; }
...
bool VendorInterface::Open(InitializeCompleteCallback initialize_complete_cb,
                           ...
                           PacketReadCallback iso_cb) {
  {
    ...
    if (vstate == VENDOR_STATE_OPENING) {
...
void VendorInterface::Close() {
  ...
  initialize_complete_cb = initialize_complete_cb_;
