...
uint8_t transmit_cb(uint16_t opcode, void* buffer, tINT_CMD_CBACK callback) {
  ...
  // TODO(!zachoverflow): this waiting internal command code feels sketchy?
  // It will only work if:
  //   * The vendor library can only send one command at a time
  //   * The vendor library doesn't send the same opcode as an outstanding
  //     command from the main stack
  VendorInterface::get()->GetHci()->SetWaitingInternalCommand(opcode, callback);
...
HciProtocol *VendorInterface::GetHci() { return hci_; }
