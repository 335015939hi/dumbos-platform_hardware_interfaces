...
using SendDataCallback = std::function<void(uint8_t*, size_t)>;
using FirmwareConfiguredCallback = std::function<void(bt_vendor_op_result_t)>;

static tINT_CMD_CBACK internal_command_cb;
static SendDataCallback send_data_cb;
static FirmwareConfiguredCallback firmware_configured_cb;
...
VendorInterface::VendorInterface() : uart_fd_(INVALID_FD) {}

VendorInterface::~VendorInterface() {
  fd_watcher_.StopWatchingFileDescriptor();
  if (lib_handle_ == nullptr) return;
  dlclose(lib_handle_);
}
...
bool VendorInterface::open(PacketReadCallback packet_read_cb) {
  ...
  send_data_cb = [this](uint8_t* buffer, size_t size) { send(buffer, size); };
  firmware_configured_cb =
      [this](bt_vendor_op_result_t) {
...
static uint8_t transmit_cb(uint16_t opcode, void* buffer,
                           tINT_CMD_CBACK callback) {
  if (send_data_cb == nullptr) return false;
  ...
  send_data_cb(&type, 1);
  send_data_cb(bt_hdr->data, bt_hdr->len);
  return true;
}

static void firmware_config_cb(bt_vendor_op_result_t result) {
  ALOGD("%s result: %d", __func__, result);
  if (firmware_configured_cb != nullptr) firmware_configured_cb(result);
}
