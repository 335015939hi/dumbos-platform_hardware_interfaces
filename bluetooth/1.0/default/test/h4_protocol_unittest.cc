...
class H4ProtocolTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    protocol_ = new H4Protocol(sockfd[0], packet_read_cb_.AsStdFunction(), &fd_watcher_);
    fake_uart_ = sockfd[1];
  ...
  void WriteAndExpectInboundAclData(char *payload) {
    ...
    WriteAndExpectInbound(preamble, 5, payload);
  }
  ...
  void WriteAndExpectInboundScoData(char *payload) {
    ...
    WriteAndExpectInbound(preamble, 4, payload);
  }
  ...
  void WriteAndExpectInboundEvent(char *payload) {
    ...
    WriteAndExpectInbound(preamble, 3, payload);
  }

  void WriteAndExpectInbound(char *preamble, size_t preamble_size, char *payload) {
    std::mutex mutex;
    std::condition_variable done;
    {
      ...
      EXPECT_CALL(
          packet_read_cb_,
          Call(
              Eq(preamble[0]), // First byte is the type
              HidlVecMatches(preamble + 1, preamble_size - 1, payload)
          )
      ).WillOnce(Notify(&mutex, &done));

      ALOGD("%s writing", __func__);
      TEMP_FAILURE_RETRY(write(fake_uart_, preamble, preamble_size));
