...
MATCHER_P3(HidlVecMatches, preamble, preamble_length, payload, "") {
  ...
  return memcmp(payload, arg.data() + preamble_length, length - preamble_length) == 0;
};
...
class MctProtocolTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    protocol_ = new MctProtocol(mct_fds, packet_read_cb_.AsStdFunction(), &fd_watcher_);
    protocol_->Start();
  }
  ...
  void WriteAndExpectInboundAclData(char *payload) {
    ...
    WriteAndExpectInbound(
        HCI_PACKET_TYPE_ACL_DATA,
        fake_uart_[CH_ACL_IN],
        preamble,
        4,
        payload);
  }
  ...
  void WriteAndExpectInbound(
      uint8_t type, int fake_fd, char *preamble, size_t preamble_size, char *payload) {
    std::mutex mutex;
    std::condition_variable done;
    {
      ...
      EXPECT_CALL(
          ...
          Call(
              Eq(type),
              HidlVecMatches(preamble, preamble_size, payload)
          )
      ).WillOnce(Notify(&mutex, &done));

      ALOGD("%s writing", __func__);
      TEMP_FAILURE_RETRY(write(fake_fd, preamble, preamble_size));
      TEMP_FAILURE_RETRY(write(fake_fd, payload, strlen(payload)));
  ...
  testing::MockFunction<void(HciPacketType, const hidl_vec<uint8_t> &)> packet_read_cb_;
...
TEST_F(MctProtocolTest, TestSendCommand) {
  SendAndReadUartOutbound(HCI_PACKET_TYPE_COMMAND, sample_data1, fake_uart_[CH_CMD]);
}
TEST_F(MctProtocolTest, TestSendAclData) {
  SendAndReadUartOutbound(HCI_PACKET_TYPE_ACL_DATA, sample_data2, fake_uart_[CH_ACL_OUT]);
}

// Ensure we properly parse data coming from the UART
TEST_F(MctProtocolTest, TestReadAclData) {
  WriteAndExpectInboundAclData(acl_data);
}
TEST_F(MctProtocolTest, TestReadEvent) {
