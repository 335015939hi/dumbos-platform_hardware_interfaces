...
#define EVENT_LOCAL_HCI_VERSION_BYTE static_cast<size_t>(EVENT_COMMAND_COMPLETE_FIRST_PARAM_BYTE)
#define EVENT_LOCAL_LMP_VERSION_BYTE EVENT_LOCAL_HCI_VERSION_BYTE + 3

#define EVENT_CONNECTION_COMPLETE_PARAM_LENGTH static_cast<size_t>(11)
#define EVENT_CONNECTION_COMPLETE_TYPE static_cast<size_t>(11)
...
TEST_F(BluetoothHidlTest, HciReset) {
  ...
  do {
    ...
    event_queue.pop();
    EXPECT_GT(event.size(), static_cast<size_t>(EVENT_COMMAND_STATUS_OPCODE_LSBYTE + 1));
...
TEST_F(BluetoothHidlTest, HciVersionTest) {
  ...
  EXPECT_GE(HCI_MINIMUM_HCI_VERSION, event[EVENT_LOCAL_HCI_VERSION_BYTE]);
  EXPECT_GE(HCI_MINIMUM_LMP_VERSION, event[EVENT_LOCAL_LMP_VERSION_BYTE]);

  bluetooth->close();
  EXPECT_EQ(1, event_cb_count);
}
...
TEST_F(BluetoothHidlTest, WriteLoopbackMode) {
  ...
  do {
    ...
    if (event[EVENT_CODE_BYTE] == EVENT_CONNECTION_COMPLETE) {
      EXPECT_GT(event.size(), EVENT_CONNECTION_COMPLETE_TYPE);
  ...
  EXPECT_GE(event_cb_count, 2);
  EXPECT_GE(connection_event_count, 0);
