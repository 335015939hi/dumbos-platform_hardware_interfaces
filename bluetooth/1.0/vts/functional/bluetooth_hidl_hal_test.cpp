...
void BluetoothHidlTest::setBufferSizes() {
  ...
  ALOGD("%s: ACL max %x num %x SCO max %x num %x", __func__,
...
TEST_F(BluetoothHidlTest, WriteLoopbackMode) {
  ...
  hidl_vec<uint16_t> temp_handles;
  enterLoopbackMode(temp_handles, temp_handles);
...
TEST_F(BluetoothHidlTest, LoopbackModeSinglePackets) {
  ...
  if (acl_connection_handles.size() > 0) {
    sendAndCheckACL(1, 480 /* should be max_acl_data_packet_length */,
...
TEST_F(BluetoothHidlTest, LoopbackModeBandwidth) {
  ...
  int acl_packets_sent = 0;
  if (acl_connection_handles.size() > 0) {
    sendAndCheckACL(NUM_ACL_PACKETS_BANDWIDTH,
                    480 /* should be max_acl_data_packet_length */,
                    acl_connection_handles[0]);
