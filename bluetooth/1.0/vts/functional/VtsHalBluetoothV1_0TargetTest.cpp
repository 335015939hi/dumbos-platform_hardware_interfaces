...
#define ACL_PACKET_BOUNDARY_FIRST_AUTO_FLUSHABLE \
    (ACL_PACKET_BOUNDARY_FLAG_FIRST_AUTO_FLUSHABLE << ACL_PACKET_BOUNDARY_FLAG_OFFSET)
...
class ThroughputLogger {
   public:
    ...
    ~ThroughputLogger() {
        ...
        ALOGD("%s %.1f KB/s (%zu bytes in %.3fs)", task_.c_str(), rate_kb, total_bytes_, s);
    }
...
class BluetoothHidlEnvironment : public ::testing::VtsHalHidlTargetTestEnvBase {
   public:
    ...
    virtual void registerTestServices() override { registerTestService<IBluetoothHci>(); }
...
class BluetoothHidlTest : public ::testing::VtsHalHidlTargetTestBase {
   public:
    virtual void SetUp() override {
        ...
        ASSERT_TRUE(bluetooth->linkToDeath(bluetooth_hci_death_recipient, 0).isOk());
        ...
        bluetooth_cb->SetWaitTimeout(kCallbackNameInitializationComplete, WAIT_FOR_INIT_TIMEOUT);
        bluetooth_cb->SetWaitTimeout(kCallbackNameHciEventReceived, WAIT_FOR_HCI_EVENT_TIMEOUT);
        bluetooth_cb->SetWaitTimeout(kCallbackNameAclEventReceived, WAIT_FOR_ACL_DATA_TIMEOUT);
        bluetooth_cb->SetWaitTimeout(kCallbackNameScoEventReceived, WAIT_FOR_SCO_DATA_TIMEOUT);
    ...
    void enterLoopbackMode(std::vector<uint16_t>& sco_handles, std::vector<uint16_t>& acl_handles);
    void handle_no_ops();
    ...
    class BluetoothHciCallbacks : public ::testing::VtsHalHidlTargetCallbackBase<BluetoothHidlTest>,
       ...
       public:
        ...
        Return<void> aclDataReceived(const ::android::hardware::hidl_vec<uint8_t>& data) override {
        ...
        Return<void> scoDataReceived(const ::android::hardware::hidl_vec<uint8_t>& data) override {
...
void BluetoothHidlTest::handle_no_ops() {
    while (event_queue.size() > 0) {
        ...
        EXPECT_GE(event.size(), static_cast<size_t>(EVENT_COMMAND_COMPLETE_STATUS_BYTE));
        bool event_is_no_op = (event[EVENT_CODE_BYTE] == EVENT_COMMAND_COMPLETE) &&
...
void BluetoothHidlTest::wait_for_event(bool timeout_is_error = true) {
    ...
    do {
        bool no_timeout = bluetooth_cb->WaitForCallback(kCallbackNameHciEventReceived).no_timeout;
...
void BluetoothHidlTest::wait_for_command_complete_event(hidl_vec<uint8_t> cmd) {
    ...
    EXPECT_GT(event.size(), static_cast<size_t>(EVENT_COMMAND_COMPLETE_STATUS_BYTE));
...
void BluetoothHidlTest::setBufferSizes() {
    ...
    max_acl_data_packet_length = event[EVENT_COMMAND_COMPLETE_STATUS_BYTE + 1] +
    ...
    ALOGD("%s: ACL max %d num %d SCO max %d num %d", __func__,
          static_cast<int>(max_acl_data_packet_length), static_cast<int>(max_acl_data_packets),
          static_cast<int>(max_sco_data_packet_length), static_cast<int>(max_sco_data_packets));
}
...
void BluetoothHidlTest::sendAndCheckHCI(int num_packets) {
    ...
    for (int n = 0; n < num_packets; n++) {
        ...
        size_t compare_length =
            (cmd.size() > static_cast<size_t>(0xff) ? static_cast<size_t>(0xff) : cmd.size());
...
void BluetoothHidlTest::sendAndCheckSCO(int num_packets, size_t size, uint16_t handle) {
    ThroughputLogger logger = {__func__};
    for (int n = 0; n < num_packets; n++) {
        // Send a SCO packet
        ...
        EXPECT_TRUE(bluetooth_cb->WaitForCallback(kCallbackNameScoEventReceived).no_timeout);
        ...
        for (size_t i = 0; i < sco_packet.size(); i++) {
            if (sco_packet[i] == sco_loopback[i]) {
                ...
                ALOGE("Miscompare at %d (expected %x, got %x)", static_cast<int>(i), sco_packet[i],
                      sco_loopback[i]);
                ALOGE("At %d (expected %x, got %x)", static_cast<int>(i + 1), sco_packet[i + 1],
                      sco_loopback[i + 1]);
                break;
            }
        }
...
void BluetoothHidlTest::sendAndCheckACL(int num_packets, size_t size, uint16_t handle) {
    ThroughputLogger logger = {__func__};
    for (int n = 0; n < num_packets; n++) {
        // Send an ACL packet
        ...
        EXPECT_TRUE(bluetooth_cb->WaitForCallback(kCallbackNameAclEventReceived).no_timeout);
        ...
        for (size_t i = 0; i < acl_packet.size(); i++) {
            if (acl_packet[i] == acl_loopback[i]) {
                ...
                ALOGE("Miscompare at %d (expected %x, got %x)", static_cast<int>(i), acl_packet[i],
                      acl_loopback[i]);
                ALOGE("At %d (expected %x, got %x)", static_cast<int>(i + 1), acl_packet[i + 1],
                      acl_loopback[i + 1]);
                break;
            }
        }
...
void BluetoothHidlTest::enterLoopbackMode(std::vector<uint16_t>& sco_handles,
    ...
    while (true) {
        ...
        if (event[EVENT_CODE_BYTE] == EVENT_CONNECTION_COMPLETE) {
            EXPECT_GT(event.size(), static_cast<size_t>(EVENT_CONNECTION_COMPLETE_TYPE));
            EXPECT_EQ(event[EVENT_LENGTH_BYTE], EVENT_CONNECTION_COMPLETE_PARAM_LENGTH);
            ...
            ALOGD("Connect complete type = %d handle = %d", event[EVENT_CONNECTION_COMPLETE_TYPE],
                  handle);
...
TEST_F(BluetoothHidlTest, HciUnknownCommand) {
    ...
    if (event[EVENT_CODE_BYTE] == EVENT_COMMAND_COMPLETE) {
        EXPECT_EQ(cmd[0], event[EVENT_COMMAND_COMPLETE_OPCODE_LSBYTE]);
        EXPECT_EQ(cmd[1], event[EVENT_COMMAND_COMPLETE_OPCODE_LSBYTE + 1]);
        EXPECT_EQ(HCI_STATUS_UNKNOWN_HCI_COMMAND, event[EVENT_COMMAND_COMPLETE_STATUS_BYTE]);
    } else {
        ...
        EXPECT_EQ(HCI_STATUS_UNKNOWN_HCI_COMMAND, event[EVENT_COMMAND_STATUS_STATUS_BYTE]);
    }
}
