...
using aidl::android::hardware::bluetooth::ranging::SecurityLevel;
...
class BluetoothRangingTest : public ::testing::TestWithParam<std::string> {
 public:
  ...
  ScopedAStatus getMaxSupportedSecurityLevel(SecurityLevel* _aidl_return);
...
ScopedAStatus BluetoothRangingTest::getMaxSupportedSecurityLevel(
    SecurityLevel* _aidl_return) {
  return bluetooth_channel_sounding_->getMaxSupportedSecurityLevel(
      _aidl_return);
}
...
TEST_P(BluetoothRangingTest, GetMaxSupportedSecurityLevel) {
  SecurityLevel security_level;
  ScopedAStatus status = getMaxSupportedSecurityLevel(&security_level);
  ASSERT_TRUE(status.isOk());
}
