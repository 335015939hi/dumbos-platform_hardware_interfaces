...
interface IBluetoothChannelSoundingSession {
    /**
     * API to obtain the filter that is used to set the Procedure Data Filter of GATT notification.
     *
     * @return a filter of GATT notifcation
     */
    @nullable Filter getFilter();

    /**
     * API to obtain preferred PctFormat.
     *
     * @return PctFormat of GATT notifcation
     */
    PctFormat getPctFormat();

    /**
     * API to get preferred config for CS.
     *
     * @return Config for CS setup
     */
    @nullable Config getPreferredConfig();
     ...
     *
     * @return ture if the HAL would like to receive raw data of abort procedures.
     */
