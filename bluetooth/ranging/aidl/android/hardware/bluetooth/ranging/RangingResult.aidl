...
import android.hardware.bluetooth.ranging.ImpulseResponse;
...
parcelable RangingResult {
    // Estimated distance in meters from the algorithm
    double resultMeters;

    // A vector of size L, where L is the number of channel taps, of pairs of each tap's distance
    // and corresponding coefficient (complex)
    @nullable List<ImpulseResponse> channelImpulseResponse;

    // Estimated delay spread in meters of the measured channel. This is an indicator of the
    // severance of the multipath.
    double delaySpreadMeters;

    // A normalized value representing the confidence of estimated distance
    byte confidenceLevel;

    // A normalized value representing the security level of the measurement
    byte securityLevel;
    ...
    // Potential distance estimate error in meters
    double errorMeters;
