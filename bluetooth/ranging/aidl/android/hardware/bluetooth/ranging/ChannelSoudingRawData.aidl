...
import android.hardware.bluetooth.ranging.ProcedureChannel;
...
parcelable ChannelSoudingRawData {
    ...
    ProcedureChannel procedureChannel;

    // A buffer storing step channel in previous CS procedures
    List<ProcedureChannel> runningProcedureChannels;

    // Toa_tod_initator from mode-1 or mode-3 steps in a CS procedures (in time order)
    // RTT = 0.5 * (toa_tod_initiator - tod_toa_reflector)
    ...
    // Reference power level (-127 to 20) of the signal in the procedure
    int referencePowerLevel;
}
