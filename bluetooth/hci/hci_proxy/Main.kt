  ...
  fun read(inputStream: DataInputStream, halWrapper: HalWrapper) {
    while (true) {
      try {
        ...
        when (type) {
          HCI_PACKET_TYPE_COMMAND -> halWrapper.sendHciCommand(buf)
          HCI_PACKET_TYPE_ACL_DATA -> halWrapper.sendAclData(buf)
          HCI_PACKET_TYPE_SCO_DATA -> halWrapper.sendScoData(buf)
          HCI_PACKET_TYPE_ISO_DATA -> halWrapper.sendIsoData(buf)
