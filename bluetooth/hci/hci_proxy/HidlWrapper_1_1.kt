...
import android.hardware.bluetooth.V1_1.IBluetoothHciCallbacks
import android.util.Log
import java.io.DataOutputStream

class HidlWrapper_1_1(service: IBluetoothHci) : HalWrapper() {
  companion object {
    private val TAG = "HciProxyHIDL_1_1"
  }
  ...
  class BluetoothHciCallbacks(out: DataOutputStream) : IBluetoothHciCallbacks.Stub() {
    ...
    fun writePacketWithType(typeByte: Byte, data: ArrayList<Byte>) {
    ...
    override fun isoDataReceived(data: ArrayList<Byte>) =
  ...
  override fun sendAclData(data: ByteArray) = service.sendAclData(data.toCollection(ArrayList()))
  override fun sendScoData(data: ByteArray) = service.sendScoData(data.toCollection(ArrayList()))
  override fun sendIsoData(data: ByteArray) = service.sendIsoData(data.toCollection(ArrayList()))
}
