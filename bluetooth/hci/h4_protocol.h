...
class H4Protocol {
 public:
  H4Protocol(int fd, PacketReadCallback cmd_cb, PacketReadCallback acl_cb,
             PacketReadCallback sco_cb, PacketReadCallback evt_cb,
             PacketReadCallback iso_cb, DisconnectCallback disconnect_cb);

  size_t Send(H4Parser::Idc type, const uint8_t* data, size_t length);
  size_t Send(H4Parser::Idc type, const std::vector<uint8_t>& data);
  void OnDataReady();

 protected:
  void SendDataToPacketizer(uint8_t const* buffer, size_t length);
