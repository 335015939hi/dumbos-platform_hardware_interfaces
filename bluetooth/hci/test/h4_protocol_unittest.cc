...
using android::hardware::bluetooth::hci::PacketType;
using ::testing::Eq;
 ...
 protected:
  void SetUp() override {
    ...
    H4Protocol* h4_hci = new H4Protocol(
        sockfd[0], cmd_cb_.AsStdFunction(), acl_cb_.AsStdFunction(),
  ...
  void WriteAndExpectInboundScoData(char* payload) {
    ...
    char preamble[4] = {static_cast<uint8_t>(PacketType::SCO_DATA), 20, 17, 0};
  ...
  void WriteAndExpectInboundIsoData(char* payload) {
    // h4 type[1] + handle[2] + size[1]
    char preamble[5] = {static_cast<uint8_t>(PacketType::ISO_DATA), 19, 92, 0,
                        0};
    int length = strlen(payload);
    preamble[3] = length & 0xFF;
