...
H4Protocol::H4Protocol(int fd, PacketReadCallback cmd_cb,
    ...
    : uart_fd_(fd),
      ...
      h4_parser_([cmd_cb = std::move(cmd_cb), acl_cb = std::move(acl_cb),
                  sco_cb = std::move(sco_cb), evt_cb = std::move(evt_cb),
                  iso_cb = std::move(iso_cb)](
                     H4Parser::Idc idc, std::vector<uint8_t> const& packet) {
        switch (idc) {
          case H4Parser::Idc::kCommand:
            cmd_cb(packet);
            break;
          case H4Parser::Idc::kAcl:
            acl_cb(packet);
            break;
          case H4Parser::Idc::kSco:
            sco_cb(packet);
            break;
          case H4Parser::Idc::kEvent:
            evt_cb(packet);
            break;
...
size_t H4Protocol::Send(H4Parser::Idc type,
                        const std::vector<uint8_t>& vector) {
  return Send(type, vector.data(), vector.size());
}

size_t H4Protocol::Send(H4Parser::Idc type, const uint8_t* data,
                        size_t length) {
