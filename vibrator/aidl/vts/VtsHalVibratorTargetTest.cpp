...
<<<<<<< TARGET BRANCH (499960 [automerger skipped] composer: VtsDisplay::setDimensions sho)
    if (!(capabilities & IVibrator::CAP_ON_CALLBACK))
        return;
=======
    if (!(capabilities & IVibrator::CAP_ON_CALLBACK)) return;
>>>>>>> SOURCE BRANCH (9edfd3 vibrator: vts: correct the checking flag when testing on() i)
