...
TEST_P(VibratorAidl, ComposeCallback) {
    constexpr uint32_t allowedLatencyMs = 10;
    ...
    if (capabilities & IVibrator::CAP_COMPOSE_EFFECTS) {
        ...
        for (auto primitive : supported) {
            ...
            EXPECT_EQ(completionFuture.wait_for(
                              std::chrono::milliseconds(duration + allowedLatencyMs)),
            ...
            if (elapsed.count() > duration) {
                EXPECT_LE(elapsed.count() - duration, allowedLatencyMs) << toString(primitive);
            } else {
                EXPECT_LE(duration - elapsed.count(), allowedLatencyMs) << toString(primitive);
            }
        }
    }
}
