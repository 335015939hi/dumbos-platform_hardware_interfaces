...
class WifiOffloadHidlTest : public ::testing::VtsHalHidlTargetTestBase {
 public:
  virtual void SetUp() override {
    wifi_offload_ = ::testing::VtsHalHidlTargetTestBase::getService<IOffload>();
    ASSERT_NE(wifi_offload_, nullptr);

    wifi_offload_cb_ = new OffloadCallback([this] (std::vector<ScanResult> scanResult) -> void {
  ...
  class OffloadCallback : public IOffloadCallback {
   ...
   public:
    OffloadCallback(OnOffloadScanResultsReadyHandler handler) : handler_(handler) {};
...
TEST_F(WifiOffloadHidlTest, configureScans) {
  ...
  auto returnObject = wifi_offload_->configureScans(
      *pScanParam, *pScanFilter);
  ASSERT_EQ(returnObject.isOk(), true);
}

TEST_F(WifiOffloadHidlTest, getScanStats) {
  ScanStats *pScanStats = new ScanStats();
  const auto& returnObject = wifi_offload_->getScanStats(
    [pScanStats] (ScanStats scanStats) -> void {
