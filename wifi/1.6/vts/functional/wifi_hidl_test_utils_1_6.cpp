...
bool findAnyModeSupportingIfaceConcurrencyType(IfaceConcurrencyType desired_type,
...
bool configureChipToSupportIfaceConcurrencyType(const sp<IWifiChip>& wifi_chip,
    ...
    if (!findAnyModeSupportingIfaceConcurrencyType(type, status_and_modes.second,
                                                   configured_mode_id)) {
        return false;
    }
...
sp<IWifiApIface> getBridgedWifiApIface_1_6(const std::string& instance_name) {
    ChipModeId mode_id;
    sp<IWifiChip> wifi_chip_ = getWifiChip_1_6(instance_name);
    configureChipToSupportIfaceConcurrencyType(wifi_chip_, IfaceConcurrencyType::AP_BRIDGED,
                                               &mode_id);
    const auto& status_and_iface = HIDL_INVOKE(wifi_chip_, createBridgedApIface);
