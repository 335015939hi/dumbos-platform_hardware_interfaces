...
using android::hardware::wifi::hostapd::AcsFrequencyRange;
...
using android::hardware::wifi::hostapd::HostapdStatus;
...
class HostapdAidl : public testing::TestWithParam<std::string> {
   public:
    virtual void SetUp() override {
        ...
        hostapd->setDebugParams(DebugLevel::EXCESSIVE, &status);
    ...
    IfaceParams getIfaceParamsWithoutAcs(String16 iface_name) {
        ...
        iface_params.channelParamsList = vec_channelParams;
        return iface_params;
    }
    ...
    IfaceParams getIfaceParamsWithBridgedModeACS(String16 iface_name) {
        ...
        iface_params.channelParamsList[0].enableAcs = true;
        iface_params.channelParamsList[0].acsShouldExcludeDfs = true;

        std::vector<ChannelParams> vec_channelParams;
        vec_channelParams.push_back(iface_params.channelParamsList[0]);
    ...
    IfaceParams getIfaceParamsWithAcs(String16 iface_name) {
        ...
        iface_params.channelParamsList[0].enableAcs = true;
        iface_params.channelParamsList[0].acsShouldExcludeDfs = true;
        iface_params.channelParamsList[0].channel = 0;
        iface_params.channelParamsList[0].bandMask =
            iface_params.channelParamsList[0].bandMask | BandMask::BAND_5_GHZ;
        return iface_params;
    }
    ...
    IfaceParams getIfaceParamsWithAcsAndFreqRange(String16 iface_name) {
        ...
        AcsFrequencyRange acsFrequencyRange;
        acsFrequencyRange.startMhz = 2412;
        acsFrequencyRange.endMhz = 2462;
        std::vector<AcsFrequencyRange> vec_acsFrequencyRange;
        vec_acsFrequencyRange.push_back(acsFrequencyRange);
        iface_params.channelParamsList[0].acsChannelFreqRangesMhz =
    ...
    IfaceParams getIfaceParamsWithAcsAndInvalidFreqRange(String16 iface_name) {
        ...
        iface_params.channelParamsList[0].acsChannelFreqRangesMhz[0].startMhz =
            222;
        iface_params.channelParamsList[0].acsChannelFreqRangesMhz[0].endMhz =
            999;
        return iface_params;
    }
    ...
    IfaceParams getIfaceParamsWithInvalidChannel(String16 iface_name) {
        IfaceParams iface_params = getIfaceParamsWithoutAcs(iface_name);
        iface_params.channelParamsList[0].channel = kIfaceInvalidChannel;
        return iface_params;
    }
...
TEST_P(HostapdAidl, AddPskAccessPointWithAcs) {
    ...
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithAcs(kIfaceName), getPskNwParams(),
                            &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddPskAccessPointWithAcsAndFreqRange) {
    if (!isAcsSupport) GTEST_SKIP() << "Missing ACS support";
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithAcsAndFreqRange(kIfaceName),
                            getPskNwParams(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddOpenAccessPointWithAcs) {
    if (!isAcsSupport) GTEST_SKIP() << "Missing ACS support";
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithAcs(kIfaceName),
                            getOpenNwParams(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddPskAccessPointWithoutAcs) {
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithoutAcs(kIfaceName),
                            getPskNwParams(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddPskAccessPointWithoutAcsAndNonMetered) {
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithoutAcs(kIfaceName),
                            getPskNwParamsWithNonMetered(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddSaeTransitionAccessPointWithoutAcs) {
    ...
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithoutAcs(kIfaceName),
                            getSaeTransitionNwParams(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, RemoveAccessPointWithAcs) {
    ...
    hostapd->removeAccessPoint(kIfaceName, &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddPskAccessPointWithInvalidChannel) {
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithInvalidChannel(kIfaceName),
                            getPskNwParams(), &status);
    EXPECT_NE(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, AddInvalidSaeTransitionAccessPointWithoutAcs) {
    ...
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithoutAcs(kIfaceName),
                            getInvalidSaeTransitionNwParams(), &status);
    EXPECT_NE(HostapdStatusCode::SUCCESS, status.code);
}
...
TEST_P(HostapdAidl, DisconnectClientWhenIfacAvailable) {
    ...
    hostapd->forceClientDisconnect(kIfaceName, kTestZeroMacAddr,
                                   kTestDisconnectReasonCode, &status);
    EXPECT_EQ(HostapdStatusCode::FAILURE_CLIENT_UNKNOWN, status.code);
}
...
TEST_P(HostapdAidl, AddAccessPointWithDualBandConfig) {
    ...
    HostapdStatus status;
    hostapd->addAccessPoint(getIfaceParamsWithBridgedModeACS(kIfaceName),
                            getOpenNwParams(), &status);
    EXPECT_EQ(HostapdStatusCode::SUCCESS, status.code);
}
