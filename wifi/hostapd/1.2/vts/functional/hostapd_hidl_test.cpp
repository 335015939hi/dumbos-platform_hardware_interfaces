...
<<<<<<< TARGET BRANCH (e6c800 Merge "In supplicant VTS test setup, make sure that the wifi)
    if (is_1_3(hostapd_))
        GTEST_SKIP() << "Ignore addAccessPoint_1_2 on hostapd 1_3";
=======
    std::string ifname = setupApIfaceIfNeededAndGetName();
>>>>>>> SOURCE BRANCH (155f20 Merge "wifi: create interface before running hostapd vts" in)
