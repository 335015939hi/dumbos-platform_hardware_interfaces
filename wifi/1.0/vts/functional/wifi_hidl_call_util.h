...
// wherein to store the argument. The buffer has the same type as
// the argument (with const and reference modifiers removed).
...
// wherein to store the arguments. The buffer is a pair which holds
// the same types as the arguments (with const and reference modifiers
// removed).
...
// buffer wherein to store the arguments. The buffer is a tuple which
// holds the same types as the arguments (with const and reference
// modifiers removed).
template <typename... ArgT>
struct functionArgSaver<std::function<void(ArgT...)>> final {
  ...
  std::function<void(ArgT...)> saveArgs = [this](ArgT... arg) {
    this->saved_values = StorageT(arg...);
  };

  StorageT saved_values;
};

// Invokes |method| on |obj|, providing |method| a CallbackT as the
// argument. Returns a copy of the parameters that |method| provided
...
typename functionArgSaver<CallbackT>::StorageT invokeMethod(
    MethodT method, ObjectT obj, ArgT&&... methodArg) {
  functionArgSaver<CallbackT> result_buffer;
  ((*obj).*method)(std::forward<ArgT>(methodArg)..., result_buffer.saveArgs);
...
//   parameter), OR
// - A pair of the result callback parameters (for callbacks with
//   two parameters), OR
