...
bool hasAnyRingBufferCapabilities(uint32_t caps) {
  return (
      caps &
      (IWifiChip::ChipCapabilityMask::
           DEBUG_RING_BUFFER_CONNECT_EVENT_SUPPORTED |
       IWifiChip::ChipCapabilityMask::DEBUG_RING_BUFFER_POWER_EVENT_SUPPORTED |
       IWifiChip::ChipCapabilityMask::
           DEBUG_RING_BUFFER_WAKELOCK_EVENT_SUPPORTED |
...
TEST_F(WifiChipHidlTest, StartLoggingToDebugRingBuffer) {
  ...
  wifi_chip_->getDebugRingBuffersStatus(
      [&](const WifiStatus& status,
        ...
        if (hasAnyRingBufferCapabilities(caps)) {
          EXPECT_EQ(status.code, WifiStatusCode::SUCCESS);
          ASSERT_GT(ring_buffers.size(), 0u);
...
TEST_F(WifiChipHidlTest, GetApIface) {
  ...
  EXPECT_EQ(createApIface(&ap_iface), WifiStatusCode::SUCCESS);
  EXPECT_NE(ap_iface.get(), nullptr);
...
TEST_F(WifiChipHidlTest, GetNanIface) {
  ...
  EXPECT_EQ(createNanIface(&nan_iface), WifiStatusCode::SUCCESS);
  EXPECT_NE(nan_iface.get(), nullptr);
...
TEST_F(WifiChipHidlTest, GetP2pIface) {
  ...
  EXPECT_EQ(createP2pIface(&p2p_iface), WifiStatusCode::SUCCESS);
  EXPECT_NE(p2p_iface.get(), nullptr);
...
TEST_F(WifiChipHidlTest, GetStaIface) {
  ...
  EXPECT_EQ(createStaIface(&sta_iface), WifiStatusCode::SUCCESS);
  EXPECT_NE(sta_iface.get(), nullptr);
