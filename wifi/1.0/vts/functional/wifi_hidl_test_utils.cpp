...
bool findModeToSupportIfaceType(IfaceType type,
  ...
  for (const auto& mode : modes) {
    hidl_vec<IWifiChip::ChipIfaceCombination> combinations =
        mode.availableCombinations;
    for (const auto& combination : combinations) {
      hidl_vec<IWifiChip::ChipIfaceCombinationLimit> iface_limits =
          combination.limits;
      for (const auto& iface_limit : iface_limits) {
        hidl_vec<IfaceType> iface_types = iface_limit.types;
