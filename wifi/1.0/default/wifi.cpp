...
using RunState = ::android::hardware::wifi::WifiLegacyHal::RunState;
...
Return<bool> WifiHal::isStarted() {
  return state_.run_state_ != RunState::STOPPED;
}

Return<void> WifiHal::start() {
  ...
  event_loop_thread_ = std::thread(&WifiHal::DoHalEventLoop, this);
...
wifi_interface_handle WifiHal::FindInterfaceHandle(const std::string& ifname) {
...
Return<void> WifiHal::getChip(getChip_cb cb) {
  cb(chip_);
  return Void();
}
