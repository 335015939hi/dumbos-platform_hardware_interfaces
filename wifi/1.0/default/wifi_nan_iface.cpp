...
WifiNanIface::WifiNanIface(
  ...
  callback_handlers.on_event_disc_eng_event = [&](
      ...
      hidl_struct.eventType = msg.event_type;
  ...
  callback_handlers.on_event_data_path_end = [&](
      ...
      vec<uint32_t> ndpInstanceIds(msg.num_ndp_instances);
...
WifiStatus WifiNanIface::terminateDataPathRequestInternal(
    uint16_t cmd_id, const std::vector<uint32_t>& ndpInstanceIds) {
  legacy_hal::NanDataPathEndRequest* legacy_msg = (NanDataPathEndRequest*)malloc(
      sizeof(NanDataPathEndRequest) + ndpInstanceIds * sizeof(uint32_t));
...
std::pair<WifiStatus, NanVersion> WifiNanIface::getVersionInternal() {
  std::pair<wifi_error, NanVersion> result = legacy_hal_.lock()->nanGetVersion();
