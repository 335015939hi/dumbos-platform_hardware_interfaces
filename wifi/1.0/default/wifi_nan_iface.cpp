...
WifiNanIface::WifiNanIface(
  ...
  callback_handlers.on_notify_response = [&](
    ...
    case legacy_hal::NAN_RESPONSE_ENABLED: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyEnableResponse(id, wifiNanStatus).getStatus().isOk()) {
    ...
    case legacy_hal::NAN_RESPONSE_DISABLED: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyDisableResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_PUBLISH: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyStartPublishResponse(id, wifiNanStatus,
                        msg.body.publish_response.publish_id).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_PUBLISH_CANCEL: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyStopPublishResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_TRANSMIT_FOLLOWUP: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyTransmitFollowupResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_SUBSCRIBE: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyStartSubscribeResponse(id, wifiNanStatus,
                        msg.body.subscribe_response.subscribe_id).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_SUBSCRIBE_CANCEL: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyStopSubscribeResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_RESPONSE_CONFIG: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyConfigResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
     }
    case legacy_hal::NAN_RESPONSE_BEACON_SDF_PAYLOAD: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyBeaconSdfPayloadResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
     }
    ...
    case legacy_hal::NAN_DP_INTERFACE_CREATE: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyCreateDataInterfaceResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_DP_INTERFACE_DELETE: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyDeleteDataInterfaceResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_DP_INITIATOR_RESPONSE: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyInitiateDataPathResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_DP_RESPONDER_RESPONSE: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyRespondToDataPathIndicationResponse(id, wifiNanStatus,
                    msg.body.data_request_response.ndp_instance_id).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
    case legacy_hal::NAN_DP_END: {
        for (const auto& callback : event_callbacks_) {
          if (!callback->notifyTerminateDataPathResponse(id, wifiNanStatus).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
          }
        }
    }
  ...
  callback_handlers.on_event_disc_eng_event = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventClusterEvent(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_disabled = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventDisabled(status).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_publish_terminated = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventPublishTerminated(msg.publish_id, status).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_subscribe_terminated = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventSubscribeTerminated(msg.subscribe_id, status).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_match = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventMatch(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_match_expired = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventMatchExpired(msg.publish_subscribe_id,
                msg.requestor_instance_id).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_followup = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventFollowupReceived(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_transmit_follow_up = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventTransmitFollowup(msg.id, status).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_data_path_request = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventDataPathRequest(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_data_path_confirm = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventDataPathConfirm(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_data_path_end = [&](
      ...
      std::vector<uint32_t> ndpInstanceIds(msg.num_ndp_instances);
      for (int i = 0; i < msg.num_ndp_instances; ++i) {
          ndpInstanceIds[i] = msg.ndp_instance_id[i];
      }

      for (const auto& callback : event_callbacks_) {
        if (!callback->eventDataPathTerminated(ndpInstanceIds).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
  ...
  callback_handlers.on_event_beacon_sdf_payload = [&](
      ...
      for (const auto& callback : event_callbacks_) {
        if (!callback->eventBeaconSdfPayload(hidl_struct).getStatus().isOk()) {
            LOG(ERROR) << "Failed to invoke the callback";
        }
      }
  };
...
Return<void> WifiNanIface::terminateDataPathRequest(uint16_t cmd_id,
    const hidl_vec<uint32_t>& ndpInstanceIds,
...
WifiStatus WifiNanIface::terminateDataPathRequestInternal(
    uint16_t cmd_id, const hidl_vec<uint32_t>& ndpInstanceIds) {
  legacy_hal::NanDataPathEndRequest* legacy_msg = (legacy_hal::NanDataPathEndRequest*)malloc(
      sizeof(legacy_hal::NanDataPathEndRequest) + ndpInstanceIds.size() * sizeof(uint32_t));
  legacy_msg->num_ndp_instances = ndpInstanceIds.size();
  for (int i = 0; i < legacy_msg->num_ndp_instances; ++i) {
    legacy_msg->ndp_instance_id[i] = ndpInstanceIds[i];
  }
...
std::pair<WifiStatus, int32_t> WifiNanIface::getVersionInternal() {
  std::pair<legacy_hal::wifi_error, legacy_hal::NanVersion> result =
        legacy_hal_.lock()->nanGetVersion();

  return {createWifiStatusFromLegacyError(result.first), result.second};
}
