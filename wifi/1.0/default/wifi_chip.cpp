...
Return<void> WifiChip::requestChipDebugInfo(requestChipDebugInfo_cb cb) {
  ...
  std::pair<wifi_error, std::string> legacy_ret =
...
Return<void> WifiChip::requestDriverDebugDump(requestDriverDebugDump_cb cb) {
  ...
  std::pair<wifi_error, std::vector<char>> legacy_ret =
      legacy_hal_.lock()->requestDriverMemoryDump();
  if (legacy_ret.first != WIFI_SUCCESS) {
    LOG(ERROR) << "Failed to get driver debug dump: "
               << legacyErrorToString(legacy_ret.first);
    cb(createWifiStatusFromLegacyError(legacy_ret.first), hidl_vec<uint8_t>());
    return Void();
  }
