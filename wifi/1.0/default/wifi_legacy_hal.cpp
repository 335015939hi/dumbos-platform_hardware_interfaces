...
static constexpr uint32_t kMaxGscanResults = 64;
...
WifiLegacyHal::getPacketFilterCapabilities() {
  uint32_t version, max_len;
  wifi_error status = global_func_table_.wifi_get_packet_filter_capabilities(
      wlan_interface_handle_, &version, &max_len);
  return std::make_pair(status, std::make_pair(version, max_len));
}
...
WifiLegacyHal::getGscanCachedResults() {
  std::array<wifi_cached_scan_results, kMaxGscanResults> scan_results;
  int32_t num_results;
  wifi_error status =
