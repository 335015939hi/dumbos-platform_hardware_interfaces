...
void onNanNotifyResponse(transaction_id id, NanResponseMsg* msg) {
  if (on_nan_notify_response_user_callback && msg) {
    const NanResponseMsg msg_data = *msg;
    on_nan_notify_response_user_callback(id, msg_data);
  }
}
...
void onNanEventPublishTerminated(NanPublishTerminatedInd* event) {
  if (on_nan_event_publish_terminated_user_callback && event) {
    const NanPublishTerminatedInd event_data = *event;
    on_nan_event_publish_terminated_user_callback(event_data);
  }
}
std::function<void(const NanMatchInd&)> on_nan_event_match_user_callback;
void onNanEventMatch(NanMatchInd* event) {
  if (on_nan_event_match_user_callback && event) {
    const NanMatchInd event_data = *event;
    on_nan_event_match_user_callback(event_data);
  }
}
...
void onNanEventMatchExpired(NanMatchExpiredInd* event) {
  if (on_nan_event_match_expired_user_callback && event) {
    const NanMatchExpiredInd event_data = *event;
    on_nan_event_match_expired_user_callback(event_data);
  }
}
...
void onNanEventSubscribeTerminated(NanSubscribeTerminatedInd* event) {
  if (on_nan_event_subscribe_terminated_user_callback && event) {
    const NanSubscribeTerminatedInd event_data = *event;
    on_nan_event_subscribe_terminated_user_callback(event_data);
  }
}
std::function<void(const NanFollowupInd&)> on_nan_event_followup_user_callback;
void onNanEventFollowup(NanFollowupInd* event) {
  if (on_nan_event_followup_user_callback && event) {
    const NanFollowupInd event_data = *event;
    on_nan_event_followup_user_callback(event_data);
  }
}
...
void onNanEventDiscEngEvent(NanDiscEngEventInd* event) {
  if (on_nan_event_disc_eng_event_user_callback && event) {
    const NanDiscEngEventInd event_data = *event;
    on_nan_event_disc_eng_event_user_callback(event_data);
  }
}
std::function<void(const NanDisabledInd&)> on_nan_event_disabled_user_callback;
void onNanEventDisabled(NanDisabledInd* event) {
  if (on_nan_event_disabled_user_callback && event) {
    const NanDisabledInd event_data = *event;
    on_nan_event_disabled_user_callback(event_data);
  }
}
std::function<void(const NanTCAInd&)> on_nan_event_tca_user_callback;
void onNanEventTca(NanTCAInd* event) {
  if (on_nan_event_tca_user_callback && event) {
    const NanTCAInd event_data = *event;
    on_nan_event_tca_user_callback(event_data);
  }
}
...
void onNanEventBeaconSdfPayload(NanBeaconSdfPayloadInd* event) {
  if (on_nan_event_beacon_sdf_payload_user_callback && event) {
    const NanBeaconSdfPayloadInd event_data = *event;
    on_nan_event_beacon_sdf_payload_user_callback(event_data);
  }
}
...
void onNanEventDataPathRequest(NanDataPathRequestInd* event) {
  if (on_nan_event_data_path_request_user_callback && event) {
    const NanDataPathRequestInd event_data = *event;
    on_nan_event_data_path_request_user_callback(event_data);
  }
}
...
void onNanEventDataPathConfirm(NanDataPathConfirmInd* event) {
  if (on_nan_event_data_path_confirm_user_callback && event) {
    const NanDataPathConfirmInd event_data = *event;
    on_nan_event_data_path_confirm_user_callback(event_data);
  }
}
...
void onNanEventDataPathEnd(NanDataPathEndInd* event) {
  if (on_nan_event_data_path_end_user_callback && event) {
    const NanDataPathEndInd event_data = *event;
    on_nan_event_data_path_end_user_callback(event_data);
  }
}
...
void onNanEventTransmitFollowUp(NanTransmitFollowupInd* event) {
  if (on_nan_event_transmit_follow_up_user_callback && event) {
    const NanTransmitFollowupInd event_data = *event;
    on_nan_event_transmit_follow_up_user_callback(event_data);
  }
}
...
wifi_error WifiLegacyHal::startGscan(
  ...
  // This callback will be used to eithe trigger |on_results_user_callback| or
      ...
      [&on_failure_user_callback, &on_results_user_callback, this](
        ...
        switch (event) {
          ...
          case WIFI_SCAN_THRESHOLD_PERCENT: {
            std::pair<wifi_error, std::vector<wifi_cached_scan_results>> ret =
                getGscanCachedResults();
            if (ret.first == WIFI_SUCCESS) {
              on_results_user_callback(id, ret.second);
              return;
            }
          }
          ...
          case WIFI_SCAN_FAILED:
            ...
            return;
          default:
            CHECK(0) << "Unexpected gscan event received: " << event;
        }
      };

  on_gscan_full_result_internal_callback = [&on_full_result_user_callback](
      wifi_request_id id, wifi_scan_result* result, unsigned buckets_scanned) {
    if (result != nullptr) {
...
wifi_error WifiLegacyHal::startRttRangeRequest(
  ...
  return global_func_table_.wifi_rtt_range_request(
      id,
      wlan_interface_handle_,
      rtt_configs.size(),
      const_cast<wifi_rtt_config*>(rtt_configs.data()),
      {onRttResults});
}
...
wifi_error WifiLegacyHal::cancelRttRangeRequest(
  ...
  wifi_error status = global_func_table_.wifi_rtt_range_cancel(
      ...
      mac_addrs.size(),
      const_cast<mac_addr*>(
          reinterpret_cast<const mac_addr*>(mac_addrs.data())));
...
WifiLegacyHal::getRttCapabilities() {
  ...
  return std::make_pair(status, rtt_caps);
}
...
std::pair<wifi_error, wifi_rtt_responder> WifiLegacyHal::getRttResponderInfo() {
  ...
  return std::make_pair(status, rtt_responder);
}
...
wifi_error WifiLegacyHal::enableRttResponder(
  ...
  return global_func_table_.wifi_enable_responder(
      ...
      channel_hint,
      max_duration_secs,
      const_cast<wifi_rtt_responder*>(&info));
}
...
wifi_error WifiLegacyHal::setRttLci(wifi_request_id id,
  ...
  return global_func_table_.wifi_set_lci(
      id, wlan_interface_handle_, const_cast<wifi_lci_information*>(&info));
}

wifi_error WifiLegacyHal::setRttLcr(wifi_request_id id,
                                    const wifi_lcr_information& info) {
  return global_func_table_.wifi_set_lcr(
      id, wlan_interface_handle_, const_cast<wifi_lcr_information*>(&info));
}
...
wifi_error WifiLegacyHal::nanEnableRequest(transaction_id id,
  ...
  return global_func_table_.wifi_nan_enable_request(
      id, wlan_interface_handle_, const_cast<NanEnableRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanPublishRequest(transaction_id id,
  ...
  return global_func_table_.wifi_nan_publish_request(
      id, wlan_interface_handle_, const_cast<NanPublishRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanPublishCancelRequest(
  ...
  return global_func_table_.wifi_nan_publish_cancel_request(
      id, wlan_interface_handle_, const_cast<NanPublishCancelRequest*>(&msg));
}

wifi_error WifiLegacyHal::nanSubscribeRequest(transaction_id id,
                                              const NanSubscribeRequest& msg) {
  return global_func_table_.wifi_nan_subscribe_request(
      id, wlan_interface_handle_, const_cast<NanSubscribeRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanSubscribeCancelRequest(
  ...
  return global_func_table_.wifi_nan_subscribe_cancel_request(
      id, wlan_interface_handle_, const_cast<NanSubscribeCancelRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanBeaconSdfPayloadRequest(
  ...
  return global_func_table_.wifi_nan_beacon_sdf_payload_request(
      id,
      wlan_interface_handle_,
      const_cast<NanBeaconSdfPayloadRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanDataInterfaceCreate(
  ...
  return global_func_table_.wifi_nan_data_interface_create(
      id, wlan_interface_handle_, const_cast<char*>(iface_name.c_str()));
}
...
wifi_error WifiLegacyHal::nanDataRequestInitiator(
  ...
  return global_func_table_.wifi_nan_data_request_initiator(
      id,
      wlan_interface_handle_,
      const_cast<NanDataPathInitiatorRequest*>(&msg));
}
...
wifi_error WifiLegacyHal::nanDataIndicationResponse(
  ...
  return global_func_table_.wifi_nan_data_indication_response(
      id,
      wlan_interface_handle_,
      const_cast<NanDataPathIndicationResponse*>(&msg));
}
...
wifi_error WifiLegacyHal::nanDataEnd(transaction_id id,
  ...
  return global_func_table_.wifi_nan_data_end(
      id, wlan_interface_handle_, const_cast<NanDataPathEndRequest*>(&msg));
}
...
WifiLegacyHal::getGscanCachedResults() {
  std::array<wifi_cached_scan_results, kMaxCachedGscanResults> scan_results;
  int32_t num_results = 0;
  wifi_error status =
      global_func_table_.wifi_get_cached_gscan_results(wlan_interface_handle_,
                                                       true /* always flush */,
                                                       scan_results.size(),
                                                       scan_results.data(),
                                                       &num_results);
