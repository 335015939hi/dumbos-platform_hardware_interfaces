...
std::pair<wifi_error, WakeReasonStats> WifiLegacyHal::getWakeReasonStats() {
  ...
  std::vector<uint32_t> cmd_event_wake_cnt_vec;
  std::vector<uint32_t> driver_fw_local_wake_cnt_vec;
  cmd_event_wake_cnt_vec.resize(kMaxWakeReasonStatsArraySize);
  driver_fw_local_wake_cnt_vec.resize(kMaxWakeReasonStatsArraySize);

  // This struct needs separate memory to store the variable sized wake
  // reason types.
  wlan_wake_reason_cnt.cmd_event_wake_cnt =
      reinterpret_cast<int32_t*>(cmd_event_wake_cnt_vec.data());
  wlan_wake_reason_cnt.cmd_event_wake_cnt_sz = cmd_event_wake_cnt_vec.size();
  wlan_wake_reason_cnt.cmd_event_wake_cnt_used = 0;
  wlan_wake_reason_cnt.driver_fw_local_wake_cnt =
      reinterpret_cast<int32_t*>(driver_fw_local_wake_cnt_vec.data());
  wlan_wake_reason_cnt.driver_fw_local_wake_cnt_sz =
      driver_fw_local_wake_cnt_vec.size();
  ...
  // This is ugly! Move over the retrieved stats/arrays to the external
  // struct.
  WakeReasonStats wake_reason_stats_ext;
  wake_reason_stats_ext.wake_reason_cnt = wlan_wake_reason_cnt;

  CHECK(wlan_wake_reason_cnt.cmd_event_wake_cnt_used >= 0 &&
        static_cast<uint32_t>(wlan_wake_reason_cnt.cmd_event_wake_cnt_used) <=
            kMaxWakeReasonStatsArraySize);
  cmd_event_wake_cnt_vec.resize(wlan_wake_reason_cnt.cmd_event_wake_cnt_used);
  wake_reason_stats_ext.cmd_event_wake_cnt = std::move(cmd_event_wake_cnt_vec);
  wake_reason_stats_ext.wake_reason_cnt.cmd_event_wake_cnt = nullptr;

  CHECK(wlan_wake_reason_cnt.driver_fw_local_wake_cnt_used >= 0 &&
        static_cast<uint32_t>(
            wlan_wake_reason_cnt.driver_fw_local_wake_cnt_used) <=
            kMaxWakeReasonStatsArraySize);
  driver_fw_local_wake_cnt_vec.resize(
      wlan_wake_reason_cnt.driver_fw_local_wake_cnt_used);
  wake_reason_stats_ext.driver_fw_local_wake_cnt =
      std::move(driver_fw_local_wake_cnt_vec);
  wake_reason_stats_ext.wake_reason_cnt.driver_fw_local_wake_cnt = nullptr;
...
WifiLegacyHal::getGscanCachedResults() {
  std::vector<wifi_cached_scan_results> scan_results;
  scan_results.resize(kMaxCachedGscanResults);
  static_assert(sizeof(scan_results) < kMaxStackAllocInBytes,
                "Size of struct too large to fit in stack");
  ...
  scan_results.resize(num_results);
  return {status, std::move(scan_results)};
}
