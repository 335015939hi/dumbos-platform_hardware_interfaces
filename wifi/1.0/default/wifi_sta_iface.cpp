...
Return<void> WifiStaIface::enableRoamingControl(
    bool enable, enableRoamingControl_cb hidl_status_cb) {
  return validateAndCall(this,
                         WifiStatusCode::ERROR_WIFI_IFACE_INVALID,
                         &WifiStaIface::enableRoamingControlInternal,
                         hidl_status_cb,
                         enable);
}
...
WifiStatus WifiStaIface::enableRoamingControlInternal(bool enable) {
  legacy_hal::wifi_error legacy_status =
      legacy_hal_.lock()->enableFirmwareRoaming(
          enable ? legacy_hal::ROAMING_ENABLE : legacy_hal::ROAMING_DISABLE);
  return createWifiStatusFromLegacyError(legacy_status);
}
