...
Return<void> WifiStaIface::getType(getType_cb hidl_status_cb) {
  ...
  HIDL_RETURN1_WITH_CB(WifiStatusCode::SUCCESS, IfaceType::STA);
}
...
Return<void> WifiStaIface::registerEventCallback(
  ...
  if (!is_valid_) {
    HIDL_RETURN0_WITH_CB(WifiStatusCode::ERROR_WIFI_IFACE_INVALID);
  }
...
Return<void> WifiStaIface::getCapabilities(getCapabilities_cb hidl_status_cb) {
  if (!is_valid_) {
    HIDL_RETURN1_WITH_CB(WifiStatusCode::ERROR_WIFI_IFACE_INVALID, 0);
  }
  ...
  if (legacy_status != legacy_hal::WIFI_SUCCESS) {
    HIDL_RETURN1_WITH_CB_FROM_LEGACY_ERROR(legacy_status, 0);
  }
  ...
  HIDL_RETURN1_WITH_CB(WifiStatusCode::SUCCESS, caps);
}
...
Return<void> WifiStaIface::getApfPacketFilterCapabilities(
  ...
  std::tie(legacy_status, apf_caps) =
      legacy_hal_.lock()->getPacketFilterCapabilities();
  if (legacy_status != legacy_hal::WIFI_SUCCESS) {
    HIDL_RETURN1_WITH_CB_FROM_LEGACY_ERROR(legacy_status,
                                           ApfPacketFilterCapabilities());
  }
...
Return<void> WifiStaIface::installApfPacketFilter(
  ...
  if (legacy_status != legacy_hal::WIFI_SUCCESS) {
    HIDL_RETURN0_WITH_CB_FROM_LEGACY_ERROR(legacy_status);
  } else {
    HIDL_RETURN0_WITH_CB(WifiStatusCode::SUCCESS);
  }
}
...
Return<void> WifiStaIface::startBackgroundScan(
  ...
  if (!convertHidlScanParamsToLegacy(params, &internal_scan_params)) {
    HIDL_RETURN0_WITH_CB(WifiStatusCode::ERROR_INVALID_ARGS);
  }
...
Return<void> WifiStaIface::getValidFrequenciesForBackgroundScan(
  ...
  if (legacy_status != legacy_hal::WIFI_SUCCESS) {
    HIDL_RETURN1_WITH_CB_FROM_LEGACY_ERROR(legacy_status, hidl_vec<uint32_t>());
  }
  hidl_vec<uint32_t> hidl_freqs;
  hidl_freqs.setToExternal(freqs.data(), freqs.size());

  HIDL_RETURN1_WITH_CB(WifiStatusCode::SUCCESS, hidl_freqs);
}
...
Return<void> WifiStaIface::getLinkLayerStats(
  ...
  if (!convertLegacyLinkLayerStatsToHidl(stats, &hidl_stats)) {
    ...
    HIDL_RETURN1_WITH_CB(WifiStatusCode::ERROR_UNKNOWN,
                         IWifiStaIface::LinkLayerStats());
  }

  HIDL_RETURN1_WITH_CB(WifiStatusCode::SUCCESS, hidl_stats);
}
