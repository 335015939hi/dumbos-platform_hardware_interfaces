...
bool convertLegacyIeBlobToHidl(uint8_t const ie_blob[],
  ...
  while (next_ie + kIeHeaderLen <= ies_end) {
    ...
    std::vector<uint8_t> ie_data(&legacy_ie.data[0],
                                 &legacy_ie.data[0] + legacy_ie.len);
    hidl_ie.data = std::move(ie_data);
    hidl_ies->push_back(hidl_ie);
    next_ie += curr_ie_len;
  }
...
bool convertLegacyScanResultToHidl(
  ...
  std::vector<uint8_t> ssid(
      &legacy_scan_result.ssid[0],
      &legacy_scan_result.ssid[0] + sizeof(legacy_scan_result.ssid));
  hidl_scan_result->ssid = std::move(ssid);
