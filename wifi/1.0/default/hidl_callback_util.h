...
// Utility that provides a class to manage callbacks for the various hidl
// interfaces and handle the death of clients to remove the corresponding
...
class HidlCallbackHandler : public hidl_death_recipient {
 public:
  ...
  bool addCallback(const android::sp<CallbackType>& cb) {
    // TODO(b/33818800): Can't compare proxies yet. So, use the cookie
    // (underlying proxy pointer) to track the death of individual clients.
    uint64_t cookie = reinterpret_cast<uint64_t>(cb.get());
    if (!cb->linkToDeath(this, cookie)) {
      LOG(ERROR) << "Failed to register for death notification";
      return false;
    }
    cb_map_[cookie] = cb;
    return true;
  ...
  void serviceDied(
    ...
    // Lookup the callback object using the cookie.
 ...
 private:
  std::map<uint64_t, android::sp<CallbackType>> cb_map_;
