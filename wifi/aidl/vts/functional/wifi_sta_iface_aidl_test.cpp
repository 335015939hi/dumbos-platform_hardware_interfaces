...
class WifiStaIfaceAidlTest : public testing::TestWithParam<std::string> {
  ...
  protected:
    ...
    // Detected TV devices by using ro.oem.key1 property.
    // https://docs.partner.android.com/tv/build/platform/props-vars/ro-oem-key1
    bool isTvDevice(const std::string oem_key1) {
        if (oem_key1.size() < 9) {
            return false;
        }
    ...
    // Detected Panel TV devices by using ro.oem.key1 property.
    // https://docs.partner.android.com/tv/build/platform/props-vars/ro-oem-key1
    bool isPanelTvDevice(const std::string oem_key1) {
...
TEST_P(WifiStaIfaceAidlTest, CheckApfIsSupported) {
    ...
    if (isTvDevice(oem_key1)) {
        ...
        if (isPanelTvDevice(oem_key1) && isMdnsOffloadPresentInNIC()) {
