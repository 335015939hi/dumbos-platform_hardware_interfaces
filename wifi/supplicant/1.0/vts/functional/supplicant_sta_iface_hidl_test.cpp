...
class SupplicantStaIfaceHidlTest : public ::testing::Test {
   public:
    virtual void SetUp() override {
        startSupplicantAndWaitForHidlService();
        EXPECT_TRUE(turnOnExcessiveLogging());
        sta_iface_ = getSupplicantStaIface();
        EXPECT_NE(sta_iface_.get(), nullptr);
    }
...
TEST_F(SupplicantStaIfaceHidlTest, InitiateTdlsDiscover) {
    ...
    sta_iface_->initiateTdlsDiscover(
        mac_addr, [](const SupplicantStatus& status) {
...
TEST_F(SupplicantStaIfaceHidlTest, InitiateTdlsSetup) {
    std::array<uint8_t, 6> mac_addr;
    memcpy(mac_addr.data(), kTestMacAddr, mac_addr.size());
    sta_iface_->initiateTdlsSetup(mac_addr, [](const SupplicantStatus& status) {
        // These requests will fail unless the MAC address mentioned is actually
        // around.
        EXPECT_EQ(SupplicantStatusCode::FAILURE_UNKNOWN, status.code);
    });
}
...
/*
 * InitiateHs20AnqpQuery.
 */
TEST_F(SupplicantStaIfaceHidlTest, InitiateHs20AnqpQuery) {
    std::array<uint8_t, 6> mac_addr;
    memcpy(mac_addr.data(), kTestMacAddr, mac_addr.size());
    ...
    sta_iface_->initiateAnqpQuery(
        mac_addr, anqp_ids, hs_types, [](const SupplicantStatus& status) {
...
TEST_F(SupplicantStaIfaceHidlTest, InitiateHs20IconQuery) {
    ...
    sta_iface_->initiateHs20IconQuery(
        mac_addr, kTestHs20IconFile, [](const SupplicantStatus& status) {
            // These requests will fail unless the bssid mentioned is actually
            // present in scan results.
            EXPECT_EQ(SupplicantStatusCode::FAILURE_UNKNOWN, status.code);
        });
