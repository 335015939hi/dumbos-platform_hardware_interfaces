...
class ServiceNotificationListener : public IServiceNotification {
 public:
  ...
  bool registerForHidlServiceNotifications(const std::string& instance_name) {
    ...
    // This starts up a binder thread even though we set the thread count to
    // zero. (b/31226656)
  ...
  bool waitForHidlService(uint32_t timeout_in_millis,
    ...
    std::unique_lock<std::mutex> lock(mutex_);
    condition_.wait_for(
        lock, std::chrono::milliseconds(timeout_in_millis), [&]() {
          return registered_.size() >= 1;
        });
    if (registered_.size() != 1) {
      return false;
    }
    std::string exptected_registered = std::string("['") +
