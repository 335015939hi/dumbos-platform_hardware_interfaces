...
TEST_P(MasterHidlTest, ListServiceAttr) {
    ...
    if (attributes.size() == 0) {
         ...
         */
        const std::vector<struct AttributePattern> unknown = {{"supports-[a-z0-9-]*", "0|1"}};
        ...
        for (IOmxStore::Attribute attr : attributes) {
            ...
            if (knownIter != known.end()) {
                ...
                bool matched = false;
                for (auto it : unknown) {
                    if (testing::internal::RE::PartialMatch(attr.key, it.key)) {
                        matched = true;
                        EXPECT_EQ(testing::internal::RE::FullMatch(attr.value, it.value), true);
                    }
                }
                if (!matched) {
...
TEST_P(MasterHidlTest, ListRoles) {
    ...
    for (IOmxStore::RoleInfo role : roleList) {
        ...
        for (IOmxStore::NodeInfo node : role.nodes) {
            ...
            for (const auto attr : node.attributes) {
                // Make sure there are no duplicates
                const auto [nodeIter, inserted] = attributeSet.insert(attr.key);
                EXPECT_EQ(inserted, true);

                // Check the value against the corresponding regular
                // expression.
                const auto it = attrRules.find(attr.key);
                if (it != attrRules.end()) {
    ...
    for (auto owner : ownerToNodes) {
        ...
        EXPECT_EQ(nodeSet, owner.second);  // TODO Failing
    }
    ...
    if (!nodeToRoles.empty()) {
        ...
        EXPECT_EQ(testing::internal::RE::PartialMatch(prefix, node_name_re), true);
