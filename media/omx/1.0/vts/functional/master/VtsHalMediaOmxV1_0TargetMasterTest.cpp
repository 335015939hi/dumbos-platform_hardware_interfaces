...
TEST_P(MasterHidlTest, ListServiceAttr) {
    ...
    if (attributes.size() == 0) {
        ...
        for (IOmxStore::Attribute attr : attributes) {
            ...
            // expression.
            const auto knownIter = knownPatterns.find(attr.key);
            if (knownIter != knownPatterns.end()) {
                ...
                // possible patterns.
                bool matched = false;
                for (auto unknownIter : unknownPatterns) {
...
TEST_P(MasterHidlTest, ListRoles) {
        ...
        for (IOmxStore::NodeInfo node : role.nodes) {
            ...
            for (const auto attr : node.attributes) {
                ...
                if (knownPattern != knownPatterns.end()) {
                    EXPECT_EQ(testing::internal::RE::FullMatch(attr.value, knownPattern->second),
                              true)
    ...
    for (auto owner : ownerToNodes) {
        ...
        const sp<IOmx> omx = omxStore->getOmx(owner.first);
