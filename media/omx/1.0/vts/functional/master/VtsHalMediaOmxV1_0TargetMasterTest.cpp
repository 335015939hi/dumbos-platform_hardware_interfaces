...
TEST_P(MasterHidlTest, ListRoles) {
    ...
    std::string product =
        ::android::base::GetProperty(
                 ...
                 ::android::base::GetProperty("ro.product.system.name", ""));
    bool isGsi = product.rfind("aosp_", 0) == 0 || product.rfind("gsi_", 0) == 0;
    ...
    for (const IOmxStore::RoleInfo& role : roleList) {
        ...
        if (role_name != nullptr) {
            ...
            EXPECT_TRUE(isGsi) << "Media type \"" << role.type << "\" does not resolve to an OMX role.";
        }
