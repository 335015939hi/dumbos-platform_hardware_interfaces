...
class MasterHidlTest : public ::testing::TestWithParam<std::tuple<std::string, std::string>> {
  public:
    virtual void SetUp() override {
        omxStore = IOmxStore::getService(std::get<1>(GetParam()));
        ASSERT_NE(omxStore, nullptr);
        omx = IOmx::getService(std::get<0>(GetParam()));
...
INSTANTIATE_TEST_CASE_P(
        ...
        testing::Combine(
                testing::ValuesIn(android::hardware::getAllHalInstanceNames(IOmx::descriptor)),
                testing::ValuesIn(
                        android::hardware::getAllHalInstanceNames(IOmxStore::descriptor))),
