...
TEST_P(MasterHidlTest, ListServiceAttr) {
    ...
    if (attributes.size() == 0) {
        ...
        /*
         * unknown is a vector of pairs of regular expressions.
         ...
         */
        const std::vector<const struct AttributePattern> unknown = {{"supports-[a-z0-9-]*", "0|1"}};

        std::set<const std::string> keys;
        for (IOmxStore::Attribute attr : attributes) {
            // Make sure there are no duplicates
            const auto [keyIter, inserted] = keys.insert(attr.key);
            EXPECT_EQ(inserted, true) << "Service attribute \"" << attr.key << "\" has duplicates.";
            ...
            if (knownIter != known.end()) {
                EXPECT_EQ(testing::internal::RE::FullMatch(attr.value, knownIter->second), true)
                        << "Service attribute \"" << attr.key << "\" has invalid value \""
                        << attr.value << ".";
            } else {
                // Failed to find exact attribute, check against
                // possible patterns.
                bool matched = false;
                for (auto unknownIter : unknown) {
                    if (testing::internal::RE::PartialMatch(attr.key, unknownIter.key)) {
                        matched = true;
                        EXPECT_EQ(testing::internal::RE::FullMatch(attr.value, unknownIter.value),
                                  true)
                                << "Service attribute \"" << attr.key << "\" has invalid value \""
                                << attr.value << ".";
                    }
...
TEST_P(MasterHidlTest, ListRoles) {
    ...
    const std::map<std::string, testing::internal::RE> attrRules = {
            {"alignment", size},
    ...
    const std::vector<struct AttributePattern> attrPatternRules = {
    ...
    for (IOmxStore::RoleInfo role : roleList) {
        ...
        for (IOmxStore::NodeInfo node : role.nodes) {
            ...
            // Check the attributes associated with this node
            std::set<const std::string> attributeKeys;
            for (const auto attr : node.attributes) {
                // Make sure there are no duplicates
                const auto [nodeIter, inserted] = attributeKeys.insert(attr.key);
                EXPECT_EQ(inserted, true) << "Attribute \"" << attr.key << "\" for node \""
                                          << node.name << "\" has duplicates.";

                // Check the value against the corresponding regular
                // expression.
                const auto attrRule = attrRules.find(attr.key);
                if (attrRule != attrRules.end()) {
                    EXPECT_EQ(testing::internal::RE::FullMatch(attr.value, attrRule->second), true)
                    ...
                    // possible patterns.
                    bool keyFound = false;
                    for (auto attrPatternRule : attrPatternRules) {
                        if (testing::internal::RE::PartialMatch(attr.key, attrPatternRule.key)) {
                            keyFound = true;
                            EXPECT_EQ(testing::internal::RE::FullMatch(attr.value,
                                                                       attrPatternRule.value),
                                      true)
                                    << "Attribute \"" << attr.key << "\" has invalid value \""
                                    << attr.value << ".";
                            ;
                        }
                    }
            ...
            if (nodeToRoles.find(node.name) == nodeToRoles.end()) {
    ...
    for (auto owner : ownerToNodes) {
        ...
        for (IOmx::ComponentInfo node : nodeList) {
            ...
            // IOmxStore::listRoles().
            if (owner.second.find(node.mName) == owner.second.end()) {
            ...
            for (auto role : nodeToRoles[node.mName]) {
                EXPECT_NE(roleKeys.find(role), roleKeys.end());
            }
        }
        ...
        std::set_difference(owner.second.begin(), owner.second.end(), nodeKeys.begin(),
                            nodeKeys.end(), std::inserter(difference, difference.begin()));
        EXPECT_EQ(difference.empty(), true)
                << "IOmx::listNodes() for IOmx "
                   "instance \""
                << owner.first
                << "\" does not report some "
                   "expected nodes: "
                << android::base::Join(difference, ", ") << ".";  // TODO Failing
    }
