...
TEST_F(VideoDecHidlTest, DecodeTest) {
    ...
    PortMode pmArray[] = {
        PortMode::DYNAMIC_ANW_BUFFER, PortMode::PRESET_ANW_BUFFER,
    ...
    for (size_t j = 0; j < sizeof(pmArray) / sizeof(pmArray[0]); j++) {
        ...
        ASSERT_NO_FATAL_FAILURE(
            decodeNFrames(omxNode, observer, &iBuffer, &oBuffer,
                          ...
                          0, (int)Info.size(), portMode[1]));
        eleStream.close();
        ASSERT_NO_FATAL_FAILURE(waitOnInputConsumption(
            omxNode, observer, &iBuffer, &oBuffer, kPortIndexInput,
            kPortIndexOutput, portMode[1]));
        ...
        // skip testing preset anw buffer testing if dynamic anw buffer is
        // supported
        if (j == 0) j++;
    }
}
...
TEST_F(VideoDecHidlTest, AdaptivePlaybackTest) {
    ...
    for (size_t j = 0; j < sizeof(pmArray) / sizeof(pmArray[0]); j++) {
        ...
        for (uint32_t i = 0; i < STREAM_COUNT * 2; i++) {
            ...
            ASSERT_NO_FATAL_FAILURE(
                decodeNFrames(omxNode, observer, &iBuffer, &oBuffer,
                              kPortIndexInput, kPortIndexOutput, eleStream,
                              &Info, 0, (int)Info.size(), portMode[1], false));
            eleStream.close();
...
TEST_F(VideoDecHidlTest, EOSTest_M) {
    ...
    for (size_t j = 0; j < sizeof(pmArray) / sizeof(pmArray[0]); j++) {
        ...
        ASSERT_NO_FATAL_FAILURE(testEOS(
            omxNode, observer, &iBuffer, &oBuffer, true, eosFlag, portMode,
            portReconfiguration, kPortIndexInput, kPortIndexOutput, nullptr));
        ASSERT_NO_FATAL_FAILURE(flushPorts(omxNode, observer, &iBuffer,
...
TEST_F(VideoDecHidlTest, FlushTest) {
    ...
    for (size_t j = 0; j < sizeof(pmArray) / sizeof(pmArray[0]); j++) {
        ...
        if (keyFrame) {
            ASSERT_NO_FATAL_FAILURE(decodeNFrames(
                omxNode, observer, &iBuffer, &oBuffer, kPortIndexInput,
                kPortIndexOutput, eleStream, &Info, index, Info.size() - index,
                portMode[1], false));
        }
        eleStream.close();
        ASSERT_NO_FATAL_FAILURE(flushPorts(omxNode, observer, &iBuffer,
