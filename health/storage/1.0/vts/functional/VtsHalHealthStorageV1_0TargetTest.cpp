...
const std::chrono::seconds DEVGC_TIMEOUT{DEVGC_TIMEOUT_SEC};
// Time accounted for RPC calls.
const auto RPC_TIME = 100ms;
...
TEST_F(HealthStorageHidlTest, GcNullCallback) {
    ...
    ASSERT_TRUE(ping(DEVGC_TIMEOUT + RPC_TIME))
        << "Service must be available after " << toString(DEVGC_TIMEOUT + RPC_TIME);
}
...
TEST_F(HealthStorageHidlTest, GcNonNullCallback) {
    ...
    ASSERT_OK(ret);
    cb->waitForResult(DEVGC_TIMEOUT + RPC_TIME, Result::SUCCESS);
}
