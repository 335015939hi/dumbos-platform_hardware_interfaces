...
static constexpr int32_t kFullChargeDesignCapMin = 100 * 1000;
static constexpr int32_t kFullChargeDesignCapMax = 100 * 1000 * 1000;
...
template <typename T>
Matcher<T> InOpenRange(const T& lo, const T& hi) {
    return AllOf(Gt(lo), Lt(hi));
}
...
TEST_P(HealthAidl, Callbacks) {
    ...
    first_callback->waitInvoke(200ms);
    second_callback->waitInvoke(200ms);
...
MATCHER(IsValidHealthInfo, "") {
    *result_listener << "value is " << arg.toString() << ".";
    ...
    if (!ExplainMatchResult(
                AnyOf(Eq(0), InOpenRange(kFullChargeDesignCapMin, kFullChargeDesignCapMax)),
