...
class Health : public std::enable_shared_from_this<Health>, public HalHealthLoop, public BnHealth {
  public:
    ...
    // Expose to HalHealthLoop for internal usage.
    // This is similar to android::hardware::health::V2_1::implementation::Health::getHealthConfig.
    const struct healthd_config* healthd_config() const { return healthd_config_.get(); }
  ...
  protected:
    ...
    Health(const std::string& name, std::unique_ptr<struct healthd_config>&& config);
    ...
    // for efficiency.
    virtual ndk::ScopedAStatus shouldKeepScreenOn(bool* out);

  private:
    ndk::ScopedAStatus updateInternal();
