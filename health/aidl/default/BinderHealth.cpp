...
BinderHealth::LinkedCallback::LinkedCallback(BinderHealth* health,
                                             std::shared_ptr<IHealthInfoCallback> callback)
    : health_(health), callback_(callback) {
    binder_status_t linkRet =
            AIBinder_linkToDeath(callback_->asBinder().get(), health->death_recipient_.get(),
...
BinderHealth::LinkedCallback::~LinkedCallback() {
    (void)AIBinder_unlinkToDeath(callback_->asBinder().get(), health_->death_recipient_.get(),
...
BinderHealth::BinderHealth(const std::string& name, std::shared_ptr<IHealth> impl)
    : HalHealthLoop(name, impl),
      death_recipient_(AIBinder_DeathRecipient_new(&BinderHealth::serviceDied)) {
...
bool BinderHealth::unregisterCallbackInternal(
        const std::shared_ptr<IHealthInfoCallback>& callback) {
    if (callback == nullptr) {
        return false;
    }
    ...
    auto matches = [callback](const auto& linked) {
        return linked.callback_.get() == callback.get();
    };
...
ndk::ScopedAStatus BinderHealth::unregisterCallback(
        const std::shared_ptr<IHealthInfoCallback>& callback) {
    return unregisterCallbackInternal(callback)
...
void BinderHealth::OnHealthInfoChanged(const HealthInfo& health_info) {
    ...
    auto is_dead = [&](const auto& linked) {
        auto res = linked.callback_->healthInfoChanged(health_info);
...
void BinderHealth::serviceDied(void* cookie) {
    BinderHealth::LinkedCallback* linked = reinterpret_cast<BinderHealth::LinkedCallback*>(cookie);
    (void)linked->health_->unregisterCallbackInternal(linked->get());
}
...
int BinderHealth::PrepareToWait(void) {
    // TODO(b/177376954): There's no flushCommands in libbinder_ndk.
