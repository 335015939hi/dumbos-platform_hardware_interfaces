...
LinkedCallback::LinkedCallback(Health* service, std::shared_ptr<IHealthInfoCallback> callback)
    : service_(service), callback_(callback) {
    binder_status_t linkRet =
            AIBinder_linkToDeath(callback_->asBinder().get(), service_->death_recipient_.get(),
...
LinkedCallback::~LinkedCallback() {
    (void)AIBinder_unlinkToDeath(callback_->asBinder().get(), service_->death_recipient_.get(),
