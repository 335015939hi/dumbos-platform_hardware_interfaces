...
template <typename T>
AssertionResult IsInRangeInclusive(T x, const std::array<T, 2>& range) {
    if (x < range[0] || range[1] < x) {
        return AssertionFailure() << x <<" not in range [" << range[0] << ", " << range[1] << "]";
    } else {
        return AssertionSuccess();
    }
}
...
void verifyStorageInfos(const hidl_vec<struct StorageInfo>& infos) {
    for (const auto& info : infos) {
        EXPECT_TRUE(IsInRangeInclusive(info.eol, {0, 3}));
        EXPECT_TRUE(IsInRangeInclusive(info.lifetimeA, {0, 0x0B}));
        EXPECT_TRUE(IsInRangeInclusive(info.lifetimeB, {0, 0x0B}));
    }
}
...
TEST_P(HealthHidlTest, getHealthInfo) {
    ...
    EXPECT_OK(mHealth->getHealthInfo([](auto result, auto& health_info) {
        ...
        EXPECT_TRUE(IsInRangeInclusive(health_info.legacy.batteryLevel, {0, 100}));
