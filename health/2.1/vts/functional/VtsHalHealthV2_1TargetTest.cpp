...
TEST_F(HealthHidlTest, Callbacks_2_0) {
    EXPECT_TRUE(TestCallbacks<Callback_2_0>(service_));
}

TEST_F(HealthHidlTest, Callbacks_2_1) {
...
TEST_F(HealthHidlTest, UnregisterNonExistentCallback_2_0) {
    EXPECT_TRUE(TestUnregisterNonExistentCallback<Callback_2_0>(service_));
}

TEST_F(HealthHidlTest, UnregisterNonExistentCallback_2_1) {
...
TEST_F(HealthHidlTest, getHealthInfo) {
    EXPECT_TRUE(IsOk(service_->getHealthInfo_2_1([](auto result, const auto& value) {
        if (result == Result::NOT_SUPPORTED) {
            // GTEST_SKIP() << "getHealthInfo_2_1 returns NOT_SUPPORTED";
            return;
        }
        ASSERT_EQ(Result::SUCCESS, result);
        const auto& legacy = value.legacy.legacy;

        // Test batteryCapacityLevel.

        EXPECT_TRUE(IsEnum(value.batteryCapacityLevel)) << "BatteryCapacityLevel";

        bool batteryPresent = legacy.batteryPresent;
        bool capacityLevelKnown = value.batteryCapacityLevel != BatteryCapacityLevel::UNKNOWN;
        EXPECT_EQ(batteryPresent, capacityLevelKnown) << toString(value.batteryCapacityLevel);

        bool batteryFull = legacy.batteryStatus == BatteryStatus::FULL;
        bool battery100 = legacy.batteryLevel == 100;
        bool capacityLevelFull = value.batteryCapacityLevel == BatteryCapacityLevel::FULL;
        EXPECT_EQ(batteryFull, battery100) << "batteryStatus is " << toString(legacy.batteryStatus)
                                           << " but batteryLevel is " << legacy.batteryLevel;
        EXPECT_EQ(batteryFull, capacityLevelFull)
                << "batteryStatus is " << toString(legacy.batteryStatus)
                << " but batteryCapacityLevel is " << toString(value.batteryCapacityLevel);

        // Test batteryChargeTimeToFullNowSeconds
        EXPECT_EQ(!capacityLevelKnown || capacityLevelFull,
                  value.batteryChargeTimeToFullNowSeconds == 0)
                << "batteryChargeTimeToFullNowSeconds must be 0 if capacity level is "
                << toString(value.batteryCapacityLevel) << ", but it is "
                << value.batteryChargeTimeToFullNowSeconds;
        EXPECT_EQ(capacityLevelKnown && !capacityLevelFull,
                  value.batteryChargeTimeToFullNowSeconds > 0)
                << "batteryChargeTimeToFullNowSeconds must be positive if capacity level is "
                << toString(value.batteryCapacityLevel) << ", but it is "
                << value.batteryChargeTimeToFullNowSeconds;

        // Test batteryFullCapacityUah
