...
class Health : public IHealth, public hidl_death_recipient {
  public:
    // Initialize with the given HealthLoop.
    // Caller is responsible for calling loop->StartLoop() in some thread to do
    ...
    // Called by health_loop_->Init().
    // A subclass can override this to modify healthd_config before calling
    // BatteryMonitor::init. This is similar to healthd_board_init().
    // By default, it initializes battery_monitor_ directly.
    // For example, if you need to call healthd_board_init:
    //    healthd_board_init(config);
    //     Health::Init(config);
    virtual void Init(HealthLoop* loop, struct healthd_config* config);

    // Called by health_loop_->ScheduleBatteryUpdate() to send out callbacks.
    virtual void ScheduleBatteryUpdate();
  ...
  protected:
    ...
    // Callback function when battery info is updated by
    // health_loop_->ScheduleBatteryUpdate.
    // By default, it notifies all callbacks.
    virtual void OnHealthInfoUpdate(const HealthInfo& health_info);

  private:
    std::string instance_name_;
    HealthLoop* health_loop_ = nullptr;
