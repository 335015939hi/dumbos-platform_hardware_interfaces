...
BinderHealth::BinderHealth(const std::string& name, const sp<IHealth>& impl) : HalHealthLoop(name) {
...
Return<void> BinderHealth::OnHealthInfoChanged(const HealthInfo& health_info) {
...
Return<void> BinderHealthCallback::healthInfoChanged_2_1(const HealthInfo& health_info) {
    return service_->OnHealthInfoChanged(health_info);
}
