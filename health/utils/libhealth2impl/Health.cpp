...
void Health::Init(struct healthd_config* config) {
    ...
    // A subclass may want to customized healthd_config before sending it to BatteryMonitor.
...
Health::Health(const std::string& name)
    : instance_name_(name), thread_(std::make_unique<std::thread>(&Health::StartLoop, this)) {}
...
void Health::UpdateHealthConfig(struct healthd_config* /* config */) {
#if 0
    // If you need to call healthd_board_init, do it here:
