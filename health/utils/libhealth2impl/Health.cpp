...
Return<Result> Health::registerCallback(const sp<V2_0::IHealthInfoCallback>& callback) {
    if (callback == nullptr) {
        return Result::SUCCESS;
    }

    std::unique_lock<decltype(callbacks_lock_)> lock(callbacks_lock_);
    Callback* wrapped = callbacks_.emplace_back(Wrap(callback)).get();
    lock.unlock();
    ...
    getHealthInfo_2_1([&](auto res, const auto& health_info) {
        ...
        wrapped->Notify(health_info);
    });

    return Result::SUCCESS;
}

Return<Result> Health::unregisterCallback(const sp<V2_0::IHealthInfoCallback>& callback) {
    if (callback == nullptr) {
...
void Health::OnHealthInfoUpdate(const HealthInfo& health_info) {
    std::lock_guard<decltype(callbacks_lock_)> lock(callbacks_lock_);
    for (const auto& callback : callbacks_) {
        callback->Notify(health_info);
    }
}
...
Return<Result> Health::update() {
    ...
    getHealthInfo_2_1([&](auto res, const auto& health_info) {
        ...
        // If this process is a binderized service wrapping a passthrough
        // service, when the passthrough service invoke callbacks, the
        // outer binderized service:
        // 1. Broadcast health info update (originally healthd_mode_ops->battery_update)
        // 2. adjusts uevent / wakealarm periods (originally healthd_board_battery_update_internal)
        OnHealthInfoUpdate(health_info);
    });
    return result;
}
...
Return<void> Health::getScreenOn(getScreenOn_cb _hidl_cb) {
