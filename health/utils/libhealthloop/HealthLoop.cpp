...
HealthLoop::HealthLoop() {
    ...
    wakealarm_wake_interval_ = DEFAULT_PERIODIC_CHORES_INTERVAL_FAST;
}
...
int HealthLoop::RegisterEvent(int fd, BoundFunction func, EventWakeup wakeup) {
    // RegisterEvent may be called from a different thread. Wait until epollfd_ is initialized.
    std::unique_lock<decltype(epoll_mutex_)> lock(epoll_mutex_);
    bool wait_success = init_signal_.wait_for(lock, 3s, [this]{return epollfd_ != -1;});
    if (!wait_success && epollfd_ == -1) {
        KLOG_ERROR(LOG_TAG, "RegisterEvent failed: timeout waiting for initialization.");
        return -1;
    }
...
int HealthLoop::InitInternal() {
    {
        std::lock_guard<decltype(epoll_mutex_)> lock(epoll_mutex_);
    ...
    // If other threads are calling RegisterEvent and waiting for epollfd_ to
    // be initialized, wake them up.
