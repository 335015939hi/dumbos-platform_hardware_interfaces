...
bool HealthLoop::RecvUevents() {
    ...
    for (;;) {
        ...
        if (n < 0 && errno == ENOBUFS) {
            static bool logged_receive_buffer_overflow;
            if (!logged_receive_buffer_overflow) {
                logged_receive_buffer_overflow = true;
                KLOG_INFO(LOG_TAG, "Receive buffer overflowed\n");
            }
            update_stats = true;
        }
        if (n <= 0) return update_stats;
        if (n >= kUeventMsgLen) /* too long -- discard */
            continue;
        if (update_stats) {
            continue;
        }
