...
int HealthLoop::RegisterEvent(int fd, BoundFunction func, EventWakeup wakeup) {
    ...
    bool wait_success = init_signal_.wait_for(lock, 3s, [this]{return epollfd_ != -1;});
    if (!wait_success && epollfd_ == -1) {
        KLOG_ERROR(LOG_TAG, "Timeout waiting for epoll_fd initialization.");
        return -1;
    }
    return RegisterEventLocked(fd, func, wakeup);
}

int HealthLoop::RegisterEventLocked(int fd, BoundFunction func, EventWakeup wakeup) {
    if (reject_event_register_) {
        KLOG_ERROR(LOG_TAG, "Should not register event after loop starts.\n");
        return -1;
    }
...
void HealthLoop::WakeAlarmSetIntervalLocked(int interval) {
...
void HealthLoop::SetChargerOnline(bool charger_online) {
    std::unique_lock<std::mutex> lock(mutex_);
    // This function and WakeAlarmSetIntervalLocked requires wakealarm_fd_ and healthd_config_
    // to be initialized. Because wakealarm_fd_ is initialized after healthd_config_, it is
    // okay to check wakealarm_fd_ alone.
    bool wait_success = init_signal_.wait_for(lock, 3s, [this]{return wakealarm_fd_ != -1;});
    if (!wait_success && wakealarm_fd_ == -1) {
        KLOG_ERROR(LOG_TAG, "Timeout waiting for wakealarm_fd initialization.");
        return;
    }
    ...
    if (new_wake_interval != wakealarm_wake_interval_) WakeAlarmSetIntervalLocked(new_wake_interval);
...
void HealthLoop::UeventInitLocked(void) {
    ...
    if (RegisterEventLocked(uevent_fd_, &HealthLoop::UeventEvent, EVENT_WAKEUP_FD))
...
void HealthLoop::WakeAlarmInitLocked(void) {
    ...
    if (RegisterEventLocked(wakealarm_fd_, &HealthLoop::WakeAlarmEvent, EVENT_WAKEUP_FD))
        KLOG_ERROR(LOG_TAG, "Registration of wakealarm event failed\n");

    WakeAlarmSetIntervalLocked(healthd_config_.periodic_chores_interval_fast);
}
