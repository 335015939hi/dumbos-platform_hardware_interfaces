...
// BPF is not supported in recovery mode.
...
void HealthLoop::UeventInit(void) {
    ...
    Result<unique_fd> filter_fd;
    // BPF is not supported in recovery mode.
#ifndef __ANDROID_RECOVERY__
    if (isAtLeastKernelVersion(5, 10, 0)) {
        filter_fd = AttachFilter(uevent_fd_);
    ...
    // From the bpf(2) man page: "Applying close(2) to the file descriptor returned by BPF_PROG_LOAD
    // will unload the eBPF program". Hence keep the BPF program file descriptor open by moving it
    // into a class member.
