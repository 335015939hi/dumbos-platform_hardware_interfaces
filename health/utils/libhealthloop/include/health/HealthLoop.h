...
class HealthLoop {
  ...
  private:
    ...
    int RegisterEventLocked(int fd, BoundFunction func, EventWakeup wakeup);
    int InitInternal();
    void MainLoop();
    void WakeAlarmInitLocked();
    void WakeAlarmEvent(uint32_t);
    void UeventInitLocked();
    void UeventEvent(uint32_t);
    void WakeAlarmSetIntervalLocked(int interval);
    void PeriodicChores();

    // Lock that guards all states.
    std::mutex mutex_;

    // Signals for InitInternal().
    std::condition_variable init_signal_;
