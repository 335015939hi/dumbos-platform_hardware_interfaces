...
int filter_netlink(struct __sk_buff* skb) {
    ...
    if (skb->protocol != NETLINK_KOBJECT_UEVENT) return 0;
    if (skb->len < NLMSG_HDRLEN) return 0;
    end = load_word(skb, offsetof(struct nlmsghdr, nlmsg_len));
    if (end > skb->len) return 0;
    i = NLMSG_HDRLEN;

    if (i + MATCH_SUBSYSTEM_LENGTH + MATCH_POWER_SUPPLY0_LENGTH > end) return 0;
    if (MATCH_SUBSYSTEM(skb, i)) return MATCH_POWER_SUPPLY0(skb, i + MATCH_SUBSYSTEM_LENGTH);

#pragma unroll
    for (int n = 0; n < 70; ++n) {
        ++i;
        if (i + MATCH_0SUBSYSTEM_LENGTH + MATCH_POWER_SUPPLY0_LENGTH > end) return 0;
