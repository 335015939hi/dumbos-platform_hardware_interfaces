...
// M2: match 2 bytes
static inline uint32_t M2(struct __sk_buff* skb, unsigned int offset, uint8_t c0, uint8_t c1) {
    return load_half(skb, offset) ^ ((c0 << 8) | c1);
}

// M1: match 1 byte
static inline uint32_t M1(struct __sk_buff* skb, unsigned int offset, uint8_t c0) {
...
found_subsystem:
    ...
    char subsys[5] = {load_byte(skb, i), load_byte(skb, i + 1), load_byte(skb, i + 2),
                      load_byte(skb, i + 3)};
    return 0;
}
