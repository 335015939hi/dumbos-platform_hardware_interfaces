...
TEST_F(RadioHidlTest, acknowledgeLastIncomingGsmSms) {
    ...
    if (cardStatus.cardState == CardState::ABSENT) {
        ...
        EXPECT_EQ(RadioError::MODEM_ERR, radioRsp->rspInfo.error);
    }
}
...
TEST_F(RadioHidlTest, acknowledgeIncomingGsmSmsWithPdu) {
    ...
    if (cardStatus.cardState == CardState::ABSENT) {
        EXPECT_EQ(RadioError::GENERIC_FAILURE, radioRsp->rspInfo.error);
    } else {
        EXPECT_EQ(RadioError::GENERIC_FAILURE, radioRsp->rspInfo.error);
    }
}
...
TEST_F(RadioHidlTest, getSmscAddress) {
    ...
    if (cardStatus.cardState == CardState::ABSENT) {
        EXPECT_EQ(RadioError::INVALID_MODEM_STATE, radioRsp->rspInfo.error);
    } else {
        EXPECT_EQ(RadioError::NONE, radioRsp->rspInfo.error);
    }
...
TEST_F(RadioHidlTest, writeSmsToSim) {
    ...
    if (cardStatus.cardState == CardState::ABSENT) {
        EXPECT_EQ(RadioError::SYSTEM_ERR, radioRsp->rspInfo.error);
    } else {
        EXPECT_EQ(RadioError::SYSTEM_ERR, radioRsp->rspInfo.error);
    }
