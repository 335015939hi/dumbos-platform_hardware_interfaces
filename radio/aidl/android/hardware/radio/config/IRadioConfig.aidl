...
import android.hardware.radio.config.SlotMapping;
...
oneway interface IRadioConfig {
     ...
     *
     * Maps the logical slots to the SlotMapping which consist of both physical slot id and port id.
     * Logical slot is the slot that is seen by modem.
     * Physical slot is the actual physical slot.
     * PortId is just an enumeration of the ports available on the UICC.
     * Each physical slot can have multiple ports which enables multi-enabled profile(MEP).
     * The portId is just an enumeration of the ports available on the physical slot.
     * If eUICC physical slot supports 2 ports, then the portId is numbered 0,1 and
     * eUICC2 supports 4 ports. Each portId is unique within a UICC physical slot but not
     * necessarily
     * unique across UICC’s. SEP(Single enabled profile) eUICC and non-eUICC will only have portId
     * 0.
     * Request maps the SlotMapping to logical slot.
     * Logical slots that are already mapped to the requested SlotMapping are not impacted.
     *
     * Example no. of logical slots 1 and physical slot 2 do not support MEP, each physical slot
     * have one port:
     ...
     * inactive.
     * slotMap[0] = SlotMapping{1 //physical slot//, 0 //port//}
     * or slotMap[0] = SlotMapping{0 //physical slot//, 0 //port//}
     *
     * Example no. of logical slots 2 and physical slot 2 supports MEP with 2 ports available:
     ...
     * slotMap[0] = SlotMapping{0, 0} and slotMap[1] = SlotMapping{1, 0} or
     * slotMap[0] = SlotMapping{0, 0} and slotMap[1] = SlotMapping{1, 1}
     ...
     * slotMap[1] = SlotMapping{0, 0} and slotMap[0] = SlotMapping{1, 0} or
     * slotMap[1] = SlotMapping{0, 0} and slotMap[0] = SlotMapping{1, 1}
     ...
     * slotMap[0] = SlotMapping{1, 0} and slotMap[1] = SlotMapping{1, 1} or
     * and slotMap[0] = SlotMapping{1, 1} and slotMap[1] = SlotMapping{1, 0}
     *
     * @param serial Serial number of request
     * @param slotMap Logical to physical slot and port mapping, size == no. of radio instances.
     *        Index is mapping to logical slot and value to physical slot and port id,need to provide
     *        all the slots mapping when sending request in case of multi slot device.
     *        
     *        EX: uint32_t slotMap[logical slot] = SlotMapping(physical slot, port id)
     ...
     */
    void setSimSlotsMapping(in int serial, in SlotMapping slotMap);
}
