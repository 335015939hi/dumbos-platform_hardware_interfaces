...
oneway interface IRadioConfig {
     ...
     * PortId is an enumeration of the ports available on the UICC.
     * Each physical slot can have multiple ports which enables multi-enabled profile(MEP).
     * If eUICC physical slot supports 2 ports, then the portId is numbered 0,1 and eUICC2 supports
     * 4 ports. Each portId is unique within a UICC physical slot but not necessarily unique across
     * UICC’s. SEP(Single enabled profile) eUICC and non-eUICC will only have portId 0.
     *
     ...
     *
     *        EX: SlotMapping(physical slot, port id)
