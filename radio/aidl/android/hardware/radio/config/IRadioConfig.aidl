...
oneway interface IRadioConfig {
     ...
     * slot. PortId is an enumeration of the ports available on the UICC. Each physical slot can
     * have multiple ports which enables multi-enabled profile(MEP). If eUICC physical slot supports
     * 2 ports, then the portId is numbered 0,1 and if eUICC2 supports 4 ports then the portID is
     * numbered 0,1,2,3. Each portId is unique within a UICC physical slot but not necessarily
     ...
     * and slotMap[0] = SlotPortMapping{1, 1} and slotMap[1] = SlotPortMapping{1, 0}
     *
     * @param serial Serial number of request
