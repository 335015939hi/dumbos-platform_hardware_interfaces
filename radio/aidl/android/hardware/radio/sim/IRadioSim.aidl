...
oneway interface IRadioSim {
     ...
     *
     * For MEP-A(Mutlitple enabled profile), only dedicated port 0 is ISDR selectable.
     * e.g., Port0 - for ISDR access and Port1/Port2 - the currently active ports/subscriptionsPort.
     * Port 0 should be transparent to AP and iccLogicalChannel API should remains the same.
     * Even the ISDR request comes over port1 or port2, Modem would just internally convert the
