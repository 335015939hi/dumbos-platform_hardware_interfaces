...
oneway interface IRadioNetworkIndication {
     ...
     *
     * Cellular indications must be sent to Android regardless of the screen state. If the screen is
     * off, the indications must still be sent to Android.
     *
     ...
     * - Note on integrity: In 4G, PDCP can be LTE-based or NR-based. Starting from release 17, only
     * the NR-based PDCP supports DRB integrity. As the PDCP version can change during a DRB's
     * operation, it becomes complicated when integrity is used to determine whether an emergency
     * call is protected or not, hence its exclusion to simplify implementation.
     *- Note for 4G and 5G: emergency calls are protected under that RAT only if all DRBs are
     * protected (including IMS DRB).
     * - Note for 4G and 5G: Since DRB integrity is not enabled in most networks, if both ciphering
     * and integrity are taken into account to determine the value of isUnprotectedEmergency, the
     * value will mostly be false, hence why it is excluded.
     *
     * @param type Type of radio indication
