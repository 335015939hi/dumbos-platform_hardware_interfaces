...
ScopedAStatus RadioSimSap::apduReq(int32_t token, SapApduType type,
                                   const std::vector<uint8_t>& command) {
    //::android::hardware::hidl_vec<uint8_t> hidlCommand{command.begin(), command.end()};
    // mSap->apduReq(token, (V1_0::SapApduType) type, hidlCommand);
