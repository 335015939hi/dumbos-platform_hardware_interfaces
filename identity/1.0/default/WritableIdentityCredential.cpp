...
bool generateCredentialData(const vector<uint8_t>& hardwareBoundKey, const string& docType,
    ...
    array.add(cppbor::Bstr(credentialBlob));
...
Return<void> WritableIdentityCredential::addAccessControlProfile(
        uint8_t id, const hidl_vec<uint8_t>& readerCertificate, bool userAuthenticationRequired,
    ...
    // Spec requires if |userAuthenticationRequired| is false, then |timeout| must also be zero.
    if (!userAuthenticationRequired && timeout != 0) {
        _hidl_cb(support::result(ResultCode::INVALID_DATA,
    ...
    if (profile.userAuthenticationRequired) {
        ...
        profileMap.add("timeout", profile.timeout);
    }
...
Return<void> WritableIdentityCredential::addEntryValue(const hidl_vec<uint8_t>& content,
    ...
    if (!support::encryptAes128Gcm(storageKey_, nonce, content, entryAdditionalData_,
                                   encryptedContent)) {
...
Return<void> WritableIdentityCredential::finishAddingEntries(finishAddingEntries_cb _hidl_cb) {
    ...
    _hidl_cb(support::resultOK(), credentialData, encodedCbor, signature);
    return Void();
}
