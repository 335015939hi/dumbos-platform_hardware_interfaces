...
bool checkUserAuthentication(const SecureAccessControlProfile& profile,
    ...
    LOG(ERROR) << "checkUserAuth:"
               << " profile.timeout=" << profile.timeout << " authChallenge=" << authChallenge
               << " authToken.challenge=" << authToken.challenge
               << " profile.secureUserId=" << profile.secureUserId
...
Return<void> IdentityCredential::startRetrieval(
    ...
    for (const auto& profile : accessControlProfiles) {
        ...
        if (profile.userAuthenticationRequired) {
            if (!haveAuthToken || !checkUserAuthentication(profile, authToken, mAuthChallenge)) {
