...
Return<void> IdentityCredential::deleteCredential(deleteCredential_cb _hidl_cb) {
    cppbor::Array deletionSignedDataArray;
    deletionSignedDataArray.add("ProofOfDeletion");
    deletionSignedDataArray.add(docType_);
    deletionSignedDataArray.add(testCredential_);
    vector<uint8_t> deletionSignedData = deletionSignedDataArray.encode();

    vector<uint8_t> signature;
    if (!support::signEcDsa(credentialPrivKey_, deletionSignedData, signature)) {
        _hidl_cb(support::result(ResultCode::FAILED, "Error signing DeletionSignedData"), {});
        return Void();
    }

    cppbor::Array proofOfDeletionArray;
    proofOfDeletionArray.add(std::move(deletionSignedDataArray));
    proofOfDeletionArray.add(signature);
    vector<uint8_t> proofOfDeletion = proofOfDeletionArray.encode();

    _hidl_cb(support::resultOK(), proofOfDeletion);
    return Void();
}
...
Return<void> IdentityCredential::startRetrieval(
    ...
    // key we previously created, is present in the SessionTranscript as a
    // COSE_Key, in uncompressed form.
    //
    ...
    if (sessionTranscript.size() > 0) {
        ...
        if (sessionTranscript.size() > 0 && !(support::cborHasBstr(sessionTranscript, ePubX) &&
                                              support::cborHasBstr(sessionTranscript, ePubY))) {
            _hidl_cb(support::result(ResultCode::EPHEMERAL_PUBLIC_KEY_NOT_FOUND,
    ...
    if (itemsRequest.size() > 0) {
        ...
        if (map == nullptr) {
            _hidl_cb(support::result(ResultCode::INVALID_DATA, "itemsRequest is not a CBOR map"));
            return Void();
        }

        // 3. The map must contain a key "NameSpaces" whose value is an map of maps, defined below.
        //
        //   NameSpaces = {
        ...
        //      'NameSpaces' : {
        //          'org.iso.18013-5.2019' : ['Last name', 'Birth date', 'First name', 'Home
        //          address'], 'org.aamva.iso.18013-5.2019' : ['Real Id']
        //      }
        //    }
        //
        ...
        if (nsMap == nullptr) {
            _hidl_cb(
                    support::result(ResultCode::INVALID_DATA, "No NameSpaces map in top-most map"));
            return Void();
        }
        ...
        for (size_t n = 0; n < nsMap->size(); n++) {
            ...
            const cppbor::Array* nsArray = nsValueItem->asArray();
            if (nsKey == nullptr || nsArray == nullptr) {
                _hidl_cb(support::result(ResultCode::INVALID_DATA,
            ...
            for (size_t m = 0; m < nsArray->size(); m++) {
                const cppbor::Tstr* nameItem = (*nsArray)[m]->asTstr();
                if (nameItem == nullptr) {
                    _hidl_cb(support::result(ResultCode::INVALID_DATA,
...
Return<void> IdentityCredential::finishRetrieval(
    ...
    if (signingKeyBlob.size() > 0 && sessionTranscript_.size() > 0) {
        ...
        array.add(docType_);
        array.add(encodedDeviceNameSpaces);
