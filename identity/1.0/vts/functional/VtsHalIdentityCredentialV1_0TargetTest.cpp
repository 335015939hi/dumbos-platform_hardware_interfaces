...
TEST_F(IdentityCredentialStoreHidlTest, createAndRetrieveCredential) {
    ...
    writableCredential->finishAddingEntries([&](const Result& _result,
                                                const hidl_vec<uint8_t>& _credentialData,
                                                const hidl_vec<uint8_t>& _signedData,
                                                const hidl_vec<uint8_t>& _personalizationReceipt) {
        result = _result;
        credentialData = _credentialData;
        signedData = _signedData;
        personalizationReceipt = _personalizationReceipt;
    });
    EXPECT_EQ("", result.message);
    ASSERT_EQ(ResultCode::OK, result.code);
    // TODO: inspect credentialData

    ASSERT_TRUE(support::cborPrettyPrint(signedData, cborPretty, 32, {"readerCertificate"}));
    EXPECT_EQ(
            "[\n"
            "  'ProofOfProvisioning',\n"
            ...
            "]",
            cborPretty);
    EXPECT_TRUE(support::checkEcDsaSignatureUsingCertificateChain(
            support::sha256(signedData), personalizationReceipt, attestationCertificate));
    ...
    cppbor::Map sessionTranscript = cppbor::Map().add("ephX", ephX).add("ephY", ephY);
    vector<uint8_t> sessionTranscriptBytes = sessionTranscript.encode();

    vector<uint8_t> itemsRequestBytes =
            cppbor::Map("NameSpaces",
                        cppbor::Map()
                                .add("PersonalData", cppbor::Array({"Last name", "Birth date",
                                                                    "First name", "Home address"}))
                                .add("Image", cppbor::Array({"Portrait image"})))
                    .encode();
    ASSERT_TRUE(
