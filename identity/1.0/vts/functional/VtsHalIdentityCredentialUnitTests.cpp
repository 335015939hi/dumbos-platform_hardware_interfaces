  ...
  public:
    ...
    bool AddAccessControlProfiles(sp<IWritableIdentityCredential>& writableCredential,
        ...
        for (const auto& testProfile : testProfiles) {
            ...
            writableCredential->addAccessControlProfile(
                    ...
                    testProfile.userAuthenticationRequired, testProfile.timeoutMillis, 0,
    ...
    bool AddEntry(sp<IWritableIdentityCredential>& writableCredential, const TestEntryData& entry,
        ...
        for (const auto& chunk : chunks) {
            writableCredential->addEntryValue(chunk, [&](const Result& result,
                                                         hidl_vec<uint8_t> encryptedContent) {
                EXPECT_EQ("", result.message) << "entry name = " << entry.name
    ...
    bool ValidateAttestationCertificate(const hidl_vec<hidl_vec<uint8_t>> inputCertificates) {
        vector<vector<uint8_t>> certificates;

        std::copy(inputCertificates.begin(), inputCertificates.end(),
                  std::back_inserter(certificates));
        return support::certificateChainValidate(support::certificateChainJoin(certificates));
    }
...
TEST_F(IdentityCredentialUnitTests, verifyAttestationFailWithEmptyChallenge) {
    ...
    hidl_vec<hidl_vec<uint8_t>> attestationCertificate;
    vector<uint8_t> attestationApplicationId = {};
    writableCredential->getAttestationCertificate(
            attestationApplicationId, attestationChallenge,
            [&](const Result& _result, const hidl_vec<hidl_vec<uint8_t>>& _attestationCertificate) {
                result = _result;
    ...
    EXPECT_FALSE(ValidateAttestationCertificate(attestationCertificate));
}
...
TEST_F(IdentityCredentialUnitTests, verifyAttestationDoubleCallFails) {
    Result result;
    sp<IWritableIdentityCredential> writableCredential;
    ASSERT_TRUE(SetupWritableCredential(writableCredential));
    ...
    //
    ASSERT_EQ(ResultCode::OK, result.code);
    ASSERT_TRUE(ValidateAttestationCertificate(attestationCertificate));

    string challenge2 = "NotSoRandomChallenge2";
    vector<uint8_t> attestationChallenge2(challenge2.begin(), challenge2.end());
    writableCredential->getAttestationCertificate(
            attestationApplicationId, attestationChallenge2,
            [&](const Result& _result, const hidl_vec<hidl_vec<uint8_t>>& _attestationCertificate) {
                result = _result;
                attestationCertificate = _attestationCertificate;
            });
...
TEST_F(IdentityCredentialUnitTests, verifystartPersonalization) {
    ...
    const vector<uint16_t> entryCounts = {2, 4};
    writableCredential->startPersonalization(5, entryCounts,
                                             [&](const Result& _result) { result = _result; });
    EXPECT_EQ("", result.message);
    ASSERT_EQ(ResultCode::OK, result.code);

    writableCredential->startPersonalization(7, entryCounts,
                                             [&](const Result& _result) { result = _result; });
...
TEST_F(IdentityCredentialUnitTests, verifystartPersonalizationZero) {
    ...
    // Verify minimal number of profile count and entry count
    const vector<uint16_t> entryCounts = {0};
...
TEST_F(IdentityCredentialUnitTests, verifystartPersonalizationLarge) {
    ...
    // Verify minimal number of profile count and entry count
    const vector<uint16_t> entryCounts = {3000};
...
TEST_F(IdentityCredentialUnitTests, verifyProfileNumberMismatchShouldFail) {
    Result result;
    sp<IWritableIdentityCredential> writableCredential;
    ASSERT_TRUE(SetupWritableCredential(writableCredential));

    // Enter mismatched entry and profile numbers
    const vector<uint16_t> entryCounts = {5, 6};
    writableCredential->startPersonalization(5, entryCounts,
                                             [&](const Result& _result) { result = _result; });
    EXPECT_EQ("", result.message);
    ASSERT_EQ(ResultCode::OK, result.code);
...
TEST_F(IdentityCredentialUnitTests, verifyDuplicateProfileId) {
    ...
    const vector<TestProfile> testProfiles = {// first profile should go though
                                              ...
                                              {0, readerCertificate2.value(), true, 1},
...
TEST_F(IdentityCredentialUnitTests, verifyEmptyNameSpaceMixedWithNonEmptyWorks) {
    Result result;
    sp<IWritableIdentityCredential> writableCredential;
    ASSERT_TRUE(SetupWritableCredential(writableCredential));

    const vector<uint16_t> entryCounts = {2u, 2u};
    writableCredential->startPersonalization(3, entryCounts,
                                             [&](const Result& _result) { result = _result; });
    EXPECT_EQ("", result.message);
    ASSERT_EQ(ResultCode::OK, result.code);
...
TEST_F(IdentityCredentialUnitTests, verifyInterleavingEntryNameSpaceOrderingFails) {
    Result result;
    sp<IWritableIdentityCredential> writableCredential;
    ASSERT_TRUE(SetupWritableCredential(writableCredential));

    // Enter mismatched entry and profile numbers.
    // Technically the 2nd name space of "PersonalData" occurs intermittently, 2
    // before "Image" and 2 after image, which is not correct.  All of same name
    // space should occur together.  Let's see if this fails.
    const vector<uint16_t> entryCounts = {2u, 1u, 2u};
    writableCredential->startPersonalization(3, entryCounts,
                                             [&](const Result& _result) { result = _result; });
    EXPECT_EQ("", result.message);
    ASSERT_EQ(ResultCode::OK, result.code);
