...
optional<vector<vector<uint8_t>>> createAttestation(
    ...
    if (expireTimeMilliSeconds == 0) {
        ...
        time_t bcNotAfter;
        if (!parseAsn1Time(X509_getm_notAfter(bc.get()), &bcNotAfter)) {
...
optional<pair<time_t, time_t>> certificateGetValidity(const vector<uint8_t>& x509Certificate) {
    ...
    time_t notBefore;
    time_t notAfter;
    if (!parseAsn1Time(X509_getm_notBefore(certs[0].get()), &notBefore)) {
        LOG(ERROR) << "Error parsing notBefore";
        return {};
    }

    if (!parseAsn1Time(X509_getm_notAfter(certs[0].get()), &notAfter)) {
...
optional<vector<uint8_t>> calcMac(const vector<uint8_t>& sessionTranscriptEncoded,
    ...
    deviceAuthentication.add(std::move(sessionTranscriptItem));
    deviceAuthentication.add(docType);
    deviceAuthentication.add(cppbor::Semantic(24, deviceNameSpacesEncoded));
