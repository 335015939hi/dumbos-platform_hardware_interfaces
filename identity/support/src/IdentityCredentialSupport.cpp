...
bool coseMac0(const vector<uint8_t>& key, const vector<uint8_t>& data,
    ...
    vector<uint8_t> encodedProtectedHeaders = coseEncodeHeaders(protectedHeaders);
    vector<uint8_t> toBeMACed = coseBuildToBeMACed(encodedProtectedHeaders, data, additionalData);

    vector<uint8_t> mac;
    if (!hmacSha256(key, toBeMACed, mac)) {
