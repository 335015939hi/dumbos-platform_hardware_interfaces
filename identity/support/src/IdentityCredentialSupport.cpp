...
bool cborHasBstrInternal(const cppbor::Item* item, const vector<uint8_t>& valueBstr) {
    switch (item->type()) {
        case cppbor::BSTR: {
            const cppbor::Bstr* bstr = item->asBstr();
            if (valueBstr.size() == bstr->value().size() &&
                memcmp(valueBstr.data(), bstr->value().data(), valueBstr.size()) == 0) {
                return true;
            }
        }
            return false;
        ...
        case cppbor::ARRAY: {
            ...
            for (size_t n = 0; n < array->size(); n++) {
                const unique_ptr<cppbor::Item>& element = (*array)[n];
                if (cborHasBstrInternal(element.get(), valueBstr)) {
                    return true;
                }
            }
        }
        ...
        case cppbor::MAP: {
            ...
            for (size_t n = 0; n < map->size(); n++) {
                auto [keyElement, valueElement] = (*map)[n];
                if (cborHasBstrInternal(keyElement.get(), valueBstr) ||
                    cborHasBstrInternal(valueElement.get(), valueBstr)) {
                    return true;
                }
            }
        }
...
bool cborHasBstr(const vector<uint8_t>& encodedCbor, const vector<uint8_t>& valueBstr) {
    if (encodedCbor.size() == 0) {
        return false;
    }
    auto [item, _, message] = cppbor::parse(encodedCbor);
    if (item == nullptr) {
        LOG(ERROR) << "Data to search is not valid CBOR: " << message;
        return false;
    }
    return cborHasBstrInternal(item.get(), valueBstr);
}
...
bool cborPrettyPrint(const vector<uint8_t>& encodedCbor, string& out, size_t maxBStrSize,
    ...
    return cborPrettyPrintInternal(item.get(), out, 0, maxBStrSize, mapKeysToNotPrint);
}
...
bool certificateCheckSignedByPublicKey(const vector<uint8_t>& certificate,
                                       const vector<uint8_t>& publicKey) {
    const unsigned char* p = (unsigned char*)certificate.data();
    const unsigned char* pEnd = p + certificate.size();
    X509_Ptr x509 = X509_Ptr(d2i_X509(nullptr, &p, pEnd - p));
    if (x509 == nullptr) {
        LOG(ERROR) << "Error parsing X509 certificate";
        return false;
    }
...
bool checkEcDsaSignatureUsingCertificateChain(const vector<uint8_t>& digest,
                                              const vector<uint8_t>& signature,
                                              const vector<uint8_t>& certificateChain) {
    vector<uint8_t> topmostPublicKey;
    if (!support::certificateChainGetTopMostKey(certificateChain, topmostPublicKey)) {
        LOG(ERROR) << "Error getting top-most public key from chain";
        return false;
    }
    return checkEcDsaSignature(digest, signature, topmostPublicKey);
}
...
bool coseSignEcDsa(const vector<uint8_t>& key, const vector<uint8_t>& data,
                   const vector<uint8_t>& additionalData, const vector<uint8_t>& certificateChain,
    ...
    if (certificateChain.size() != 0) {
        ...
        if (!support::certificateChainSplit(certificateChain, certs)) {
            LOG(ERROR) << "Error splitting certificate chain";
            return false;
        }
        if (certs.size() == 1) {
            unprotectedHeaders.add(COSE_LABEL_X5CHAIN, certs[0]);
        } else {
            cppbor::Array certArray;
            for (const vector<uint8_t>& cert : certs) {
                certArray.add(cert);
            }
    ...
    vector<uint8_t> toBeSigned = coseBuildToBeSigned(encodedProtectedHeaders, data, additionalData);

    // string out;
    // cborPrettyPrint(toBeSigned, out, 1024);
    // fprintf(stderr, "toBeSigned: \n%s\n\n", out.c_str());

    vector<uint8_t> signature;
    if (!signEcDsa(key, toBeSigned, signature)) {
...
bool coseCheckEcDsaSignature(const vector<uint8_t>& signatureCoseSign1,
    ...
    const vector<uint8_t>& signature = signatureBstr->value();

    vector<uint8_t> toBeSigned = coseBuildToBeSigned(encodedProtectedHeaders, data, additionalData);

    // string out;
    // cborPrettyPrint(toBeSigned, out, 1024);
    // fprintf(stderr, "Verifying toBeSigned: \n%s\n\n", out.c_str());

    if (!checkEcDsaSignature(support::sha256(toBeSigned), signature, publicKey)) {
...
// From https://tools.ietf.org/html/rfc8152 table 3
const int COSE_KEY_LABEL_KTY = 1;

// From https://tools.ietf.org/html/rfc8152 table 21
const int COSE_KEY_TYPE_EC2 = 2;  // Elliptic Curve Keys w/ x- and y-coordinate pair

// From https://tools.ietf.org/html/rfc8152, 13.1.1. Double Coordinate Curves
const int COSE_KEY_TYPE_EC2_CRV = -1;
const int COSE_KEY_TYPE_EC2_X = -2;
const int COSE_KEY_TYPE_EC2_Y = -3;

// From https://tools.ietf.org/html/rfc8152, 13.1. Elliptic Curve Keys
const int COSE_KEY_EC2_P256 = 1;
...
bool coseCreateKey(const vector<uint8_t>& key, vector<uint8_t>& coseKey) {
    ...
    map.add(COSE_KEY_LABEL_KTY, COSE_KEY_TYPE_EC2);
    map.add(COSE_KEY_TYPE_EC2_CRV, COSE_KEY_EC2_P256);
    map.add(COSE_KEY_TYPE_EC2_X, x);
    map.add(COSE_KEY_TYPE_EC2_Y, y);
...
bool validateAuthToken(const vector<uint8_t>& authToken) {
    LOG(INFO) << "TODO: authenticate the authToken";
    return true;
}
...
bool checkReaderAuthentication(const SecureAccessControlProfile& profile,
    ...
    if (!certificateChainGetTopMostKey(profile.readerCertificate, acpPubKey)) {
        LOG(ERROR) << "Error extracting public key from readerCertificate in profile";
        return false;
    }

    vector<vector<uint8_t>> certificatesInChain;
    if (!certificateChainSplit(readerCertificateChain, certificatesInChain)) {
    ...
    for (const vector<uint8_t>& certInChain : certificatesInChain) {
        ...
        if (!certificateChainGetTopMostKey(certInChain, certPubKey)) {
            LOG(ERROR)
...
bool checkUserAuthentication(const SecureAccessControlProfile& profile,
    ...
    LOG(ERROR) << "TODO: implement checkUserAuthentication()";
    return true;
}
...
bool secureAccessControlProfileEncodeCbor(const SecureAccessControlProfile& profile,
    ...
    if (profile.userAuthenticationRequired) {
        ...
        map.add("timeout", profile.timeout);
    }
