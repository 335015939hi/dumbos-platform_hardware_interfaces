...
optional<vector<uint8_t>> createAttestation(EVP_PKEY* key, const vector<uint8_t>& applicationId,
    ...
    for (int i = 0; i < cert_chain_out->entry_count; i++) {
        attestationCertificate.insert(
                attestationCertificate.end(), cert_chain_out->entries[i].data,
...
optional<std::pair<vector<uint8_t>, vector<uint8_t>>> createEcKeyPairAndAttestation(
