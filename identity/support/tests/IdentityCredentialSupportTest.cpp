...
TEST(IdentityCredentialSupport, CoseSignatures) {
    ...
    ASSERT_TRUE(support::coseSignEcDsa(privKey, data, {} /* detachedContent */, {} /* x5chain */,
                                       coseSign1));
    ASSERT_TRUE(support::coseCheckEcDsaSignature(coseSign1, {} /* detachedContent */, pubKey));
...
TEST(IdentityCredentialSupport, CoseSignaturesX5ChainWithSingleCert) {
    ...
    vector<uint8_t> coseSign1;
    ASSERT_TRUE(
            support::coseSignEcDsa(privKey, {} /* data */, detachedContent, certChain, coseSign1));
    ASSERT_TRUE(support::coseCheckEcDsaSignature(coseSign1, detachedContent, pubKey));
...
TEST(IdentityCredentialSupport, hmacSha256) {
    ...
    ASSERT_TRUE(support::decodeHex(
            "f7bc83f430538424b13298e6aa6fb143ef4d59a14946175997479dbc2d1a3cd8", expected));

    vector<uint8_t> hmac;
    ASSERT_TRUE(support::hmacSha256(key, data, hmac));
...
TEST(IdentityCredentialSupport, CoseMac0) {
    ...
    ASSERT_TRUE(support::coseMac0(key, data, detachedContent, mac));
    string out;
    support::cborPrettyPrint(mac, out);
    EXPECT_EQ(
            "[\n"
            "  {0xa1, 0x01, 0x05},\n"
            "  {},\n"
