...
// Checks that |signature| (in DER format) is a valid signature of |digest|,
// made with the top-most certificate in |certificateChain|.
//
...
// ecKeyPairGetPublicKey()) where |additionalData| is the detached content.
//
...
// Returns |true| only if |authToken| fulfills the user authentication
// requirements encoded in |profile|.
//
// Note that this doesn't check |profile.readerCertificate|, only checks specific
// to |authToken| are performed.
//
