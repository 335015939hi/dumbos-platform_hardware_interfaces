...
class TestCredentialTests : public testing::TestWithParam<testing::tuple<string, int>> {
  public:
...
TEST_P(TestCredentialTests, testCredential) {
    ...
    int expectedNumCredentialKeyItems = 2;
    if (featureLevel_ >= IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_12) {
        expectedNumCredentialKeyItems = 3;
    }
    ASSERT_EQ(dckArrayItem->size(), expectedNumCredentialKeyItems);
    ...
    // Check that SHA-256(ProofOfProvisioning) matches
    if (featureLevel_ >= IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_12) {
