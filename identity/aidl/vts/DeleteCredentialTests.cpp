...
TEST_P(DeleteCredentialTests, Delete) {
    ...
    EXPECT_EQ("['ProofOfDeletion', 'org.iso.18013-5.2019.mdl', true, ]",
              cborPretty);
    EXPECT_TRUE(support::coseCheckEcDsaSignature(proofOfDeletionSignature,
                                                 {},  // Additional data
                                                 credentialPubKey_));
}
...
TEST_P(DeleteCredentialTests, DeleteWithChallenge) {
    ...
    vector<uint8_t> proofOfDeletionSignature;
    ASSERT_TRUE(credential->deleteCredentialWithChallenge(challenge,
                                                          &proofOfDeletionSignature).isOk());
    optional<vector<uint8_t>> proofOfDeletion =
            support::coseSignGetPayload(proofOfDeletionSignature);
    ASSERT_TRUE(proofOfDeletion);
    string cborPretty = support::cborPrettyPrint(proofOfDeletion.value(), 32, {});
    // Challenge isn't included because we haven't selected FEATURE_LEVEL_ANDROID_12
    EXPECT_EQ("['ProofOfDeletion', 'org.iso.18013-5.2019.mdl', true, ]",
              cborPretty);
    EXPECT_TRUE(support::coseCheckEcDsaSignature(proofOfDeletionSignature,
                                                 {},  // Additional data
...
TEST_P(DeleteCredentialTests, Delete_FL12) {
    provisionData();
    ...
    ASSERT_TRUE(credentialStore_
                        ->getCredential(
                                ...
                                credentialData_, &credential)
                        .isOk());
    ASSERT_TRUE(credential->setFeatureLevel(
        IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_12).isOk());
    ...
    ASSERT_TRUE(proofOfDeletion);
    string cborPretty = support::cborPrettyPrint(proofOfDeletion.value(), 32, {});
    // As per the API contract, this contains an empty challenge because we've selected
...
TEST_P(DeleteCredentialTests, DeleteWithChallenge_FL12) {
    provisionData();
    ...
    ASSERT_TRUE(proofOfDeletion);
    string cborPretty = support::cborPrettyPrint(proofOfDeletion.value(), 32, {});
    // Challenge *is* included because we have selected FEATURE_LEVEL_ANDROID_12
