...
bool verifyAuthKeyCertificate(const vector<uint8_t>& authKeyCertificate,
    ...
    const uint8_t* data = authKeyCertificate.data();
    auto cert = X509_Ptr(d2i_X509(nullptr, &data, authKeyCertificate.size()));
    ...
    EXPECT_EQ("CN=Android Identity Credential Authentication Key",
              x509NameToRfc2253String(subject));
    ...
    EXPECT_EQ("CN=Android Identity Credential Key", x509NameToRfc2253String(issuer));

    //  - subjectPublicKeyInfo: must contain attested public key.

    //  - validity: should be from current time and one year in the future (365 days).
    time_t notBefore, notAfter;
    if (!parseAsn1Time(X509_getm_notAfter(cert.get()), &notAfter)) {
        return false;
    }
    if (!parseAsn1Time(X509_getm_notBefore(cert.get()), &notBefore)) {
        return false;
    }
