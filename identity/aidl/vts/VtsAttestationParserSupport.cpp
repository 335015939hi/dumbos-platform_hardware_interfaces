...
optional<vector<uint8_t>> AttestationCertificateParser::GetApplicationId() {
    keymaster_blob_t id = {nullptr, 0};
    if (!att_sw_enforced_.GetTagValue(TAG_ATTESTATION_APPLICATION_ID, &id)) {
        return {};
    }

    vector<uint8_t> idVector(id.data, id.data + id.data_length);
    return idVector;
}
...
bool AttestationCertificateParser::GetSwEnforcedBool(TypedTag<KM_BOOL, Tag> tag) {
...
template <keymaster_tag_t Tag>
bool AttestationCertificateParser::GetHwEnforcedBool(TypedTag<KM_BOOL, Tag> tag) {
