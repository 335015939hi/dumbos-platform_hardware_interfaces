...
class UserAuthTests : public testing::TestWithParam<string> {
  public:
    ...
    void retrieveData(HardwareAuthToken authToken, VerificationToken verificationToken,
                      bool expectSuccess, bool leaveOutAccessibleToAllFromRequestMessage);
    ...
    cppbor::Array sessionTranscript_;
...
void UserAuthTests::provisionData() {
    ...
    ASSERT_TRUE(wc->addAccessControlProfile(0, {}, true, 0, 0, &sacp0_).isOk());

    // Access control profile 1: user auth, 60 seconds timeout
    ASSERT_TRUE(wc->addAccessControlProfile(1, {}, true, 60000, 0, &sacp1_).isOk());
...
cppbor::Array calcSessionTranscript(const vector<uint8_t>& ePublicKey) {
...
void UserAuthTests::retrieveData(HardwareAuthToken authToken, VerificationToken verificationToken,
    ...
    auto [readerPublicKey, readerPrivateKey] = generateReaderKey();
    vector<uint8_t> certReaderSelfSigned = generateReaderCert(readerPublicKey, readerPrivateKey);

    optional<vector<uint8_t>> readerSignature =
            support::coseSignEcDsa(readerPrivateKey,  // private key for reader
                                   {},                // content
                                   dataToSign,        // detached content
                                   certReaderSelfSigned);
    ...
    vector<uint8_t> sessionTranscriptBytes = sessionTranscript_.encode();
    Status status = credential_->startRetrieval(
            {sacp0_, sacp1_, sacp2_}, authToken, itemsRequestBytes, signingKeyBlob,
            sessionTranscriptBytes, readerSignature.value(), {4 /* numDataElementsPerNamespace */});
    if (expectSuccess) {
