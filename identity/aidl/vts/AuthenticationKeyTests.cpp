...
class AuthenticationKeyTests : public testing::TestWithParam<testing::tuple<string, int>> {
  public:
    virtual void SetUp() override {
      featureLevel_ = std::get<1>(GetParam());
      string halInstanceName = std::get<0>(GetParam());
...
TEST_P(AuthenticationKeyTests, proofOfProvisionInAuthKeyCert) {
    ...
    ASSERT_TRUE(credentialStore_->createCredential(docType,
                                                   true,  // testCredential
                                                   &wc).isOk());
    wc->setFeatureLevel(featureLevel_);
    ...
    optional<vector<uint8_t>> optCredentialPubKey = support::certificateChainGetTopMostKey(
            certChain[0].encodedCertificate);
    ...
    ASSERT_TRUE(credentialStore_
                        ...
                        .isOk());
    ASSERT_TRUE(credential->setFeatureLevel(featureLevel_).isOk());
    ...
    // On Feature Level 12 and later, SHA-256(ProofOfProvisioning) is embedded in CBOR
    ...
    optional<vector<uint8_t>> proofOfBinding = support::certificateGetExtension(
        signingKeyCertificate.encodedCertificate,
        "1.3.6.1.4.1.11129.2.1.25");
    if (featureLevel_ == IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_11) {
      ASSERT_FALSE(proofOfBinding);
    } else {
...
INSTANTIATE_TEST_SUITE_P(
        ...
        testing::Combine(
            ...
            testing::Values(IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_11,
                            IIdentityCredentialStore::FEATURE_LEVEL_ANDROID_12)),
        test_utils::printInstanceNameAndFeatureLevel);
