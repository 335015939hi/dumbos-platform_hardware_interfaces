...
TEST_P(IdentityCredentialUnitTests, verifyAttestationDoubleCallFails) {
   ...
   */
    ...
    /*    vector<uint8_t> attestationChallenge2(challenge2.begin(), challenge2.end());
        result = writableCredential->getAttestationCertificate(
                attestationApplicationId, attestationChallenge2, &attestationCertificate);
    */
...
TEST_P(IdentityCredentialUnitTests, verifyDuplicateProfileId) {
    ...
    for (const auto& testProfile : testProfiles) {
        ...
        Certificate cert;
        cert.encodedCertificate = testProfile.readerCertificate;
        writableCredential->addAccessControlProfile(testProfile.id, cert,
...
TEST_P(IdentityCredentialUnitTests, verifyOneProfileAndEntryPass) {
    ...
    for (const auto& entry : testEntries1) {
        ASSERT_TRUE(AddEntry(writableCredential, entry, hwInfo.dataChunkSize, encryptedBlobs));
    }
...
TEST_P(IdentityCredentialUnitTests, verifyManyProfilesAndEntriesPass) {
    ...
    map<const TestEntryData*, vector<vector<uint8_t>>> encryptedBlobs;
    for (const auto& entry : testEntries1) {
        EXPECT_TRUE(AddEntry(writableCredential, entry, hwInfo.dataChunkSize, encryptedBlobs));
    }
...
TEST_P(IdentityCredentialUnitTests, verifyInterleavingEntryNameSpaceOrderingFails) {
    ...
    EXPECT_TRUE(AddEntry(writableCredential, testEntry2, hwInfo.dataChunkSize, encryptedBlobs));
    ...
    for (const auto& entry : testEntries3) {
        EXPECT_FALSE(AddEntry(writableCredential, entry, hwInfo.dataChunkSize, encryptedBlobs));
    }
