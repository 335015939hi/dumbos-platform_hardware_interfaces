...
TEST_P(ProveOwnershipTests, proveOwnership_FL11) {
    provisionData();
    ...
    // Should fail with STATUS_FAILED on feature level 11
    Status result = credential->proveOwnership(challenge, &proofOfOwnershipSignature);
...
TEST_P(ProveOwnershipTests, proveOwnership) {
    ...
    EXPECT_EQ("['ProofOfOwnership', 'org.iso.18013-5.2019.mdl', {0x11, 0x12}, true, ]",
              cborPretty);
    EXPECT_TRUE(support::coseCheckEcDsaSignature(proofOfOwnershipSignature,
                                                 {},  // Additional data
                                                 credentialPubKey_));
}
