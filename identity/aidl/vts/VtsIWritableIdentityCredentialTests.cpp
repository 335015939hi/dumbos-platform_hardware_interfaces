...
TEST_P(IdentityCredentialTests, verifyAttestationWithEmptyChallenge) {
    ...
    EXPECT_TRUE(result.isOk()) << result.exceptionCode() << "; " << result.exceptionMessage()
                               << endl;
    EXPECT_EQ(binder::Status::EX_NONE, result.exceptionCode());
    EXPECT_EQ(IIdentityCredentialStore::STATUS_OK, result.serviceSpecificErrorCode());
...
TEST_P(IdentityCredentialTests, verifyStartPersonalization) {
    ...
    // Call personalization again to verify if repeat call is allowed.EX_SECURITY
...
TEST_P(IdentityCredentialTests, verifyProfileNumberMismatchShouldFail) {
    ...
    const vector<test_utils::TestProfile> testProfiles = {// Profile 0 (reader authentication)
                                                          ...
                                                          {4, {}, false, 0}};
    ASSERT_TRUE(test_utils::AddAccessControlProfiles(writableCredential, testProfiles));
...
TEST_P(IdentityCredentialTests, verifyDuplicateProfileId) {
    ...
    optional<vector<uint8_t>> readerCertificate1 = test_utils::GenerateReaderCertificate("12345");
    ASSERT_TRUE(readerCertificate1);

    optional<vector<uint8_t>> readerCertificate2 =
            test_utils::GenerateReaderCertificate("1234567890");
...
TEST_P(IdentityCredentialTests, verifyOneProfileAndEntryPass) {
    ...
    EXPECT_EQ(attData.result.exceptionCode(), binder::Status::EX_NONE);
    EXPECT_EQ(IIdentityCredentialStore::STATUS_OK, result.serviceSpecificErrorCode());
