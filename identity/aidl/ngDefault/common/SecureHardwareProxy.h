...
class SecureHardwareProvisioningProxy : public RefBase {
  public:
    ...
    virtual optional<vector<uint8_t>> finishGetCredentialData(bool testCredential,
...
class SecureHardwarePresentationProxy : public RefBase {
  public:
    ...
    virtual bool setTopLevelReaderCert(const vector<uint8_t>& certX509) = 0;
    ...
    virtual bool validateNextReaderCert(const vector<uint8_t>& certX509) = 0;

    virtual bool setHardwareAuthToken(uint64_t challenge, uint64_t secureUserId,
                                      uint64_t authenticatorId, int hardwareAuthenticatorType,
                                      uint64_t timeStamp, const vector<uint8_t>& mac) = 0;
