...
bool eicOpsAttestToEcKey(const uint8_t publicKey[EIC_P256_PUB_KEY_SIZE], const uint8_t* challenge,
    ...
    optional<vector<vector<uint8_t>>> chain =
            android::hardware::identity::support::createAttestationForEcPublicKey(
                    pubKeyVec, challengeVec, applicationIdVec);
    if (!chain) {
        eicDebug("Error generating attestation");
        return false;
    }
    vector<uint8_t> flatChain =
            android::hardware::identity::support::certificateChainJoin(chain.value());
...
bool eicOpsValidateAuthToken(uint64_t challenge, uint64_t secureUserId, uint64_t authenticatorId,
                             ...
                             size_t macSize, uint64_t verificationTokenTimestamp,
