...
// The maximum size we support for X.509 certificates.
#define EIC_PRESENTATION_MAX_X509_READER_CERT_SIZE 512
...
typedef struct {
    ...
    // Place-holder for the current X509 certificate being verified.
    //
    // TODO: change the code to feed in X.509 certs in smaller bits
    // so we don't need to store an entire cert at one time...
    //
    uint8_t curX509ReaderCert[EIC_PRESENTATION_MAX_X509_READER_CERT_SIZE];
    size_t curX509ReaderCertSize;

    // This is a bitmask indicating which access control profiles are
    // authrorized. Built up by eicPresentationValidateAccessControlProfile().
...
// certificate set with eicPresentationSetTopLevelReaderCert().
//
// The format of the signature is the same encoding as the 'signature' field of
// COSE_Sign1 - that is, it's the R and S integers both with the same length as
// the key-size.
//
// Must be called right after that function and before
// eicPresentationValidateNextReaderCert().
//
...
//
// Returns in |outAccessGranted| whether access is granted.
//
...
typedef enum {
    ...
    EIC_ACCESS_CHECK_RESULT_NOT_IN_REQUEST_MESSAGE,
