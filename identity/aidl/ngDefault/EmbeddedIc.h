...
typedef struct {
    ...
    // The SHA-256 for the reader certificate.
    //
    // (During the process of pushing reader certificates, this is also used to store
    // the SHA-256 of the previously pushed certificate.)
    uint8_t readerCertificateSha256[EIC_SHA256_DIGEST_SIZE];

    // This is a bitmask indicating which access control profiles are
    // authorized. Built up by eicPresentationValidateAccessControlProfile().
    //
    // Since we need to return detailed information of why a certain data element
    // isn't authorized (see eicPresentationStartRetrieveEntryValue()), we record
    // three bits per profile:
    //
...
// Doesn't actually return if access is granted, that's instead recorded internally.
//
// If |readerCertificate| is non-empty and one of those certificates appear in the
// chain presented by the reader, this function must be called after pushing that
// certificate using eicPresentationPushReaderCert().
//
