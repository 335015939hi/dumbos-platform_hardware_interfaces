...
bool eicPresentationPushReaderCert(EicPresentation* ctx, const uint8_t* certX509,
    ...
    // Store SHA-256 of cert for use in readerAuth checks...
    EicSha256Ctx digester;
    eicOpsSha256Init(&digester);
    eicOpsSha256Update(&digester, certX509, certX509Size);
    eicOpsSha256Final(&digester, ctx->readerCertificateSha256);
...
static bool checkReaderAuth(EicPresentation* ctx, const uint8_t* readerCertificate,
                            size_t readerCertificateSize) {
    uint8_t readerCertificateSha256[EIC_SHA256_DIGEST_SIZE];
    EicSha256Ctx digester;
    eicOpsSha256Init(&digester);
    eicOpsSha256Update(&digester, readerCertificate, readerCertificateSize);
    eicOpsSha256Final(&digester, readerCertificateSha256);

    if (eicMemCmp(ctx->readerCertificateSha256, readerCertificateSha256, EIC_SHA256_DIGEST_SIZE) ==
        0) {
        return true;
    }
    return false;
...
// Note: This function returns false _only_ if an error occurred check for access, _not_ if
...
bool eicPresentationValidateAccessControlProfile(EicPresentation* ctx, int id,
    ...
    bool passedUserAuth = checkUserAuth(ctx, userAuthenticationRequired, timeoutMillis);
...
EicAccessCheckResult eicPresentationStartRetrieveEntryValue(EicPresentation* ctx,
    ...
    //
    EicAccessCheckResult result = EIC_ACCESS_CHECK_RESULT_OK;
    for (size_t n = 0; n < numAccessControlProfileIds; n++) {
        ...
        // fail immediately.
        if ((ctx->accessControlProfileMaskValidated & idBitMask) == 0) {
            eicDebug("No ACP for profile id %d", id);
            return EIC_ACCESS_CHECK_RESULT_FAILED;
        }
        // Otherwise, we _did_ validate the profile. If none of the checks
        // failed, we're done
        if ((ctx->accessControlProfileMaskFailedReaderAuth & idBitMask) == 0 &&
