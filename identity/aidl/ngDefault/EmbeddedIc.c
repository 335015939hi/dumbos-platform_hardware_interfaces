...
#define EIC_MAX_READER_PUBLIC_KEY_SIZE 65
...
static size_t eicCborAdditionalLengthBytesFor(size_t size) {
    if (size < 24) {
        return 0;
    } else if (size <= 0xff) {
        return 1;
...
static bool setCurReaderCert(EicPresentation* ctx, const uint8_t* certX509, size_t certX509Size) {
    if (certX509Size > EIC_PRESENTATION_MAX_X509_READER_CERT_SIZE) {
        eicDebug("X.509 certificate size %zd exceeds maximum size %zd", certX509Size,
                 EIC_PRESENTATION_MAX_X509_READER_CERT_SIZE);
        return false;
    }
    eicMemCpy(ctx->curX509ReaderCert, certX509, certX509Size);
    ctx->curX509ReaderCertSize = certX509Size;
...
bool eicPresentationSetTopLevelReaderCert(EicPresentation* ctx, const uint8_t* certX509,
                                          size_t certX509Size) {
    if (!setCurReaderCert(ctx, certX509, certX509Size)) {
        return false;
    }
    return true;
}
...
bool eicPresentationValidateRequestMessage(EicPresentation* ctx, const uint8_t* sessionTranscript,
    ...
    uint8_t publicKey[EIC_MAX_READER_PUBLIC_KEY_SIZE];
    size_t publicKeySize = sizeof(publicKey);

    if (!eicOpsX509GetPublicKey(ctx->curX509ReaderCert, ctx->curX509ReaderCertSize, publicKey,
                                &publicKeySize)) {
        eicDebug("Error extracting public key from top-level certificate");
        return false;
    }
    ...
    eicDebug("SessionTranscriptSize 0x%02zx at offset 0x%02zx", sessionTranscriptSize,
             cbor.size - payloadOffset);
    ...
    eicDebug("requestMessage bstr size 0x%02zx at offset 0x%02zx", requestMessageSize,
             cbor.size - payloadOffset);
    ...
    if (!eicOpsEcDsaVerifyWithPublicKey(
                ...
                readerSignatureOfToBeSignedSize, publicKey, publicKeySize)) {
...
bool eicPresentationValidateNextReaderCert(EicPresentation* ctx, const uint8_t* certX509,
    ...
    if (!eicOpsX509GetPublicKey(certX509, certX509Size, publicKey, &publicKeySize)) {
        eicDebug("Error extracting public key from certificate");
        return false;
    }
    if (!eicOpsX509CertSignedByPublicKey(ctx->curX509ReaderCert, ctx->curX509ReaderCertSize,
                                         publicKey, publicKeySize)) {
        eicDebug("Previous certificate not signed by public key in certificate");
        return false;
    }
