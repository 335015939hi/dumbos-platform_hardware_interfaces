...
optional<vector<uint8_t>> FakeSecureHardwareProvisioningProxy::finishAddingEntries(
    ...
    if (!eicProvisioningFinishAddingEntries(&ctx_, testCredential, signatureOfToBeSigned.data())) {
        return {};
    }
...
optional<vector<uint8_t>> FakeSecureHardwareProvisioningProxy::finishGetCredentialData(
    ...
    if (!eicProvisioningFinishGetCredentialData(&ctx_, testCredential, docType.c_str(),
...
bool FakeSecureHardwarePresentationProxy::setTopLevelReaderCert(const vector<uint8_t>& certX509) {
    return eicPresentationSetTopLevelReaderCert(&ctx_, certX509.data(), certX509.size());
}
...
bool FakeSecureHardwarePresentationProxy::validateNextReaderCert(const vector<uint8_t>& certX509) {
    return eicPresentationValidateNextReaderCert(&ctx_, certX509.data(), certX509.size());
}

bool FakeSecureHardwarePresentationProxy::setHardwareAuthToken(
    ...
    return eicPresentationSetHardwareAuthToken(&ctx_, challenge, secureUserId, authenticatorId,
                                               hardwareAuthenticatorType, timeStamp, mac.data(),
                                               mac.size());
}
...
optional<bool> FakeSecureHardwarePresentationProxy::validateAccessControlProfile(
    ...
    if (!eicPresentationValidateAccessControlProfile(
                &ctx_, id, readerCertificate.data(), readerCertificate.size(),
                userAuthenticationRequired, timeoutMillis, secureUserId, mac.data(), &hasAccess)) {
        return {};
    }
    return hasAccess;
}
...
AccessCheckResult FakeSecureHardwarePresentationProxy::startRetrieveEntryValue(
    ...
    EicAccessCheckResult result = eicPresentationStartRetrieveEntryValue(
            &ctx_, nameSpace.c_str(), name.c_str(), entrySize, accessControlProfileIds.data(),
            accessControlProfileIds.size());
    switch (result) {
        ...
        case EIC_ACCESS_CHECK_RESULT_NOT_IN_REQUEST_MESSAGE:
            return AccessCheckResult::kNotInRequestMessage;
...
optional<vector<uint8_t>> FakeSecureHardwarePresentationProxy::retrieveEntryValue(
    ...
    if (!eicPresentationRetrieveEntryValue(&ctx_, encryptedContent.data(), encryptedContent.size(),
                                           content.data(), nameSpace.c_str(), name.c_str(),
                                           accessControlProfileIds.data(),
                                           accessControlProfileIds.size())) {
        return {};
    }
    return content;
...
static const uint8_t eicAttestationPublicKey[EIC_P256_PUB_KEY_SIZE] = {
        0x3a, 0x16, 0x66, 0x7e, 0xc6, 0x70, 0x5c, 0x81, 0x36, 0x9c, 0x53, 0xe2, 0xb8,
        0x39, 0x15, 0x2b, 0x71, 0xad, 0x08, 0xd0, 0xa0, 0x63, 0x05, 0x12, 0xe6, 0xb9,
        0x1b, 0x96, 0x58, 0xbe, 0x91, 0xbf, 0x9c, 0xc2, 0x53, 0x02, 0x46, 0x0c, 0xd9,
        0xd3, 0x53, 0x36, 0xf4, 0xf9, 0x9b, 0x42, 0x5d, 0xa1, 0x91, 0x91, 0xcb, 0x23,
        0x23, 0xb8, 0xb5, 0x3b, 0xe9, 0x45, 0xd9, 0xd8, 0x06, 0x29, 0xac, 0x47};

static const uint8_t eicAttestationPrivateKey[EIC_P256_PRIV_KEY_SIZE] = {
        0x81, 0xdb, 0x31, 0x67, 0x52, 0xb9, 0xab, 0xc5, 0xe8, 0xc3, 0x1c,
        0xb9, 0x92, 0xb3, 0x06, 0x27, 0x33, 0x81, 0xf7, 0xc0, 0xdb, 0x98,
        0x93, 0x37, 0x4f, 0xae, 0x08, 0xc0, 0x5e, 0x3f, 0x7e, 0x4a};
...
bool eicOpsAttestToEcKey(const uint8_t publicKey[EIC_P256_PUB_KEY_SIZE], const uint8_t* challenge,
    ...
    optional<vector<uint8_t>> certVec =
            android::hardware::identity::support::ecPublicKeyGenerateCertificate(
                    pubKeyVec, signingKeyVec, "1", eicOpsGetIssuerName(),
                    "Android Identity Credential Credential Key", validityNotBefore,
                    validityNotAfter);
    if (!certVec) {
...
bool eicOpsGetTimestamp(uint64_t* timeStamp) {
    ...
    uint64_t milliSeconds = time.tv_sec * 1000 + time.tv_nsec / 1000000;
...
bool eicOpsX509CertSignedByPublicKey(const uint8_t* x509Cert, size_t x509CertSize,
    ...
    eicDebug("TODO: eicOpsX509CertSignedByPublicKey");
    return true;
}
