...
bool eicPresentationGenerateSigningKeyPair(EicPresentation* ctx, const char* docType, time_t now,
    ...
    // and stored at 1.3.6.1.4.1.11129.2.1.25 (TODO: allocate it).
    //
    const uint8_t* proofOfBinding = NULL;
    size_t proofOfBindingSize = 0;
    if (ctx->featureLevel >= EIC_FEATURE_LEVEL_ANDROID_12) {
        EicCbor cbor;
        ...
        proofOfBinding = cborBuf;
        proofOfBindingSize = cbor.size;
    }
...
bool eicPresentationCalcMacKey(EicPresentation* ctx, const uint8_t* sessionTranscript,
                               ...
                               size_t expectedDeviceNamespacesSize, uint8_t* scratchSpace,
    ...
    if (scratchSpaceSize < EIC_P256_COORDINATE_SIZE) {
        eicDebug("Insufficient scratch space buffer");
        return false;
    }
    uint8_t* sharedSecret = scratchSpace;
    ...
    EicCbor cbor;
    size_t scratchSpaceSizeForCbor = scratchSpaceSize - EIC_P256_COORDINATE_SIZE;
    eicCborInit(&cbor, scratchSpace + EIC_P256_COORDINATE_SIZE, scratchSpaceSizeForCbor);
    eicCborAppendSemantic(&cbor, EIC_CBOR_SEMANTIC_TAG_ENCODED_CBOR);
    eicCborAppendByteString(&cbor, sessionTranscript, sessionTranscriptSize);
    if (cbor.size > scratchSpaceSizeForCbor) {
        eicDebug("Insufficient scratch space buffer (need %zd bytes, usable buffer is %zd)",
                 cbor.size, scratchSpaceSizeForCbor);
        return false;
    }

    const uint8_t* sharedSecretWithSessionTranscriptBytes = scratchSpace;
    size_t sharedSecretWithSessionTranscriptBytesSize = EIC_P256_COORDINATE_SIZE + cbor.size;

    const uint8_t salt[1] = {0x00};
    const uint8_t info[0] = {};
    uint8_t derivedKey[32];
    if (!eicOpsHkdf(sharedSecretWithSessionTranscriptBytes,
                    sharedSecretWithSessionTranscriptBytesSize, salt, sizeof(salt), info,
