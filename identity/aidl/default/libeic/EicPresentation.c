...
bool eicPresentationInit(EicPresentation* ctx, uint64_t sessionId, bool testCredential,
                         const char* docType, const uint8_t* encryptedCredentialKeys,
...
bool eicPresentationValidateRequestMessage(EicPresentation* ctx, const uint8_t* sessionTranscript,
    ...
    if (ctx->sessionId != 0) {
        ...
        if (session == NULL) {
            eicDebug("Error looking up session for sessionId %" PRIu64, ctx->sessionId);
            return false;
        }
        EicSha256Ctx sha256;
...
bool eicPresentationSetAuthToken(EicPresentation* ctx, uint64_t challenge, uint64_t secureUserId,
    ...
    uint64_t authChallenge = ctx->authChallenge;
    if (ctx->sessionId != 0) {
        EicSession* session = eicSessionGetForId(ctx->sessionId);
        if (session == NULL) {
            eicDebug("Error looking up session for sessionId %" PRIu64, ctx->sessionId);
