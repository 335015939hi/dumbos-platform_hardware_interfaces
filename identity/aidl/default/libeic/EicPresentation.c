...
bool eicPresentationInit(EicPresentation* ctx, bool testCredential, const char* docType,
    ...
    uint8_t credentialKeys[EIC_CREDENTIAL_KEY_SIZE_V2];
    size_t credentialKeysLength = sizeof(credentialKeys);
    ...
    //
    if (encryptedCredentialKeysSize == EIC_CREDENTIAL_KEY_SIZE_V1 + EIC_NONCE_TAG_SIZE) {
        /* do nothing */
    } else if (encryptedCredentialKeysSize == EIC_CREDENTIAL_KEY_SIZE_V2 + EIC_NONCE_TAG_SIZE) {
    ...
    // In order to prevent NULL pointer access Hmac ctx, use zero derivedKey to init ctx at beginning.
    eicMemSet(derivedKey, 0, 32);
...
bool eicPresentationGenerateSigningKeyPair(EicPresentation* ctx, const char* docType,
    ...
    eicCborAppendString(&cbor, "ProofOfBinding", sizeof("ProofOfBinding") - 1);
...
bool eicPresentationValidateRequestMessage(EicPresentation* ctx, const uint8_t* sessionTranscript,
    ...
    eicCborAppendString(&cbor, "Signature1", sizeof("Signature1") - 1);
...
bool eicPresentationValidateAccessControlProfile(EicPresentation* ctx, int id,
    ...
    *accessGranted = false;
    ...
    // Validate the MAC
    uint8_t* cborBuffer = scratchSpace;
    EicCbor cborBuilder;
...
bool eicPresentationCalcMacKey(EicPresentation* ctx, const uint8_t* sessionTranscript,
    ...
    uint8_t signingKeyPriv[EIC_P256_PRIV_KEY_SIZE];
    size_t signingKeyPrivLength = sizeof(signingKeyPriv);
    if (!eicOpsDecryptAes128Gcm(ctx->storageKey, signingKeyBlob, 60, (const uint8_t*)docType,
                                docTypeLength, signingKeyPriv, &signingKeyPrivLength)) {
    ...
    eicCborAppendString(&ctx->cbor, "MAC0", sizeof("MAC0") - 1);
    ...
    eicCborAppendArray(&ctx->cbor, 4);
    eicCborAppendString(&ctx->cbor, "DeviceAuthentication", sizeof("DeviceAuthentication") - 1);
...
EicAccessCheckResult eicPresentationStartRetrieveEntryValue(
        ...
        unsigned int newNamespaceNumEntries, int32_t entrySize __unused,
...
bool eicPresentationRetrieveEntryValue(EicPresentation* ctx, const uint8_t* encryptedContent,
                                       ...
                                       size_t* contentLength, const char* nameSpace,
...
bool eicPresentationDeleteCredential(EicPresentation* ctx, const char* docType, size_t docTypeLength,
    ...
    eicCborAppendString(&cbor, "ProofOfDeletion", sizeof("ProofOfDeletion") - 1);
...
bool eicPresentationProveOwnership(EicPresentation* ctx, const char* docType,
    ...
    // Finally, the CBOR that we're actually signing.
    eicCborAppendArray(&cbor, 4);
    eicCborAppendString(&cbor, "ProofOfOwnership", sizeof("ProofOfOwnership") -1 );
    eicCborAppendString(&cbor, docType, docTypeLength);
    eicCborAppendByteString(&cbor, challenge, challengeSize);
