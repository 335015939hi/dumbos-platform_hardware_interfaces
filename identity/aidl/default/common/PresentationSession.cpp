...
ndk::ScopedAStatus PresentationSession::getCredential(
    ...
    shared_ptr<IdentityCredential> credential = ndk::SharedRefBase::make<IdentityCredential>(
            hwProxyFactory_, hwProxy, credentialData, p);
...
const vector<uint8_t>& PresentationSession::getSessionTranscript() {
    return sessionTranscript_;
}

const vector<uint8_t>& PresentationSession::getReaderEphemeralPublicKey() {
