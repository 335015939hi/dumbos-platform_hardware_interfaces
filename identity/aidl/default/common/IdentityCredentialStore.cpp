...
ndk::ScopedAStatus IdentityCredentialStore::getHardwareInformation(
    ...
    hw.isRemoteKeyProvisioningSupported = remotelyProvisionedComponent_.has_value();
    *hardwareInformation = hw;
...
ndk::ScopedAStatus IdentityCredentialStore::getRemotelyProvisionedComponent(
    ...
    if (!remotelyProvisionedComponent_) {
        return ndk::ScopedAStatus(AStatus_fromServiceSpecificErrorWithMessage(
                IIdentityCredentialStore::STATUS_FAILED,
                "Remote key provisioning is not supported"));
    }

    ndk::SpAIBinder binder(AServiceManager_waitForService(remotelyProvisionedComponent_->c_str()));
