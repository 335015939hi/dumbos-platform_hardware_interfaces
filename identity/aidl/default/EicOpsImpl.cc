...
bool eicOpsCreateCredentialKey(uint8_t privateKey[EIC_P256_PRIV_KEY_SIZE],
                               uint8_t publicKey[EIC_P256_PUB_KEY_SIZE], const uint8_t* challenge,
                               size_t challengeSize, const uint8_t* applicationId,
                               size_t applicationIdSize, bool isTestCredential, uint8_t* certChain,
    ...
    optional<std::pair<vector<uint8_t>, vector<vector<uint8_t>>>> ret =
            android::hardware::identity::support::createEcKeyPairAndAttestation(
                    challengeVec, applicationIdVec, isTestCredential);
    if (!ret) {
    ...
    if (*certChainSize < flatChain.size()) {
        eicDebug("Buffer for certificate is only %zd bytes long, need %zd bytes", *certChainSize,
                 flatChain.size());
        return false;
    }
    memcpy(certChain, flatChain.data(), flatChain.size());
    *certChainSize = flatChain.size();
    ...
    optional<vector<uint8_t>> pubKey =
            android::hardware::identity::support::ecKeyPairGetPublicKey(ret.value().first);
    if (!pubKey) {
        eicDebug("Error extracting public key");
        return false;
    }
