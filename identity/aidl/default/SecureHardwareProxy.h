...
class SecureHardwareProvisioningProxy : public RefBase {
  public:
    ...
    virtual bool initialize(bool testCredential);
    ...
    virtual bool startPersonalization(int accessControlProfileCount, vector<int> entryCounts,
                                      string docType, size_t expectedProofOfProvisioningSize);
    ...
    virtual bool beginAddEntry(const vector<int>& accessControlProfileIds, string nameSpace,
                               string name, uint64_t entrySize);
    ...
    virtual optional<vector<uint8_t>> finishAddingEntries(bool testCredential);

    // Returns encryptedCredentialKeys (80 bytes).
    virtual optional<vector<uint8_t>> finishGetCredentialData(bool testCredential, string docType);
...
class SecureHardwarePresentationProxy : public RefBase {
  public:
    ...
    virtual optional<pair<vector<uint8_t>, vector<uint8_t>>> createEphemeralKeyPair();
