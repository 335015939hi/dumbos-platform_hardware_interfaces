...
bool eicProvisionInit(EicProvision* ctx, bool testCredential) {
...
bool eicProvisionCreateCredentialKey(EicProvision* ctx,
    ...
    if (!eicOpsCreateEcKey(ctx->credentialPrivateKey,
                           publicKey)) {
      return false;
    }
    if (!eicOpsAttestToEcKey(publicKey,
                             challenge, challengeSize,
                             applicationId, applicationIdSize,
                             publicKeyCert,
                             publicKeyCertSize)) {
      return false;
    }
...
#ifdef EIC_DEBUG
static uint8_t eicProvisionStaticBuf[1024*1024] = {0};
#endif

bool eicProvisionStartPersonalization(EicProvision* ctx, int accessControlProfileCount,
...
#ifdef EIC_DEBUG
    eicCborInit(&ctx->cbor, eicProvisionStaticBuf, sizeof(eicProvisionStaticBuf));
#else
    ...
    eicCborAppendByteString(&ctx->cbor, coseEncodedProtectedHeaders, sizeof(coseEncodedProtectedHeaders));
...
bool eicProvisionAddAccessControlProfile(EicProvision* ctx, int id, const uint8_t* readerCertificate,
...
bool eicProvisionBeginAddEntry(EicProvision* ctx, const int* accessControlProfileIds,
...
bool eicProvisionAddEntryValue(EicProvision* ctx, const int* accessControlProfileIds,
                               size_t numAccessControlProfileIds, const char* nameSpace, const char* name,
...
bool eicProvisionFinishAddingEntries(EicProvision* ctx, bool testCredential,
    ...
    if (ctx->cbor.size != ctx->expectedCborSizeAtEnd) {
        eicDebug("CBOR size is %zd, was expecting %zd",
                 ctx->cbor.size,
                 ctx->expectedCborSizeAtEnd);
        return false;
    }

    if (!eicOpsEcDsa(ctx->credentialPrivateKey, cborSha256, signatureToBeSignedWithProofOfProvisioning)) {
...
bool eicProvisionFinishGetCredentialData(EicProvision* ctx,
                                         bool testCredential,
                                         const char* docType,
    ...
    if (!eicOpsEncryptAes128Gcm(eicOpsGetHardwareBoundKey(testCredential),
                                nonce,
                                cborBuf,
                                cbor.size,
                                // DocType is the additionalAuthenticatedData
                                (const uint8_t*) docType, eicStrLen(docType),
...
bool eicPresentationInit(EicPresentation* ctx,
                         bool testCredential,
                         const char* docType,
                         const uint8_t encryptedCredentialKeys[80]) {
    uint8_t credentialKeys[52];

    if (!eicOpsDecryptAes128Gcm(eicOpsGetHardwareBoundKey(testCredential),
                                encryptedCredentialKeys, 80,
                                // DocType is the additionalAuthenticatedData
                                (const uint8_t*) docType, eicStrLen(docType),
                                credentialKeys)) {
        eicDebug("Error encrypting CredentialKeys");
    ...
    //
    if (credentialKeys[0] != 0x82 || credentialKeys[1] != 0x50 ||
        credentialKeys[18] != 0x58 || credentialKeys[19] != 0x20) {
...
bool eicPresentationGenerateSigningKeyPair(EicPresentation* ctx,
    ...
    if (!eicOpsSignEcKey(signingKeyPub,
                         ctx->credentialPrivateKey,
                         1,
                         "Android Identity Credential Authentication Key",
                         eicOpsGetIssuerName(),
                         validityNotBefore,
                         validityNotAfter,
                         publicKeyCert,
    ...
    if (!eicOpsEncryptAes128Gcm(ctx->storageKey,
                                nonce,
                                signingKeyPriv,
                                sizeof(signingKeyPriv),
                                // DocType is the additionalAuthenticatedData
                                (const uint8_t*) docType, eicStrLen(docType),
                                signingKeyBlob)) {
