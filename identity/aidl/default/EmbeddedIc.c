...
bool eicStartPersonalization(EicProvision* ctx, int accessControlProfileCount,
    ...
    eicDebug("CBOR size before ProofOfProvisioning: %zd", ctx->cbor.size);
    ctx->expectedProofOfProvisioningSize = expectedProofOfProvisioningSize;
...
bool eicFinishAddingEntries(EicProvision* ctx, bool testCredential,
    ...
    // The Sig_structure CBOR appended in eicStartPersonalization() takes up 22
    // bytes so need to discount that when checking if the expected size of
    // ProofOfProvisioning CBOR checks out.
    size_t proofOfProvisioningSize = ctx->cbor.size - 22;
    if (proofOfProvisioningSize != ctx->expectedProofOfProvisioningSize) {
        eicDebug("proofOfProvisioningSize size is %zd, was told to expect %zd",
                 proofOfProvisioningSize,
                 ctx->expectedProofOfProvisioningSize);
        return false;
    }
