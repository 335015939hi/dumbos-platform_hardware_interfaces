...
ndk::ScopedAStatus WritableIdentityCredential::finishAddingEntries(
    ...
    optional<vector<uint8_t>> signatureToBeSignedWithProofOfProvisioning =
        hwProxy_->finishAddingEntries(testCredential_);
    if (!signatureToBeSignedWithProofOfProvisioning) {
      return ndk::ScopedAStatus(AStatus_fromServiceSpecificErrorWithMessage(
    ...
    optional<vector<uint8_t>> signature = support::coseSignEcDsaWithSignature(
        signatureToBeSignedWithProofOfProvisioning.value(),
        encodedCbor,  // data
        {});          // certificateChain
    if (!signature) {
      return ndk::ScopedAStatus(AStatus_fromServiceSpecificErrorWithMessage(
