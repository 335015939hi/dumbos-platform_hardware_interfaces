...
void eicPrint(const char* message);
...
// |privateKey|.
//
// Also generates a certificate using |challenge| and |applicationId|, and
// returns the generated certificate in X.509 format. This is returned in |cert|
...
//
// If |signingKey| is not NULL, it will be used to sign the generated certificate.
//
// TODO: include other attestation data, e.g. current time, expiration, and so on.
//
bool eicOpsCreateEcKey(const uint8_t* challenge,
                       size_t challengeSize,
                       const uint8_t* applicationId,
                       size_t applicationIdSize,
                       const uint8_t signingKey[EIC_P256_PRIV_KEY_SIZE],
                       uint8_t privateKey[EIC_P256_PRIV_KEY_SIZE],
                       uint8_t* cert,
...
typedef struct {
    ...
    uint8_t credentialPrivateKey[32];
...
bool eicCreateCredentialKey(EicProvision* ctx,
                            ...
                            const uint8_t* applicationId,
                            size_t applicationIdSize,
                            const uint8_t attestationKey[EIC_P256_PRIV_KEY_SIZE],
