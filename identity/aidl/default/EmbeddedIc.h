...
bool eicOpsEncryptAes128Gcm(const uint8_t* key,    // Must be 16 bytes
...
typedef struct {
    ...
    // Placeholder for buffer - the actuall buffer is allocated/reserved by the
    // caller, following this struct.
    uint8_t buffer[0];
} EicCbor;

/* Initializes an EicCbor.
 *
 * If zero is used for bufferSize, the CBOR isn't incrementally built up, it's
 * only digested. The memory usage is constrained to sizeof(EicCbor).
 *
 * If non-zero is used for bufferSize, this number of bytes after the EicCbor
 * struct will be used for storing the incrementally built-up CBOR.
 */
...
typedef struct {
    uint8_t numEntryCounts;
    uint8_t entryCounts[EIC_MAX_NUM_NAMESPACES];
    uint8_t accessControlProfileCount;

    uint8_t curNumAccessControlProfileIds;
    uint8_t curAccessControlProfileIds[EIC_MAX_NUM_ACCESS_CONTROL_PROFILE_IDS];

    uint8_t curNamespace;
    uint8_t curNamespaceNumProcessed;
