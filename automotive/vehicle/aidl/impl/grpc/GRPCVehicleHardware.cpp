...
constexpr static size_t kMaxRetryCount = 5;
...
bool GRPCVehicleHardware::updateTimestamp(aidlvhal::VehiclePropValue* propValue) const {
    ...
    if (it == mLatestUpdateTimestamps.end() || externalTimestamp > (it->second)[0]) {
        mLatestUpdateTimestamps[propIdAreaId][0] = externalTimestamp;
        mLatestUpdateTimestamps[propIdAreaId][1] = now;
        propValue->timestamp = now;
        return true;
    }
    if (externalTimestamp == (it->second)[0]) {
        propValue->timestamp = (it->second)[1];
        return true;
    }
    // externalTimestamp < (it->second)[0]
    return false;
}
