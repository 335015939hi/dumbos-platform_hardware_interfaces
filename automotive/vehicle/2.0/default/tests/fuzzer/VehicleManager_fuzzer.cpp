...
void VehicleHalManagerFuzzer::invokeDebug() {
    ...
    hidl_handle fd = {};
    native_handle_t* const nativeHandle1 = native_handle_create(/*numFds=*/1, /*numInts=*/0);
    fd.setTo(nativeHandle1, /*shouldOwn=*/false);

    const hidl_vec<hidl_string> optionsVector1{};
    const hidl_vec<hidl_string> optionsVector2{debugOption};
...
void VehicleHalManagerFuzzer::invokePropConfigs() {
    ...
    mManager->getPropConfigs(properties,
                             [](StatusCode status, const hidl_vec<VehiclePropConfig>& c) {
                                 (void)status;
                                 (void)c;
                             });
    ...
    mManager->getAllPropConfigs(
            [](const hidl_vec<VehiclePropConfig>& propConfigs) { (void)propConfigs; });
}
