<<<<<<< HEAD   (79819a Work around fix for "libvtswidevine32.so" not found (HIDL ve)
=======
/*
...
void VehicleHalHidlTest::checkIsSupported(int32_t propertyId) {
    hidl_vec<int32_t> properties = {propertyId};
    mVehicle->getPropConfigs(properties, [propertyId](StatusCode status,
                                                      const hidl_vec<VehiclePropConfig>& cfgs) {
        ASSERT_EQ(StatusCode::OK, status);
        if (cfgs.size() == 0) {
...
>>>>>>> CHANGE (8c20c3 Allow CDD required properties to be absent in VTS.)
