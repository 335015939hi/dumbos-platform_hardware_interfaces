...
void SurroundViewFuzzer::invoke2dSessionAPI() {
                ...
                mFuzzedDataProvider->PickValueInArray<const std::function<void()>>({
                        ...
                        [&]() {
                            ...
                            if (mFuzzedDataProvider->ConsumeBool()) {
                                ...
                                config.blending = mFuzzedDataProvider->ConsumeBool()
                                                          ? (SvQuality::HIGH)
                                                          : (SvQuality::LOW);
                            }
                        ...
                        [&]() {
                            ...
                            if (cameraIds.size() > 0 && mFuzzedDataProvider->ConsumeBool()) {
                                ...
                                cameraId = mFuzzedDataProvider->ConsumeRandomLengthString(
                                        kMaxCharacters);
                            }
    ...
    mSurroundViewService->stop2dSession(mFuzzedDataProvider->ConsumeBool() ? surroundView2dSession
                                                                           : nullptr);
}
...
void SurroundViewFuzzer::invoke3dSessionAPI() {
    ...
    while (mFuzzedDataProvider->remaining_bytes() > 0) {
        auto surroundView3dFunc =
                mFuzzedDataProvider->PickValueInArray<const std::function<void()>>({
                        [&]() {
                            const size_t numViews =
                                    mFuzzedDataProvider->ConsumeIntegralInRange<size_t>(1,
                                                                                        kMaxViews);
    ...
    mSurroundViewService->stop3dSession(mFuzzedDataProvider->ConsumeBool() ? surroundView3dSession
                                                                           : nullptr);
}
