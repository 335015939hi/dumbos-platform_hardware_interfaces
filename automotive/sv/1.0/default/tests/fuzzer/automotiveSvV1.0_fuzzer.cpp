...
void SurroundViewFuzzer::invoke2dSessionAPI() {
    ...
    sp<SurroundViewStream> handler = new SurroundViewStream(surroundView2dSession);
    ...
    size_t camPoints = mFuzzedDataProvider->ConsumeIntegralInRange<size_t>(1, kMaxCameraPoints);
...
void SurroundViewFuzzer::invoke3dSessionAPI() {
    ...
    sp<SurroundViewStream> handler = new SurroundViewStream(surroundView3dSession);

    size_t numViews = mFuzzedDataProvider->ConsumeIntegralInRange<size_t>(1, kMaxViews);
...
bool SurroundViewFuzzer::init() {
    mSurroundViewService = new SurroundViewService();
