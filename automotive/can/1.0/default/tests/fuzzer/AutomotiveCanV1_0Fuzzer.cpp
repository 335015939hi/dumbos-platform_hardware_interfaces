...
void CanFuzzer::invokeUpInterface() {
    CanController::InterfaceType iftype;
    if (mFuzzedDataProvider->ConsumeBool()) {
        iftype = (CanController::InterfaceType)mFuzzedDataProvider->ConsumeIntegral<uint8_t>();
    } else {
        iftype = kInterfaceType[mFuzzedDataProvider->ConsumeIntegralInRange<size_t>(
    ...
    } else if (iftype == CanController::InterfaceType::INDEXED) {
