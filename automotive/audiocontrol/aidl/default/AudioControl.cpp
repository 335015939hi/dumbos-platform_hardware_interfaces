...
binder_status_t AudioControl::parseMetaData(
        int fd, const std::string &metadataLiteral,
    ...
    trackMetadata = {
        .usage = static_cast<audiomediacommon::AudioUsage>(usage),
...
binder_status_t AudioControl::cmdRequestFocusWithMetaData(
...
binder_status_t AudioControl::cmdAbandonFocusWithMetaData(
        int fd, const char** args, uint32_t numArgs) {
    if (!checkCallerHasWritePermissions(fd)) {
        return STATUS_PERMISSION_DENIED;
    }
    if (numArgs != 3) {
        dprintf(fd, "Invalid number of arguments: please provide:\n"
                    "--abandonFocusWithMetaData <METADATA> <ZONE_ID>: "
                    "abandons audio focus for specified metadata and audio zone ID (int)\n");
        return STATUS_BAD_VALUE;
    }
    ...
    dprintf(fd, "Abandoned focus for metadata %s and zoneId %d\n",
            trackMetadata.toString().c_str(), zoneId);
    return STATUS_OK;
}
