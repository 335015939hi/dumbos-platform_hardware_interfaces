...
bool verify_attestation_record(const string& challenge, const string& app_id,
    ...
    EXPECT_GE(att_keymint_version, 5U);
...
/*
 * AttestationTest.RsaAttestationEncryptKeyDummySigned
 *
 * Verifies that attesting to RSA encryption key with no challenge and app id returns
 * a dummy certificate.
 */
TEST_P(AttestationTest, RsaAttestationEncryptKeyDummySigned) {
    ASSERT_EQ(ErrorCode::OK, GenerateKey(AuthorizationSetBuilder()
                                                 .Authorization(TAG_NO_AUTH_REQUIRED)
                                                 .RsaEncryptionKey(2048, 65537)
                                                 .Padding(PaddingMode::NONE)));
...
TEST_P(ClearOperationsTest, TooManyOperations) {
    ...
    for (int i = 0; i < max_operations; i++) {
        EXPECT_EQ(ErrorCode::OK,
                  Begin(KeyPurpose::ENCRYPT, key_blob_, params, &out_params, op_handles[i]));
    }
    ...
    for (int i = 0; i < max_operations; i++) {
        EXPECT_EQ(ErrorCode::OK, Abort(op_handles[i]))
                << "Aboort failed for i = " << i << std::endl;
    }
