...
namespace aidl::android::hardware::keymint {
...
StatusOr<bytevec /* pubkeys */> validateAndExtractPubkeys(bool /* testMode */,
    ...
    for (auto& keyToSign : keysToSign) {
        auto [macedKeyItem, _, coseMacErrMsg] = cppbor::parse(keyToSign.key);
        if (!macedKeyItem) {
            return Status("Invalid COSE_Mac0 structure");
        }

        auto coseKey = verifyAndParseCoseMac0(macedKeyItem.get(), macKey);
        if (!coseKey) return Status(STATUS_INVALID_MAC, coseKey.moveMessage());

        auto pubKey = CoseKey::parse(*coseKey, EDDSA, ED25519);
        if (!pubKey) return Status(pubKey.moveMessage());

        pubKeysToMac.add(pubKey->getBstrValue(CoseKey::PUBKEY_X).value());
    }
...
ScopedAStatus RemotelyProvisionedComponent::generateEd25519KeyPair(bool testMode,
    ...
    macedPublicKey->key = macedKey->encode();
...
ScopedAStatus RemotelyProvisionedComponent::generateCertificateRequest(
    ...
    auto pubKeysToSign = validateAndExtractPubkeys(testMode, keysToSign, macKey_);
    if (!pubKeysToSign.isOk()) return pubKeysToSign.moveError();

    bytevec ephemeralMacKey = randomBytes(SHA256_DIGEST_LENGTH);
    ...
    *keysToSignMac = *std::move(pubKeysToSignMac);
    ...
    auto coseEncrypted = constructCoseEncrypt(*sessionKey, randomBytes(kAesGcmNonceLength),
...
bytevec RemotelyProvisionedComponent::randomBytes(size_t numBytes) const {
