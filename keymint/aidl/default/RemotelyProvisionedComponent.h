...
#include <aidl/android/hardware/keymint/BnRemotelyProvisionedComponent.h>
...
class RemotelyProvisionedComponent : public BnRemotelyProvisionedComponent {
  public:
    RemotelyProvisionedComponent() { std::tie(devicePrivKey_, bcc_) = generateBcc(); }

    ScopedAStatus generateEd25519KeyPair(bool testMode, MacedPublicKey* macedPublicKey,
  ...
  private:
    ...
    std::vector<uint8_t> randomBytes(size_t numBytes) const;
