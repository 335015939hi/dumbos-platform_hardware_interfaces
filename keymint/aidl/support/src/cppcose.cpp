...
// std::ostream& operator<<(std::ostream& os, const bytevec& v) {
//     for (auto c : v) {
//         char buf[3];
...
ErrMsgOr<EVP_CIPHER_CTX_Ptr> aesGcmInitAndProcessAad(const bytevec& key, const bytevec& nonce,
                                                     const bytevec& aad, bool encrypt) {
...
ErrMsgOr<bytevec /* payload */> verifyAndParseCoseMac0(const cppbor::Item* macItem,
    ...
    if (macTag->size() != tag->value().size() ||
        CRYPTO_memcmp(macTag->data(), tag->value().data(), macTag->size())) {
