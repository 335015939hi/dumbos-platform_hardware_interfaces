...
enum Tag {
     ...
     * ErrorCode::LIMITED_USE_KEY_DELETED. The key deletion may not happen synchronously in KeyMint,
     * so the implementation must not allow further operations to finish successfully if the value
     * of this tag is zero and the IkeyMintDevice must return ErrorCode::USAGE_LIMIT_EXHAUSTED.
     *
