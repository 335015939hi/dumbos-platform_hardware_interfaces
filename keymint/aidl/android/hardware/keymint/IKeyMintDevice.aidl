 ...
 *
 * IKeyMintDevice do not implementations softwareEnforced list or returns it.  For example,
 * Tag::ORIGINATION_EXPIRE_DATETIME provides the date and time after which a key may not be
 * used to encrypt or sign new messages.  Unless the IKeyMintDevice has access to a secure
 * source of current date/time information, it is not possible for the IKeyMintDevice to
 * enforce this tag.  An IKeyMintDevice implementation will not rely on the non-secure world's
 * notion of time, because it could be controlled by an attacker. Similarly, it cannot rely
 * on GPSr time, even if it has exclusive control of the GPSr, because that might be spoofed
 * by attacker RF signals.
 *
...
@VintfStability interface IKeyMintDevice {
     ...
     *        appropriate error if any of those requirements fail.  TODO(seleneh) list errors.
