...
import android.hardware.keymint.MacedPublicKey;
 ...
 *     components.  The "decrypter" decrypts the BCC, verifies DK_pub and the device's right to receive
 *     provisioned certificates, but does not see the public keys to be signed or the resulting
 *     certificates.  The "certifier" gets informed of the results of the the decrypter's validation
 *     and sees the public keys to be signed and resulting certificates, but does not see the BCC.
 *
 * Test Mode
 * =========
 *
...
interface IRemotelyProvisionedComponent {
     ...
     *        details.
     *
     * @param out keysToSignMac contains A MAC over the KeysToSign in the
     *        CertificateRequest structure. Specifically, it contains:
     *
     *            HMAC-256(EK_mac, .cbor KeysToMacStructure)
     *
     *        Where EK_mac is an ephemeral MAC key, found in EncryptedBcc.  The MACed data is:
     *
     *             KeysToMacStructure = [
     *                 context : "MAC0",
     ...
     *             ]
     *
     * @param out byte[] ephemeralKey contains an ephemeral X25519 public key in little-endian
     *        order. The secure area must generate a new, random ephemeral key pair for every
     *        request.
     *
     * @param out byte[] bccCiphertext contains the encrypted BCC and MAC key. Specifically,
     *        it contains:
     *
     *             AES-GCM(K, .cbor ProtectedDataPayload)
     *
     *        where K is the key derived by performing ECDH key agreement between the provided EEK
     *        public key and an ephemeral private key generated by the secure area (see the
     *        ephemeralKey output argument above) and then applying HKDF-SHA256 (COSE algorithm is
     *        ECDH-ES + HKDF-256). The 128-bit authentication tag is appended to the ciphertext, as
     *        per COSE.
     *
     *        The MAC key that is signed by DK_pub and embedded in the encrypted data along with the
     *        BCC is an ephemeral MAC key generated only for this request. The MAC key is signed by
     *        DK_priv to prove that this request is from the device identified by DK_pub (which is
     *        at index dk_index in the BCC) and is in turn used to authenticate the keysToSign (see
     *        keysToSignMac output argument).
     *
     *        The payload is:
     *
     ...
     *
     *            Bcc = [
     *                device_key_entry : int,         // 0-based index of device key entry in array
     *                [ + BccEntry ]
     *            ]
     *
     *            BccEntry = [                       // COSE_Sign1
    ...
    void generateCertificateRequest(in boolean testMode,
                                    ...
                                    out byte[] ephemeralKey,
                                    out byte[] bccCiphertext);
}
