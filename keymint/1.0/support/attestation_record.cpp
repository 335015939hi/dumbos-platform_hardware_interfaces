...
#include <keymintV1_0/authorization_set.h>
...
std::tuple<ErrorCode, AttestationRecord> parse_attestation_record(const vector<uint8_t>& cert) {
    ...
    result.keymint_version = ASN1_INTEGER_get(record->keymint_version);
    result.keymint_security_level =
    ...
    rot.verified_boot_state = static_cast<keymint_verified_boot_t>(
