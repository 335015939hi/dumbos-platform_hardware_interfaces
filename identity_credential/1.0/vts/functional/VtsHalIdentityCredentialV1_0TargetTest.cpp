...
TEST_F(IdentityCredentialStoreHidlTest, createAndRetrieveCredential) {
    ...
    ASSERT_TRUE(support::cborPrettyPrint(encodedSignedData, cborPretty, 32, {"readerAuthPubKey"}));
    EXPECT_EQ(
        "{\n"
    ...
    vector<uint8_t> previousAuditSignatureHash = credentialData;
    vector<uint8_t> signature;
    AuditLogEntry auditLogEntry;
    credential->finishRetrieval(
        generatedSigningKeyBlob, previousAuditSignatureHash,
        [&](ResultCode hidl_error, const hidl_vec<uint8_t> returnedSignature,
