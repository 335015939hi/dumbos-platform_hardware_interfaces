...
TEST_F(IdentityCredentialStoreHidlTest, createAndRetrieveCredential) {
    ...
    const vector<TestProfile> testProfiles = {
        ...
        // Profile 1 (user authentication)
        {1, {}, 0x1234567890abcdef, CapabilityType::ANY, 100},
        // Profile 2 (no authentication)
        {2, {}, 0, CapabilityType::NOT_APPLICABLE, 0}};

    // Here's the actual test data:
    const vector<TestEntryData> testEntries = {
        {"PersonalData", "Last name", string("Turing"), false, vector<uint8_t>{1}},
        {"PersonalData", "Birth date", string("19120623"), false, vector<uint8_t>{1}},
        {"PersonalData", "First name", string("Alan"), false, vector<uint8_t>{2}},
        {"PersonalData", "Home address", string("Maida Vale, London, England"), false,
         vector<uint8_t>{0, 1}},
        {"Image", "Portrait image", portraitImage, false, vector<uint8_t>{1, 2}},
    };
    ...
    credential->createEphemeralKeyPair(
        ...
        [&](const hidl_vec<uint8_t>& keyPair) { ephemeralKeyPair = keyPair; });
    ...
    startArguments.authToken = {123456,
                                {1},
                                CapabilityType::ANY,
                                666666,
                                vector<uint8_t>{0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef}};
