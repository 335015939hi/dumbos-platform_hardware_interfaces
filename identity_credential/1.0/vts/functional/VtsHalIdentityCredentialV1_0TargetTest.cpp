...
struct TestEntryData {
    ...
    TestEntryData(string nameSpace, string name, string str, bool directlyAvailable,
    ...
    virtual void SetUp() override {
        ...
        credentialStore_->getHardwareInformation(
            [&](const Result& result, const hidl_string& credentialStoreName,
                const hidl_string& credentialStoreAuthorName, SecurityType /* securityType */,
                const hidl_vec<Certification>& /* certifications */,
                const hidl_vec<hidl_string>& /* directAccessDocTypes */, uint32_t chunkSize) {
                EXPECT_EQ("", result.message);
                ASSERT_EQ(ResultCode::OK, result.code);
                ASSERT_GT(credentialStoreName.size(), 0u);
                ASSERT_GT(credentialStoreAuthorName.size(), 0u);
                ASSERT_GE(chunkSize,
                          256u);  // Chunk sizes smaller than APDU buffer won't be supported
...
TEST_F(IdentityCredentialStoreHidlTest, createAndRetrieveCredential) {
    ...
    credentialStore_->createCredential(
        ...
        [&](const Result& result, const sp<IWritableIdentityCredential>& newCredential) {
            EXPECT_EQ("", result.message);
            ASSERT_EQ(ResultCode::OK, result.code);
            writableCredential = newCredential;
        });
    ...
    writableCredential->getAttestationCertificate(
        ...
        [&](const Result& result, const hidl_vec<uint8_t>& returnedAttestationCertificate) {
            EXPECT_EQ("", result.message);
            ASSERT_EQ(ResultCode::OK, result.code);
    ...
    for (const auto& testProfile : testProfiles) {
        writableCredential->addAccessControlProfile(
            testProfile.id, testProfile.readerCertificate, testProfile.capabilityId,
            testProfile.capabilityType, testProfile.timeout,
            [&](const Result& result, SecureAccessControlProfile profile) {
                EXPECT_EQ("", result.message);
                ASSERT_EQ(ResultCode::OK, result.code);
    ...
    vector<uint8_t> personalizationReceipt;
    writableCredential->finishAddingEntries([&](const Result& result,
                                                const hidl_vec<uint8_t>& returnedCredentialData,
                                                hidl_vec<uint8_t> returnedPersonalizationReceipt) {
        EXPECT_EQ("", result.message);
        ASSERT_EQ(ResultCode::OK, result.code);
        credentialData = returnedCredentialData;
        personalizationReceipt = returnedPersonalizationReceipt;
    });
    // TODO: inspect credentialData

    vector<uint8_t> encodedSignedData;
    ASSERT_TRUE(signedDataBuilder.getEncodedCbor(encodedSignedData));
    ASSERT_TRUE(support::cborPrettyPrint(encodedSignedData, cborPretty, 32, {"ReaderCertificate"}));
    EXPECT_EQ(
        "{\n"
        ...
        "}",
        cborPretty);
    EXPECT_TRUE(support::checkEcDsaSignature(support::sha256(encodedSignedData),
                                             personalizationReceipt, attestationCertificate));
    ...
    credentialStore_->getCredential(
        credentialData, [&](const Result& result, const sp<IIdentityCredential>& newCredential) {
            EXPECT_EQ("", result.message);
            ASSERT_EQ(ResultCode::OK, result.code);
    ...
    credential->createEphemeralKeyPair([&](const Result& result, const hidl_vec<uint8_t>& keyPair) {
        EXPECT_EQ("", result.message);
        ASSERT_EQ(ResultCode::OK, result.code);
    ...
    vector<uint8_t> encodedRequestData;
    ASSERT_TRUE(support::generateRequestData(
        ...
        encodedRequestData));
    ASSERT_TRUE(
        support::cborPrettyPrint(encodedRequestData, cborPretty, 32, {"EphemeralPublicKey"}));
    EXPECT_EQ(
        "{\n"
        "  'SessionTranscript' : {\n"
    ...
    ASSERT_TRUE(support::signEcDsa(readerKey, encodedRequestData, readerSignature));
    ...
    startArguments.requestData = encodedRequestData;
    startArguments.readerSignature = readerSignature;
    startArguments.requestCounts = testEntriesEntryCounts;
    credential->startRetrieval(startArguments, [&](const Result& result) {
        EXPECT_EQ("", result.message);
        ASSERT_EQ(ResultCode::OK, result.code);
    });
    ...
    for (const auto& entry : testEntries) {
        ...
        credential->startRetrieveEntryValue(entry.nameSpace, entry.name, entrySize,
                                            entry.profileIds, [&](const Result& result) {
                                                EXPECT_EQ("", result.message);
                                                ASSERT_EQ(ResultCode::OK, result.code);
                                            });
        ...
        for (const auto& chunk : chunks) {
            ...
            credential->retrieveEntryValue(
                encryptedContent, [&](const Result& result, const EntryValue& value) {
                    EXPECT_EQ("", result.message);
                    ASSERT_EQ(ResultCode::OK, result.code);
    ...
    vector<uint8_t> encodedAuthenticatedData;
    ASSERT_TRUE(authenticatedDataBuilder.getEncodedCbor(encodedAuthenticatedData));
    ASSERT_TRUE(
        support::cborPrettyPrint(encodedAuthenticatedData, cborPretty, 32, {"EphemeralPublicKey"}));
    ASSERT_EQ(
        "{\n"
        "  'SessionTranscript' : {\n"
        "    'EphemeralPublicKey' : <not printed>\n"
    ...
    credential->generateSigningKeyPair([&](const Result& result,
    ...
    credential->finishRetrieval(generatedSigningKeyBlob, previousAuditSignatureHash,
                                [&](const Result& result, const hidl_vec<uint8_t> returnedSignature,
                                    const AuditLogEntry& returnedAuditLogEntry) {
                                    EXPECT_EQ("", result.message);
                                    ASSERT_EQ(ResultCode::OK, result.code);
                                    signature = returnedSignature;
                                    auditLogEntry = returnedAuditLogEntry;
                                });
    EXPECT_TRUE(support::checkEcDsaSignature(support::sha256(encodedAuthenticatedData), signature,
                                             generatedSigningKeyCertificate));

    vector<uint8_t> requestHash;
    for (size_t n = 0; n < auditLogEntry.requestHash.size(); n++) {
        requestHash.push_back(auditLogEntry.requestHash[n]);
    }
    vector<uint8_t> responseHash;
    for (size_t n = 0; n < auditLogEntry.responseHash.size(); n++) {
        responseHash.push_back(auditLogEntry.responseHash[n]);
    }
    EXPECT_EQ(requestHash, support::sha256(encodedRequestData));
    EXPECT_EQ(responseHash, support::sha256(encodedAuthenticatedData));
    vector<uint8_t> encodedAuditLogData;
    ASSERT_TRUE(support::buildAndEncodeAuditLogData(
        requestHash, responseHash, previousAuditSignatureHash, encodedAuditLogData));
    EXPECT_TRUE(support::checkEcDsaSignature(support::sha256(encodedAuditLogData),
                                             auditLogEntry.signature, attestationCertificate));
}
