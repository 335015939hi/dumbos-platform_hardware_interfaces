...
bool IdentityCredential::initialize() {
    ...
    if (cbor.get() == nullptr) {
        LOG(ERROR) << "Error decoding CBOR in decryptedCredentialKeys";
        return false;
    }
...
Return<void> IdentityCredential::startRetrieval(const StartRetrievalArguments& args,
    ...
    // TODO: check/calculate failedAccessControlProfileIds by checking authToken

    // TODO: check/validate that readerSignature signs requestData (reader
    // public key is from one of the access control profiles).

    std::string docType = "org.iso.18013-5.2019.mdl";  // TODO: don't hardcode
    ...
    authenticatedDataBuilder_.reset(docType, encodedSessionTranscript);
...
Return<void> IdentityCredential::retrieveEntryValue(const hidl_vec<uint8_t>& encryptedContent,
    ...
    authenticatedDataBuilder_.addDataItem(currentNameSpace_, currentName_, value);
...
Return<void> IdentityCredential::finishRetrieval(
    ...
    hexdump("encodedCbor", encodedCbor);
    ...
    _hidl_cb(ResultCode::FAILED, signature, auditLogEntry);
    return Void();
}
...
Return<void> IdentityCredential::generateSigningKeyPair(KeyType keyType,
    ...
    // hexdump("encryptedSigningKey", encryptedSigningKey);
