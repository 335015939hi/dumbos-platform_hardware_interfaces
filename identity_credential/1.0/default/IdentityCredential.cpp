...
Return<void> IdentityCredential::createEphemeralKeyPair(KeyType keyType,
    ...
    if (keyType != KeyType::EC_NIST_P_256) {
        ...
        // TODO: want to return FAILED
        _hidl_cb({});
        return Void();
    }
    if (!support::createEmpheralKeyPair(keyPair)) {
        _hidl_cb({});
        return Void();
    }
...
Return<ResultCode> IdentityCredential::startRetrieval(const StartRetrievalArguments& args) {
    ...
    if (args.requestData.size() > 0) {
        ...
        for (size_t n = 0; n < (size_t)arrayDataReqs->length; n++) {
            ...
            // TODO: collect all DataItemNames for said DocType and store them somewhere
            // so they can be checked against in startRetrievalEntryValue().
        }
    }
...
Return<ResultCode> IdentityCredential::startRetrieveEntryValue(
    ...
    // TODO: check (nameSpace, name) was in RequestData and bail if not.
