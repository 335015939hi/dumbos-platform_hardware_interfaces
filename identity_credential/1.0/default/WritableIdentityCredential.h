...
struct WritableIdentityCredential : public IWritableIdentityCredential {
    ...
    Return<void> addAccessControlProfile(uint8_t id, const hidl_vec<uint8_t>& readerCertificate,
                                         uint64_t capabilityId, CapabilityType capabilityType,
                                         uint32_t timeout,
                                         addAccessControlProfile_cb _hidl_cb) override;

    Return<void> beginAddEntry(const hidl_vec<uint8_t>& accessControlProfileIds,
   ...
   private:
    ...
    std::vector<uint8_t> credentialPubKey_;

    // These fields are initialized during startPersonalization()
    size_t numAccessControlProfileRemaining_;
    std::vector<uint16_t> remainingEntryCounts_;
    support::SignedDataBuilder signedDataBuilder_;

    // This is built in addAccessControlProfile
    ...
    std::string entryNameSpace_;
    std::string entryName_;
    std::vector<uint8_t> entryAccessControlProfileIds_;
    bool entryDirectlyAvailable_;
