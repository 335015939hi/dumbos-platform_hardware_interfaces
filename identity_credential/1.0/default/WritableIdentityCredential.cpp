...
Return<void> WritableIdentityCredential::getAttestationCertificate(
    ...
    if (!createEcKeyAndAttestationChain(credentialPrivKey_, certificate)) {
        _hidl_cb(ResultCode::FAILED, {});
        return Void();
    }
    ...
    // Calculate |credentialBlob_| which is used in finishAddingEntries().
    vector<uint8_t> nonce;
    if (!getRandom(12, nonce)) {
        _hidl_cb(ResultCode::FAILED, {});
        return Void();
    }
    if (!encryptAes128Gcm(getTestHardwareBoundKey(), nonce, credentialKeys, {}, credentialBlob_)) {
        _hidl_cb(ResultCode::FAILED, {});
        return Void();
    }
...
Return<void> WritableIdentityCredential::addEntryValue(const EntryValue& value,
    ...
    if (chunkSize < 0) {
        ...
        if (entryRemainingBytes_ > 0) {
            // (the whole value wasn't transferred).. TODO: ensure chunkSize is gcmChunkSize
        }
    }
...
Return<void> WritableIdentityCredential::finishAddingEntries(finishAddingEntries_cb _hidl_cb) {
    ...
    _hidl_cb(ResultCode::OK, credentialBlob_, signature);
    return Void();
}
