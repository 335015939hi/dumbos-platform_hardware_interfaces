...
bool cborPrettyPrint(const std::vector<uint8_t>& encodedCbor, std::string& out, size_t maxBStrSize,
                     const vector<string>& mapKeysToNotPrint) {
    out.clear();
...
bool createEmpheralKeyPair(std::vector<uint8_t>& keyPair) {
    auto ec_key = EC_KEY_Ptr(EC_KEY_new());
    auto pkey = EVP_PKEY_Ptr(EVP_PKEY_new());
    // TODO: maybe allow specifying curve instead of hard-coding it here?
    auto group = EC_GROUP_Ptr(EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1));
...
bool generateRequestData(const std::vector<uint8_t>& encodedSessionTranscript,
