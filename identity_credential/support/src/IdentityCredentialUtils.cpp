...
bool createEcKeyAndAttestationChain(vector<uint8_t>& key, vector<uint8_t>& certificate) {
...
bool ecPublicKeyGenerateCertificate(const vector<uint8_t>& publicKey,
    ...
    BIO* outbio = BIO_new_file("/data/dzcert.pem", "w");
    if (!PEM_write_bio_X509(outbio, x509.get())) {
        LOG(ERROR) << "Error printing the signed certificate";
    }
