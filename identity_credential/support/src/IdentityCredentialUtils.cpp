...
bool createEcKeyAndAttestationChain(std::vector<uint8_t>& key, std::vector<uint8_t>& certificate) {
    ...
    if (context.GenerateAttestation(*(keymasterKey.get()), attestationParams, &cert_chain) !=
        KM_ERROR_OK) {
        LOG(INFO) << "Error generating attestation";
    }
...
cn_cbor* SignedDataBuilderEntry::toCbor() const {
    cn_cbor_errback err;
...
void SignedDataBuilder::addEntry(
    ...
    entries_.push_back(
        SignedDataBuilderEntry(nameSpace, name, accessControlProfileIds, value, directlyAvailable));
}
    ...
    switch (value->type) {
        ...
        case CN_CBOR_ARRAY:
            ...
            for (size_t n = 0; n < value->length; n++) {
                ...
                if (!cborPrettyPrint(item, out, indent + 2)) {
                    return false;
                }
            }
            out.append("]");
