...
// ecKeyPairGetPrivateKey()). Signature is returned in |signature| and will by
// in DER format.
...
// returned by ecKeyPairGetPublicKey()). The certificate is signed by
// |signingKey| (which must be in the format returned by
// ecKeyPairGetPrivateKey())
...
// Returns |true| only if |authToken| is authentic.
...
// Gets milliseconds since boot. Guaraneteed to be monotonic.
bool getMillisecondsSinceBoot(uint64_t& milliseconds);

// Returns |true| only if |authToken| fulfills the requirements encoded in |profile|.
//
// The |sessionTranscriptDigest| field must be set to the SHA-256 of the
// SessionTranscript. If no SessionTranscript is available it must be empty.
//
// Note that this doesn't check |profile.readerCertificate|, only checks specific
// to |KeymasterCapability| are performed.
bool checkAccess(const SecureAccessControlProfile& profile, const KeymasterCapability& authToken,
                 const std::vector<uint8_t>& sessionTranscriptDigest);
...
// Builds the AuditLogData CBOR from |requestHash|, |responseHash|, and
// |previousAuditSignatureHash|.
...
const std::vector<uint8_t>& getTestHardwareBoundKey();
