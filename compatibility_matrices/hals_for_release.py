...
def ParseArgs():
  ...
  if not args.show_deprecated and not args.show_unchanged and not args.show_introduced:
    args.show_deprecated = args.show_introduced = True
  ...
  logger.debug("Show deprecated HALs? %s", args.show_deprecated)
  logger.debug("Show unchanged HALs? %s", args.show_unchanged)
  logger.debug("Show introduced HALs? %s", args.show_introduced)
...
def GetLevel(analyze_matrix, file):
  """
  :param analyze_matrix: Path of analyze_matrix
  :param file: a file, possibly a compatibilty matrix
...
def GetReport(tuple1, tuple2, args):
  ...
  for package in all_packages:
    ...
    if args.show_deprecated:
      package_report += ["- Deprecated %s" % StripPackage(instance) for instance in deprecated]
    if args.show_unchanged:
      package_report += ["  Unchanged  %s" % StripPackage(instance) for instance in unchanged]
    if args.show_introduced:
      package_report += ["+ Introduced %s" % StripPackage(instance) for instance in introduced]
