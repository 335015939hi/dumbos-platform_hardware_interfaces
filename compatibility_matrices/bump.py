...
class Bump(object):
    ...
    def __init__(self, cmdline_args):
        ...
        self.current_module_name = "framework_compatibility_matrix.{}.xml".format(
            self.current_level)
        self.next_module_name = "framework_compatibility_matrix.{}.xml".format(
            self.next_level)
        self.interfaces_dir = self.top / "hardware/interfaces"
        self.bug = cmdline_args.bug
        self.bugline = "Bug: {}\n".format(
            cmdline_args.bug) if cmdline_args.bug else ""
    ...
    def copy_matrix(self):
        current_xml = self.interfaces_dir / "compatibility_matrices/compatibility_matrix.{}.xml".format(
            self.current_level)
        self.next_xml = self.interfaces_dir / "compatibility_matrices/compatibility_matrix.{}.xml".format(
            self.next_level)
        shutil.copyfile(current_xml, self.next_xml)

        rel_next_xml = self.next_xml.relative_to(self.interfaces_dir)
        check_call(f"git -C {self.interfaces_dir} add {rel_next_xml}", shell=True)
    ...
    def edit_android_bp(self):
        ...
        rel_android_bp = android_bp.relative_to(self.interfaces_dir)
        check_call(f"git -C {self.interfaces_dir} add {rel_android_bp}", shell=True)
    ...
    def edit_android_mk(self):
        ...
        rel_android_mk = android_mk.relative_to(self.interfaces_dir)
        check_call(f"git -C {self.interfaces_dir} add {rel_android_mk}", shell=True)


    def commit(self):
        check_call(
            f'git -C {self.interfaces_dir} commit -m '
            f'"Create compatibility matrix for '
            f'{self.level_to_letter[self.next_level]}.\n\n'
            f'{self.bugline}'
            f'Test: builds"',
            shell=True)

    def print_summary(self):
        print("*** Please submit these changes: ***")
        check_call("git -C {} show -s --format=%B HEAD".format(
            self.interfaces_dir),
                   shell=True)
