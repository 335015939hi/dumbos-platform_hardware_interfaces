...
type vintfCompatibilityMatrixProperties struct {
	...
	// list of extra xml defined by board config
	Env_srcs []string
}
...
func (g *vintfCompatibilityMatrixRule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
    ...
    fmt.Printf("Bill_1 %s\n", g.properties.Env_srcs)
    fmt.Printf("Bill_2 %s\n", strings.Join(g.properties.Env_srcs, ":"))
	...
	ctx.Build(pctx, android.BuildParams{
		...
		Output:      g.genFile,
		Args: map[string]string{
			"inputs": strings.Join(inputPaths.Strings(), ":") + strings.Join(g.properties.Env_srcs, ":"),
		},
	})
