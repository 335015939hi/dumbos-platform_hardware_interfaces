...
type vintfCompatibilityMatrixProperties struct {
	...
	// Boolean variable to indicate generating the System Compatibility Matrix (common to all FCM versions)
	Use_framework_compatibility_env bool

	// Boolean variable to indicate generating the Product Compatibility Matrix
...
func (g *vintfCompatibilityMatrixRule) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if g.properties.Use_framework_compatibility_env && g.properties.Use_product_compatibility_env {
		panic(fmt.Errorf("Attribute use_framework_compatibility_env and use_product_compatibility_env should not set simultaneously!"))
	}
	...
	// For framework_compatibility_matrix.device.xml the source will from the environment variabe
	...
	// For product_compatibility_matrix.xml the source may come from the environment variabe
	...
	// If there's no any inputs means it will not need to generate any installed targets and will treat it as a phony target.
	if len(inputPaths) == 0 {
		g.phonyOnly = true
		return
	}
...
func (g *vintfCompatibilityMatrixRule) getAssembleVintfEnv(ctx android.ModuleContext) string {
	if g.properties.Use_framework_compatibility_env {
		assembleVintfEnvs := []string{
			fmt.Sprintf("POLICYVERS=%s", policyVers),
