...
bool ShouldCheckMissingHalsInFcm(const std::string& interfaceName) {
    ...
    auto interface_name_has_prefix = [&](const std::string& prefix) {
    ...
    if (!std::any_of(included_prefixes.begin(), included_prefixes.end(),
                     interface_name_has_prefix)) {
        return false;
    }

    if (std::find(excluded_exact.begin(), excluded_exact.end(), interfaceName) !=
        excluded_exact.end()) {
        return false;
    }

    return !std::any_of(excluded_prefixes.begin(), excluded_prefixes.end(),
                        interface_name_has_prefix);
}
