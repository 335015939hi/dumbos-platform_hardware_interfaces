...
use android_hardware_rpmb::aidl::android::hardware::rpmb::IRpmb::{BnRpmb, BpRpmb, IRpmb};
use android_hardware_rpmb::aidl::android::hardware::rpmb::{RpmbMessageFrameNormal::RpmbMessageFrameNormal, RpmbGeometry::RpmbGeometry};
...
impl IRpmb for MockRpmbService {
    // void initialize_device(out Geometry geometry);
    // void operate(in RpmbMessageFrameNormal[] requestFrames, out RpmbMessageFrameNormal[] responseFrames);
    fn initialize_device(&self, geometry: &mut RpmbGeometry) -> BinderResult<()> {
        // Do something?
        info!("geometry is : {:?}", geometry);
        Ok(())
    }

    fn operate(&self, _request_frames: &[RpmbMessageFrameNormal], _response_frames: &mut Vec<RpmbMessageFrameNormal>)
            -> BinderResult<()>{
        info!("Request frame : {:?}", _request_frames);
        // TODO: Do something
        Ok(())
    }
}
...
fn main() {
    ...
    binder::add_service(
        &format!("{}/default", <BpRpmb as IRpmb>::get_descriptor()),
