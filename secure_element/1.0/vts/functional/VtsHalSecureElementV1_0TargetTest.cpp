   ...
   public:
    virtual void SetUp() override {
        std::string serviceName =
            SecureElementHidlEnvironment::Instance()->getServiceName<ISecureElement>();
        ...
        se_->init(se_cb_);
        // Wait for onStateChange to send state connected
...
TEST_F(SecureElementHidlTest, transmit) {
    std::vector<uint8_t> aid = {0x00, 0x00, 0x00, 0x00, 0x00};
 ...
 */
TEST_F(SecureElementHidlTest, openCloseBasicChannel) {
    std::vector<uint8_t> aid = {0x00, 0x00, 0x00, 0x00, 0x00};
    SecureElementStatus statusReturned;
    ...
    if (statusReturned == SecureElementStatus::SUCCESS) {
        ...
        EXPECT_EQ(SecureElementStatus::SUCCESS, se_->closeChannel(0));
        return;
    }
 ...
 * checks for status
 */
TEST_F(SecureElementHidlTest, factoryReset) {
    EXPECT_EQ(SecureElementStatus::SUCCESS, se_->factoryReset());
}
