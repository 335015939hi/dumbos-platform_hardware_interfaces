...
interface ISecureElement {
    ...
    byte[] getAtr();
    void init(in android.hardware.secure_element.ISecureElementHalCallback clientCallback);
    boolean isCardPresent();
    byte[] openBasicChannel(in byte[] aid, in byte p2);
    android.hardware.secure_element.LogicalChannelResponse openLogicalChannel(
            in byte[] aid, in byte p2);
    void reset();
