...
class MySecureElement : public BnSecureElement {
  public:
    ...
    ScopedAStatus openLogicalChannel(
        ...
        // FIXME: need to support multiple logical channels
        if (mLogicChannel) {
            return ScopedAStatus::fromServiceSpecificError(CHANNEL_NOT_AVAILABLE);
        }
        *_aidl_return = LogicalChannelResponse{.channelNumber = 1, .selectResponse = {}};

        // note - channels and responses are not modeled correctly or isolated. This is
        // written as a tool to test the framework implementation.
        size_t size = 2050;
        _aidl_return->selectResponse.resize(size);
        _aidl_return->selectResponse[size - 1] = 0x00;
        _aidl_return->selectResponse[size - 2] = 0x90;
        mLogicChannel = true;
        return ScopedAStatus::ok();
    }
