...
TEST_P(SecureElementAidl, transmit) {
    ...
    // FIXME: not sure what 3rd element should be
    EXPECT_THAT(transmitResponse, ElementsAre(0x90, 0x00, 0x00));
...
TEST_P(SecureElementAidl, closeInvalidChannel) {
    auto status = se->closeChannel(42);
    EXPECT_EQ(status.getServiceSpecificError(), ISecureElement::CHANNEL_NOT_AVAILABLE)
            << status.getDescription();
}
