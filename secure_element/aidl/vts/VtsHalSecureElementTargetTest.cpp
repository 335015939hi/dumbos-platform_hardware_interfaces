...
class SecureElementAidl : public ::testing::TestWithParam<std::string> {
  public:
    ...
    // Call init() on the bound secure element interface.
    void init() {
    ...
    uint16_t transmit(uint8_t channel_number) {
        ...
        return (response[response.size() - 2] << 8) | (response[response.size() - 1] << 0);
    }
...
TEST_P(SecureElementAidl, init) {
    ...
    // init() called a second time shall fail.
    EXPECT_ERR(secure_element_->init(secure_element_callback_));
}

TEST_P(SecureElementAidl, reset) {
    // reset called before init shall fail.
    EXPECT_ERR(secure_element_->reset());
...
TEST_P(SecureElementAidl, isCardPresent) {
    ...
    // isCardPresent called before init shall fail.
    EXPECT_ERR(secure_element_->isCardPresent(&res));
...
TEST_P(SecureElementAidl, getAtr) {
    ...
    // getAtr called before init shall fail.
    EXPECT_ERR(secure_element_->getAtr(&atr));
...
TEST_P(SecureElementAidl, openBasicChannel) {
    ...
    // openBasicChannel called before init shall fail.
    ...
    EXPECT_OK(secure_element_->openBasicChannel(kNonSelectableAid, 0x00, &response));
}

TEST_P(SecureElementAidl, openLogicalChannel) {
    LogicalChannelResponse response;

    // openLogicalChannel called before init shall fail.
    EXPECT_ERR(secure_element_->openLogicalChannel(kSelectableAid, 0x00, &response));
...
TEST_P(SecureElementAidl, closeChannel) {
    ...
    // closeChannel called on basic or logical channel before init shall fail.
    ...
    EXPECT_OK(secure_element_->openBasicChannel(kNonSelectableAid, 0x00, &basic_channel_response));
    EXPECT_OK(secure_element_->closeChannel(0));
    ...
    EXPECT_OK(secure_element_->openLogicalChannel(kNonSelectableAid, 0x00,
                                                  &logical_channel_response));
...
TEST_P(SecureElementAidl, transmit) {
    ...
    // transmit called before init shall fail.
