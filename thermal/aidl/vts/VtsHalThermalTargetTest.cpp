...
class ThermalAidlTest : public testing::TestWithParam<std::string> {
  public:
    void SetUp() override {
        ...
        auto ret = mThermal->registerThermalChangedCallback(mThermalCallback, false,
                                                            TemperatureType::SKIN, &thermalStatus);
        EXPECT_EQ(ThermalStatusCode::SUCCESS, thermalStatus.code);
        ASSERT_TRUE(ret.isOk());
        // Expect to fail if register again
        ret = mThermal->registerThermalChangedCallback(mThermalCallback, false,
                                                       TemperatureType::SKIN, &thermalStatus);
        EXPECT_NE(ThermalStatusCode::SUCCESS, thermalStatus.code);
        ASSERT_TRUE(ret.isOk());
    }

    void TearDown() override {
        ThermalStatus thermalStatus;
        auto ret = mThermal->unregisterThermalChangedCallback(mThermalCallback, &thermalStatus);
        EXPECT_EQ(ThermalStatusCode::SUCCESS, thermalStatus.code);
        ASSERT_TRUE(ret.isOk());
        // Expect to fail if unregister again
        ret = mThermal->unregisterThermalChangedCallback(mThermalCallback, &thermalStatus);
        EXPECT_NE(ThermalStatusCode::SUCCESS, thermalStatus.code);
        ASSERT_TRUE(ret.isOk());
    }
