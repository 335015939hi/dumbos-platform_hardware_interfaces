...
TEST_F(OffloadConfigHidlTest, TestSetNonNetfilterNetlinkHandles) {
    unique_fd fd1(netlinkSocket(NETLINK_ROUTE, RTM_NEWLINK));
    if (fd1.get() < 0) {
        ALOGE("Unable to create conntrack handles: %d/%s", errno, strerror(errno));
        FAIL();
    }
    ...
    unique_fd fd2(netlinkSocket(NETLINK_ROUTE, RTM_NEWADDR));
    if (fd2.get() < 0) {
        ALOGE("Unable to create conntrack handles: %d/%s", errno, strerror(errno));
        FAIL();
    }
...
TEST_F(OffloadConfigHidlTest, TestSetNonNetlinkHandles) {
    for (sa_family_t family : {AF_UNIX, AF_INET, AF_INET6}) {
        SCOPED_TRACE(StringPrintf("address family: %d", family));

        unique_fd fd1(socket(family, SOCK_DGRAM, 0));
        if (fd1.get() < 0) {
            ALOGE("Unable to create conntrack handles: %d/%s", errno, strerror(errno));
            FAIL();
        }
        ...
        unique_fd fd2(socket(family, SOCK_DGRAM, 0));
        if (fd2.get() < 0) {
            ALOGE("Unable to create conntrack handles: %d/%s", errno, strerror(errno));
            FAIL();
        }
