...
int netlinkSocket(int protocol, unsigned groups) {
    android::base::unique_fd s(socket(AF_NETLINK, SOCK_DGRAM, protocol));
    if (s.get() < 0) {
        return -errno;
    }
...
TEST_F(OffloadConfigHidlTest, TestSetHandles) {
    android::base::unique_fd fd1(netlinkSocket(kFd1Groups)), fd2(netlinkSocket(kFd2Groups));
...
TEST_F(OffloadConfigHidlTest, TestSetHandle1Only) {
    android::base::unique_fd fd1(netlinkSocket(kFd1Groups));
...
TEST_F(OffloadConfigHidlTest, TestSetHandle2OnlyNotOk) {
    android::base::unique_fd fd2(netlinkSocket(kFd2Groups));
...
// Ensure passing non-nfnetlink handles is recognized by the HAL as an error.
TEST_F(OffloadConfigHidlTest, TestSetNonNetfilterNetlinkHandles) {
    android::base::unique_fd fd1(netlinkSocket(NETLINK_ROUTE, RTM_NEWLINK)),
        fd2(netlinkSocket(NETLINK_ROUTE, RTM_NEWADDR));
...
// Ensure passing non-netlink handles is recognized by the HAL as an error.
TEST_F(OffloadConfigHidlTest, TestSetNonNetlinkHandles) {
    android::base::unique_fd fd1(socket(AF_UNIX, SOCK_DGRAM, 0)),
