...
ndk::ScopedAStatus Offload::setLocalPrefixes(const std::vector<std::string>& in_prefixes) {
    ...
    for (std::string prefix : in_prefixes) {
        if (!isValidIpAddress(prefix) && !isValidIpPrefix(prefix)) {
            return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                    "Invalid IP prefix");
        }
    }
...
bool Offload::isValidInterface(const std::string& iface) {
    ...
    const auto ifidx = if_nametoindex(iface.c_str());
    // upstream interface doesn't exist in cuttlefish, hardcode its name here
    return ifidx != 0 || iface == "rmnet_data0";
}
