...
class TetheroffloadAidlTestBase : public testing::TestWithParam<std::string> {
  ...
  protected:
    ...
    void setHandles(const ExpectBoolean expectedResult) {
        ...
        ndk::ScopedAStatus status = mOffload->setHandles(fd1, fd2);
...
// Check that calling setHandles() without first having called initOffload() returns error.
TEST_P(TetheroffloadAidlPreInitTest, SetHandlesWithoutInitReturnsError) {
    setHandles(ExpectBoolean::False);
}
...
TEST_P(TetheroffloadAidlGeneralTest, TestSetHandle2OnlyNotOk) {
    ndk::ScopedFileDescriptor fd1 = ndk::ScopedFileDescriptor(-1);
    unique_fd ufd2(netlinkSocket(kFd2Groups));
    if (ufd2.get() < 0) {
        ALOGE("Unable to create conntrack handles: %d/%s", errno, strerror(errno));
