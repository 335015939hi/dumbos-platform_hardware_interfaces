...
import android.hardware.tetheroffload.control.NetworkStats;
...
interface IOffloadControl {
     ...
     * forwarded.
     *
     * The prefix may be an IPv4 or an IPv6 address to signify which family can be offloaded from
     ...
     * downstream interface that have IP addresses within the specified prefix. Traffic from the
     * same downstream interfaces is unaffected and must be forwarded if and only if it was already
     * being forwarded.
     *
     ...
     *
     * This API does not replace any previously configured downstreams and must be explicitly
     * removed by calling removeDownstream.
     *
     ...
     *            status.success: true if successful, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     *
     ...
     *          being forwarded.
     */
    void addDownstream(in String iface, in String prefix, out Status status);
     ...
     *
     * @param out netStats @NetworkStats
     */
    void getForwardedStats(in String upstream, out NetworkStats netStats);
     ...
     *
     * @param out status @Status
     *            status.success: true if initialization is successful, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     *
     * Remarks: Initializing offload does not imply that any upstreams or downstreams have yet
     * been, or even will be, chosen.  This API is symmetrical with stopOffload.
     */
    void initOffload(in ITetheringOffloadCallback cb, out Status status);
     ...
     *
     * @param out status @Status
     *            status.success: true if success, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     */
    void removeDownstream(in String iface, in String prefix, out Status status);
     ...
     * addDownstream again.  Note that it is not necessary to call initOffload again to resume
     * offload if stopOffload was not called by the client.
     *
     ...
     *
     * @param out status @Status
     *            status.success: true if limit is applied, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     */
    void setDataLimit(in String upstream, in long limit, out Status status);
     ...
     *
     * @param out status @Status
     *            status.success: true if quota is applied, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     */
     ...
     */
    void setLocalPrefixes(in String[] prefixes, out Status status);
    ...
    void setUpstreamParameters(in String iface, in String v4Addr, in String v4Gw,
        in String[] v6Gws, out Status status);
     ...
     *
     * @param out status @Status
     *            status.success: true if offload is stopped, false otherwise
     *            status.errMsg: a human readable string if eror has occurred
     *
     * Remarks: Statistics must be reset by this API.
     */
    void stopOffload(out Status status);
}