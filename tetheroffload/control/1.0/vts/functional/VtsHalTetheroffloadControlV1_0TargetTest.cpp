...
class OffloadControlHidlTestBase : public testing::TestWithParam<std::string> {
   public:
    ...
    void prepareControlHal() {
        control = IOffloadControl::getService(GetParam());
...
INSTANTIATE_TEST_CASE_P(
    ...
    [](const testing::TestParamInfo<OffloadControlHidlTest::ParamType>&
           info) {
        return "OffloadConfig_" + android::hardware::Sanitize(std::get<0>(info.param)) +
               "_OffloadControl_" +
               android::hardware::Sanitize(std::get<1>(info.param));
    });
