...
impl Uwb {
    pub fn from_chips(chips: impl IntoIterator<Item = uwb_chip::UwbChip>, handle: TokioHandle) -> Self {
        Self {
            chips: chips
                .into_iter()
                .map(|chip| {
                    IUwbChip::BnUwbChip::new_async_binder(chip, TokioRuntime(handle.clone()), binder::BinderFeatures::default())
                })
                .collect(),
        }
    }
